...
//
// Manages a list of categories and their associated indexes.
...
public:
    ...
    NVCategory(const char** strs, int count);
    // Parameter is a list of pointers and lengths to char-arrays in host or device memory.
    NVCategory(std::pair<const char*,size_t>* strs, size_t count, bool devmem=true);
    // Create an instance from a list of strings.
    NVCategory(NVStrings& strs);
    ...
    int get_value(const char* str, unsigned int bytes);
