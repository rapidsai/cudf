...
typedef struct {
	 ...
	 */
	char		*file_path			= NULL;			/**< file location to read from	- currently the file cannot be compressed 							*/
	char		*buffer				= NULL;			// process data from a buffer,  pointer to Host memory
	char 		*object				= NULL;			// this is a URL path

	bool 		windowslinetermiation = false;		/**< States if we should \r\n as our line termination>**/
	...
	int			header				= -1;			// Row of the header data,  zero based counting. Default states that header should not be read from file.

	int			num_cols			= -1;			/**< number of columns in the names and dtype arrays												*/
	const char	**names				= NULL;			/**< ordered List of column names, this is a required field 										*/
	const char	**dtype				= NULL;			/**< ordered List of data types, this is required													*/

	int			*index_col			= NULL;			// array of int:	Column to use as the row labels of the DataFrame.

	int			*use_cols_int		= NULL;			// array of int:	Return a subset of the columns.  CSV reader will only process those columns,  another read is needed to get full data
	int			use_cols_int_len	= 0;			// int:	number of elements in list of returned columns
	const char	**use_cols_char		= NULL;			// array of char:	Return a subset of the columns.  CSV reader will only process those columns,  another read is needed to get full data
	int			use_cols_char_len	= 0;			// int:	number of elements in list of returned columns

	long		skiprows			= 0;			/**< number of rows at the start of the files to skip, default is 0									*/
	long		skipfooter			= 0;			/**< number of rows at the bottom of the file to skip - default is 0								*/
	...
	bool		mangle_dupe_cols	= true;		// if true: duplicate columns will be specified as ‘X’, ‘X.1’, …’X.N’, rather than ‘X’…’X’
