...
if [ "$BUILD_LIBCUDF" == '1' ]; then
  ...
  gpuci_conda_retry build --old-build-string --prefix-length 100 --no-build-id --croot ${CONDA_BLD_DIR} conda/recipes/libcudf $CONDA_BUILD_ARGS
  ...
  gpuci_conda_retry build --old-build-string --prefix-length 100 --no-build-id --croot ${CONDA_BLD_DIR} conda/recipes/libcudf_kafka $CONDA_BUILD_ARGS
