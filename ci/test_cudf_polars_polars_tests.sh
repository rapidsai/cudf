...
RAPIDS_PY_WHEEL_NAME="libcudf_${RAPIDS_PY_CUDA_SUFFIX}" rapids-download-wheels-from-s3 cpp ./local-deps
RAPIDS_PY_WHEEL_NAME="pylibcudf_${RAPIDS_PY_CUDA_SUFFIX}" rapids-download-wheels-from-s3 python ./local-deps

# clone polars to get its test requirements files
...
python -m pip install \
    ...
    "$(echo ./local-deps/libcudf_${RAPIDS_PY_CUDA_SUFFIX}*.whl)" \
    "$(echo ./local-deps/pylibcudf_${RAPIDS_PY_CUDA_SUFFIX}*.whl)"
