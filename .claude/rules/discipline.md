...
## Rule 6 — Circuit breaker on 3 consecutive failures
If you hit 3 consecutive `discard` or `crash` results, you are going down the wrong path. STOP entirely. Do NOT keep hammering variations of the same approach. Instead:
1. Re-read the source code from disk (not from memory — your cached understanding may be wrong after many edits)
2. Re-read results.tsv end-to-end to see what's been tried
3. Do fresh web research for a completely different optimization strategy
4. Pick a fundamentally different approach
...
3. Re-read results.tsv end-to-end
...
## Rule 12 — Use memory to persist discoveries
