...
1. **Pick a run tag** autonomously based on today's date (e.g. `apr05-csv`). Branch `autoresearch/<tag>` must not exist yet.
...
9. **Begin the loop immediately** — do not pause or ask for confirmation.
