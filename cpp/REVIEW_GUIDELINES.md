...
- Extended `__device__` lambdas passed to device algorithms should declare their return type via `cuda::proclaim_return_type<T>(...)` or a trailing `-> T`. Some host-side APIs (e.g. CUB `DeviceSelect`, transform iterators) query the return type in host code and fail to compile without it; declaring it uniformly avoids surprises as APIs change
