  ...
  std::reference_wrapper<cudf::io::datasource> datasource)
{
  ...
  size_t const len          = datasource.get().size();

  auto header_buffer = datasource.get().host_read(0, header_len);
  auto const header  = reinterpret_cast<file_header_s const*>(header_buffer->data());
  auto ender_buffer  = datasource.get().host_read(len - ender_len, ender_len);
  ...
  return datasource.get().host_read(len - ender->footer_len - ender_len, ender->footer_len);
}
...
{
  return datasource.get().host_read(page_index_bytes.offset(), page_index_bytes.size());
}
                  ...
                  rmm::device_async_resource_ref mr)
{
  ...
  column_chunk_buffers.reserve(byte_ranges.size());
  {
    ...
    for (size_t chunk = 0; chunk < byte_ranges.size();) {
      ...
      // Such kernels operate on aligned data pointers, which require padding to the buffers so that
      // the pointers can shift along the address space to satisfy their alignment requirement.
      constexpr size_t buffer_padding_multiple = 8;

      if (io_size != 0) {
        column_chunk_buffers.emplace_back(
          cudf::util::round_up_safe(io_size, buffer_padding_multiple), stream, mr);

        // Directly read the column chunk data to the device
        // buffer if supported
        if (datasource.get().supports_device_read() and
            datasource.get().is_device_read_preferred(io_size)) {
          read_tasks.emplace_back(datasource.get().device_read_async(
            io_offset, io_size, static_cast<uint8_t*>(column_chunk_buffers.back().data()), stream));
        } else {
          // Read the column chunk data to the host buffer and
          // copy it to the device buffer
          read_tasks.emplace_back(std::async(
            std::launch::deferred,
            [datasource, io_offset, io_size, dest = column_chunk_buffers.back().data(), stream]() {
              auto host_buffer = datasource.get().host_read(io_offset, io_size);
        ...
        auto d_compdata = static_cast<uint8_t const*>(column_chunk_buffers.back().data());
        do {
          column_chunk_data[chunk] = cudf::device_span<uint8_t const>{
            d_compdata, static_cast<size_t>(byte_ranges[chunk].size())};
          d_compdata += byte_ranges[chunk].size();
