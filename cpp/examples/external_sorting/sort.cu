 ...
 */
bool data_files_exist(std::string const& output_dir, int num_files)
{
...
int main(int argc, char** argv)
{
  ...
  if (argc >= 5) output_dir = std::string{argv[4]};
  ...
  if (files_exist) {
    ...
    // Define column types for generation
    ...
    for (int i = 0; i < num_files; ++i) {
      std::cout << "Generating table " << i << "/" << num_files << std::endl;
      auto table = cudf::examples::generate_random_table(column_types, n_columns, m_rows_per_file, default_stream, cudf::get_current_device_resource_ref());
      generated_tables.push_back(std::move(table));
      table_views.push_back(generated_tables.back()->view());
    }
