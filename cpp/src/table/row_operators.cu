 ...
 *         their generated ranks (size_type column(s)) and possibly new list offsets
 */
  ...
  if (lhs.type().id() == type_id::LIST) {
    ...
    if (child_lhs.type().id() == type_id::STRUCT) {
      if (rhs_opt) {
        ...
        auto [transformed_lhs, new_offsets_lhs] = replace_child(lhs, out_cols_lhs.back()->view());
        ...
        auto [transformed_rhs, new_offsets_rhs] =
          replace_child(rhs_opt.value(), out_cols_rhs.back()->view());
      ...
      if (out_cols_child_lhs.size() > 0 || out_cols_child_rhs.size() > 0) {
        auto [transformed_lhs, new_offsets_lhs] = replace_child(lhs, new_child_lhs);
        if (new_offsets_lhs) { out_cols_lhs.emplace_back(std::move(new_offsets_lhs)); }
        if (!rhs_opt) {
          return {transformed_lhs, std::nullopt, std::move(out_cols_lhs), std::move(out_cols_rhs)};
        } else {
          auto [transformed_rhs, new_offsets_rhs] =
            replace_child(rhs_opt.value(), new_child_rhs_opt.value());
          if (new_offsets_rhs) { out_cols_rhs.emplace_back(std::move(new_offsets_rhs)); }
    ...
    // else == child is not STRUCT or LIST: just go to the end of this function.
