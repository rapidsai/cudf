 ...
 * new ones. This is because the new child was generated by ranking and always has zero
 ...
 *
 * Dense rank type is used for compute ranking of the input for later lexicographic comparison.
 *
 ...
 *
 * In addition, since the being ranked input column is always a nested child column instead of
 * top-level column, the column order for ranking should be fixed to the same value
...
/**
 * @copydoc transform_lists_of_structs(column_view const&, null_order, rmm::cuda_stream_view)
 *
