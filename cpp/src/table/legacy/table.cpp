  ...
  : _columns(dtypes.size())
{
  std::transform(
    ...
    dtypes.begin(),
    _columns.begin(),
    [num_rows, allocate_bitmasks, all_valid, stream](gdf_column*& col, gdf_dtype dtype) {
      ...
      CUDF_EXPECTS(GDF_SUCCESS == gdf_column_view_augmented(
                                    col, nullptr, nullptr, num_rows, dtype, 0, extra_info),
      ...
      if (allocate_bitmasks) {
        ...
        RMM_ALLOC(
          &col->valid, gdf_valid_allocation_size(num_rows) * sizeof(cudf::valid_type), stream);

        CUDA_TRY(cudaMemsetAsync(col->valid,
                                 fill_value,
...
table table::select(std::vector<cudf::size_type> const& column_indices) const
{
  ...
  for (auto index : column_indices) { desired_columns.push_back(_columns.at(index)); }
...
std::vector<gdf_dtype> column_dtypes(cudf::table const& table)
{
  ...
  std::transform(
    table.begin(), table.end(), dtypes.begin(), [](gdf_column const* col) { return col->dtype; });
  return dtypes;
}
...
table concat(cudf::table const& table1, cudf::table const& table2)
{
  ...
  std::transform(std::cbegin(table1), std::cend(table1), std::begin(columns), [](auto col) {
    return const_cast<gdf_column*>(col);
  });
  std::transform(std::cbegin(table2),
                 std::cend(table2),
                 std::begin(columns) + table1.num_columns(),
                 [](auto col) { return const_cast<gdf_column*>(col); });
