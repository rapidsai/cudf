...
struct elements_are_equal {
                                             ...
                                             bool nulls_are_equal = false)
  {
    if (nullable) {
      ...
      if (not lhs_is_valid and not rhs_is_valid) { return nulls_are_equal; }
                                  ...
                                  bool nulls_are_equal = false)
{
  auto equal_elements = [lhs_index, rhs_index, nulls_are_equal](gdf_column const &l,
    ...
    return cudf::type_dispatcher(
      l.dtype, elements_are_equal<nullable>{}, l, lhs_index, r, rhs_index, nulls_are_equal);
  };
...
struct typed_row_inequality_comparator {
  ...
  __device__ State operator()(cudf::size_type lhs_row,
...
struct row_inequality_comparator {
  ...
  __device__ inline bool operator()(cudf::size_type lhs_index, cudf::size_type rhs_index) const
  {
    ...
    for (cudf::size_type col_index = 0; col_index < _lhs.num_columns(); ++col_index) {
      ...
      if (asc) {
        state = cudf::type_dispatcher(col_type,
                                      typed_row_inequality_comparator<nullable>{},
                                      lhs_index,
                                      rhs_index,
