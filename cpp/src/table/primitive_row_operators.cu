 ...
 */
template <typename Element, CUDF_ENABLE_IF(cuda::std::is_floating_point_v<Element>)>
 ...
 */
template <typename Element, CUDF_ENABLE_IF(not cuda::std::is_floating_point_v<Element>)>
__device__ weak_ordering relational_compare(Element lhs, Element rhs)
{
  return detail::compare_elements(lhs, rhs);
}
