...
struct point_in_polygon_functor {
    ...
    void operator()(gdf_column const & d_poly_lats, gdf_column const & d_poly_lons,
                    gdf_column const & d_point_lats, gdf_column const & d_point_lons,
                    gdf_column const & d_point_is_in_polygon)
    {
                            ...
                            gdf_column const & point_lats, gdf_column const & point_lons)
{
    ...
    CUDF_EXPECTS(point_lats.data != nullptr && point_lons.data != nullptr, "query points are empty");
