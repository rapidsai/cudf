...
template <typename Exec,
          ...
          typename IndexType = typename thrust::iterator_difference<InputIterator>::type>
                                ...
                                cudaStream_t stream        = 0)
{
  ...
  CUDF_TRY(gdf_column_view(
    &sorted_indices_col, (void*)(sorted_indices.data().get()), nullptr, nrows, GDF_INT32));
  CUDF_TRY(gdf_order_by(keys.begin(), nullptr, keys.num_columns(), &sorted_indices_col, &context));
  ...
  if (cudf::has_nulls(keys)) {
    auto comp  = row_equality_comparator<true>(*device_input_table, nulls_are_equal);
    ...
    auto comp  = row_equality_comparator<false>(*device_input_table, nulls_are_equal);
    result_end = unique_copy(rmm::exec_policy(stream)->on(stream),
                             sorted_indices.begin(),
                             sorted_indices.end(),
                             unique_indices.begin(),
                             ...
                             cudaStream_t stream        = 0)
{
  ...
  CUDF_TRY(gdf_column_view(&sorted_indices_col,
                           static_cast<void*>(sorted_indices.data().get()),
                           nullptr,
                           nrows,
                           GDF_INT32));
  CUDF_TRY(gdf_order_by(keys.begin(), nullptr, keys.num_columns(), &sorted_indices_col, &context));
  ...
  if (cudf::has_nulls(keys)) {
    ...
    return thrust::count_if(
      ...
      [sorted_row_index, comp] __device__(const cudf::size_type i) {
...
rows in input table should be equal to number of rows in key colums table");
  ...
  if (0 == input.num_rows() || 0 == input.num_columns() || 0 == keys.num_columns()) {
  ...
  cudf::table destination_table(
    unique_count, cudf::column_dtypes(input), cudf::column_dtype_infos(input), true);
