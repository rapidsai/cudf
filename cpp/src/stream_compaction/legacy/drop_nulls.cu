...
struct valid_table_filter {
                     ...
                     cudaStream_t stream = 0)
  {
    ...
    std::transform(std::cbegin(table), std::cend(table), std::begin(h_masks), [](auto col) {
      return reinterpret_cast<bit_mask_t *>(col->valid);
    });
    ...
    CUDA_TRY(
      cudaMemcpyAsync(device_masks, h_masks.data(), masks_size, cudaMemcpyHostToDevice, stream));
    CHECK_CUDA(stream);

    auto deleter = [stream](valid_table_filter *f) { f->destroy(stream); };
    std::unique_ptr<valid_table_filter, decltype(deleter)> p{
      new valid_table_filter(device_masks, table.num_columns(), keep_threshold), deleter};
...
table drop_nulls(table const &input, table const &keys, cudf::size_type keep_threshold)
{
  ...
  CUDF_EXPECTS(keys.num_rows() <= input.num_rows(), "Column size mismatch");
