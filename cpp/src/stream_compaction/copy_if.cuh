...
template <bool has_validity, int block_size>
struct scatterer {
  template <typename T>
  __device__ void operator()(gdf_column * __restrict__ out_col,
                             gdf_column const * __restrict__ in_col,
                             gdf_size_type *null_count,
                             bool mask_true, 
                             int tid, 
                             gdf_index_type local_index,
                             gdf_size_type block_sum,
                             gdf_size_type block_offset) {
    T const * __restrict__ input_data = reinterpret_cast<T*>(in_col->data);
    T * __restrict__ output_data = reinterpret_cast<T*>(out_col->data);
    ...
    T* temp_data = reinterpret_cast<T*>(sdata);
    ...
    if (has_validity && threadIdx.x < warp_size)
      warp_valid_counts[threadIdx.x] = 0;
    ...
    if (has_validity) {
      ...
      if (threadIdx.x < warp_size) {
        ...
        if (lane == 0) // one thread computes and adds to null count
          atomicAdd(null_count, block_sum - block_valid_count);
      }
    }
  }
};
                               ...
                               Filter filter)
{
  ...
  for (int i = 0; i < per_thread; i++) {
    ...
    for(int col_i = 0; col_i < input.num_columns(); col_i++) {
      ...
      gdf_size_type *null_count = &null_counts[col_i];
      
      cudf::type_dispatcher(in_col->dtype, scatterer<has_validity, block_size>{},
                            out_col, in_col, null_count,
                            mask_true, tid, local_index,
                            block_sum, block_offset);
    }
...
template <typename T, typename Filter, int block_size, int per_thread, bool has_validity>
__launch_bounds__(block_size, 2048/block_size)
__global__ void scatter_kernel_single(device_table output,
                                      device_table const input,
                                      gdf_size_type *null_count,
                               //T* __restrict__ output_data,
                               //bit_mask_t * __restrict__ output_valid,
                                      ...
                                      Filter filter)
{
  ...
  for (int i = 0; i < per_thread; i++) {
    ...
    gdf_column const *in_col = input.get_column(0);
    gdf_column *out_col = const_cast<gdf_column*>(output.get_column(0));

    scatterer<has_validity, block_size> scatter{};
    scatter.template operator()<T>(out_col, in_col, null_count, 
                                   mask_true, tid, local_index, 
                                   block_sum, block_offset);
...
// Dispatch functor which performs the scatter
template <typename Filter, int block_size, int per_thread>
struct scatter_single_functor 
{
  template <typename T>
  void operator()(cudf::table & output,
                  cudf::table const & input,
                  gdf_size_type  *block_offsets,
                  Filter filter,
                  bool has_valid,
                  cudaStream_t stream = 0) {
    cudf::util::cuda::grid_config_1d grid{input.num_rows(),
                                          block_size, per_thread};
    
    auto scatter = (has_valid) ?
      scatter_kernel_single<T, Filter, block_size, per_thread, true> :
      scatter_kernel_single<T, Filter, block_size, per_thread, false>;

    auto d_input = device_table::create(input, stream);
    const auto d_output = device_table::create(output, stream);
    rmm::device_vector<gdf_size_type> output_null_count(1, 0);
      gdf_size_type *d_null_count = 
        thrust::raw_pointer_cast(output_null_count.data());
    
    scatter<<<grid.num_blocks, block_size, block_size * sizeof(int64_t), stream>>>
      (*d_output, *d_input, d_null_count, block_offsets, filter);

    cudaMemcpyAsync(&output.get_column(0)->null_count, d_null_count, 
                    sizeof(gdf_size_type), cudaMemcpyDefault, stream);
  }
};
...
*/
    ...
    if (input.num_columns() == 1) {
      cudf::type_dispatcher(output.get_column(0)->dtype,
                            scatter_single_functor<Filter, block_size, per_thread>{},
      ...
      rmm::device_vector<gdf_size_type> output_null_counts(input.num_columns(), 0);
      gdf_size_type *d_null_counts = 
        thrust::raw_pointer_cast(output_null_counts.data());

      scatter<<<grid.num_blocks, block_size, block_size * sizeof(int64_t), stream>>>
        (*d_output, *d_input, d_null_counts, block_offsets, filter);

      gdf_size_type *h_null_counts = nullptr;
      CUDA_TRY(cudaMallocHost(&h_null_counts, 
                              input.num_columns() * sizeof(gdf_size_type)));
      CUDA_TRY(cudaMemcpyAsync(h_null_counts, d_null_counts,
                               input.num_columns() * sizeof(gdf_size_type),
                               cudaMemcpyDefault, stream));
      int c = 0;
      for (auto col : output) { col->null_count = h_null_counts[c++]; }
      CUDA_TRY(cudaFreeHost(h_null_counts));
    }
