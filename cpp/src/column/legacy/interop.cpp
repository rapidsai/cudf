...
data_type gdf_dtype_to_data_type(gdf_dtype dtype)
{
  switch (dtype) {
    ...
    default: CUDF_FAIL("Unsupported `gdf_dtype` for conversion to `data_type`.");
  }
}
...
gdf_dtype data_type_to_gdf_dtype(data_type type)
{
  switch (type.id()) {
    ...
    default: CUDF_FAIL("Unsupported `data_type` for conversion to `gdf_dtype.");
  }
}
...
mutable_column_view gdf_column_to_mutable_view(gdf_column* col)
{
  ...
  return mutable_column_view(gdf_dtype_to_data_type(col->dtype),
                             col->size,
                             col->data,
                             reinterpret_cast<bitmask_type*>(col->valid),
                             col->null_count);
}
...
gdf_column view_to_gdf_column(mutable_column_view view)
{
  ...
  return gdf_column{view.head(),
                    ...
                    view.size(),
                    data_type_to_gdf_dtype(view.type()),
                    view.null_count()};
}
}  // namespace legacy
}  // namespace cudf
