...
create_device_view_from_view(ColumnView const& source, stream_t const& stream)
{
  size_type num_children = source.num_children();
...
column_device_view::create(column_view source, stream_t const& stream)
{
  size_type num_children = source.num_children();
  if (num_children == 0) {
...
mutable_column_device_view::create(mutable_column_view source, stream_t const& stream)
{
