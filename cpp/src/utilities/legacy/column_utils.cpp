...
void validate(const gdf_column& column)
{
  ...
  CUDF_EXPECTS((is_nullable(column) or (column.null_count == 0)),
               "Null validity mask with non-zero null count");
...
bool have_same_type(const gdf_column& validated_column_1,
                    ...
                    bool ignore_extra_type_info) noexcept
{
  ...
  return detail::extra_type_info_is_compatible(
