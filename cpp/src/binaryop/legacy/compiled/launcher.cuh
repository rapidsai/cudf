                              ...
                              F functor)
{
  ...
  for (int i = start; i < size; i += step) { results[i] = functor.apply(lhs_data[i], rhs_data[i]); }
}
...
struct BinaryOp {
  static gdf_error launch(gdf_column *lhs, gdf_column *rhs, gdf_column *output)
  {
    ...
    CUDA_TRY(
      cudaOccupancyMaxPotentialBlockSize(&mingridsize, &blocksize, gpu_binary_op<T, Tout, F>));
