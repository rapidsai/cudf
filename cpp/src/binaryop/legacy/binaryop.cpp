                           ...
                           cudf::size_type num_values)
{
  ...
  if (valid_left != nullptr && valid_right != nullptr) {
    ...
    auto error = apply_bitmask_to_bitmask(
      out_null_count, valid_out, valid_left, valid_right, stream, num_values);
                      ...
                      const std::string& output_type)
{
  ...
  cudf::jit::launcher(ptx_hash, cuda_source, headers_name, compiler_flags, headers_code)
    .set_kernel_inst("kernel_v_v",                         // name of the kernel we are launching
                     {cudf::jit::getTypeName(out->dtype),  // list of template arguments
                      cudf::jit::getTypeName(lhs->dtype),
                      cudf::jit::getTypeName(rhs->dtype),
...
void binary_operation(gdf_column* out, gdf_scalar* lhs, gdf_column* rhs, gdf_binary_operator ope)
{
  ...
  CUDF_EXPECTS((out->data != nullptr) && (rhs->data != nullptr), "Column data pointers are null");
  ...
  binops::scalar_col_valid_mask_and(
    out->null_count, out->valid, rhs->valid, lhs->is_valid, rhs->size);
...
void binary_operation(gdf_column* out, gdf_column* lhs, gdf_scalar* rhs, gdf_binary_operator ope)
{
  ...
  // Check for null data pointer
  CUDF_EXPECTS((out->data != nullptr) && (lhs->data != nullptr), "Column data pointers are null");
  ...
  binops::scalar_col_valid_mask_and(
    out->null_count, out->valid, lhs->valid, rhs->is_valid, lhs->size);
...
void binary_operation(gdf_column* out, gdf_column* lhs, gdf_column* rhs, gdf_binary_operator ope)
{
  ...
  CUDF_EXPECTS((out->size == lhs->size) && (lhs->size == rhs->size), "Column sizes don't match");

  // Check for null data pointer
  CUDF_EXPECTS((out->data != nullptr) && (lhs->data != nullptr) && (rhs->data != nullptr),
  ...
  if (lhs->dtype == GDF_STRING_CATEGORY && rhs->dtype == GDF_STRING_CATEGORY) {
    ...
    NVCategory::destroy(reinterpret_cast<NVCategory*>(temp_lhs.dtype_info.category));
    NVCategory::destroy(reinterpret_cast<NVCategory*>(temp_rhs.dtype_info.category));
    return;
  }
                            ...
                            gdf_dtype output_type)
{
  ...
  // Check for null data pointer
  CUDF_EXPECTS((lhs.data != nullptr) && (rhs.data != nullptr), "Column data pointers are null");

  // Check for datatype
  CUDF_EXPECTS(lhs.dtype == GDF_FLOAT32 || lhs.dtype == GDF_FLOAT64 || lhs.dtype == GDF_INT64 ||
                 lhs.dtype == GDF_INT32 || lhs.dtype == GDF_INT16,
               "Invalid/Unsupported lhs datatype");
  CUDF_EXPECTS(rhs.dtype == GDF_FLOAT32 || rhs.dtype == GDF_FLOAT64 || rhs.dtype == GDF_INT64 ||
                 rhs.dtype == GDF_INT32 || rhs.dtype == GDF_INT16,
               "Invalid/Unsupported rhs datatype");

  binops::binary_valid_mask_and(output.null_count, output.valid, lhs.valid, rhs.valid, rhs.size);

  binops::jit::binary_operation(&output, &lhs, &rhs, ptx, cudf::jit::getTypeName(output_type));
