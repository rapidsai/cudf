...
struct Add {
  ...
  CUDF_DI auto operator()(T1 const& lhs, T2 const& rhs) -> decltype(lhs + rhs)
  {
    return lhs + rhs;
  }
};
...
struct TrueDiv {
  template <typename T1, typename T2>
  CUDF_DI auto operator()(T1 const& lhs, T2 const& rhs)
...
struct Mod {
  ...
  CUDF_DI auto operator()(T1 const& lhs, T2 const& rhs) -> float
  {
  ...
  CUDF_DI auto operator()(T1 const& lhs, T2 const& rhs) -> double
  {
...
struct PMod {
  ...
  CUDF_DI auto operator()(TypeLhs x, TypeRhs y)
  {
...
struct PyMod {
  template <typename TypeLhs,
            typename TypeRhs,
            std::enable_if_t<(std::is_integral_v<std::common_type_t<TypeLhs, TypeRhs>>)>* = nullptr>
  CUDF_DI auto operator()(TypeLhs x, TypeRhs y) -> decltype(((x % y) + y) % y)
  {
    return ((x % y) + y) % y;
  }
  ...
  template <
    ...
    typename TypeRhs,
    std::enable_if_t<(std::is_floating_point_v<std::common_type_t<TypeLhs, TypeRhs>>)>* = nullptr>
  CUDF_DI auto operator()(TypeLhs x, TypeRhs y) -> double
  {
...
struct ShiftLeft {
  template <typename TypeLhs, typename TypeRhs>
  CUDF_DI auto operator()(TypeLhs x, TypeRhs y) -> decltype(x << y)
  {
    return (x << y);
  }
};
...
struct NullEquals {
  template <typename TypeLhs, typename TypeRhs>
  CUDF_DI auto operator()(TypeLhs x, TypeRhs y, bool lhs_valid, bool rhs_valid, bool& output_valid)
    -> decltype(x == y)
  {
    output_valid = true;
