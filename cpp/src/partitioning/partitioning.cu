...
__global__ void compute_row_partition_numbers(
    row_hasher_t the_hasher, const size_type num_rows,
    const size_type num_partitions, const partitioner_type the_partitioner,
  ...
  while (partition_number < num_partitions) {
    const size_type block_partition_size =
 ...
 */
/* ----------------------------------------------------------------------------*/
__global__ void compute_row_output_locations(
    size_type* __restrict__ row_partition_numbers, const size_type num_rows,
...
__global__ void copy_block_partitions(
    InputIter input_iter, DataType* __restrict__ output_buf,
    const size_type num_rows, const size_type num_partitions,
  ...
  for (int i = 0; i < ELEMENTS_PER_THREAD; ++i) {
    if (ELEMENTS_PER_THREAD * threadIdx.x + i < num_partitions) {
      temp_histo[i] =
          block_partition_sizes[blockIdx.x +
                                (ELEMENTS_PER_THREAD * threadIdx.x + i) *
                                    gridDim.x];
    } else {
      temp_histo[i] = 0;
    }
  ...
  for (int i = 0; i < ELEMENTS_PER_THREAD; ++i) {
    if (ELEMENTS_PER_THREAD * threadIdx.x + i < num_partitions) {
      partition_offset_shared[ELEMENTS_PER_THREAD * threadIdx.x + i + 1] =
          temp_histo[i];
    }
  }
  ...
  for (size_type row_number = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    block_output[partition_offset_shared[ipartition] +
                 row_partition_offset[row_number]] = input_iter[row_number];
  }
  ...
  for (size_type ipartition = threadIdx.x / nthreads_partition;
    ...
    size_type const nelements_partition =
        partition_offset_shared[ipartition + 1] -
...
void copy_block_partitions_impl(InputIter const input, OutputIter output,
                                size_type num_rows, size_type num_partitions,
                                ...
                                size_type grid_size, cudaStream_t stream) {
  ...
  int const smem =
      OPTIMIZED_BLOCK_SIZE * OPTIMIZED_ROWS_PER_THREAD * sizeof(*output) +
  ...
  copy_block_partitions<<<grid_size, OPTIMIZED_BLOCK_SIZE, smem, stream>>>(
      ...
      row_partition_offset, block_partition_sizes,
...
rmm::device_vector<size_type> compute_gather_map(
    ...
    size_type const* row_partition_offset,
    size_type const* block_partition_sizes,
    size_type const* scanned_block_partition_sizes, size_type grid_size,
    cudaStream_t stream) {
  auto sequence = thrust::make_counting_iterator(0);
  rmm::device_vector<size_type> gather_map(num_rows);

  copy_block_partitions_impl(sequence, gather_map.data().get(), num_rows,
                             num_partitions, row_partition_numbers,
                             row_partition_offset, block_partition_sizes,
                             scanned_block_partition_sizes, grid_size, stream);
...
struct copy_block_partitions_dispatcher {
  ...
  std::unique_ptr<column> operator()(
      column_view const& input, const size_type num_partitions,
      size_type const* row_partition_numbers,
      size_type const* row_partition_offset,
      size_type const* block_partition_sizes,
      size_type const* scanned_block_partition_sizes, size_type grid_size,
    ...
    copy_block_partitions_impl(
        input.data<DataType>(), static_cast<DataType*>(output.data()),
        input.size(), num_partitions, row_partition_numbers,
        row_partition_offset, block_partition_sizes,
        scanned_block_partition_sizes, grid_size, stream);

    return std::make_unique<column>(input.type(), input.size(),
                                    std::move(output));
  }
  ...
  std::unique_ptr<column> operator()(
    ...
    auto gather_map =
        compute_gather_map(input.size(), num_partitions, row_partition_numbers,
                           row_partition_offset, block_partition_sizes,
                           scanned_block_partition_sizes, grid_size, stream);

    // Use gather instead for non-fixed width types
    return experimental::type_dispatcher(
        input.type(), experimental::detail::column_gatherer{}, input,
        gather_map.begin(), gather_map.end(), false, mr, stream);
  }
};
...
hash_partition_table(table_view const& input, table_view const& table_to_hash,
                     size_type num_partitions,
                     rmm::mr::device_memory_resource* mr, cudaStream_t stream) {
  auto const num_rows = table_to_hash.num_rows();

  bool const use_optimization{num_partitions <=
                              THRESHOLD_FOR_OPTIMIZED_PARTITION_KERNEL};
  auto const block_size =
      use_optimization ? OPTIMIZED_BLOCK_SIZE : FALLBACK_BLOCK_SIZE;
  ...
  auto block_partition_sizes =
      rmm::device_vector<size_type>(grid_size * num_partitions);
  ...
  auto global_partition_sizes =
      rmm::device_vector<size_type>(num_partitions, size_type{0});
  ...
  auto const hasher =
      experimental::row_hasher<MurmurHash3_32, hash_has_nulls>(*device_input);
  ...
  if (is_power_two(num_partitions)) {
    ...
    compute_row_partition_numbers<<<
        ...
        row_partition_numbers.data().get(), row_partition_offset.data().get(),
  ...
  thrust::exclusive_scan(
      rmm::exec_policy(stream)->on(stream), block_partition_sizes.begin(),
      block_partition_sizes.end(), scanned_block_partition_sizes.data().get());
  ...
  size_type* scanned_global_partition_sizes{
      global_partition_sizes.data().get()};
  thrust::exclusive_scan(
      rmm::exec_policy(stream)->on(stream), global_partition_sizes.begin(),
      global_partition_sizes.end(), scanned_global_partition_sizes);
  ...
  CUDA_TRY(cudaMemcpyAsync(
      partition_offsets.data(), scanned_global_partition_sizes,
      num_partitions * sizeof(size_type), cudaMemcpyDeviceToHost, stream));
  ...
  if (use_optimization) {
    ...
    auto scanned_block_partition_sizes_ptr{
        scanned_block_partition_sizes.data().get()};

    // Copy input to output by partition per column
    std::transform(
        input.begin(), input.end(), output_cols.begin(), [=](auto const& col) {
          return cudf::experimental::type_dispatcher(
              col.type(), copy_block_partitions_dispatcher{}, col,
              num_partitions, row_partition_numbers_ptr,
              row_partition_offset_ptr, block_partition_sizes_ptr,
              scanned_block_partition_sizes_ptr, grid_size, mr, stream);
        });

    if (has_nulls(input)) {
      // Use copy_block_partitions to compute a gather map
      auto gather_map = compute_gather_map(
          num_rows, num_partitions, row_partition_numbers_ptr,
          row_partition_offset_ptr, block_partition_sizes_ptr,
          scanned_block_partition_sizes_ptr, grid_size, stream);

      // Handle bitmask using gather to take advantage of ballot_sync
      experimental::detail::gather_bitmask(
          input, gather_map.begin(), output_cols,
    ...
    compute_row_output_locations<<<
        grid_size, block_size, num_partitions * sizeof(size_type), stream>>>(
        row_output_locations, num_rows, num_partitions,
...
hash_partition(table_view const& input,
  ...
  if (num_partitions <= 0 || input.num_rows() == 0 ||
      table_to_hash.num_columns() == 0) {
    return std::make_pair(experimental::empty_like(input),
                          std::vector<size_type>{});
  }

  if (has_nulls(table_to_hash)) {
    return hash_partition_table<true>(input, table_to_hash, num_partitions, mr,
                                      stream);
  } else {
    return hash_partition_table<false>(input, table_to_hash, num_partitions, mr,
                                       stream);
  }
}
