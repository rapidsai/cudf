...
struct DeviceCast {
  __device__ TypeTo apply(TypeFrom data) { return static_cast<TypeTo>(cudf::detail::unwrap(data)); }
};
...
struct CastDateTo_Dispatcher {
  ...
  operator()(gdf_column const* input, gdf_column* output)
  {
    if (input->dtype == GDF_DATE64 && output->dtype == GDF_DATE32)
      ...
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, MILLISECONDS_IN_DAY>>::
        launch(input, output);
    ...
    if (input->dtype == GDF_DATE32 &&
      ...
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, SECONDS_IN_DAY>>::launch(
        input, output);
             ...
             (output->dtype == GDF_TIMESTAMP && output->dtype_info.time_unit == TIME_UNIT_ms))
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, MILLISECONDS_IN_DAY>>::
        launch(input, output);
    else if (input->dtype == GDF_DATE32 &&
             (output->dtype == GDF_TIMESTAMP && output->dtype_info.time_unit == TIME_UNIT_us))
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, MICROSECONDS_IN_DAY>>::
        launch(input, output);
    else if (input->dtype == GDF_DATE32 &&
             (output->dtype == GDF_TIMESTAMP && output->dtype_info.time_unit == TIME_UNIT_ns))
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, NANOSECONDS_IN_DAY>>::
        launch(input, output);
    else if (input->dtype == GDF_DATE64 && output->dtype == GDF_TIMESTAMP &&
             output->dtype_info.time_unit == TIME_UNIT_us)
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, METRIC_FACTOR>>::launch(
        input, output);
    else if (input->dtype == GDF_DATE64 && output->dtype == GDF_TIMESTAMP &&
             output->dtype_info.time_unit == TIME_UNIT_s)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, METRIC_FACTOR>>::launch(
        input, output);
    else if (input->dtype == GDF_DATE64 && output->dtype == GDF_TIMESTAMP &&
             output->dtype_info.time_unit == TIME_UNIT_ms)
      cudf::unary::Launcher<TypeFrom, TypeTo, DeviceCast<TypeFrom, TypeTo>>::launch(input, output);
    else if (input->dtype == GDF_DATE64 && output->dtype == GDF_TIMESTAMP &&
             output->dtype_info.time_unit == TIME_UNIT_ns)
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, METRIC_FACTOR_SQ>>::
        launch(input, output);
    else
  ...
  template <typename TypeTo>
  typename std::enable_if_t<std::is_arithmetic<TypeTo>::value ||
...
struct CastTimestampTo_Dispatcher {
  ...
  operator()(gdf_column const* input, gdf_column* output)
  {
    if ((input->dtype == GDF_TIMESTAMP && input->dtype_info.time_unit == TIME_UNIT_s) &&
      ...
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, SECONDS_IN_DAY>>::
        launch(input, output);
    else if ((input->dtype == GDF_TIMESTAMP && input->dtype_info.time_unit == TIME_UNIT_ms) &&
             output->dtype == GDF_DATE32)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, MILLISECONDS_IN_DAY>>::
        launch(input, output);
    else if ((input->dtype == GDF_TIMESTAMP && input->dtype_info.time_unit == TIME_UNIT_us) &&
             output->dtype == GDF_DATE32)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, MICROSECONDS_IN_DAY>>::
        launch(input, output);
    else if ((input->dtype == GDF_TIMESTAMP && input->dtype_info.time_unit == TIME_UNIT_ns) &&
             output->dtype == GDF_DATE32)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, NANOSECONDS_IN_DAY>>::
        launch(input, output);
    else if ((input->dtype == GDF_TIMESTAMP && input->dtype_info.time_unit == TIME_UNIT_s) &&
             output->dtype == GDF_DATE64)
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, METRIC_FACTOR>>::launch(
        input, output);
    else if ((input->dtype == GDF_TIMESTAMP && input->dtype_info.time_unit == TIME_UNIT_ms) &&
             output->dtype == GDF_DATE64)
      ...
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, METRIC_FACTOR>>::launch(
        input, output);
             ...
             output->dtype == GDF_DATE64)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, METRIC_FACTOR_SQ>>::
        launch(input, output);
    else
      CUDF_FAIL("Unsupported datatype");
  }
    ...
    gdf_column const* input, gdf_column* output)
  {
    if (input->dtype_info.time_unit == output->dtype_info.time_unit)
      ...
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, METRIC_FACTOR>>::launch(
        input, output);
    else if (input->dtype_info.time_unit == TIME_UNIT_ms &&
             output->dtype_info.time_unit == TIME_UNIT_s)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, METRIC_FACTOR>>::launch(
        input, output);
    else if (input->dtype_info.time_unit == TIME_UNIT_s &&
      ...
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, METRIC_FACTOR_SQ>>::
        launch(input, output);
    else if (input->dtype_info.time_unit == TIME_UNIT_us &&
             output->dtype_info.time_unit == TIME_UNIT_s)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, METRIC_FACTOR_SQ>>::
        launch(input, output);
    else if (input->dtype_info.time_unit == TIME_UNIT_s &&
             output->dtype_info.time_unit == TIME_UNIT_ns)
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, METRIC_FACTOR_CUBE>>::
        launch(input, output);
    else if (input->dtype_info.time_unit == TIME_UNIT_ns &&
             output->dtype_info.time_unit == TIME_UNIT_s)
      cudf::unary::Launcher<TypeFrom, TypeTo, DownCasting<TypeFrom, TypeTo, METRIC_FACTOR_CUBE>>::
        launch(input, output);
    else if (input->dtype_info.time_unit == TIME_UNIT_us &&
             output->dtype_info.time_unit == TIME_UNIT_ns)
      cudf::unary::Launcher<TypeFrom, TypeTo, UpCasting<TypeFrom, TypeTo, METRIC_FACTOR>>::launch(
...
gdf_column cast(gdf_column const& input, gdf_dtype out_type, gdf_dtype_extra_info out_info)
{
  ...
  cudf::type_dispatcher(input.dtype, detail::CastFrom_Dispatcher{}, &input, &output);
