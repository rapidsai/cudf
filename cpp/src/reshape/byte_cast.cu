...
struct byte_list_conversion_dispatcher {
  ...
  {
    return byte_list_conversion_fn<T>::invoke(input, configuration, stream, resources);
  }
};
...
struct byte_list_conversion_fn<T, std::enable_if_t<cudf::is_numeric<T>()>> {
  ...
  {
    if (input.size() == 0) {
      return cudf::lists::detail::make_empty_lists_column(output_type, stream, resources);
    }
    ...
    auto byte_column =
      make_numeric_column(output_type, num_bytes, mask_state::UNALLOCATED, stream, resources);
    ...
    if (auto const result_cv = result->view();
      ...
      return cudf::detail::purge_nonempty_nulls(result_cv, stream, resources);
    }
...
{
  CUDF_FUNC_RANGE();
  return detail::byte_cast(input, endian_configuration, stream, resources);
}
