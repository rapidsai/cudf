...
/**
 * @copydoc
 * stable_sorted_order(column_view&,order,null_order,cuda::stream_ref,memory_resources)
 */
template <>
...
{
  auto sorted_indices              = cudf::make_numeric_column(data_type(type_to_id<size_type>()),
                                                  input.size(),
  ...
  if (is_radix_sortable(input)) {
    sorted_order_radix(
      input, indices_view, column_order == order::ASCENDING, stream, mr.get_temporary_mr());
  } else {
