...
std::unique_ptr<table> stable_sort_by_key(
    ...
    auto sorted_order = detail::stable_sorted_order(keys, column_order, null_precedence, mr, stream);
...
std::unique_ptr<table> stable_sort(table_view input,
                             std::vector<order> const& column_order,
                             std::vector<null_order> const& null_precedence,
                             rmm::mr::device_memory_resource* mr) {
    return detail::stable_sort_by_key(input, input, column_order, null_precedence, mr);
}
...
std::unique_ptr<table> stable_sort_by_key(
    ...
    return detail::stable_sort_by_key(values, keys, column_order, null_precedence, mr);
}
