               ...
               rmm::cuda_stream_view stream)
{
  ...
  if (cudf::detail::has_nested_columns(in)) {
    ...
    // in thrust::is_sorted.
    auto d_results = rmm::device_uvector<bool>(in.num_rows(), stream, resources.get_temporary_mr());
    ...
    return thrust::count(rmm::exec_policy(stream, resources.get_temporary_mr()), d_results.begin(), d_results.end(), false) == 0;
  } else {
    auto const device_comparator = comparator.less<false>(has_nested_nulls(in));

    return thrust::is_sorted(rmm::exec_policy(stream, resources.get_temporary_mr()),
                             thrust::counting_iterator<size_type>(0),
                             thrust::counting_iterator<size_type>(in.num_rows()),
                             device_comparator);
  }
