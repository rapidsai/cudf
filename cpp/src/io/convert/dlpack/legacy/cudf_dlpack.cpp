...
gdf_dtype DLDataType_to_gdf_dtype(DLDataType type) {
  ...
  switch (type.bits) {
    ...
    case 32:
      return (type.code == kDLInt) ? GDF_INT32
                                   : (type.code == kDLFloat) ? GDF_FLOAT32 : GDF_invalid;
    case 64:
      return (type.code == kDLInt) ? GDF_INT64
...
gdf_error gdf_from_dlpack(gdf_column **columns,
  ...
  if (tensor->dl_tensor.ndim > 1) GDF_REQUIRE(tensor->dl_tensor.shape[1] > 0, GDF_DATASET_EMPTY);

  GDF_REQUIRE(tensor->dl_tensor.shape[0] < std::numeric_limits<cudf::size_type>::max(),
  ...
  *columns = new gdf_column[*num_columns]{};
  ...
  for (cudf::size_type c = 0; c < *num_columns; ++c) {
    void *tensor_data =
      reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(tensor->dl_tensor.data) +
                               tensor->dl_tensor.byte_offset + col_stride * c);
    ...
    gdf_error status = gdf_column_view(&(*columns)[c], col_data, nullptr, num_rows, dtype);
...
gdf_error gdf_to_dlpack(DLManagedTensor *tensor,
  ...
  tensor->dl_tensor.dtype = cudf::type_dispatcher(type, gdf_dtype_to_DLDataType());
  ...
  for (cudf::size_type i = 0; i < num_columns; ++i) {
    CUDA_TRY(cudaMemcpy(d, columns[i]->data, column_bytesize, cudaMemcpyDefault));
    d += column_bytesize;
  }
  ...
  tensor->deleter = [](DLManagedTensor *arg) {
    ...
    if (arg->dl_tensor.ctx.device_type == kDLGPU) RMM_TRY(RMM_FREE(arg->dl_tensor.data, 0));
