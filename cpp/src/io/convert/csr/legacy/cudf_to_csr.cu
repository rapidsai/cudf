...
template <typename T>
__global__ void cudaCreateCSR(void *data,
...
__global__ void determineValidRecCount(cudf::valid_type *validArray,
                                       cudf::size_type numRows,
...
gdf_error gdf_to_csr(gdf_column **gdfData, int numCol, csr_gdf *csrReturn)
{
  ...
  CUDA_TRY(
    cudaMemcpy((void *)&nnz, (void *)&offsets[numRows], sizeof(int64_t), cudaMemcpyDeviceToHost));

  if (nnz == 0) return GDF_CUDA_ERROR;
  ...
  CUDA_TRY(
    cudaMemcpy(IA, offsets, (sizeof(cudf::size_type) * (numRows + 2)), cudaMemcpyDeviceToDevice));
  ...
  switch (dType) {
    case gdf_dtype::GDF_INT8: status = runConverter<int8_t>(gdfData, csrReturn, offsets); break;
    case gdf_dtype::GDF_INT16: status = runConverter<int16_t>(gdfData, csrReturn, offsets); break;
    case gdf_dtype::GDF_INT32: status = runConverter<int32_t>(gdfData, csrReturn, offsets); break;
    case gdf_dtype::GDF_INT64: status = runConverter<int64_t>(gdfData, csrReturn, offsets); break;
    case gdf_dtype::GDF_FLOAT32: status = runConverter<float>(gdfData, csrReturn, offsets); break;
    case gdf_dtype::GDF_FLOAT64: status = runConverter<double>(gdfData, csrReturn, offsets); break;
    default:
...
gdf_error runConverter(gdf_column **gdfData, csr_gdf *csrReturn, cudf::size_type *offsets)
{
  ...
  for (cudf::size_type colId = 0; colId < numCols; colId++) {
    ...
    cudaCreateCSR<T><<<blocks, threads>>>(
      gdf->data, gdf->valid, gdf->dtype, colId, A, csrReturn->JA, offsets, numRows);
                                       ...
                                       cudf::size_type *offset)
{
  ...
  if (checkBitCSR(bitmap, bitIdx)) ++offset[tid];
}
