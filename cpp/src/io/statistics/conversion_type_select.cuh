...
class DetectInnerIteration<N0, 0, std::tuple<T...>> {
  ...
  static constexpr bool is_duplicate =
    std::is_same_v<
    typename std::tuple_element<N0, std::tuple<T...>>::type,
...
class Detect<std::tuple<T...>> {
  public:
    static constexpr bool is_duplicate = DetectIteration<(sizeof...(T)-1), std::tuple<T...>>::is_duplicate;
};
...
class ConversionTypeSelect<std::tuple<I0>> {
  ...
  using type = std::conditional_t<
    std::is_same_v<T, typename std::tuple_element<0, I0>::type>,
    typename std::tuple_element<1, I0>::type, T>;
};
...
class ConversionTypeSelect<std::tuple<I0, In...>> {
  ...
  static_assert(not Detect<std::tuple<I0, In...>>::is_duplicate, "Type tuple has duplicate first entries");
};
