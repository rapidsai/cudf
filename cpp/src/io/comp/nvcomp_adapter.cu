...
std::pair<size_t, size_t> max_chunk_and_total_input_size(device_span<size_t const> input_sizes,
                                                         rmm::cuda_stream_view stream)
{
  auto const max = thrust::reduce(rmm::exec_policy_nosync(stream, resources.get_temporary_mr()),
                                  input_sizes.begin(),
                                  input_sizes.end(),
                                  0ul,
                                  cuda::maximum<size_t>());
  auto const sum = thrust::reduce(rmm::exec_policy_nosync(stream, resources.get_temporary_mr()),
                                  input_sizes.begin(),
                                  input_sizes.end());
  return {max, sum};
}
