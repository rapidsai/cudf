...
public:
    ...
    BitReader(const uint8_t *buffer, int buffer_len)
      : buffer_(buffer), max_bytes_(buffer_len), byte_offset_(0),
        bit_offset_(0) {
    ...
    /// or false if there are not enough bytes left. num_bits must be <= 32.
    template <typename T>
    bool GetValue(int num_bits, T *v);

    template <typename T>
    void SetGpuBatchMetadata(int               num_bits,
                             T *               v,
                             int               batch_size,
                             int               values_read,
    ...
    /// current byte (i.e., there may be an additional fraction of a byte).
    int bytes_left() {
        return max_bytes_
               - (byte_offset_
                  + static_cast<int>(
                    ::arrow::BitUtil::CeilDiv(bit_offset_, 8)));
    }
...
template <typename T>
inline void GetValue_(int            num_bits,
                      T *            v,
                      int            max_bytes,
...
#endif
    *v = static_cast<T>(
      ::arrow::BitUtil::TrailingBits(*buffered_values, *bit_offset + num_bits)
      >> *bit_offset);
#ifdef _MSC_VER
#pragma warning(pop)
#endif
...
#pragma warning(disable : 4800 4805)
        ...
        *v = *v
             | static_cast<T>(
               ::arrow::BitUtil::TrailingBits(*buffered_values, *bit_offset)
               << (num_bits - *bit_offset));
#ifdef _MSC_VER
#pragma warning(pop)
#endif
...
template <typename T>
inline void
BitReader::SetGpuBatchMetadata(int               num_bits,
                               T *               v,
                               int               batch_size,
                               int               values_read,
                               std::vector<int> &unpack32InputOffsets,
    ...
    if (ARROW_PREDICT_FALSE(bit_offset != 0)) {
        ...
        int count = 0;
        for (; i < batch_size && bit_offset != 0;
             ++i) {  // TODO this loop can be replaced with math
            bit_offset += num_bits;
            if (bit_offset >= 64) {
                byte_offset += 8;
                bit_offset -= 64;
...
inline bool BitReader::GetAligned(int num_bytes, T *v) {
    ...
    int bytes_read =
      static_cast<int>(::arrow::BitUtil::CeilDiv(bit_offset_, 8));
