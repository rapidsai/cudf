...
void reader_impl::compute_output_chunks_for_subpass()
{
  ...
  // Check for string column constraints upfront
  auto const has_string_limit =
    (subpass.kernel_mask & STRINGS_MASK) && !cudf::strings::is_large_strings_enabled();

  // simple case : no chunk size, no splits
  if (_output_chunk_read_limit <= 0 && !has_string_limit) {
  ...
  // calculate string limit if needed
  auto const str_limit =
    has_string_limit
      ? static_cast<size_t>(std::max(int64_t{0}, cudf::strings::get_offset64_threshold() - 1))
      : 0;
  subpass.output_chunk_read_info = compute_page_splits_by_row(c_info,
                                                              subpass.pages,
                                                              subpass.skip_rows,
                                                              subpass.num_rows,
