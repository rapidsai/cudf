...
static inline int CountLeadingZeros32(uint32_t value) {
  ...
  unsigned long index;
  return (_BitScanReverse(&index, static_cast<unsigned long>(value)))
...
#undef DECL_PARQUET_STRUCT
 ...
 protected:
  int WalkSchema(std::vector<SchemaElement> &schema, int idx = 0,
                 int parent_idx = 0, int max_def_level = 0,
                 int max_rep_level = 0);
