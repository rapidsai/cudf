...
inline __device__ void gpuOutputString(volatile page_state_s *s, int src_pos, void *dstv)
{
  ...
  if (s->dict_base) {
    ...
    uint32_t dict_pos = (s->dict_bits > 0)
                          ? s->dict_idx[src_pos & (non_zero_buffer_size - 1)] * sizeof(string_pair)
                          : 0;
    if (dict_pos < (uint32_t)s->dict_size) {
      const string_pair *src = reinterpret_cast<const string_pair *>(s->dict_base + dict_pos);
      ptr                    = src->first;
      len                    = src->second;
    }
  } else {
  ...
  if (s->dtype_len == 4) {
    ...
    *static_cast<uint32_t *>(dstv) = device_str2hash32(ptr, len);
    ...
    string_pair *dst = static_cast<string_pair *>(dstv);
    dst->first       = ptr;
    dst->second      = len;
  }
}
  ...
  if (!t) {
    ...
    if (s->page.num_input_values > 0) {
      ...
      switch (s->col.data_type & 7) {
        ...
        case BYTE_ARRAY: s->dtype_len = sizeof(string_pair); break;
      ...
      switch (s->page.encoding) {
        ...
        case Encoding::RLE_DICTIONARY:
          ...
          if (((s->col.data_type & 7) == BYTE_ARRAY) && (s->col.str_dict_index)) {
            ...
            s->dict_size = s->col.page_info[0].num_input_values * sizeof(string_pair);
          } else {
            s->dict_base =
