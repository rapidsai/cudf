...
void hybrid_scan_reader_impl::decode_page_data(size_t skip_rows, size_t num_rows)
{
  ...
  size_t const sum_max_depths = std::accumulate(
    ...
    [&](auto cursum, ColumnChunkDesc const& chunk) {
      return cursum + _extended_metadata->get_output_nesting_depth(chunk.src_col_schema);
    });
  ...
  for (size_t c = 0, chunk_off = 0; c < pass.chunks.size(); c++) {
    ...
    size_t const max_depth =
      _extended_metadata->get_output_nesting_depth(pass.chunks[c].src_col_schema);
  ...
  auto decode_data = [&](decode_kernel_mask decoder_mask) {
    parquet::detail::decode_page_data(subpass.pages,
                                      pass.chunks,
                                      num_rows,
                                      skip_rows,
                                      level_type_size,
  ...
  if (BitAnd(kernel_mask, decode_kernel_mask::DELTA_BYTE_ARRAY) != 0) {
    parquet::detail::decode_delta_byte_array(subpass.pages,
                                             pass.chunks,
                                             num_rows,
                                             skip_rows,
                                             level_type_size,
  ...
  if (BitAnd(kernel_mask, decode_kernel_mask::DELTA_LENGTH_BA) != 0) {
    parquet::detail::decode_delta_length_byte_array(subpass.pages,
                                                    pass.chunks,
                                                    num_rows,
                                                    skip_rows,
                                                    level_type_size,
  ...
  if (BitAnd(kernel_mask, decode_kernel_mask::DELTA_BINARY) != 0) {
    parquet::detail::decode_delta_binary(subpass.pages,
                                         pass.chunks,
                                         num_rows,
                                         skip_rows,
                                         level_type_size,
  ...
  if (BitAnd(kernel_mask, decode_kernel_mask::BYTE_STREAM_SPLIT) != 0) {
    parquet::detail::decode_split_page_data(subpass.pages,
                                            pass.chunks,
                                            num_rows,
                                            skip_rows,
                                            level_type_size,
  ...
  parquet::detail::write_final_offsets(final_offsets, out_buffers, _stream);
