...
void check_input_row_group_indices(
  cudf::host_span<std::vector<cudf::size_type> const> row_group_indices)
{
  CUDF_EXPECTS(row_group_indices.size() == 1 and row_group_indices.front().size(),
               "Encountered invalid size of input row group indices");
}
...
std::vector<std::vector<cudf::size_type>> hybrid_scan_reader::filter_row_groups_with_stats(
  cudf::host_span<std::vector<cudf::size_type> const> row_group_indices,
  cudf::io::parquet_reader_options const& options,
  rmm::cuda_stream_view stream) const
{
  check_input_row_group_indices(row_group_indices);
...
hybrid_scan_reader::filter_row_groups_with_dictionary_pages(
...
{
  check_input_row_group_indices(row_group_indices);
  CUDF_EXPECTS(row_group_indices.front().size() == dictionary_page_data.size(),
...
std::vector<std::vector<cudf::size_type>> hybrid_scan_reader::filter_row_groups_with_bloom_filters(
...
{
  check_input_row_group_indices(row_group_indices);
  CUDF_EXPECTS(row_group_indices.front().size() == bloom_filter_data.size(),
