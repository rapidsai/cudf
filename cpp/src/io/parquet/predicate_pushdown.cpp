...
stats_caster_base::host_column<T>::host_column(size_type total_row_groups,
  ...
  : val{cudf::detail::make_host_vector<T>(total_row_groups, stream)},
    null_mask(cudf::util::div_rounding_up_safe<size_type>(
...
template <typename T>
void stats_caster_base::host_column<T>::set_index(
...
template <typename T>
std::tuple<rmm::device_uvector<char>, rmm::device_uvector<cudf::size_type>>
stats_caster_base::host_column<T>::make_strings_children(host_span<string_view> host_strings,
                                                         rmm::cuda_stream_view stream,
                                                         rmm::device_async_resource_ref mr)
{
  ...
  for (auto const& str : host_strings) {
    auto tmp = str.empty() ? std::string_view{} : std::string_view(str.data(), str.size_bytes());
...
template <typename T>
std::unique_ptr<column> stats_caster_base::host_column<T>::to_device(
  cudf::data_type dtype, rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr)
{
