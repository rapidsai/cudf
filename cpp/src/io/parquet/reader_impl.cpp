...
void reader::impl::decode_page_data(size_t skip_rows, size_t num_rows)
{
  ...
  // chunked reader).
  auto const has_strings = (kernel_mask & BitOr(gpu::decode_kernel_mask::STRING,
                                                gpu::decode_kernel_mask::DELTA_BYTE_ARRAY)) != 0;
  ...
  int s_idx = 0;
  if (BitAnd(kernel_mask, gpu::decode_kernel_mask::STRING) != 0) {
  ...
  if (BitAnd(kernel_mask, gpu::decode_kernel_mask::DELTA_BYTE_ARRAY) != 0) {
    gpu::DecodeDeltaByteArray(
      pages, chunks, num_rows, skip_rows, level_type_size, error_code.data(), streams[s_idx++]);
  }

  // launch delta binary decoder
  if (BitAnd(kernel_mask, gpu::decode_kernel_mask::DELTA_BINARY) != 0) {
    gpu::DecodeDeltaBinary(
      pages, chunks, num_rows, skip_rows, level_type_size, error_code.data(), streams[s_idx++]);
  }

  // launch the catch-all page decoder
  if (BitAnd(kernel_mask, gpu::decode_kernel_mask::GENERAL) != 0) {
    gpu::DecodePageData(
      pages, chunks, num_rows, skip_rows, level_type_size, error_code.data(), streams[s_idx++]);
  }
