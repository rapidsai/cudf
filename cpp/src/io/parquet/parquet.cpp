...
struct CompactProtocolWriter::CPWField
{
  ...
  template <typename T>
  inline void field_struct_list(int f, const std::vector<T> &v)
  {
    cpr->put_fldh(f, cur_fld, ST_FLD_LIST);
    cpr->putb((uint8_t)((std::min(v.size(), (size_t)0xfu) << 4) | ST_FLD_STRUCT));
    if (v.size() >= 0xf) cpr->put_uint(v.size());
    for (auto i = 0; i < v.size(); i++) { cpr->write(&v[i]); }
    cur_fld = f;
  }
