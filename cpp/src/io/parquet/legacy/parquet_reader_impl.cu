...
constexpr int32_t to_clockrate(gdf_time_unit time_unit)
{
  switch (time_unit) {
    ...
    case TIME_UNIT_us: return 1000000;
    case TIME_UNIT_ns: return 1000000000;
    default: return 0;
  }
}
 ...
 **/
constexpr std::pair<gdf_dtype, gdf_dtype_extra_info> to_dtype(parquet::Type physical,
                                                              ...
                                                              int32_t decimal_scale)
{
  ...
  switch (logical) {
    case parquet::UINT_8:
    case parquet::INT_8: return std::make_pair(GDF_INT8, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case parquet::UINT_16:
    case parquet::INT_16: return std::make_pair(GDF_INT16, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case parquet::DATE: return std::make_pair(GDF_DATE32, gdf_dtype_extra_info{TIME_UNIT_NONE});
  ...
  switch (physical) {
    case parquet::BOOLEAN: return std::make_pair(GDF_BOOL8, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case parquet::INT32: return std::make_pair(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case parquet::INT64: return std::make_pair(GDF_INT64, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case parquet::FLOAT: return std::make_pair(GDF_FLOAT32, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case parquet::DOUBLE: return std::make_pair(GDF_FLOAT64, gdf_dtype_extra_info{TIME_UNIT_NONE});
...
struct ParquetMetadata : public parquet::FileMetaData {
  ...
  std::string get_pandas_index_name()
  {
    ...
    if (it != key_value_metadata.end()) {
      ...
      if (pos != std::string::npos) {
        ...
        if ((end - begin) > 1) { return it->value.substr(begin + 2, end - begin - 3); }
      }
    }
    return "";
  }
  ...
  auto select_row_groups(int row_group, int &row_start, int &row_count)
  {
    ...
    if (row_group != -1) {
      ...
      for (int i = 0; i < row_group; ++i) { row_start += row_groups[i].num_rows; }
      selection.emplace_back(row_group, row_start);
      row_count = row_groups[row_group].num_rows;
    } else {
      row_start = std::max(row_start, 0);
      if (row_count == -1) { row_count = get_total_rows(); }
                      ...
                      const std::string &pandas_index)
  {
    ...
    if (use_names.empty()) {
      ...
      if (include_index) {
        if (std::find(use_names.begin(), use_names.end(), pandas_index) == use_names.end()) {
...
size_t reader::Impl::count_page_headers(hostdevice_vector<parquet::gpu::ColumnChunkDesc> &chunks)
{
  ...
  CUDA_TRY(cudaMemcpyAsync(
    chunks.device_ptr(), chunks.host_ptr(), chunks.memory_size(), cudaMemcpyHostToDevice));
  CUDA_TRY(parquet::gpu::DecodePageHeaders(chunks.device_ptr(), chunks.size()));
  CUDA_TRY(cudaMemcpyAsync(
    chunks.host_ptr(), chunks.device_ptr(), chunks.memory_size(), cudaMemcpyDeviceToHost));
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    LOG_PRINTF(
      ...
      c,
      (uint64_t)chunks[c].compressed_data,
      chunks[c].compressed_size,
      chunks[c].num_values,
      chunks[c].start_row,
      chunks[c].num_rows,
      chunks[c].max_def_level,
      chunks[c].max_rep_level,
      chunks[c].data_type,
      chunks[c].def_level_bits,
      chunks[c].rep_level_bits,
      chunks[c].num_data_pages,
      chunks[c].num_dict_pages,
                                       ...
                                       hostdevice_vector<parquet::gpu::PageInfo> &pages)
{
  ...
  CUDA_TRY(parquet::gpu::DecodePageHeaders(chunks.device_ptr(), chunks.size()));
  CUDA_TRY(cudaMemcpyAsync(
    pages.host_ptr(), pages.device_ptr(), pages.memory_size(), cudaMemcpyDeviceToHost));
  CUDA_TRY(cudaStreamSynchronize(0));
  ...
  for (size_t i = 0; i < pages.size(); i++) {
    LOG_PRINTF(
      ...
      pages[i].flags,
      pages[i].encoding,
      pages[i].definition_level_encoding,
      pages[i].repetition_level_encoding,
      pages[i].valid_count);
  }
}
  ...
  hostdevice_vector<parquet::gpu::PageInfo> &pages)
{
  ...
  for (const auto &codec : codecs) {
    if (codec.second > 0) {
      ...
      switch (codec.first) {
        ...
        case parquet::SNAPPY:
          CUDA_TRY(gpu_unsnap(
            inflate_in.device_ptr(start_pos), inflate_out.device_ptr(start_pos), argc - start_pos));
          break;
        case parquet::BROTLI:
  ...
  CUDA_TRY(cudaMemcpyAsync(
    pages.device_ptr(), pages.host_ptr(), pages.memory_size(), cudaMemcpyHostToDevice));

  return decomp_pages;
}

void reader::Impl::decode_page_data(hostdevice_vector<parquet::gpu::ColumnChunkDesc> &chunks,
                                    hostdevice_vector<parquet::gpu::PageInfo> &pages,
                                    ...
                                    size_t total_rows)
{
  ...
  CUDA_TRY(DecodePageData(
    pages.device_ptr(), pages.size(), chunks.device_ptr(), chunks.size(), total_rows, min_row));
  CUDA_TRY(cudaMemcpyAsync(
    pages.host_ptr(), pages.device_ptr(), pages.memory_size(), cudaMemcpyDeviceToHost));
  CUDA_TRY(cudaStreamSynchronize(0));
  ...
  for (size_t i = 0; i < pages.size(); i++) {
    if (pages[i].num_rows > 0) {
      LOG_PRINTF(" %2zd: valid_count=%d/%d\n", i, pages[i].valid_count, pages[i].num_rows);
  ...
  : source_(std::move(source))
{
  ...
  selected_cols_ =
    md_->select_columns(options.columns, options.use_pandas_metadata, pandas_index_col_);

  // Override output timestamp resolution if requested
  if (options.timestamp_unit != TIME_UNIT_NONE) { timestamp_unit_ = options.timestamp_unit; }
...
table reader::Impl::read(int skip_rows, int num_rows, int row_group)
{
  ...
  if (selected_row_groups.empty() || selected_cols_.empty()) { return cudf::table{}; }
  ...
  for (const auto &col : selected_cols_) {
    ...
    auto dtype_info  = to_dtype(col_schema.type,
    ...
    columns.emplace_back(
      static_cast<cudf::size_type>(num_rows), dtype_info.first, dtype_info.second, col.second);
  ...
  for (const auto &rg : selected_row_groups) {
    ...
    for (size_t i = 0; i < num_columns; ++i) {
      ...
      int32_t type_width =
        (col_schema.type == parquet::FIXED_LEN_BYTE_ARRAY) ? col_schema.type_length : 0;
      ...
      chunks.insert(parquet::gpu::ColumnChunkDesc(col_meta.total_compressed_size,
                                                  d_compdata,
                                                  col_meta.num_values,
                                                  col_schema.type,
                                                  type_width,
      ...
      LOG_PRINTF(
        ...
        chunks[chunks.size() - 1].data_type,
        type_width,
        col_schema.max_definition_level,
        col_schema.max_repetition_level,
  ...
  if (total_pages > 0) {
    ...
    for (auto &column : columns) { column.allocate(); }
...
table reader::read_row_group(size_t row_group) { return impl_->read(0, -1, row_group); }
