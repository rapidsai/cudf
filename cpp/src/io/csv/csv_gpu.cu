...
/**
 * @brief CUDA kernel for type detection using precomputed field-end offsets.
 *
 * Same logic as data_type_detection but uses precomputed field-end offsets
 * instead of calling seek_field_end. This eliminates redundant field scanning.
 */
CUDF_KERNEL void __launch_bounds__(csvparse_block_dim)
  data_type_detection_with_offsets(parse_options_view const opts,
...
/**
 * @brief Compute optimal rows per tile so that field_ends + CSV data fit in ~half of L2 cache.
 * Assumes ~128 bytes per CSV row and 4 bytes per field_end entry.
 */
inline size_t compute_rows_per_tile(int num_cols)
{
  constexpr size_t target_tile_bytes = 48ULL * 1024 * 1024;  // half of ~96MB L2
  constexpr size_t est_bytes_per_row = 128;                  // estimated CSV bytes per row
  auto const field_ends_per_row      = static_cast<size_t>(num_cols) * sizeof(uint32_t);
  auto const total_per_row           = field_ends_per_row + est_bytes_per_row;
  // Clamp to reasonable range: at least 32K rows, at most 512K rows
  return std::clamp(target_tile_bytes / total_per_row,
  ...
  rmm::cuda_stream_view stream)
{
  ...
  auto const rows_per_tile = compute_rows_per_tile(num_actual_cols);
                            ...
                            rmm::cuda_stream_view stream)
{
  ...
  auto const rows_per_tile =
    compute_rows_per_tile(num_actual_cols);  // ~32MB of CSV data for typical rows
