...
NVStrings* column_to_strings_fn::operator()<cudf::bool8>() {
  if (sizeof(bool) == sizeof(cudf::bool8))
    ...
    return NVStrings::create_from_bools(
      bool_buffer.data().get(), rows, true_string, false_string, valid);
  }
}
...
//
NVStrings* column_to_strings_csv(const gdf_column* column,
  ...
  if (column->dtype == GDF_STRING)
    ...
    rtn = cudf::type_dispatcher(
      column->dtype,
      column_to_strings_fn{column, valid, row_offset, rows, true_string, false_string});
  ...
  bool bquoted =
    (column->dtype == GDF_STRING || column->dtype == GDF_DATE64 || column->dtype == GDF_TIMESTAMP);
...
gdf_error write_csv(csv_write_arg* args) {
  ...
  char delimiter[2]          = {',', '\0'};
  if (args->delimiter) delimiter[0] = args->delimiter;
  const char* terminator = "\n";
  if (args->line_terminator) terminator = args->line_terminator;
  const char* narep = "";
  if (args->na_rep) narep = args->na_rep;
  ...
  if (count > 0) CUDF_EXPECTS(columns != 0, "write_csv: invalid data values");

  // check all columns are the same size
  const bool all_sizes_match = std::all_of(columns, columns + count, [total_rows](auto col) {
  ...
  while (rows > 0) {
    ...
    for (int idx = 0; idx < count; ++idx) {
      ...
      NVStrings* strs =
        column_to_strings_csv(col, row_offset, rows, delim, narep, true_value, false_value);
    ...
    //
    rmm::device_vector<size_t> string_locations(rows *
                                                count);  // all the memory pointers for each column
    ...
    size_t* d_string_locations = string_locations.data().get();
    {
      ...
      thrust::for_each_n(execpol->on(0),
                         ...
                         rows,
                         [d_sums, d_string_lengths, rows, count, d_string_locations] __device__(
                           cudf::size_type idx) {
                           size_t sum = 0;
    ...
    {
      rmm::device_vector<std::pair<const char*, size_t>> indices(
        rows);  // this is used to retrieve pointers
      std::pair<const char*, size_t>* d_indices =
        indices.data().get();  // to each element in strings instance
      for (int idx = 0; idx < count; ++idx) {
        const gdf_column* col = columns[idx];
        const char* delim     = ((idx + 1) < count ? delimiter : terminator);
        NVStrings* strs =
    ...
    if (include_header) {
      for (int idx = 0; idx < count; ++idx) {
        const gdf_column* col = columns[idx];
        const char* delim     = ((idx + 1) < count ? delimiter : terminator);
        if (col->col_name) filecsv << "\"" << col->col_name << "\"";
        filecsv << delim;
      }
    }
