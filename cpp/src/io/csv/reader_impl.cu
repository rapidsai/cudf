...
table_with_metadata reader::impl::read(cudaStream_t stream)
{
  ...
  if (!source_->is_empty()) {
    auto h_uncomp = host_span<char const>();
    ...
    size_t const data_start_offset = (range_offset != 0) ? find_first_row_start(h_uncomp) : 0;
    ...
    // Gather row offsets
    gather_row_offsets(h_uncomp,
                       data_start_offset,
                       (range_size) ? range_size : h_uncomp.size(),
...
void reader::impl::gather_row_offsets(host_span<char const> data,
                                      size_t range_begin,
                                      size_t range_end,
                                      size_t skip_rows,
                                      int64_t num_rows,
                                      ...
                                      cudaStream_t stream)
{
  ...
  do {
    ...
    data_.insert(data_.end(), data.data() + buffer_pos + data_.size(), data.data() + target_pos);
  ...
  if (header_row_index + 1 < row_offsets_.size()) {
    ...
    header_.assign(data.data() + header_start, data.data() + header_end);
    if (header_rows > 0) {
