...
struct reader_column_meta {
  ...
  std::vector<uint64_t> num_child_rows;
  ...
  std::vector<uint64_t> child_start_row;

  // Number of rows of child columns [stripe][column].
  std::vector<uint64_t> num_child_rows_per_stripe;

  struct row_group_meta {
    uint32_t num_rows;   // number of rows in a column in a row group
    uint64_t start_row;  // start row in a column in a row group
  };
