  ...
  rmm::cuda_stream_view stream)
{
  ...
  auto const d_stripes = cudf::detail::make_device_uvector_async(
    segmentation.stripes, stream, resources.get_temporary_mr());
...
#pragma GCC diagnostic pop
  ...
  for (auto const& stripe : segmentation.stripes) {
    std::generate_n(std::back_inserter(encoded_data[stripe.id]), streams.size(), [stream]() {
      return rmm::device_uvector<uint8_t>(0, stream, resources.get_temporary_mr());
    });
...
#pragma GCC diagnostic pop
          ...
          encoded_data[stripe.id][strm_id] =
            rmm::device_uvector<uint8_t>(stripe_size, stream, resources.get_temporary_mr());
        }
      ...
      for (int k = 0; k < CI_INDEX; k++) {
        ...
        if (stream_id != -1) {
          ...
          if (stripe.size > 1 and allocated_stripe_size > actual_stripe_size) {
            gathered_stripes[stripe.id][stream_id] = rmm::device_uvector<uint8_t>(
              actual_stripe_size, stream, resources.get_temporary_mr());
          }
                                                 ...
                                                 rmm::cuda_stream_view stream)
{
  ...
  if (num_stripe_blobs == 0) {
    ...
    auto const d_stat_chunks = cudf::detail::make_device_uvector_async<statistics_chunk>(
      h_stat_chunks, stream, resources.get_temporary_mr());
  ...
  auto const d_srcs = cudf::detail::make_device_uvector_async(h_srcs, stream, resources);
  auto const d_dsts = cudf::detail::make_device_uvector_async(h_dsts, stream, resources);
  auto const d_lens = cudf::detail::make_device_uvector_async(h_lens, stream, resources);
                                             ...
                                             rmm::cuda_stream_view stream)
{
  ...
  auto const d_mask_ptrs =
    cudf::detail::make_device_uvector_async(mask_ptrs, stream, resources.get_temporary_mr());
  thrust::for_each_n(
    rmm::exec_policy_nosync(stream, resources.get_temporary_mr()),
    thrust::make_counting_iterator(0ul),
                                   ...
                                   rmm::cuda_stream_view stream)
{
  ...
  auto const d_type_kinds =
    cudf::detail::make_device_uvector_async(type_kinds, stream, resources.get_temporary_mr());
  ...
  return {std::move(orc_columns),
          ...
          str_col_indexes,
          cudf::detail::make_device_uvector(str_col_indexes, stream, resources.get_temporary_mr())};
}
                                         ...
                                         rmm::cuda_stream_view stream)
{
  ...
  auto d_tmp_rowgroup_sizes = rmm::device_uvector<uint32_t>(
    segmentation.num_rowgroups(), stream, resources.get_temporary_mr());
                              ...
                              rmm::cuda_stream_view stream)
{
  ...
  auto counts = rmm::device_uvector<size_type>(
    num_str_cols * num_rowgroups, stream, resources.get_temporary_mr());
  ...
  if (sort_dictionaries) {
    ...
    for (auto& sd : dictionaries_flat) {
      ...
      auto order_copy = cudf::detail::make_device_uvector_async<uint32_t>(
        sd.data_order, current_stream, resources.get_temporary_mr());
