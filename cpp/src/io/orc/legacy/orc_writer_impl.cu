...
constexpr TypeKind to_orc_type(gdf_dtype dtype) {
  switch (dtype) {
    ...
    case GDF_INT32: return INT;
    case GDF_INT64: return LONG;
    case GDF_FLOAT32: return FLOAT;
    case GDF_FLOAT64: return DOUBLE;
    case GDF_BOOL8: return BOOLEAN;
    case GDF_DATE32: return DATE;
    case GDF_DATE64:
    case GDF_TIMESTAMP: return TIMESTAMP;
    case GDF_CATEGORY: return INT;
    case GDF_STRING:
    case GDF_STRING_CATEGORY: return STRING;
    default: return INVALID_TYPE_KIND;
  }
}
 ...
 public:
  ...
  explicit orc_column(size_t id, size_t str_id, const gdf_column *col)
    ...
    if (col->dtype == GDF_STRING) {
      ...
      CUDF_EXPECTS(str->create_index(static_cast<str_pair *>(index.data())) == 0,
      ...
      index.resize(col->size * sizeof(str_pair));
      CUDF_EXPECTS(cat->create_index(static_cast<str_pair *>(index.data())) == 0,
...
void writer::Impl::write(const cudf::table &table) {
  ...
  for (size_t g = 0, stripe_start = 0, stripe_size = 0; g < num_rowgroups; g++) {
    ...
    for (int i = 0; i < num_columns; i++) {
      if (orc_columns[i]->dtype == GDF_STRING || orc_columns[i]->dtype == GDF_STRING_CATEGORY) {
        const auto dt = orc_columns[i].host_dict_chunk(g);
        rowgroup_size += 1 * row_index_stride_;
        rowgroup_size += dt->string_char_count;
      } else {
        rowgroup_size += cudf::size_of(orc_columns[i]->dtype) * row_index_stride_;
      }
    }
  ...
  if (compression_kind_ != NONE) {
    ...
    CUDA_TRY(CompressOrcDataStreams(static_cast<uint8_t *>(compressed_data.data()),
    ...
    CUDA_TRY(cudaMemcpyAsync(
      comp_out.host_ptr(), comp_out.device_ptr(), comp_out.memory_size(), cudaMemcpyDeviceToHost));
...
void writer::Impl::init_dictionaries(orc_column *columns,
                                     size_t num_rows,
  ...
  for (size_t i = 0; i < str_col_ids.size(); ++i) {
    ...
    for (size_t g = 0; g < num_rowgroups; g++) {
      ...
      ck->num_rows =
        std::min<uint32_t>(row_index_stride_, std::max<int>(str_column->size - ck->start_row, 0));
      ck->num_strings       = 0;
  ...
  CUDA_TRY(cudaMemcpyAsync(
    dict.device_ptr(), dict.host_ptr(), dict.memory_size(), cudaMemcpyHostToDevice));
  CUDA_TRY(InitDictionaryIndices(dict.device_ptr(), str_col_ids.size(), num_rowgroups));
  CUDA_TRY(cudaMemcpyAsync(
    dict.host_ptr(), dict.device_ptr(), dict.memory_size(), cudaMemcpyDeviceToHost));
  CUDA_TRY(cudaStreamSynchronize(0));
}

void writer::Impl::build_dictionaries(orc_column *columns,
                                      size_t num_rows,
                                      const std::vector<int> &str_col_ids,
  ...
  CUDA_TRY(cudaMemcpyAsync(stripe_dict.device_ptr(),
                           ...
                           cudaMemcpyHostToDevice));
  CUDA_TRY(BuildStripeDictionaries(stripe_dict.device_ptr(),
                                   stripe_dict.host_ptr(),
                                   dict.device_ptr(),
                                   stripe_list.size(),
                                   num_rowgroups,
...
std::vector<Stream> writer::Impl::gather_streams(orc_column *columns,
                                                 size_t num_columns,
                                                 size_t num_rows,
                                                 const std::vector<uint32_t> &stripe_list,
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    if (columns[i]->null_count != 0 || columns[i]->size != static_cast<cudf::size_type>(num_rows)) {
...
rmm::device_buffer writer::Impl::encode_columns(orc_column *columns,
                                                size_t num_columns,
                                                size_t num_rows,
  ...
  auto output          = [&]() {
    ...
    for (size_t i = 0; i < streams.size(); ++i) {
      ...
      if (((stream.kind == DICTIONARY_DATA || stream.kind == LENGTH) &&
          ...
          ((stream.kind == DATA) &&
           (column.orc_kind() == STRING && column.orc_encoding() == DIRECT_V2))) {
  ...
  if (!str_col_ids.empty()) {
    ...
    CUDA_TRY(EncodeStripeDictionaries(
      d_stripe_dict, chunks.device_ptr(), str_col_ids.size(), num_columns, stripe_list.size()));
  }
  CUDA_TRY(EncodeOrcColumnData(chunks.device_ptr(), num_columns, num_rowgroups));
  CUDA_TRY(cudaStreamSynchronize(0));
...
std::vector<StripeInformation> writer::Impl::gather_stripes(
  ...
  CUDA_TRY(cudaMemcpyAsync(
    strm_desc.device_ptr(), strm_desc.host_ptr(), strm_desc.memory_size(), cudaMemcpyHostToDevice));
  CUDA_TRY(CompactOrcDataStreams(
    strm_desc.device_ptr(), chunks.device_ptr(), strm_desc.size(), num_columns));
  CUDA_TRY(cudaMemcpyAsync(
    strm_desc.host_ptr(), strm_desc.device_ptr(), strm_desc.memory_size(), cudaMemcpyDeviceToHost));
  CUDA_TRY(cudaMemcpyAsync(
...
void writer::Impl::write_index_stream(int32_t stripe_id,
  ...
  auto find_record = [=, &strm_desc](const gpu::EncChunk &chunk, const gpu::StreamIndexType type) {
  ...
  if (stream_id != 0) {
    ...
    if (kind == STRING && columns[stream_id - 1].orc_encoding() == DICTIONARY_V2) { kind = INT; }
  }
  ...
  for (size_t g = group; g < group + groups_in_stripe; g++) {
    pbw_.put_row_index_entry(present[2], present[0], data[2], data[0], data2[2], data2[0], kind);
...
void writer::Impl::write_data_stream(const gpu::StripeStream &strm_desc,
  ...
  if (length != 0) {
    ...
    CUDA_TRY(cudaMemcpyAsync(stream_out, stream_in, length, cudaMemcpyDeviceToHost, 0));
    CUDA_TRY(cudaStreamSynchronize(0));
