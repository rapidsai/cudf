...
static_assert(sizeof(orc::gpu::ColumnDesc) <= 256 && !(sizeof(orc::gpu::ColumnDesc) & 7),
                                                              ...
                                                              bool decimals_as_float)
{
  switch (schema.kind) {
    ...
    case orc::SHORT: return std::make_pair(GDF_INT16, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case orc::INT: return std::make_pair(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case orc::LONG: return std::make_pair(GDF_INT64, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case orc::FLOAT: return std::make_pair(GDF_FLOAT32, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case orc::DOUBLE: return std::make_pair(GDF_FLOAT64, gdf_dtype_extra_info{TIME_UNIT_NONE});
    case orc::STRING:
    case orc::BINARY:
    case orc::VARCHAR:
    case orc::CHAR:
    ...
    case orc::DECIMAL:
      ...
      return (decimals_as_float) ? std::make_pair(GDF_FLOAT64, gdf_dtype_extra_info{TIME_UNIT_NONE})
  ...
  const orc::StreamKind kind, uint32_t skip_count, bool non_child)
{
  switch (kind) {
    ...
    case orc::DICTIONARY_DATA: return std::make_pair(orc::gpu::CI_DICTIONARY, skip_count);
    case orc::PRESENT:
      skip_count += (non_child ? 1 : 0);
      return std::make_pair(orc::gpu::CI_PRESENT, skip_count);
    case orc::ROW_INDEX: return std::make_pair(orc::gpu::CI_INDEX, skip_count);
    default:
    ...
    for (size_t i = 0; i < ff.types.size(); i++) {
      ...
      if (ff.types[i].fieldNames.size() > 0) {
        ...
        for (size_t j = 0; j < ff.types[i].fieldNames.size(); j++) {
          LOG_PRINTF("%c\"%s\"", (j) ? ',' : '{', ff.types[i].fieldNames[j].c_str());
        }
        LOG_PRINTF("}\n");
      }
    }
...
size_t reader::Impl::gather_stream_info(const size_t stripe_index,
  ...
  size_t row_index_stride)
{
  ...
  CUDA_TRY(cudaMemcpyAsync(
    compinfo.device_ptr(), compinfo.host_ptr(), compinfo.memory_size(), cudaMemcpyHostToDevice));
  CUDA_TRY(ParseCompressedStripeData(compinfo.device_ptr(),
                                     compinfo.size(),
                                     decompressor->GetBlockSize(),
                                     decompressor->GetLog2MaxCompressionRatio()));
  CUDA_TRY(cudaMemcpyAsync(
    compinfo.host_ptr(), compinfo.device_ptr(), compinfo.memory_size(), cudaMemcpyDeviceToHost));
  ...
  if (num_compressed_blocks > 0) {
    switch (decompressor->GetKind()) {
      case orc::ZLIB:
        CUDA_TRY(
          gpuinflate(inflate_in.data().get(), inflate_out.data().get(), num_compressed_blocks, 0));
        break;
      case orc::SNAPPY:
        CUDA_TRY(
          gpu_unsnap(inflate_in.data().get(), inflate_out.data().get(), num_compressed_blocks));
        break;
  ...
  if (not row_groups.empty()) {
    ...
    CUDA_TRY(ParseRowGroupIndex(row_groups.data().get(),
                                compinfo.device_ptr(),
                                chunks.device_ptr(),
                                num_columns,
                                num_stripes,
...
void reader::Impl::decode_stream_data(hostdevice_vector<orc::gpu::ColumnDesc> &chunks,
                                      size_t num_dicts,
                                      size_t skip_rows,
                                      ...
                                      const std::vector<gdf_column_wrapper> &columns)
{
  ...
  for (size_t i = 0; i < num_stripes; ++i) {
    for (size_t j = 0; j < num_columns; ++j) {
      ...
      chunk.dtype_len = (columns[j]->dtype == GDF_STRING) ? sizeof(std::pair<const char *, size_t>)
  ...
  CUDA_TRY(DecodeNullsAndStringDictionaries(
    chunks.device_ptr(), global_dict.data().get(), num_columns, num_stripes, num_rows, skip_rows));
  CUDA_TRY(DecodeOrcColumnData(chunks.device_ptr(),
  ...
  for (size_t i = 0; i < num_columns; ++i) {
    ...
    LOG_PRINTF(
      ...
      chunks[i].start_row,
      chunks[i].num_rows,
      chunks[i].strm_len[orc::gpu::CI_PRESENT]);
  }
}
...
table reader::Impl::read(int skip_rows, int num_rows, int stripe)
{
  ...
  for (const auto &col : selected_cols_) {
    ...
    columns.emplace_back(static_cast<cudf::size_type>(selected_stripes.size() > 0 ? num_rows : 0),
                         dtype_info.first,
                         dtype_info.second,
                         md_->ff.GetColumnName(col));
  ...
  if (num_rows > 0 && selected_stripes.size() > 0) {
    ...
    const bool use_index =
      ...
      (num_rows > md_->get_row_index_stride() && !(md_->get_row_index_stride() & 7) &&
       md_->get_row_index_stride() > 0 && num_columns * selected_stripes.size() < 8 * 128) &&
    ...
    for (size_t i = 0; i < selected_stripes.size(); ++i) {
      ...
      if (use_index) {
        num_rowgroups += (stripe_info->numberOfRows + md_->get_row_index_stride() - 1) /
    ...
    rmm::device_vector<orc::gpu::RowGroup> row_groups(num_rowgroups * num_columns);

    if (md_->ps.compression != orc::NONE) {
      auto decomp_data = decompress_stripe_data(chunks,
                                                stripe_data,
                                                md_->decompressor.get(),
                                                stream_info,
