  ...
  rmm::cuda_stream_view stream)
{
  ...
  rmm::device_buffer decomp_data(cudf::util::round_up_safe(total_decomp_size, PADDING_MODULUS),
                                 stream);
    ...
    if (rows_to_read == 0 || selected_stripes.empty()) {
      ...
      for (auto const& stripe_source_mapping : selected_stripes) {
        ...
        for (auto const& stripe : stripe_source_mapping.stripe_info) {
          ...
          // Buffer needs to be padded.
          stripe_data.emplace_back(cudf::util::round_up_safe(total_data_size, PADDING_MODULUS),
                                   stream);
