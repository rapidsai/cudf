...
template <typename T>
inline bool ProtobufReader::function_builder_return(T *s,
    const uint8_t *end)
{
  return m_cur <= end;
}
...
template <>
inline bool ProtobufReader::function_builder_return<FileFooter>(FileFooter *s,
    const uint8_t *end)
{
...
inline bool ProtobufReader::function_builder(
    T *s,
    size_t maxlen,
...
struct ProtobufReader::FieldInt32
{
  ...
  inline bool
    operator()(ProtobufReader *pbr, const uint8_t *end)
    {
      val = pbr->get_i32();
      return false;
    }
...
struct ProtobufReader::FieldRepeatedString
{
  ...
  FieldRepeatedString(int f, std::vector<std::string> &v) : field((f*8) + PB_TYPE_FIXEDLEN), val(v) {}
...
struct ProtobufReader::FieldRepeatedStructFunctor
{
  ...
  FieldRepeatedStructFunctor(int f, std::vector<Enum> &v) : field((f*8) + PB_TYPE_FIXEDLEN), val(v) {}
...
struct ProtobufReader::FieldRepeatedStructBlobFunctor
{
  ...
  FieldRepeatedStructBlobFunctor(int f, std::vector<Enum> &v) : field((f*8) + PB_TYPE_FIXEDLEN), val(v) {}
