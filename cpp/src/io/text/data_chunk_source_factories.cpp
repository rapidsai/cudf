...
class device_uvector_data_chunk : public device_data_chunk {
 public:
  device_uvector_data_chunk(rmm::device_uvector<char>&& data)
    : _data(std::move(data)), _size{_data.size()}, _offset{0}
  {
  }
  device_uvector_data_chunk(rmm::device_uvector<char>&& data, size_t size, size_t offset)
    : _data(std::move(data)), _size{size}, _offset{offset}
  {
  }

  [[nodiscard]] char const* data() const override { return _data.data() + _offset; }
  [[nodiscard]] std::size_t size() const override { return _size; }
  operator device_span<char const>() const override { return {_data.data() + _offset, _size}; }
...
class gds_data_chunk_reader : public data_chunk_reader {
 public:
  gds_data_chunk_reader(std::string filename) : _filename{filename}
  {
    device_reader = cudf::io::detail::make_cufile_input(_filename);
    if (!device_reader) {
      host_reader = std::make_unique<istream_data_chunk_reader>(
                                                    ...
                                                    rmm::cuda_stream_view stream) override
  {
    if (device_reader) {
      auto chunk = rmm::device_uvector<char>(read_size, stream);
      auto size  = device_reader->read(bytes_read, read_size, chunk.data(), stream);
      bytes_read += size;
      return std::make_unique<device_uvector_data_chunk>(std::move(chunk), size, 0);
    } else {
      return host_reader->get_next_chunk(read_size, stream);
    }
  }

 private:
  std::string _filename;
  std::unique_ptr<cudf::io::detail::cufile_input_impl> device_reader;
  std::unique_ptr<istream_data_chunk_reader> host_reader;
  size_t bytes_read = 0;
};
...
class file_data_chunk_source : public data_chunk_source {
 public:
  ...
  {
    static const bool use_gds = std::getenv("MULTIBYTE_SPLIT_USE_GDS") != nullptr;
    if (use_gds) {
      return std::make_unique<istream_data_chunk_reader>(
        std::make_unique<std::ifstream>(_filename, std::ifstream::in));
    } else {
      return std::make_unique<gds_data_chunk_reader>(_filename);
    }
  }
