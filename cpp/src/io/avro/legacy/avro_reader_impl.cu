...
gdf_dtype to_dtype(const avro::schema_entry *col)
{
  switch (col->kind) {
    ...
    case avro::type_long: return GDF_INT64;
    case avro::type_float: return GDF_FLOAT32;
    case avro::type_double: return GDF_FLOAT64;
    case avro::type_bytes:
    case avro::type_string: return GDF_STRING;
    case avro::type_enum: return (!col->symbols.empty()) ? GDF_STRING : GDF_INT32;
 ...
 public:
      ...
      for (const auto &use_name : use_names) {
        for (int i = 0; i < num_avro_columns; ++i, ++index) {
          ...
          if (columns[index].name == use_name &&
              GDF_invalid != to_dtype(&schema[columns[index].schema_data_idx])) {
  ...
  void print_metadata() const
  {
    ...
    for (size_t i = 0; i < schema.size(); ++i) {
      LOG_PRINTF("  [%zd] num_children=%d, parent_idx=%d, type=%d, name=\"%s\"\n",
                 i,
                 schema[i].num_children,
                 schema[i].parent_idx,
                 schema[i].kind,
                 schema[i].name.c_str());
    }
...
table reader::Impl::read(int skip_rows, int num_rows)
{
  ...
  if (selected_cols_.empty()) { return table(); }
  ...
  for (const auto &col : selected_cols_) {
    ...
    LOG_PRINTF(" %2zd: name=%s size=%zd type=%d data=%lx valid=%lx\n",
               columns.size() - 1,
               columns.back()->col_name,
               (size_t)columns.back()->size,
               columns.back()->dtype,
               (uint64_t)columns.back()->data,
               (uint64_t)columns.back()->valid);
  }
  ...
  if (md_->total_data_size > 0) {
    ...
    if (md_->codec != "" && md_->codec != "null") {
      ...
      for (size_t i = 0; i < md_->block_list.size(); i++) { md_->block_list[i].offset -= dst_ofs; }
    }
    ...
    // Write out columns
    decode_data(block_data, dict, global_dictionary, total_dictionary_entries, columns);

    // Perform any final column preparation (may reference decoded data)
    for (auto &column : columns) { column.finalize(); }
  } else {
  ...
  for (size_t i = 0; i < columns.size(); ++i) { out_cols[i] = columns[i].release(); }
...
rmm::device_buffer reader::Impl::decompress_data(const rmm::device_buffer &comp_block_data)
{
  ...
  for (int loop_cnt = 0; loop_cnt < 2; loop_cnt++) {
    CUDA_TRY(cudaMemcpyAsync(inflate_in.device_ptr(),
                             ...
                             cudaMemcpyHostToDevice));
    CUDA_TRY(cudaMemsetAsync(inflate_out.device_ptr(), 0, inflate_out.memory_size()));
    if (md_->codec == "deflate") {
      CUDA_TRY(gpuinflate(inflate_in.device_ptr(), inflate_out.device_ptr(), inflate_in.size(), 0));
    } else if (md_->codec == "snappy") {
      CUDA_TRY(gpu_unsnap(inflate_in.device_ptr(), inflate_out.device_ptr(), inflate_in.size()));
    } else {
      CUDF_FAIL("Unsupported compression codec\n");
    }
                               ...
                               const std::vector<gdf_column_wrapper> &columns)
{
  ...
  for (size_t i = 0; i < md_->schema.size(); i++) {
    ...
    if (kind == type_enum && !md_->schema[i].symbols.size()) { kind = type_int; }
    ...
    CUDF_EXPECTS(kind != type_union || md_->schema[i].num_children < 2 ||
                   (md_->schema[i].num_children == 2 &&
                    (md_->schema[i + 1].kind == type_null || md_->schema[i + 2].kind == type_null)),
  ...
  CUDA_TRY(
    DecodeAvroColumnData(static_cast<block_desc_s *>(block_list.data()),
  ...
  for (size_t i = 0; i < columns.size(); i++) {
    if (valid_alias[i] != nullptr) {
      CUDA_TRY(cudaMemcpyAsync(columns[i]->valid,
                               valid_alias[i],
