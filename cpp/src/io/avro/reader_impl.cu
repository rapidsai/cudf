                                   ...
                                   rmm::cuda_stream_view stream)
{
  if (meta.codec == "deflate") {
    ...
    decompress(compression_type::SNAPPY,
               ...
               stream);
    CUDF_EXPECTS(thrust::equal(rmm::exec_policy_nosync(stream, resources.get_temporary_mr()),
                                       ...
                                       cudf::memory_resources resources)
{
  ...
  auto block_list =
    cudf::detail::make_device_uvector_async(meta.block_list, stream, resources.get_temporary_mr());
                              ...
                              cudf::memory_resources resources)
{
  ...
  if (not selected_columns.empty()) {
    ...
    if (meta.num_rows > 0) {
      ...
      auto d_global_dict =
        rmm::device_uvector<string_index_pair>(0, stream, resources.get_temporary_mr());
  ...
  out_columns =
    cudf::structs::detail::enforce_null_consistency(std::move(out_columns), stream, resources);
