  ...
  rmm::cuda_stream_view stream)
{
  ...
  auto construct_row_idx = [&stream](NodeIndexT num_columns,
    ...
    if (num_columns > 1) {
      thrust::transform_inclusive_scan(
        rmm::exec_policy_nosync(stream),
        thrust::make_zip_iterator(cuda::counting_iterator{1}, row_idx.begin() + 1),
        thrust::make_zip_iterator(cuda::counting_iterator{1} + num_columns, row_idx.end()),
        row_idx.begin() + 1,
  ...
  auto construct_col_idx = [&stream](NodeIndexT num_columns,
    ...
    thrust::scatter(rmm::exec_policy_nosync(stream),
                    cuda::counting_iterator{1},
                    cuda::counting_iterator{1} + num_columns - 1,
                    map.begin(),
                    col_idx.begin());
