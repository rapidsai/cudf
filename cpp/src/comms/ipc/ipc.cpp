...
protected:
    ...
    void read_schema(const uint8_t *schema_buf, size_t length)
    {
        ...
        if (!_schema)
            throw ParseError("failed to parse schema");
        // Parse the schema
    ...
    void read_record_batch(const uint8_t *recordbatches, size_t length)
    {
        ...
        if (header.version != flatbuf::MetadataVersion_V4)
            throw ParseError("unsupported metadata version, expected V4 got " + std::string(flatbuf::EnumNameMetadataVersion(header.version)));
