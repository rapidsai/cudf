 ...
 * struct elements.
 *
 * The input of this class is a flattenned table generated by flattenning a structs column.
 * Using the binary operator provided by this class, nulls STRUCT are compared as larger than all
 * other non-null STRUCT elements (if finding for ARGMIN), or smaller than all other non-null STRUCT
 * elements (if finding for ARGMAX). This helps achieve the results of finding the min or max STRUCT
 * element when nulls are excluded from the operations, returning null only when all the input
 * elements are nulls.
 */
class comparison_binop_generator {
 private:
  table_device_view const& tview;
 ...
 public:
  comparison_binop_generator(cudf::structs::detail::flattened_table const& flattened_input,
                             table_device_view const& tview,
                             bool is_min_op,
                             size_type n_rows,
                             bool has_nulls,
  ...
  auto binop() const
  {
    return row_arg_minmax_fn(n_rows, tview, has_nulls, null_orders_dvec.data(), is_min_op);
  }
};
