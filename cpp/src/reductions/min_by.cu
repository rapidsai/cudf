...
std::unique_ptr<scalar> min_by(column_view const& input,
                               rmm::cuda_stream_view stream,
                               rmm::device_async_resource_ref mr)
{
  CUDF_EXPECTS(input.type().id() == type_id::STRUCT,
               "MIN_BY reduction expects a struct column as input");

  auto const struct_view = structs_column_view(input);
  CUDF_EXPECTS(struct_view.num_children() == 2,
               "MIN_BY reduction expects a struct column with exactly 2 children");
  ...
  // Extract the index
  auto const min_index = static_cast<cudf::numeric_scalar<size_type> const&>(*argmin_result).value(stream);

  // Get the element at that index from the value column
  return get_element(value_column, min_index, stream, mr);
}
