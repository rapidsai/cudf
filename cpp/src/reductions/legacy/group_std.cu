...
struct var_functor {
  ...
  std::enable_if_t<std::is_arithmetic<T>::value, void> operator()(
    ...
    thrust::reduce_by_key(
      ...
      thrust::make_transform_iterator(
        ...
        [=] __device__(cudf::size_type i) -> T {
          return (values_valid and not bit_mask::is_valid(values_valid, i)) ? 0 : values_data[i];
        }),
    ...
    auto values_it = thrust::make_transform_iterator(
      thrust::make_counting_iterator(0), [=] __device__(cudf::size_type i) {
        if (values_valid and not bit_mask::is_valid(values_valid, i)) return 0.0;
...
std::pair<cudf::table, cudf::table> group_var_std(cudf::table const& keys,
  ...
  for (cudf::size_type i = 0; i < values.num_columns(); i++) {
    ...
    std::tie(sorted_values, group_sizes) = gb_obj.sort_values(*(values.get_column(i)));

    gdf_column* result_col = result_table.get_column(i);

    if (is_std) {
      detail::group_std(sorted_values, group_labels, group_sizes, result_col, ddof);
    } else {
      detail::group_var(sorted_values, group_labels, group_sizes, result_col, ddof);
    }
