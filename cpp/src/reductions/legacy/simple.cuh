...
gdf_scalar simple_reduction(gdf_column const& col,
                            ...
                            cudaStream_t stream)
{
  ...
  CUDA_TRY(cudaMemcpyAsync(result, &identity, sizeof(ResultType), cudaMemcpyHostToDevice, stream));
  CHECK_CUDA(stream);

  // reduction by iterator
  auto it = Op::template make_iterator<has_nulls, ElementType, ResultType>(col);
  cudf::reduction::detail::reduce(result, it, col.size, identity, typename Op::Op{}, stream);

  // read back the result to host memory
  // TODO: asynchronous copy
  CUDA_TRY(cudaMemcpy(&scalar.data, result, sizeof(ResultType), cudaMemcpyDeviceToHost));
  ...
  static constexpr bool is_supported_v()
  {
    ...
    return std::is_convertible<ElementType, ResultType>::value ||
           (std::is_arithmetic<ElementType>::value &&
            std::is_same<ResultType, cudf::bool8>::value) ||
 ...
 public:
  ...
  gdf_scalar operator()(gdf_column const& col, gdf_dtype const output_dtype, cudaStream_t stream)
  {
    return cudf::type_dispatcher(
      output_dtype, result_type_dispatcher<ElementType, Op>(), col, output_dtype, stream);
  }
