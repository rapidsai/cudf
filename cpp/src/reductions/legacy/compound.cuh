...
gdf_scalar compound_reduction(gdf_column const& col,
  ...
  CUDA_TRY(cudaMemcpyAsync(
    dev_result, &intermediate, sizeof(IntermediateType), cudaMemcpyHostToDevice, stream));
  CHECK_CUDA(stream);

  // reduction by iterator
  auto it = Op::template make_iterator<has_nulls, ElementType, ResultType>(col);
  cudf::reduction::detail::reduce(static_cast<IntermediateType*>(dev_result),
                                  it,
                                  col.size,
                                  intermediate,
                                  typename Op::Op{},
  ...
  CUDA_TRY(cudaMemcpy(&intermediate, dev_result, sizeof(IntermediateType), cudaMemcpyDeviceToHost));
 ...
 public:
  ...
  gdf_scalar operator()(gdf_column const& col,
    ...
    if (cudf::has_nulls(col)) {
      ...
      return compound_reduction<ElementType, ResultType, Op, false>(
        col, output_dtype, ddof, stream);
    }
  }
