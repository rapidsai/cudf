...
/* ----------------------------------------------------------------------------*/
template <typename T>
inline void copy_and_replace_nulls(const T *data,
                                   const cudf::valid_type *mask,
                                   cudf::size_type size,
                                   T *results,
                                   T identity,
                                   cudaStream_t stream) {
...
struct Scan {
  static void call(const gdf_column *input,
    ...
    scan_function(temp_storage, temp_storage_bytes, d_input, d_output, size, stream);
    RMM_TRY(RMM_ALLOC(&temp_storage, temp_storage_bytes, stream));

    if (nullptr != input->valid) {
      // copy null bitmask
      CUDA_TRY(cudaMemcpyAsync(output->valid,
                               input->valid,
    ...
    bool const input_has_nulls{nullptr != input->valid && input->null_count > 0};
    if (input_has_nulls) {
      ...
      copy_and_replace_nulls(static_cast<const T *>(input->data),
                             input->valid,
                             size,
                             temp_input,
      ...
      // Do scan
      scan_function(temp_storage, temp_storage_bytes, temp_input, d_output, size, stream);
  ...
  static void exclusive_scan(void *&temp_storage,
  ...
  static void inclusive_scan(void *&temp_storage,
                             size_t &temp_storage_bytes,
                             const T *input,
                             T *output,
                             size_t size,
  ...
  switch (op) {
    case GDF_SCAN_SUM:
      cudf::type_dispatcher(
        input->dtype, PrefixSumDispatcher<cudf::DeviceSum>(), input, output, inclusive);
      return;
    case GDF_SCAN_MIN:
      cudf::type_dispatcher(
        input->dtype, PrefixSumDispatcher<cudf::DeviceMin>(), input, output, inclusive);
      return;
    case GDF_SCAN_MAX:
      cudf::type_dispatcher(
        input->dtype, PrefixSumDispatcher<cudf::DeviceMax>(), input, output, inclusive);
      return;
    case GDF_SCAN_PRODUCT:
      cudf::type_dispatcher(
        input->dtype, PrefixSumDispatcher<cudf::DeviceProduct>(), input, output, inclusive);
      return;
    default: CUDF_FAIL("The input enum `gdf_scan_op` is out of the range");
  }
}
