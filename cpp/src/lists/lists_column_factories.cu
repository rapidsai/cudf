...
std::unique_ptr<column> make_lists_column(
   size_type num_rows,
   std::unique_ptr<column> offsets_column,    
   std::unique_ptr<column> child_column, size_type null_count,
   rmm::device_buffer&& null_mask, cudaStream_t stream,
   rmm::mr::device_memory_resource* mr)
{
   if(null_count > 0){
      CUDF_EXPECTS(null_mask.size() > 0, "Column with nulls must be nullable.");
   }
   CUDF_EXPECTS(num_rows == offsets_column->size()-1, "Invalid offsets column size for lists column.");
   ...
   return std::make_unique<column>(
          cudf::data_type{LIST}, num_rows, rmm::device_buffer{0, stream, mr},
