...
auto __device__ distance([[maybe_unused]] Iterator begin, Iterator end, Iterator find_iter)
  -> size_type
{
  if (find_iter == end) {
...
struct finder {
  ...
  __device__ auto operator()(list_device_view const& list, ElementType const& search_key) const
    -> size_type
  {
