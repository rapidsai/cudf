...
{   
   ...
   std::transform(lists_columns.begin(), lists_columns.end(), std::back_inserter(leaves), [&](lists_column_view const& l){
   ...
   return make_lists_column(total_list_count, std::move(offsets), std::move(data), 0, rmm::device_buffer{0, stream, mr}, stream, mr);   
}
