...
//         lhs.column(child_index), rhs.column(child_index), op, out.type());
...
//                                              return col.data<output_type>()[row_index];
//                                            });
//                    });
// }

// template <typename output_type, bool const has_nulls>
// void lt_row(table_view lhs, table_view rhs, mutable_column_view out, rmm::cuda_stream_view
// stream)
// {
//   auto d_lhs = table_device_view::create(lhs);
//   auto d_rhs = table_device_view::create(rhs);
...
// template <typename output_type, bool const has_nulls>
// void gt_row(table_view lhs, table_view rhs, mutable_column_view out, rmm::cuda_stream_view
// stream)
// {
//   auto d_lhs = table_device_view::create(lhs);
//   auto d_rhs = table_device_view::create(rhs);
...
//     });
//   // auto comparator = row_lexicographic_comparator<has_nulls>(*d_lhs, *d_rhs,
//   op_modifier.data(), norder.data()); auto comparator =
//   row_lexicographic_comparator<has_nulls>(*d_lhs, *d_rhs, op_modifier.data(), nullptr);
...
//   mr); auto out_view = out->mutable_view();

//   auto const lhs_has_nulls = has_nested_nulls(lhs_flat);
//   auto const rhs_has_nulls = has_nested_nulls(rhs_flat);
//   bool const has_nulls = lhs_has_nulls || rhs_has_nulls;

//   if(has_nulls){
//     switch (op) {
//       case binary_operator::EQUAL: and_merge<ot>(lhs_flat, rhs_flat, out_view, op, stream);
//       break; case binary_operator::NOT_EQUAL: and_merge<ot>(lhs_flat, rhs_flat, out_view, op,
//       stream); break; case binary_operator::LESS: lt_row<ot, true>(lhs_flat, rhs_flat, out_view,
//       stream); break; case binary_operator::GREATER: gt_row<ot, true>(lhs_flat, rhs_flat,
//       out_view, stream); break; case binary_operator::LESS_EQUAL: gt_row<ot, true>(rhs_flat,
//       lhs_flat, out_view, stream); break; case binary_operator::GREATER_EQUAL: lt_row<ot,
//       true>(rhs_flat, lhs_flat, out_view, stream); break;
...
//       case binary_operator::EQUAL: and_merge<ot>(lhs_flat, rhs_flat, out_view, op, stream);
//       break; case binary_operator::NOT_EQUAL: and_merge<ot>(lhs_flat, rhs_flat, out_view, op,
//       stream); break; case binary_operator::LESS: lt_row<ot, false>(lhs_flat, rhs_flat, out_view,
//       stream); break; case binary_operator::GREATER: gt_row<ot, false>(lhs_flat, rhs_flat,
//       out_view, stream); break; case binary_operator::LESS_EQUAL: gt_row<ot, false>(rhs_flat,
//       lhs_flat, out_view, stream); break; case binary_operator::GREATER_EQUAL: lt_row<ot,
//       false>(rhs_flat, lhs_flat, out_view, stream); break;
//       // case binary_operator::NULL_EQUALS: break;
//       default: CUDF_FAIL("Unsupported operator for these types");
//     }
//   }
