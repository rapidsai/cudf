...
void filtered_join_insert_primitive(
  ...
  rmm::cuda_stream_view stream)
{
  auto const d_build_comparator = fj::primitive_row_comparator{
    nullate::DYNAMIC{true}, preprocessed_build, preprocessed_build, nulls_equal};
  cuco::static_set_ref set_ref{fj::empty_sentinel_key,
                               fj::insertion_adapter{d_build_comparator},
                               fj::primitive_probing_scheme{},
                               cuco::thread_scope_device,
                               bucket_storage.ref()};
  auto insert_ref = set_ref.rebind_operators(cuco::insert);

  constexpr auto CGSize = fj::primitive_probing_scheme::cg_size;
  auto insert           = [&]<typename Iterator>(Iterator build_iter) {
    auto const grid_size = cuco::detail::grid_size(build.num_rows(), CGSize);
    if (has_nested_nulls && nulls_equal == null_equality::UNEQUAL) {
  ...
  auto const d_build_hasher = fj::primitive_row_hasher{nullate::DYNAMIC{true}, preprocessed_build};
  auto const build_iter     = cudf::detail::make_counting_transform_iterator(
    size_type{0}, fj::key_pair_fn<lhs_index_type, fj::primitive_row_hasher>{d_build_hasher});
  insert(build_iter);
}
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  auto const d_build_probe_comparator = fj::primitive_row_comparator{
    nullate::DYNAMIC{true}, preprocessed_build, preprocessed_probe, nulls_equal};
  cuco::static_set_ref set_ref{fj::empty_sentinel_key,
                               fj::comparator_adapter{d_build_probe_comparator},
                               fj::primitive_probing_scheme{},
                               cuco::thread_scope_device,
                               bucket_storage.ref()};
  ...
  auto const d_probe_hasher = fj::primitive_row_hasher{nullate::DYNAMIC{true}, preprocessed_probe};
  auto const probe_iter     = cudf::detail::make_counting_transform_iterator(
    size_type{0}, fj::key_pair_fn<rhs_index_type, fj::primitive_row_hasher>{d_probe_hasher});
