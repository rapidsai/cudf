                                ...
                                rmm::cuda_stream_view stream)
{
  ...
  // equal to `cudf::detail::JoinNoneValue`, then `idx` has a match in the hash set.
  ...
  : _has_nested_columns{cudf::has_nested_columns(build)},
    ...
    _hash_table{build.num_rows(),
                ...
                cuco::empty_key{cuco::pair{std::numeric_limits<hash_value_type>::max(),
                                           rhs_index_type{cudf::detail::JoinNoneValue}}},
                always_not_equal{},
                {},
  ...
  auto const output_end =
    thrust::copy_if(rmm::exec_policy_nosync(stream),
                    ...
                    cuda::proclaim_return_type<bool>(
                      [] __device__(size_type idx) { return idx != cudf::detail::JoinNoneValue; }));
