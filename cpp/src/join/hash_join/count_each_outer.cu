...
template void launch_count_each<true, primitive_count_ref_t>(probe_key_type const*,
                                                             cuda::std::int64_t,
                                                             size_type*,
                                                             primitive_count_ref_t,
                                                             rmm::cuda_stream_view);

template void launch_count_each<true, nested_count_ref_t>(
  probe_key_type const*, cuda::std::int64_t, size_type*, nested_count_ref_t, rmm::cuda_stream_view);

template void launch_count_each<true, flat_count_ref_t>(
