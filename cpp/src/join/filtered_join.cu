...
auto filtered_join::compute_bucket_storage_size(cudf::table_view tbl, double load_factor)
{
  auto const size_with_primitive_probe =
    static_cast<size_t>(cuco::make_valid_extent<primitive_probing_scheme, storage_type, size_t>(
      tbl.num_rows(), load_factor));
  auto const size_with_nested_probe =
    static_cast<size_t>(cuco::make_valid_extent<nested_probing_scheme, storage_type, size_t>(
      tbl.num_rows(), load_factor));
  auto const size_with_simple_probe =
    static_cast<size_t>(cuco::make_valid_extent<simple_probing_scheme, storage_type, size_t>(
      tbl.num_rows(), load_factor));
...
filtered_join::filtered_join(cudf::table_view const& build,
  ...
  : _build_props{build_properties{cudf::has_nested_columns(build)}},
    ...
    _bucket_storage{cuco::extent<size_t>{compute_bucket_storage_size(build, load_factor)},
