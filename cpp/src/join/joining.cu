...
gdf_error trivial_full_join(
    ...
    gdf_column_view_augmented( left_result, l_ptr, nullptr, result_size, dtype, 0,  left_result->dtype_info,  left_result->col_name);
    gdf_column_view_augmented(right_result, r_ptr, nullptr, result_size, dtype, 0, right_result->dtype_info, right_result->col_name);
    CUDA_CHECK_LAST();
    return GDF_SUCCESS;
}
...
gdf_error construct_join_output_df(
  ...
  PUSH_RANGE("LIBGDF_JOIN_OUTPUT", JOIN_COLOR);
    ...
    for (int i = 0; i < left_table_end; ++i) {
        ...
        gdf_column_view_augmented(result_cols[i], nullptr, nullptr, join_size, col->dtype, 0, col->dtype_info, col->col_name);
