...
template <cudf::size_type block_size, bool has_nulls, typename hash_set_ref_type>
...
template <typename hash_set_ref_type>
...
{
  if (has_nulls) {
    mixed_join_semi<DEFAULT_JOIN_BLOCK_SIZE, true, hash_set_ref_type>
    ...
    mixed_join_semi<DEFAULT_JOIN_BLOCK_SIZE, false, hash_set_ref_type>
      <<<config.num_blocks, config.num_threads_per_block, shmem_size_per_block, stream.value()>>>(
        left_table,
        right_table,
        probe,
