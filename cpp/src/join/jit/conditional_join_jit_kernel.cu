...
namespace detail {
template <int block_size, bool has_nulls, typename LeftTableAccessor, typename RightTableAccessor>
CUDF_KERNEL void compute_conditional_join_output_size(table_device_view_core const* tables,
                                                      join_kind join_type,
                                                      ...
                                                      std::size_t* output_size)
{
  ...
  for (cudf::thread_index_type outer_row_index = start_idx; outer_row_index < outer_num_rows;
    ...
    for (cudf::thread_index_type inner_row_index = 0; inner_row_index < inner_num_rows;
      ...
      auto row = pair_table_accessor<LeftTableAccessor, RightTableAccessor>::element(
        tables, tables, left_row_index, right_row_index);

      // [ ] handle nulls and null-aware
      auto left_row        = LeftTableAccessor::element(left_table, left_row_index);
      auto right_row       = RightTableAccessor::element(right_table, right_row_index);
      bool predicate_match = false;
      auto args = cuda::std::tuple_cat(cuda::std::tuple{&predicate_match}, left_row, right_row);

      cuda::std::apply(GENERIC_BINARY_PREDICATE, args);
