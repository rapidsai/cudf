...
rmm::device_vector<index_type> create_missing_indices(index_type const *const index_ptr,
  ...
  size_type compacted_size = thrust::copy_if(rmm::exec_policy(stream)->on(stream),
...
gdf_error expand_buffer(data_type **buffer,
  ...
  CUDA_TRY(
    cudaMemcpy(new_buffer, old_buffer, buffer_size * sizeof(data_type), cudaMemcpyDeviceToDevice));
...
/* ----------------------------------------------------------------------------*/
template <typename index_type, typename size_type>
gdf_error append_full_join_indices(index_type **l_index_ptr,
  ...
  rmm::device_vector<index_type> unmatched_indices =
    create_missing_indices(*r_index_ptr, max_index_value, index_size, stream);
  CHECK_CUDA(stream);
