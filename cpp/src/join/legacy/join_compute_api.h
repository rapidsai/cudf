 ...
 */
/* ----------------------------------------------------------------------------*/
template <JoinType join_type, typename multimap_type>
                                    ...
                                    cudf::size_type *join_output_size_estimate)
{
  ...
  if (build_table_num_rows > 0) {
    probe_to_build_ratio = static_cast<cudf::size_type>(
    ...
    switch (join_type) {
      ...
      default: return GDF_UNSUPPORTED_JOIN_TYPE;
    }
    return GDF_SUCCESS;
  }
  ...
  *d_size_estimate = 0;
  ...
  CUDA_TRY(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
    &numBlocks,
    compute_join_output_size<join_type, multimap_type, block_size, DEFAULT_CUDA_CACHE_SIZE>,
    block_size,
    0));
  ...
  do {
    ...
    *d_size_estimate = 0;
    ...
    compute_join_output_size<join_type, multimap_type, block_size, DEFAULT_CUDA_CACHE_SIZE>
      <<<numBlocks * num_sms, block_size>>>(
        hash_table, build_table, probe_table, sample_probe_num_rows, d_size_estimate);
    ...
    if (0 == h_size_estimate) {
      ...
      probe_to_build_ratio = static_cast<cudf::size_type>(
        std::ceil(static_cast<float>(probe_to_build_ratio) / GROW_RATIO));
    }
                            ...
                            bool flip_results = false)
{
  ...
  using multimap_type = concurrent_unordered_multimap<
    hash_value_type,
    output_index_type,
    size_t,
  ...
  size_t const hash_table_size = std::max(compute_hash_table_size(build_table_num_rows), size_t{1});
  ...
  *d_gdf_error_code = GDF_SUCCESS;
  ...
  if (build_table_num_rows > 0) {
    ...
    build_hash_table<<<build_grid_size, block_size>>>(
      *hash_table, *build_table, build_table_num_rows, d_gdf_error_code);
  ...
  if (GDF_SUCCESS != gdf_error_code) { return gdf_error_code; }
  ...
  if (0 == estimated_join_output_size) { return GDF_SUCCESS; }
  ...
  RMM_TRY(RMM_ALLOC(
    (void **)&d_global_write_index, sizeof(cudf::size_type), 0));  // TODO non-default stream?
  ...
  while (cont) {
    ...
    RMM_TRY(
      RMM_ALLOC((void **)&output_l_ptr, estimated_join_output_size * sizeof(output_index_type), 0));
    RMM_TRY(
      RMM_ALLOC((void **)&output_r_ptr, estimated_join_output_size * sizeof(output_index_type), 0));
    ...
    CUDA_TRY(cudaMemcpy(
      &h_actual_found, d_global_write_index, sizeof(cudf::size_type), cudaMemcpyDeviceToHost));
  ...
  if (join_type == JoinType::FULL_JOIN) {
    append_full_join_indices(&output_l_ptr,
                             &output_r_ptr,
                             estimated_join_output_size,
                             h_actual_found,
  ...
  if (estimated_join_output_size > h_actual_found) {
    ...
    RMM_TRY(RMM_ALLOC((void **)&copy_output_l_ptr,
                      h_actual_found * sizeof(output_index_type),
                      0));  // TODO non-default stream?
    RMM_TRY(RMM_ALLOC((void **)&copy_output_r_ptr, h_actual_found * sizeof(output_index_type), 0));
    CUDA_TRY(cudaMemcpy(copy_output_l_ptr,
                        output_l_ptr,
                        h_actual_found * sizeof(output_index_type),
                        cudaMemcpyDeviceToDevice));
    CUDA_TRY(cudaMemcpy(copy_output_r_ptr,
                        output_r_ptr,
                        h_actual_found * sizeof(output_index_type),
                        cudaMemcpyDeviceToDevice));
    RMM_TRY(RMM_FREE(output_l_ptr, 0));
