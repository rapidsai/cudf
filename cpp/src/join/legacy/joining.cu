...
void trivial_full_join(const cudf::size_type left_size,
  ...
  if (left_size == 0) {
    allocValueBuffer(&l_ptr, right_size, static_cast<output_index_type>(-1));
    allocSequenceBuffer(&r_ptr, right_size);
    result_size = right_size;
  } else if (right_size == 0) {
    allocValueBuffer(&r_ptr, left_size, static_cast<output_index_type>(-1));
...
void join_call(cudf::table const& left,
  ...
  if ((0 == left.num_rows()) && (0 == right.num_rows())) { return; }

  // If left join and the left table is empty, return immediately
  if ((JoinType::LEFT_JOIN == join_type) && (0 == left.num_rows())) { return; }

  // If Inner Join and either table is empty, return immediately
  if ((JoinType::INNER_JOIN == join_type) && ((0 == left.num_rows()) || (0 == right.num_rows()))) {
    return;
  }

  // If Full Join and either table is empty, compute trivial full join
  if ((JoinType::FULL_JOIN == join_type) && ((0 == left.num_rows()) || (0 == right.num_rows()))) {
  ...
  for (int i = 0; i < left.num_columns(); i++) {
    CUDF_EXPECTS(!((left.num_rows() > 0) && (nullptr == left.get_column(i)->data)),
                 "Null column data in left set");
    CUDF_EXPECTS(!((right.num_rows() > 0) && (nullptr == right.get_column(i)->data)),
                 "Null column data in right set");
  ...
  switch (join_method) {
    case GDF_HASH: {
      gdf_error_code =
        join_hash<join_type, output_index_type>(left, right, left_result, right_result);
    ...
    case GDF_SORT: {
      ...
      if (1 == left.num_columns()) {
        gdf_error_code =
          sort_join<join_type, output_index_type>(const_cast<gdf_column*>(left.get_column(0)),
                                                  const_cast<gdf_column*>(right.get_column(0)),
                                                  left_result,
                                                  right_result);
        CUDF_EXPECTS(gdf_error_code != GDF_VALIDITY_UNSUPPORTED,
                     "GDF Validity is unsupported by sort_join");
        CUDF_EXPECTS(gdf_error_code == GDF_SUCCESS, "GDF Error");
      } else {
    ...
    default: CUDF_FAIL("Unsupported join Method");
  }
...
auto non_common_column_indices(cudf::size_type num_columns,
  ...
  std::vector<cudf::size_type> sorted_common_column_indices{common_column_indices};
...
cudf::table construct_join_output_df(
  ...
  if (not right_non_common_indices.empty()) {
    ...
    std::iota(std::begin(result_right_non_common_indices),
              std::end(result_right_non_common_indices),
              left.num_columns());
...
cudf::table join_call_compute_df(
  ...
  CUDF_EXPECTS(
    std::none_of(
      std::cbegin(left), std::cend(left), [](auto col) { return col->dtype == GDF_invalid; }),
    "Unsupported left column dtype");
  CUDF_EXPECTS(
    std::none_of(
      std::cbegin(right), std::cend(right), [](auto col) { return col->dtype == GDF_invalid; }),
    "Unsupported right column dtype");
  ...
  // If left join and the left table is empty, return immediately
  if ((JoinType::LEFT_JOIN == join_type) && (0 == left.num_rows())) { return tmp_table; }
  ...
  std::transform(std::cbegin(left), std::cend(left), std::begin(new_left_cols), [](auto col) {
    return const_cast<gdf_column*>(col);
  });
  std::transform(std::cbegin(right), std::cend(right), std::begin(new_right_cols), [](auto col) {
    return const_cast<gdf_column*>(col);
  });

  std::vector<gdf_column*> temp_columns_to_free;
  if (at_least_one_category_column) {
    for (unsigned int join_column_index = 0; join_column_index < left_on.size();
         join_column_index++) {
      if (left.get_column(left_on[join_column_index])->dtype == GDF_STRING_CATEGORY) {
        CUDF_EXPECTS(right.get_column(right_on[join_column_index])->dtype == GDF_STRING_CATEGORY,
                     "GDF type mismatch");
        ...
        gdf_column* new_join_columns[2]         = {new_left_column_ptr, new_right_column_ptr};
        gdf_column_view(
          new_left_column_ptr, nullptr, nullptr, left_original_column->size, GDF_STRING_CATEGORY);
        gdf_column_view(
          new_right_column_ptr, nullptr, nullptr, right_original_column->size, GDF_STRING_CATEGORY);

        int col_width = cudf::byte_width(*new_left_column_ptr);
        RMM_TRY(RMM_ALLOC(&(new_left_column_ptr->data),
                          col_width * left_original_column->size,
                          0));  // TODO: non-default stream?
        if (left_original_column->valid != nullptr) {
          RMM_TRY(RMM_ALLOC(
            &(new_left_column_ptr->valid),
            sizeof(cudf::valid_type) * gdf_valid_allocation_size(left_original_column->size),
            0));
          CUDA_TRY(cudaMemcpy(
            new_left_column_ptr->valid,
            left_original_column->valid,
            sizeof(cudf::valid_type) * gdf_num_bitmask_elements(left_original_column->size),
        ...
        RMM_TRY(RMM_ALLOC(&(new_right_column_ptr->data),
                          col_width * right_original_column->size,
                          0));  // TODO: non-default stream?
        if (right_original_column->valid != nullptr) {
          RMM_TRY(RMM_ALLOC(
            &(new_right_column_ptr->valid),
            sizeof(cudf::valid_type) * gdf_valid_allocation_size(right_original_column->size),
            0));
          CUDA_TRY(cudaMemcpy(
            new_right_column_ptr->valid,
            right_original_column->valid,
            sizeof(cudf::valid_type) * gdf_num_bitmask_elements(right_original_column->size),
            cudaMemcpyDeviceToDevice));
        } else {
          new_right_column_ptr->valid = nullptr;
        }
        new_right_column_ptr->null_count = right_original_column->null_count;
        gdf_error err = sync_column_categories(input_join_columns_merge, new_join_columns, 2);
  ...
  cudf::table result = construct_join_output_df<join_type, index_type>(
    updated_left_table, updated_right_table, columns_in_common, left_index_out, right_index_out);
