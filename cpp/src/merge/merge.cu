...
table merge(table const& left_table,
    ...
    for (gdf_size_type i = 0; i < left_table.num_columns(); i++) {
        ...
        *new_left_column_ptr = allocate_like(*left_col, mask_allocation_policy::RETAIN, stream);
        ...
        *new_right_column_ptr = allocate_like(*right_col, mask_allocation_policy::RETAIN, stream);
