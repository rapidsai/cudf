  ...
  index_type const* const __restrict__ merged_indices)
{
  ...
  while (destination_row < num_destination_rows) {
    ...
    // bitmask element
    bit_mask_t const result_mask{__ballot_sync(active_threads, source_bit_is_valid)};

    cudf::size_type const output_element =
      cudf::util::detail::bit_container_index<bit_mask_t, cudf::size_type>(destination_row);

    // Only one thread writes output
    if (0 == threadIdx.x % warpSize) { destination_mask[output_element] = result_mask; }
...
rmm::device_vector<index_type> generate_merged_indices(device_table const& left_table,
                                                       ...
                                                       cudaStream_t stream)
{
  ...
  if (nullable) {
    auto ineq_op = row_inequality_comparator<true>(
      right_table, left_table, nulls_are_smallest, asc_desc.data().get());
            ...
            cudaStream_t stream = 0)
{
  ...
  CUDF_EXPECTS(std::equal(left_table_dtypes.cbegin(),
                          left_table_dtypes.cend(),
                          right_table_dtypes.cbegin(),
                          right_table_dtypes.cend()),
               "Mismatched column dtypes");
  CUDF_EXPECTS(key_cols.size() > 0, "Empty key_cols");
  CUDF_EXPECTS(key_cols.size() <= static_cast<size_t>(left_table.num_columns()),
               "Too many values in key_cols");
  CUDF_EXPECTS(asc_desc.size() > 0, "Empty asc_desc");
  CUDF_EXPECTS(asc_desc.size() <= static_cast<size_t>(left_table.num_columns()),
               "Too many values in asc_desc");
  ...
  std::vector<gdf_column*> left_cols_sync(const_cast<gdf_column**>(left_table.begin()),
                                          const_cast<gdf_column**>(left_table.end()));
  std::vector<gdf_column*> right_cols_sync(const_cast<gdf_column**>(right_table.begin()),
                                           const_cast<gdf_column**>(right_table.end()));
  for (cudf::size_type i = 0; i < left_table.num_columns(); i++) {
    ...
    if (left_col->dtype != GDF_STRING_CATEGORY) { continue; }
    ...
    *new_left_column_ptr = allocate_like(*left_col, RETAIN, stream);
    if (new_left_column_ptr->valid) {
      CUDA_TRY(cudaMemcpyAsync(new_left_column_ptr->valid,
                               left_col->valid,
                               sizeof(cudf::valid_type) * gdf_num_bitmask_elements(left_col->size),
                               cudaMemcpyDefault,
                               stream));
    ...
    *new_right_column_ptr = allocate_like(*right_col, RETAIN, stream);
    if (new_right_column_ptr->valid) {
      CUDA_TRY(cudaMemcpyAsync(new_right_column_ptr->valid,
                               right_col->valid,
                               sizeof(cudf::valid_type) * gdf_num_bitmask_elements(right_col->size),
                               cudaMemcpyDefault,
                               stream));
  ...
  std::transform(key_cols.cbegin(),
                 key_cols.cend(),
                 left_key_cols_vect.begin(),
  ...
  auto right_key_table =
    device_table::create(right_key_cols_vect.size(), right_key_cols_vect.data());
  rmm::device_vector<int8_t> asc_desc_d(asc_desc);

  rmm::device_vector<index_type> merged_indices = generate_merged_indices(
    *left_key_table, *right_key_table, asc_desc_d, nulls_are_smallest, stream);
  ...
  auto index_start_it = thrust::make_zip_iterator(thrust::make_tuple(
    thrust::make_counting_iterator(static_cast<cudf::size_type>(0)), merged_indices.begin()));
  ...
  thrust::for_each(rmm::exec_policy(stream)->on(stream),
                   ...
                   [=] __device__(auto const& idx_tuple) {
                     ...
                     device_table const& src_device_table =
                       src_side == side::LEFT ? left_device_table : right_device_table;
