                ...
                cudf::size_type in_begin)
{
  ...
  if (num_elements != 0) {  // otherwise no-op
    ...
    if (cudf::has_nulls(in_column)) {
      CUDF_EXPECTS(cudf::is_nullable(*out_column), "Expected nullable output column");
    }
    ...
    if (out_column->dtype == GDF_STRING_CATEGORY) {
      ...
      gdf_column *input_cols[2] = {&temp_out, const_cast<gdf_column *>(&in_column)};
      gdf_column *temp_cols[2]  = {out_column, &temp_in};

      // sync categories
      CUDF_EXPECTS(GDF_SUCCESS == sync_column_categories(input_cols, temp_cols, 2),
                   "Failed to synchronize NVCategory");

      detail::copy_range(
        out_column, detail::column_range_factory{temp_in, in_begin}, out_begin, out_end);

      gdf_column_free(&temp_out);
      gdf_column_free(&temp_in);
    } else {
      detail::copy_range(
        out_column, detail::column_range_factory{in_column, in_begin}, out_begin, out_end);
    }
  }
}
