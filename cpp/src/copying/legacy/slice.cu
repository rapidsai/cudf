                                     ...
                                     cudf::size_type const indices_position)
{
  ...
  cudf::size_type input_offset =
    cudf::util::detail::bit_container_index<bit_mask_t, cudf::size_type>(input_index_begin);
  cudf::size_type rotate_input =
    cudf::util::detail::intra_container_index<bit_mask_t, cudf::size_type>(input_index_begin);
  bit_mask_t mask_last =
    (bit_mask_t{1} << ((input_index_end - input_index_begin) % bit_mask::bits_per_element)) -
    bit_mask_t{1};

  cudf::size_type input_block_length = bit_mask::num_elements(input_size);
  cudf::size_type partition_block_length =
    bit_mask::num_elements(input_index_end - input_index_begin);
 ...
 public:
  ...
  void operator()()
  {
    ...
    for (cudf::size_type index = 0; index < columns_quantity; ++index) {
      // Empty output column
      if (output_columns_[index]->size == 0) { continue; }
      ...
      if (input_column_.valid != nullptr) {
        RMM_TRY(RMM_ALLOC(&(output_column->valid),
                          sizeof(cudf::valid_type) * gdf_valid_allocation_size(output_column->size),
                          stream));
      } else {
      ...
      slice_data_kernel<ColumnType><<<data_grid_config.num_blocks,
                                      data_grid_config.num_threads_per_block,
                                      NO_DYNAMIC_MEMORY,
                                      stream>>>(static_cast<ColumnType*>(output_column->data),
      ...
      if (input_column_.valid != nullptr) {
        ...
        auto valid_grid_config =
          cudf::util::cuda::grid_config_1d(gdf_num_bitmask_elements(output_column->size), 256);

        // Make a copy of the bitmask in the gdf_column
        slice_bitmask_kernel<<<valid_grid_config.num_blocks,
                               valid_grid_config.num_threads_per_block,
                               NO_DYNAMIC_MEMORY,
                               stream>>>(reinterpret_cast<bit_mask_t*>(output_column->valid),
        ...
        CUDA_TRY(cudaMemcpyAsync(&num_nulls,
                                 bit_set_counter.data().get(),
      ...
      if (output_column->dtype == GDF_STRING_CATEGORY) {
        CUDF_TRY(nvcategory_gather(output_column,
                                   static_cast<NVCategory*>(input_column_.dtype_info.category)));
      }
    }
  }
                               ...
                               std::vector<cudaStream_t> const& streams)
{
  ...
  if (input_column.size == 0) { return output_columns; }
  ...
  CUDA_TRY(cudaMemcpy(
    host_indices.data(), indices, num_indices * sizeof(cudf::size_type), cudaMemcpyDeviceToHost));
