                           ...
                           const cudf::size_type source_rows)
{
  ...
  if (tid < source_rows) {
    ...
    if (destination_row < destination_rows) { gather_map[destination_row] = tid; }
  }
}
...
struct dispatch_map_type {
                  ...
                  bool allow_negative_indices)
  {
    ...
    switch (gdf_dtype_of<map_type>()) {
      case GDF_INT8: fill_value.data.si08 = default_value; break;
      case GDF_INT16: fill_value.data.si16 = default_value; break;
      case GDF_INT32: fill_value.data.si32 = default_value; break;
      case GDF_INT64: fill_value.data.si64 = default_value; break;
      default: CUDF_FAIL("Invalid scatter map type");
    }
    ...
    if (allow_negative_indices) {
      ...
      invert_map<<<grid_config.num_blocks, grid_config.num_threads_per_block>>>(
        static_cast<map_type*>(gather_map.data),
        destination_table->num_rows(),
        thrust::make_transform_iterator(typed_scatter_map,
             ...
             bool allow_negative_indices)
{
  ...
  if (0 == source_table->num_rows()) { return; }

  type_dispatcher(scatter_map.dtype,
                  dispatch_map_type{},
                  source_table,
                  scatter_map,
                  destination_table,
             ...
             bool allow_negative_indices)
{
  ...
  detail::scatter(
    source_table, scatter_map_column, destination_table, check_bounds, allow_negative_indices);
}
                    ...
                    table* destination_table)
{
  ...
  for (int i = 0; i < n_cols; i++) {
    ...
    type_dispatcher(source[i].dtype,
                    scalar_scatterer{},
                    source[i],
                    scatter_map,
                    num_scatter_rows,
                    destination_table->get_column(i),
                    v_streams[i]);
  }
  ...
  for (int i = 0; i < n_cols; i++) {
    ...
    if (dest_col->valid) {
      ...
      auto bitmask_kernel =
        source[i].is_valid ? marking_bitmask_kernel<true> : marking_bitmask_kernel<false>;
...
inline bool validate_scatter_map(gdf_column const& scatter_map, cudf::table const& input)
{
  ...
  if (scatter_map.size == 0 || input.num_columns() == 0 || input.num_rows() == 0) return false;
  return true;
}
                                                   ...
                                                   cudf::size_type num_groups)
{
  ...
  for (cudf::size_type groupid = 0; groupid < num_groups; groupid++) {
    output_tables.push_back(
      detail::copy_if(input, [scatter_array, groupid] __device__(cudf::size_type row) {
...
std::vector<cudf::table> scatter_to_tables(cudf::table const& input, gdf_column const& scatter_map)
{
  ...
  return detail::ordered_scatter_to_tables(input, scatter_array, num_groups);
}
