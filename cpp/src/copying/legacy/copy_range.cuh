...
__global__ void copy_range_kernel(T *__restrict__ const data,
  ...
  const cudf::size_type begin_mask_idx = cudf::util::detail::bit_container_index<bit_mask_t>(begin);
  const cudf::size_type end_mask_idx   = cudf::util::detail::bit_container_index<bit_mask_t>(end);
  ...
  while (mask_idx <= end_mask_idx) {
    ...
    if (has_validity) {  // update bitmask
      ...
      if (lane_id == 0) {
        bit_mask_t new_mask = (old_mask & ~active_mask) | (warp_mask & active_mask);
  ...
  if (has_validity) {
    ...
    if (threadIdx.x < warp_size) {
      ...
      uint32_t block_null_change = cub::WarpReduce<uint32_t>(temp_storage).Sum(my_null_change);
  ...
  void operator()(gdf_column *column,
    ...
    if (cudf::is_nullable(*column)) {
      ...
      CUDA_TRY(cudaMemcpyAsync(
        null_count, &column->null_count, sizeof(cudf::size_type), cudaMemcpyHostToDevice, stream));
    ...
    bit_mask_t *__restrict__ bitmask = reinterpret_cast<bit_mask_t *>(column->valid);
    ...
    if (null_count != nullptr) {
      CUDA_TRY(cudaMemcpyAsync(
        &column->null_count, null_count, sizeof(cudf::size_type), cudaMemcpyDefault, stream));
...
void copy_range(gdf_column *out_column,
  ...
  cudf::type_dispatcher(
    out_column->dtype, copy_range_dispatch<InputFunctor>{input}, out_column, begin, end);

  // synchronize nvcategory after filtering
  if (out_column->dtype == GDF_STRING_CATEGORY) {
    CUDF_EXPECTS(
      GDF_SUCCESS ==
        nvcategory_gather(out_column, static_cast<NVCategory *>(out_column->dtype_info.category)),
