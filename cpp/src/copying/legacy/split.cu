                                                            ...
                                                            cudf::size_type const split_end)
{
  ...
  thrust::tabulate(slice_indices.begin() + 1, slice_indices.end() - 1, [splits] __device__(auto i) {
    return splits[i / 2];
  });
                               ...
                               cudf::size_type num_splits)
{
  if (num_splits == 0 || splits == nullptr) {
    ...
    return cudf::detail::slice(input_column, slice_indices.data().get(), slice_indices.size());
  }
}
