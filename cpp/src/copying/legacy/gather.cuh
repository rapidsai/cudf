  ...
  for (cudf::size_type i = 0; i < num_columns; i++) {
    ...
    if (dest_has_nulls) {
      ...
      while (destination_row_base < num_destination_rows) {
        ...
        cudf::size_type source_row = thread_active ? gather_map[destination_row] : 0;
        ...
        if (ignore_out_of_bounds) {
          if (0 <= source_row && source_row < num_source_rows) {
            source_bit_is_valid =
              src_has_nulls ? bit_mask::is_valid(source_valid_col, source_row) : true;
          } else {
      ...
      if (threadIdx.x == 0) { atomicAdd(d_count + i, valid_count_accumulate); }
    }
  }
}
...
struct column_gatherer {
                  ...
                  cudaStream_t stream)
  {
    ...
    bool const sync_category =
      std::is_same<column_type, cudf::nvstring_category>::value && sync_nvstring_category;
    ...
    if (sync_category) {
      ...
      CUDF_EXPECTS(GDF_SUCCESS == sync_column_categories(input_columns, output_columns, 2),
    ...
    if (ignore_out_of_bounds) {
      thrust::gather_if(rmm::exec_policy(stream)->on(stream),
                        gather_map,
                        gather_map + num_destination_rows,
                        gather_map,
                        source_data,
                        destination_data,
...
struct index_converter<map_type, index_conversion::NEGATIVE_TO_POSITIVE> {
  index_converter(cudf::size_type n_rows) : n_rows(n_rows) {}
...
struct index_converter<map_type, index_conversion::NONE> {
  ...
  __device__ map_type operator()(map_type in) const { return in; }
  cudf::size_type n_rows;
};
            ...
            bool allow_negative_indices)
{
  ...
  for (cudf::size_type i = 0; i < source_n_cols; i++) {
    ...
    cudf::type_dispatcher(src_col->dtype,
                          column_gatherer{},
                          src_col,
                          gather_map,
                          dest_col,
  ...
  for (cudf::size_type i = 0; i < source_n_cols; i++) {
    ...
    source_bitmasks_host[i]    = reinterpret_cast<bit_mask_t *>(source_table->get_column(i)->valid);
    // Allocate inplace buffer
    if (cudf::is_nullable(*dest_col) && dest_col->valid == source_table->get_column(i)->valid) {
  ...
  auto bitmask_kernel = ignore_out_of_bounds ? gather_bitmask_kernel<true, decltype(gather_map)>
                                             : gather_bitmask_kernel<false, decltype(gather_map)>;

  int gather_grid_size;
  int gather_block_size;
  CUDA_TRY(
    cudaOccupancyMaxPotentialBlockSize(&gather_grid_size, &gather_block_size, bitmask_kernel));

  bitmask_kernel<<<gather_grid_size, gather_block_size>>>(source_bitmasks.data().get(),
                                                          source_table->num_rows(),
                                                          gather_map,
                                                          destination_bitmasks.data().get(),
                                                          destination_table->num_rows(),
                                                          null_counts.data().get(),
                                                          source_n_cols);

  std::vector<cudf::size_type> h_count(source_n_cols);
  CUDA_TRY(cudaMemcpy(h_count.data(),
                      null_counts.data().get(),
                      sizeof(cudf::size_type) * source_n_cols,
  ...
  for (cudf::size_type i = 0; i < destination_table->num_columns(); i++) {
    ...
    if (is_nullable(*dest_col)) {
      ...
      if (dest_col->valid == source_table->get_column(i)->valid) {
        cudf::size_type num_bitmask_elements = gdf_num_bitmask_elements(dest_col->size);
        CUDA_TRY(cudaMemcpy(dest_col->valid,
                            destination_bitmasks_host[i],
