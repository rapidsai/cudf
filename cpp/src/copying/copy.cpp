 ...
 * Does not copy data.
 */
gdf_column allocate_like(gdf_column const& input, bool allocate_mask, cudaStream_t stream)
{
  ...
  if (input.size > 0) {
    ...
    if ((input.valid != nullptr) && allocate_mask) {
...
table allocate_like(table const& t, bool allocate_mask, cudaStream_t stream) {
  std::vector<gdf_column*> columns(t.num_columns());
  std::transform(columns.begin(), columns.end(), t.begin(), columns.begin(),
                 [allocate_mask,stream](gdf_column* out_col, gdf_column const* in_col) {
                   out_col = new gdf_column;
                   *out_col = allocate_like(*in_col,allocate_mask,stream);
                   return out_col;
                 });
