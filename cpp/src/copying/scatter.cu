...
{
  CUDF_FUNC_RANGE();
  return detail::scatter(source, scatter_map, target, check_bounds, cudf::get_default_stream(), mr);
}
...
{
  CUDF_FUNC_RANGE();
  return detail::scatter(source, indices, target, check_bounds, cudf::get_default_stream(), mr);
}
