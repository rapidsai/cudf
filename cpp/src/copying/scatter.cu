                               ...
                               rmm::mr::device_memory_resource* mr)
{
  ...
  // note: the intermediate ((in % n_rows) + n_rows) will overflow a size_type for any value of `in` > (2^31)/2, 
  // but the end result after the final (% n_rows) will fit. so we'll do the computation using a signed 64 bit value.
  auto scatter_iter = thrust::make_transform_iterator(
    map_begin, [n_rows = static_cast<int64_t>(n_rows)] __device__(size_type in) -> size_type { return ((static_cast<int64_t>(in) % n_rows) + n_rows) % n_rows; });
