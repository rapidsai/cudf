                                    ...
                                    cudf::memory_resources resources)
{
  ...
  if (target.nullable() or not source_is_valid) {
    if (not target.nullable()) {
      ...
      auto mask = detail::create_null_mask(target.size(), mask_state::ALL_VALID, stream, resources);
...
struct column_scalar_scatterer_impl {
                                     ...
                                     cudf::memory_resources resources) const
  {
    ...
    auto result      = std::make_unique<column>(target, stream, resources);
...
struct column_scalar_scatterer {
                                     ...
                                     cudf::memory_resources resources) const
  {
    ...
    return scatterer(source, scatter_iter, scatter_rows, target, stream, resources);
  }
};
                               ...
                               cudf::memory_resources resources)
{
  ...
  if (scatter_map.is_empty()) { return std::make_unique<table>(target, stream, resources); }
  ...
  return detail::scatter(source, map_begin, map_end, target, stream, resources);
}
                               ...
                               cudf::memory_resources resources)
{
  ...
  auto map_col = column_view(data_type{type_to_id<size_type>()},
                             ...
                             nullptr,
                             0);
  return detail::scatter(source, map_col, target, stream, resources);
}
                               ...
                               cudf::memory_resources resources)
{
  ...
  if (indices.is_empty()) { return std::make_unique<table>(target, stream, resources); }
...
std::unique_ptr<column> boolean_mask_scatter(scalar const& input,
                                             ...
                                             cudf::memory_resources resources)
{
  return detail::copy_if_else(input, target, boolean_mask, stream, resources);
}
                                            ...
                                            cudf::memory_resources resources)
{
  ...
  if (target.num_rows() != 0) {
    ...
    std::transform(
      ...
      target.begin(),
      out_columns.begin(),
      [&boolean_mask, resources, stream](auto const& input_column, auto const& target_column) {
        return boolean_mask_scatter(input_column, target_column, boolean_mask, stream, resources);
      });
...
{
  CUDF_FUNC_RANGE();
  return detail::scatter(source, scatter_map, target, stream, resources);
}
...
{
  CUDF_FUNC_RANGE();
  return detail::scatter(source, indices, target, stream, resources);
}
...
{
  CUDF_FUNC_RANGE();
  return detail::boolean_mask_scatter(input, target, boolean_mask, stream, resources);
}
