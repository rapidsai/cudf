...
bool needs_sanitize(cudf::column_view const& input, rmm::cuda_stream_view stream)
{
  ...
  if ((type == type_id::STRING || type == type_id::LIST) && input.nullable()) {
    ...
    if (thrust::any_of(rmm::exec_policy(stream), row_begin, row_end, is_dirty_row)) { return true; }
  }

  // For complex types, check child columns.
  if (type == type_id::STRUCT || type == type_id::LIST) {
    if (std::any_of(input.child_begin(), input.child_end(), [](auto const& child) {
          return needs_sanitize(child, rmm::cuda_stream_view{});
        })) {
      return true;
    }
  }
