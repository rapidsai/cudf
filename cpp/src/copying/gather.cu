...
struct dispatch_map_type {
      ...
      bool transform_negative_indices = false)
  {
    ...
    if (check_bounds) {
      ...
      gdf_index_type begin = (transform_negative_indices) ? -source_table->num_rows() : 0;
      CUDF_EXPECTS(
	...
	typed_gather_map,
	map_transform<map_type>{source_table->num_rows(), transform_negative_indices});

    gather(source_table, gather_map_iterator,
	destination_table, check_bounds,
	ignore_out_of_bounds, sync_nvstring_category, transform_negative_indices);
  }
