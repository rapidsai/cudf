  ...
  rmm::mr::device_memory_resource* mr)
{
  ...
  if (value.is_empty()) {
    return std::make_pair(empty_like(_keys),
                          scan_aggregation_result{make_empty_column(value.type())});
  }
  ...
  if (not value.has_nulls()) {
    return std::make_pair(std::move(sorted_keys),
                          scan_aggregation_result{std::move(grouped_values)});
  }
  ...
  return std::make_pair(std::move(sorted_keys), scan_aggregation_result{std::move(result)});
}
