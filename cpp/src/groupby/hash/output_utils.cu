                     ...
                     cudf::detail::result_cache* cache)
{
  ...
  for (size_t i = 0; i < aggregations.size(); i++) {
    auto const& agg = aggregations[i];
    auto& result    = result_cols[i];
    if (agg->kind != aggregation::COUNT_VALID && agg->kind != aggregation::COUNT_ALL &&
        result->nullable()) {
      // Aggregation results for non-null keys should not be nulls, even all the values
      // corresponding to these keys are nulls.
      result->set_null_count(values.num_rows() - num_keys);
    }
