  ...
  rmm::device_async_resource_ref mr)
{
  ...
  thrust::transform(
    ...
    cuda::counting_iterator{static_cast<size_type>(num_groups)},
    ...
    [d_offsets = offsets.template begin<size_type>(), not_null_pred] __device__(auto i) {
      return thrust::count_if(thrust::seq,
                              cuda::counting_iterator{static_cast<size_type>(d_offsets[i])},
                              cuda::counting_iterator{static_cast<size_type>(d_offsets[i + 1])},
                              not_null_pred);
    });
