                                  ...
                                  rmm::mr::device_memory_resource* mr)
{
  ...
  if (order_by.type().id() == type_id::STRUCT) {
    ...
    if (is_nested) {
      ...
      generate_rank_struct_comparisons<false>(
        order_by, mutable_ranks, group_labels, group_offsets, stream);
    }
  } else if (order_by.has_nulls()) {
    generate_rank_comparisons<true>(order_by, mutable_ranks, group_labels, group_offsets, stream);
  } else {
    generate_rank_comparisons<false>(order_by, mutable_ranks, group_labels, group_offsets, stream);
  }
