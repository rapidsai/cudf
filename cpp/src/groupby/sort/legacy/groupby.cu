      ...
      switch (element.second) {
        case MEDIAN: {
          *result_col = cudf::allocate_column(GDF_FLOAT64, groupby.num_groups(), false);
          ...
          cudf::detail::group_medians(
            sorted_values, groupby.group_offsets(), group_sizes, result_col, stream);
          break;
        }
        case QUANTILE: {
          ...
          *result_col = cudf::allocate_column(
          ...
          cudf::detail::group_quantiles(sorted_values,
                                        groupby.group_offsets(),
                                        group_sizes,
                                        result_col,
                                        args->quantiles,
                                        args->interpolation,
        ...
        case VARIANCE: {
          ...
          *result_col = cudf::allocate_column(GDF_FLOAT64, groupby.num_groups());

          cudf::detail::group_var(
            sorted_values, groupby.group_labels(), group_sizes, result_col, args->ddof, stream);
          break;
        }
        case STD: {
          auto args = static_cast<std_args*>(input_ops_args[i]);
                                        ...
                                        cudaStream_t& stream)
{
  ...
  cudf::size_type const output_size_estimate =
    cudf::util::round_up_safe((int64_t)groupby.num_groups(), (int64_t)sizeof(int32_t));

  cudf::table simple_values_table{simple_values_columns};

  cudf::table simple_output_values{
    output_size_estimate,
    target_dtypes(column_dtypes(simple_values_table), simple_operators),
    column_dtype_infos(simple_values_table),
    values_have_nulls,
    false,
    stream};
  ...
  cudf::groupby::sort::aggregate_all_rows<keys_have_nulls, values_have_nulls>
    <<<grid_params.num_blocks, grid_params.num_threads_per_block, 0, stream>>>(
      ...
      *d_output_values,
      ...
      options.ignore_null_keys,
      d_ops.data().get(),
      row_bitmask.data().get());

  std::transform(simple_output_values.begin(),
                 simple_output_values.end(),
                 simple_output_values.begin(),
...
template <bool keys_have_nulls, bool values_have_nulls>
std::pair<cudf::table, std::vector<gdf_column*>> compute_sort_groupby(
  cudf::table const& input_keys,
  cudf::table const& input_values,
  ...
  cudaStream_t stream)
{
  ...
  if (groupby.num_groups() == 0) {
    cudf::table output_values(
      0, target_dtypes(column_dtypes(input_values), input_ops), column_dtype_infos(input_values));
    return std::make_pair(cudf::empty_like(input_keys),
  ...
  std::transform(input_values.begin(),
                 input_values.end(),
                 input_ops.begin(),
  ...
  std::vector<SimpleAggRequestCounter> simple_requests = compound_to_simple(original_requests);
  ...
  if (simple_values_columns.size() > 0) {
    ...
    cudf::table simple_output_values =
      compute_simple_aggregations<keys_have_nulls, values_have_nulls>(
        input_keys, options, groupby, simple_values_columns, simple_operators, stream);
    ...
    current_output_values =
      compute_original_requests(original_requests, simple_requests, simple_output_values, stream);
  }
  // If there are "ordered" aggregation requests like MEDIAN, QUANTILE, compute these aggregations
  std::vector<gdf_column*> final_output_values = compute_ordered_aggregations(
    groupby, original_requests, input_ops_args, current_output_values, stream);

  // Update size and null count of output columns
  std::transform(final_output_values.begin(),
                 final_output_values.end(),
                 final_output_values.begin(),
                 [](gdf_column* col) {
                   CUDF_EXPECTS(col != nullptr, "Attempt to update Null column.");
                   set_null_count(*col);
                   return col;
  ...
  for (size_t i = 0; i < ops.size(); i++) {
    if (ops[i] == QUANTILE) {
      ...
      if (q_args == nullptr or q_args->quantiles.size() == 0) {
        CUDF_FAIL("Missing quantile aggregation arguments.");
      }
    }
  }
}
                                                         ...
                                                         cudaStream_t stream = 0)
{
  ...
  std::transform(
    ops.begin(), ops.end(), optype_list.begin(), [](auto const& op) { return op.op_name; });
  std::vector<operation_args*> ops_args(ops.size());
  std::transform(
    ops.begin(), ops.end(), ops_args.begin(), [](auto const& op) { return op.args.get(); });
  ...
  if (keys.num_rows() == 0) {
    cudf::table output_values(
      0, target_dtypes(column_dtypes(values), optype_list), column_dtype_infos(values));
    return std::make_pair(cudf::empty_like(keys),
                          std::vector<gdf_column*>{output_values.begin(), output_values.end()});
  }
