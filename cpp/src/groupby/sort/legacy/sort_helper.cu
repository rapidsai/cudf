...
cudf::size_type helper::num_keys()
{
  ...
  if (not _include_nulls and has_nulls(_keys)) {
    ...
    CUDF_TRY(
      gdf_count_nonzero_mask(reinterpret_cast<cudf::valid_type*>(keys_row_bitmask().data().get()),
                             _keys.num_rows(),
                             &_num_keys));
  } else {
...
gdf_column const& helper::key_sort_order()
{
  ...
  _key_sorted_order = gdf_col_pointer(
    new gdf_column(allocate_column(
      gdf_dtype_of<cudf::size_type>(), _keys.num_rows(), false, gdf_dtype_extra_info{}, _stream)),
  ...
  context.flag_null_sort_behavior =
    (_null_sort_behavior == null_order::AFTER) ? GDF_NULL_AS_LARGEST : GDF_NULL_AS_SMALLEST;

  if (_include_nulls || !cudf::has_nulls(_keys)) {  // SQL style
    CUDF_TRY(
      gdf_order_by(_keys.begin(), nullptr, _keys.num_columns(), _key_sorted_order.get(), &context));
  } else {  // Pandas style
    ...
    cudf::table modified_keys_table(modified_keys.data(), modified_keys.size());

    CUDF_TRY(gdf_order_by(modified_keys_table.begin(),
                          nullptr,
                          modified_keys_table.num_columns(),
                          _key_sorted_order.get(),
                          &context));
...
rmm::device_vector<cudf::size_type> const& helper::group_labels()
{
  ...
  thrust::inclusive_scan(
    exec->on(_stream), group_labels.begin(), group_labels.end(), group_labels.begin());
...
gdf_column const& helper::unsorted_keys_labels()
{
  ...
  _unsorted_keys_labels =
    gdf_col_pointer(new gdf_column(allocate_column(gdf_dtype_of<cudf::size_type>(),
                                                   key_sort_order().size,
                                                   true,
  ...
  CUDA_TRY(cudaMemsetAsync(_unsorted_keys_labels->valid,
                           0,
  ...
  gdf_column const& values)
{
  ...
  auto bitmask_iterator = thrust::make_transform_iterator(
    thrust::make_counting_iterator(0), [col_valid] __device__(cudf::size_type i) -> int {
...
cudf::table helper::unique_keys()
{
  ...
  thrust::transform(exec->on(_stream),
                    group_offsets().begin(),
                    group_offsets().end(),
                    transformed_group_ids.begin(),
                    [=] __device__(cudf::size_type i) { return idx_data[i]; });

  cudf::gather(&_keys, transformed_group_ids.data().get(), &unique_keys);
  return unique_keys;
}
