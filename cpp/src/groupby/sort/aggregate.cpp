  ...
  cache.add_result(
    ...
    get_grouped_values().nullable()
      ...
      : detail::group_count_all(
          helper.group_offsets(stream), helper.num_groups(stream), stream, resources));
}
...
void aggregate_result_functor::operator()<aggregation::HISTOGRAM>(aggregation const& agg)
{
  ...
  cache.add_result(values,
                   agg,
                   detail::group_histogram(get_grouped_values(),
                                           helper.group_labels(stream),
                                           helper.num_groups(stream),
                                           stream,
                                           resources));
...
void aggregate_result_functor::operator()<aggregation::SUM>(aggregation const& agg)
{
  ...
  cache.add_result(values,
                   agg,
                   detail::group_sum(get_grouped_values(),
                                     helper.num_groups(stream),
                                     helper.group_labels(stream),
                                     stream,
                                     resources));
...
void aggregate_result_functor::operator()<aggregation::PRODUCT>(aggregation const& agg)
{
  ...
  cache.add_result(values,
                   agg,
                   detail::group_product(get_grouped_values(),
                                         helper.num_groups(stream),
                                         helper.group_labels(stream),
                                         stream,
                                         resources));
...
void aggregate_result_functor::operator()<aggregation::MIN>(aggregation const& agg)
{
  ...
  auto result = [&]() {
    ...
    if (cudf::is_fixed_width(values_type)) {
      return detail::group_min(get_grouped_values(),
                               helper.num_groups(stream),
                               helper.group_labels(stream),
                               stream,
                               resources);
...
void aggregate_result_functor::operator()<aggregation::MAX>(aggregation const& agg)
{
  ...
  auto result = [&]() {
    auto values_type = cudf::is_dictionary(values.type())
                         ...
                         : values.type();
    if (cudf::is_fixed_width(values_type)) {
      return detail::group_max(get_grouped_values(),
                               helper.num_groups(stream),
                               helper.group_labels(stream),
                               stream,
                               resources);
...
void aggregate_result_functor::operator()<aggregation::M2>(aggregation const& agg)
{
  ...
  cache.add_result(
    ...
    agg,
    detail::group_m2(
      get_grouped_values(), mean_result, helper.group_labels(stream), stream, resources));
}
...
void aggregate_result_functor::operator()<aggregation::STD>(aggregation const& agg)
{
  ...
  auto result = cudf::detail::unary_operation(var_result, unary_operator::SQRT, stream, resources);
  cache.add_result(values, agg, std::move(result));
}
...
void aggregate_result_functor::operator()<aggregation::MERGE_HISTOGRAM>(aggregation const& agg)
{
  ...
  cache.add_result(values,
                   agg,
                   detail::group_merge_histogram(get_grouped_values(),
                                                 helper.group_offsets(stream),
                                                 helper.num_groups(stream),
                                                 stream,
                                                 resources));
...
void aggregate_result_functor::operator()<aggregation::COVARIANCE>(aggregation const& agg)
{
  ...
  auto mean_agg = make_mean_aggregation();
  aggregate_result_functor(values_child0, helper, cache, stream, resources)
    .operator()<aggregation::MEAN>(*mean_agg);
  aggregate_result_functor(values_child1, helper, cache, stream, resources)
    .operator()<aggregation::MEAN>(*mean_agg);
...
void aggregate_result_functor::operator()<aggregation::CORRELATION>(aggregation const& agg)
{
  ...
  cache.add_result(
    values, agg, detail::group_correlation(covariance, stddev0, stddev1, stream, resources));
}
...
void aggregate_result_functor::operator()<aggregation::TOP_K>(aggregation const& agg)
{
  ...
  auto result = detail::group_top_k(
    k, topk_order, get_grouped_values(), helper.group_offsets(stream), stream, resources);
  cache.add_result(values, agg, std::move(result));
}
...
void aggregate_result_functor::operator()<aggregation::HOST_UDF>(aggregation const& agg)
{
  ...
  cache.add_result(values, agg, (*udf_ptr)(stream, resources));
}
  ...
  cudf::memory_resources resources)
{
  ...
  for (auto const& request : requests) {
    auto store_functor =
      detail::aggregate_result_functor(request.values, helper(), cache, stream, resources);
  ...
  auto results = detail::extract_results(requests, cache, stream, resources);

  return std::pair(helper().unique_keys(stream, resources), std::move(results));
}
}  // namespace groupby
}  // namespace cudf
