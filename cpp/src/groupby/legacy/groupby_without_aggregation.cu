...
gdf_column gdf_unique_indices(cudf::table const& input_table, gdf_context const& context)
{
  ...
  RMM_TRY(RMM_ALLOC(&unique_indices.data, sizeof(cudf::size_type) * nrows, nullptr));
  ...
  if (nullable) {
    ...
    result_end = thrust::unique_copy(rmm::exec_policy(stream)->on(stream),
                                     counting_iter,
                                     counting_iter + nrows,
                                     static_cast<cudf::size_type*>(unique_indices.data),
                                     comp);
  } else {
  ...
  unique_indices.size =
    thrust::distance(static_cast<cudf::size_type*>(unique_indices.data), result_end);
  ...
  gdf_context* context)
{
  ...
  if (context->flag_groupby_include_nulls || !cudf::has_nulls(key_col_table)) {  // SQL style
    CUDF_TRY(gdf_order_by(
      key_col_table.begin(), nullptr, key_col_table.num_columns(), &sorted_indices_col, context));
  } else {  // Pandas style
    ...
    CUDF_TRY(gdf_count_nonzero_mask(
      reinterpret_cast<cudf::valid_type*>(key_cols_bitmask.data().get()), nrows, &valid_count));

    std::for_each(destination_table.begin(),
                  destination_table.end(),
