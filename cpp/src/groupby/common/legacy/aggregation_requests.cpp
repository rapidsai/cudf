...
std::vector<SimpleAggRequestCounter> compound_to_simple(
  ...
  std::for_each(compound_requests.begin(),
                compound_requests.end(),
...
gdf_column* compute_average(gdf_column sum, gdf_column count, cudaStream_t stream) {
  ...
  if (cudf::is_nullable(sum) or cudf::is_nullable(count)) {
    RMM_TRY(RMM_ALLOC(
      &avg->valid, sizeof(cudf::size_type) * gdf_valid_allocation_size(sum.size), stream));
  }
...
table compute_original_requests(std::vector<AggRequestType> const& original_requests,
  ...
  for (std::size_t i = 0; i < simple_requests.size(); ++i) {
    ...
    simple_requests_to_outputs[simple_requests[i].first] = {simple_outputs.get_column(i),
                                                            simple_requests[i].second};
  }
  ...
  for (size_t i = 0; i < original_requests.size(); ++i) {
    ...
    if (req.second == MEAN) {
      ...
      CUDF_EXPECTS(found != simple_requests_to_outputs.end(), "SUM request missing.");
      ...
      CUDF_EXPECTS(found != simple_requests_to_outputs.end(), "COUNT request missing.");
  ...
  for (size_t i = 0; i < original_requests.size(); ++i) {
    ...
    if (is_simple(req.second)) {
      ...
      CUDF_EXPECTS(found != simple_requests_to_outputs.end(), "Aggregation missing!");

      auto& pair_col_counter = found->second;
      CUDF_EXPECTS(pair_col_counter.first != nullptr, "Simple aggregation result is null.");
