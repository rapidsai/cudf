...
__device__ __inline__ cudf::size_type single_lane_popc_block_reduce(bit_container bit_mask) {
  ...
  // reduction
  if (lane_id == lane) { warp_count[warp_id] = __popc(bit_mask); }
  __syncthreads();
  ...
  if (warp_id == 0) {
    ...
    cudf::size_type count = (lane_id < (blockDim.x / warp_size)) ? warp_count[lane_id] : 0;

    __shared__ typename cub::WarpReduce<cudf::size_type>::TempStorage temp_storage;
...
__global__ void valid_if_kernel(const bit_container* source_mask,
  ...
  while (bit_index_base < num_bits) {
    ...
    bit_container active_threads = __ballot_sync(0xffffffff, thread_active);

    bit_container result_mask = 0;

    if (thread_active) {
      bool const predicate_is_true      = p(bit_index);
      const bit_container ballot_result = __ballot_sync(active_threads, predicate_is_true);
      ...
      if (0 == threadIdx.x % warp_size) {
        ...
        result_mask =
          source_mask_valid ? source_mask[container_index] & ballot_result : ballot_result;
        destination_mask[container_index] = result_mask;
      }
    }

    result_mask = single_lane_popc_block_reduce(result_mask);
    if (0 == threadIdx.x) { atomicAdd(p_valid_count, result_mask); }
...
std::pair<bit_container*, size_type> valid_if(const bit_container* source_mask,
  ...
  auto kernel = source_mask ? detail::valid_if_kernel<true, bit_container, predicate, size_type>
  ...
  CUDA_TRY(cudaMemcpyAsync(&valid_count_host,
                           valid_count.data().get(),
                           sizeof(size_type),
