...
__global__ void gpu_transpose(ColumnType **in_cols,
  ...
  for (cudf::size_type i = x; i < ncols; i += stride_x) {
    for (cudf::size_type j = y; j < nrows; j += stride_y) { out_cols[j][i] = in_cols[i][j]; }
  }
}
 ...
 */
__global__ void gpu_transpose_valids(cudf::valid_type **in_cols_valid,
  ...
  while (i < ncols) {
    ...
    while (j < nrows) {
      ...
      MaskType *const __restrict__ out_mask32 = reinterpret_cast<MaskType *>(out_cols_valid[j]);
...
struct launch_kernel {
  template <typename ColumnType>
  gdf_error operator()(void **in_cols_data_ptr,
                       void **out_cols_data_ptr,
                       cudf::valid_type **in_cols_valid_ptr,
                       cudf::valid_type **out_cols_valid_ptr,
    ...
    gpu_transpose<ColumnType>
      <<<dimGrid, dimBlock>>>(reinterpret_cast<ColumnType **>(in_cols_data_ptr),
                              reinterpret_cast<ColumnType **>(out_cols_data_ptr),
                              ncols,
                              nrows);
    if (has_null) {
      gpu_transpose_valids<<<dimGrid, dimBlock>>>(
        in_cols_valid_ptr, out_cols_valid_ptr, out_cols_nullct_ptr, ncols, nrows);
    }
...
gdf_error gdf_transpose(cudf::size_type ncols, gdf_column **in_cols, gdf_column **out_cols) {
  ...
  bool const has_null{
    std::any_of(in_cols, in_cols + ncols, [](gdf_column *col) { return col->null_count > 0; })};
