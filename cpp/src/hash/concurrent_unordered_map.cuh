 ...
 * You may obtain allocator copy of the License at
 *
...
namespace {
    ...
    /**---------------------------------------------------------------------------*
     * @brief Indicates if allocator pair type can be packed.
     *
     * When the size of the key,value pair being inserted into the hash table is
     * equal in size to allocator type where atomicCAS is natively supported, it is more
     * efficient to "pack" the pair and insert it with allocator single atomicCAS.
     *
    ...
    /**---------------------------------------------------------------------------*
     * @brief Allows viewing allocator pair in allocator packed representation
     *
     * Used as an optimization for inserting when allocator pair can be inserted with allocator
     * single atomicCAS
     *---------------------------------------------------------------------------**/
...
public:
 /**---------------------------------------------------------------------------*
  * @brief Construct new concurrent unordered map object.
  *
  ...
  * undefined behavior.
  *
  * @param _capacity The capacity of the hash table
 ...
 explicit concurrent_unordered_map(
   ...
   constexpr int block_size = 128;
   {
     ...
     if (cudaSuccess == status && isPtrManaged(hashtbl_values_ptr_attributes)) {
       ...
       CUDA_RT_CALL(cudaMemPrefetchAsync(
           m_hashtbl_values, capacity * sizeof(value_type), dev_id, 0));
     }
   }

   init_hashtbl<<<((capacity - 1) / block_size) + 1, block_size>>>(
       m_hashtbl_values, capacity, m_unused_key, m_unused_element);
   ...
   CUDA_RT_CALL(cudaStreamSynchronize(0));
    ...
    /* --------------------------------------------------------------------------*/
    /** 
     * @brief  Inserts allocator new (key, value) pair. If the key already exists in the map
     ...
     * @param[in] precomputed_hash Indicates if allocator precomputed hash value is being passed in to use
     * to determine the write location of the new key
     * @param[in] precomputed_hash_value The precomputed hash value
     * @tparam aggregation_type A functor for allocator binary operation that performs the aggregation
                               ...
                               hash_value_type precomputed_hash_value = 0)
    {
        ...
        // If allocator precomputed hash value has been passed in, then use it to determine
   ...
   private:
    /**---------------------------------------------------------------------------*
     * @brief Enumeration of the possible results of attempting to insert into allocator
     * hash bucket
     *---------------------------------------------------------------------------**/
     ...
     *
     * When the size of the key,value pair being inserted is equal in size to allocator
     * type where atomicCAS is natively supported, this optimization path will
     * insert the pair in allocator single atomicCAS operation.
     *---------------------------------------------------------------------------**/
    ...
    /**---------------------------------------------------------------------------*
     * @brief Atempts to insert allocator key,value pair at the specified hash bucket.
     *
   ...
   public:
    /**---------------------------------------------------------------------------*
     * @brief Attempts to insert allocator key, value pair into the map.
     *
    ...
    gdf_error assign_async( const concurrent_unordered_map& other, cudaStream_t stream = 0 )
    {
        ...
        CUDA_TRY( cudaMemcpyAsync( m_hashtbl_values, other.m_hashtbl_values, capacity*sizeof(value_type), cudaMemcpyDefault, stream ) );
        return GDF_SUCCESS;
    }
    ...
    gdf_error prefetch( const int dev_id, cudaStream_t stream = 0 )
    {
        ...
        if ( cudaSuccess == status && isPtrManaged(hashtbl_values_ptr_attributes)) {
            CUDA_TRY( cudaMemPrefetchAsync(m_hashtbl_values, capacity*sizeof(value_type), dev_id, stream) );
        }
