...
struct dispatch_to_arrow {
    ...
    cudaStream_t stream)
  {
    ...
    std::transform(child_indices.begin(),
                   ...
                   [&input_view, &field_names, &ar_mr, &stream](auto const& i) {
                     auto c = input_view.child(i);
                     return type_dispatcher(
                       c.type(), dispatch_to_arrow{}, c, c.type().id(), field_names, ar_mr, stream);
                   });
    return child_arrays;
  }
...
template <>
std::shared_ptr<arrow::Array> dispatch_to_arrow::operator()<bool>(
  column_view input,
  cudf::type_id id,
  std::vector<std::string> const& field_names,
  arrow::MemoryPool* ar_mr,
  ...
  cudaStream_t stream)
{
  ...
  auto child_arrays      = fetch_child_array(input_view, field_names, ar_mr, stream);
...
std::shared_ptr<arrow::Array> dispatch_to_arrow::operator()<cudf::struct_view>(
  ...
  cudaStream_t stream)
{
  CUDF_EXPECTS(field_names.size() == input.num_children(),
  ...
  cudaStream_t stream)
{
  ...
  auto dictionary = type_dispatcher(dict_keys.type(),
                                    dispatch_to_arrow{},
                                    dict_keys,
                                    dict_keys.type().id(),
                                    field_names,
  ...
  cudaStream_t stream)
{
  ...
  std::transform(input.begin(), input.end(), std::back_inserter(arrays), [&](auto const& c) {
    auto field_names = (c.type().id() == type_id::STRUCT) ? nested_type_field_names[index++]
                                                          : std::vector<std::string>{};
    return c.type().id() != type_id::EMPTY ? type_dispatcher(c.type(),
                                                             detail::dispatch_to_arrow{},
                                                             c,
                                                             c.type().id(),
                                                             field_names,
                                                             ar_mr,
  ...
  arrow::MemoryPool* ar_mr)
{
  ...
  return detail::to_arrow(input, column_names, nested_type_field_names, ar_mr);
}
