...
gdf_error nvcategory_scatter_table(cudf::table source_table, cudf::table destination_table){
  ...
  CUDF_EXPECTS(n_cols == destination_table.num_columns(), "column number mismatch");
  
  for (gdf_size_type i = 0; i < n_cols; i++) {
    
    gdf_column* dest_col = destination_table.get_column(i);
    gdf_column* src_col = source_table.get_column(i);
    
    if(dest_col->dtype == GDF_STRING_CATEGORY){
      
      CUDF_EXPECTS(src_col->dtype == dest_col->dtype, "Column type mismatch");
      gdf_column* input_cols[2] = {src_col, dest_col};
      
      gdf_column temp_src = cudf::copy(*src_col);
      gdf_column temp_dest  = cudf::copy(*dest_col);
      gdf_column* temp_cols[2] = {&temp_src, &temp_dest};
      ...
      std::swap(dest_col->data, temp_dest.data);
      std::swap(dest_col->valid, temp_dest.valid);
      std::swap(dest_col->null_count, temp_dest.null_count);
      std::swap(dest_col->dtype_info.category, temp_dest.dtype_info.category);
    
      std::swap(src_col->data, temp_src.data);
      std::swap(src_col->valid, temp_src.valid);
      std::swap(src_col->null_count, temp_src.null_count);
      std::swap(src_col->dtype_info.category, temp_src.dtype_info.category);
