...
std::unique_ptr<cudf::column> generate_ngrams(
  ...
  std::unique_ptr<cudf::column> non_empty_offsets_column = [&] {
    cudf::column_view offsets_view(cudf::data_type{cudf::INT32},
                                   strings_count + 1,
                                   strings.offsets().data<int32_t>());
    auto table_offsets =
      cudf::experimental::detail::copy_if(
  ...
  auto offsets_column = cudf::strings::detail::make_offsets_child_column(
    ...
    offsets_transformer_itr + ngrams_count,
    mr,
    stream);
  ...
  cudf::size_type const total_bytes =
    thrust::device_pointer_cast(d_offsets)[ngrams_count];
  auto chars_column = cudf::strings::detail::create_chars_child_column(
    ngrams_count, 0, total_bytes, mr, stream);
