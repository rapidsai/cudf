...
__device__ __forceinline__ void __init_data_and_mark_word_start_and_ends(
...
__device__ __forceinline__ void __mark_sentence_start_and_ends(uint32_t* code_points,
...
/*
  ...
  start_word_indices: An array which will contain the starting index for each word scattered
  throughout. If an index does not represent a word start, the max uint32_t value is written to
  indicate this. A post processing step is required to select all the relevant values from this
  array.

  end_word_indices: An array which will contain the one past the end index for each word scattered
  throughout. If an index does not represent a word end, the max uint32_t value is written to
  indicate this. A post processing step is required to select all the relevant values from this
  array.
  ...
  token_ids: The array which will hold the token ids. This kernel initialized all values in this
  array to the max uint32_t. It is assumed that the length of this array is num_code_points.

  tokens_per_word: The array which will hold the number of tokens in each word. This kernel
                                                       ...
                                                       uint8_t* tokens_per_word)
{
  ...
  __init_data_and_mark_word_start_and_ends(code_points,
                                           start_word_indices,
                                           end_word_indices,
                                           num_code_points,
                                           token_ids,
...
/*
  Writes the indicies of the characters that start sentences in the start_word_indices array and
  index of the character after the last character in the sentence to the end_word_indices array.
  ...
  sentence_offsets: an array containing the index of the starting character of each sentence with
                    an extra space at the end containing the total number of characters. As a
  result, this array is of length num_sentences + 1.
...
{
  uint32_t char_for_thread = blockDim.x * blockIdx.x + threadIdx.x;
  __mark_sentence_start_and_ends(code_points,
                                 sentence_offsets,
                                 start_word_indices,
                                 end_word_indices,
                                 num_sentences,
...
*/
__device__ __forceinline__ void __wordPieceTokenize(uint32_t* code_points,
                                                    uint64_t* hash_table,
...
/*
  ...
  token_ids: The index for each token found during tokenization. This is of length num_code_points.
             ...
             uint32_t will be in place. Cub will be used later to filter out these invalid ids
  later.
  ...
  word_ends: An array of length num_code_points. The first total_words elements contains the
             ...
             array containing the word start code points. Thus, word_ends[word] -
  filtered_start_indices[word] = word_length

  tokens_per_word: An array of size num_code_points that will contain the number of tokens in each
                   word in a sentence.
                   This array can be exclusive summed and the result used in conjunction with the
  sentence lengths array to find the tokens in each sentence. This is possible since the number of
                   tokens in each word will be placed at the index corresponding to the start
  character of a word. If we assume prefix_summed is the prefix sum of the tokens_per_word array,
  then prefix_summed[sentence_lengths[sentence] - 1] is the number of tokens found before the start
  of sentence.
...
*/
__global__ void gpuWordPieceTokenizer(uint32_t* code_points,
                                      uint64_t* hash_table,
                                      uint64_t* bin_coefficients,
                                      uint16_t* bin_offsets,
                                      uint32_t* token_ids,
                                      ...
                                      uint16_t num_outer_bins)
{
  ...
  if (word_to_tokenize < total_words) {
    ...
    __wordPieceTokenize(code_points,
                        hash_table,
                        bin_coefficients,
                        bin_offsets,
                        token_ids,
...
GpuWordPieceTokenizer::GpuWordPieceTokenizer(std::string const& vocab_file,
                                             uint32_t max_num_chars,
                                             uint32_t max_inp_chars_per_word)
  : device_token_ids{},
    device_word_indices{},
                                     ...
                                     ptr_length_pair<uint32_t*>& offsets_and_length)
{
  ...
  detail::init_data_and_mark_word_start_and_ends<<<num_blocks, threads_per_block>>>(
    device_code_points,
  ...
  detail::mark_sentence_start_and_ends<<<word_split_blocks, threads_per_block>>>(
    device_code_points,
  ...
  device_num_selected.resize(1);
  assertCudaSuccess(cudaMemcpy(&num_words,
                               thrust::raw_pointer_cast(device_num_selected.data()),
                               sizeof(num_words),
  ...
  detail::gpuWordPieceTokenizer<<<num_wp_blocks, wp_threads_per_block>>>(
    device_code_points,
  ...
  update_sentence_lengths<<<SEN_KERNEL_BLOCKS, sen_update_num_threads>>>(
  ...
  assertCudaSuccess(cudaMemcpy(&total_token_ids,
