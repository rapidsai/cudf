...
std::unique_ptr<cudf::column> token_count_fn(
  cudf::size_type strings_count,
  TokenCounter tokenizer,
  rmm::mr::device_memory_resource* mr,
  cudaStream_t stream) {
  // create output column
  auto token_counts   = cudf::make_numeric_column(cudf::data_type{cudf::INT32},
                                                strings_count,
...
std::unique_ptr<cudf::column> tokenize_fn(cudf::size_type strings_count,
  ...
  auto const token_counts =
    token_count_fn(strings_count, tokenizer, mr, stream);
...
std::unique_ptr<cudf::column> tokenize(cudf::strings_column_view const& strings,
  ...
  return tokenize_fn(strings.size(),
                     strings_tokenizer{*strings_column, d_delimiter},
                     mr,
                     stream);
}
...
std::unique_ptr<cudf::column> tokenize(
  ...
  CUDF_EXPECTS(!delimiters.has_nulls(),
               "Parameter delimiters must not have nulls");
  auto strings_column =
    cudf::column_device_view::create(strings.parent(), stream);
  auto delimiters_column =
    cudf::column_device_view::create(delimiters.parent(), stream);
  return tokenize_fn(strings.size(),
