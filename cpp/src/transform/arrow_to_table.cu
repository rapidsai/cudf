...
struct dispatch_to_cudf_col {
    template <typename T>
    std::enable_if_t<is_fixed_width<T>(), std::pair<std::unique_ptr<column>, column_view>>
    ...
    template <typename T>
    std::enable_if_t<is_compound<T>(), std::pair<std::unique_ptr<column>, column_view>>
    operator()(const std::shared_ptr<arrow::Array> arrow_col,
               data_type type,
               bool skip_mask,
               rmm::mr::device_memory_resource* mr,
        ...
        if (std::is_same<T, cudf::string_view>::value) {
            ...
            out_col = make_dictionary_column(std::move(keys_column), std::move(indices_column), std::move(*mask), static_cast<size_type>(arrow_col->null_count()));
        } else {
            auto list_array = static_cast<arrow::ListArray *>(arrow_col.get());
            auto offset_array = std::make_shared<arrow::Int32Array>(list_array->value_offsets()->size()/sizeof(int32_t), list_array->value_offsets(), nullptr);
            auto offsets = type_dispatcher(data_type(type_id::INT32), dispatch_to_cudf_col{}, offset_array, data_type(type_id::INT32), true, mr, stream);
            auto offsets_column = (offsets.first->size() == offset_array->length())? std::move(offsets.first) : std::make_unique<column>(offsets.second, stream, mr);

            auto child_type = arrow_to_cudf_type(list_array->values()->type()->id());
            auto child = type_dispatcher(child_type, dispatch_to_cudf_col{}, list_array->values(), child_type, false, mr, stream);
            auto child_column = (child.first->size() == list_array->values()->length())? std::move(child.first) : std::make_unique<column>(child.second, stream, mr);

            auto num_rows = offsets_column->size()-1;
            out_col = make_lists_column(num_rows, std::move(offsets_column), std::move(child_column), static_cast<size_type>(arrow_col->null_count()), std::move(*mask)); 
        }

        return std::make_pair<std::unique_ptr<column>, column_view> (std::move(out_col), slice(out_col->view(), static_cast<size_type>(arrow_col->offset()), static_cast<size_type>(arrow_col->length())));
    }

    template <typename T>
    std::enable_if_t<((!is_fixed_width<T>()) and (!is_compound<T>())), std::pair<std::unique_ptr<column>, column_view>>
    operator()(const std::shared_ptr<arrow::Array> arrow_col,
               data_type type,
               bool skip_mask,
               rmm::mr::device_memory_resource* mr,
...
std::unique_ptr<table> arrow_to_table(std::shared_ptr<arrow::Table> input_table,
    ...
    for (auto const& chunks_col : input_table->columns()) {
        ...
        for (auto const& c : chunks_col->chunks()) {
            auto col_and_view = type_dispatcher(cudf_type, dispatch_to_cudf_col{}, c, cudf_type, false, mr, stream);
