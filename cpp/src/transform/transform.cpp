...
void transform(gdf_column* out, const gdf_column* in, const std::string& ptx,
               const std::string& output_type) {
  // Check for null pointers in input
  CUDF_EXPECTS((out != nullptr) && (in != nullptr), "Input pointers are null");

  // Check for 0 sized data
  if ((out->size == 0) && (in->size == 0)) return;
  CUDF_EXPECTS((out->size == in->size), "Column sizes don't match");

  // Check for null data pointer
  CUDF_EXPECTS((out->data != nullptr) && (in->data != nullptr),
               "Column data pointers are null");

  // Check for datatype
  // Input and output data types can be different but they have to be one of the
  // following four.
  ...
  transformation::col_valid_mask_and(out->null_count, out->valid, in->valid,
                                     in->size);
...
gdf_column transform(const gdf_column& input,
  ...
  transformation::transform(&output_col, &input, ptx_unary_function,
