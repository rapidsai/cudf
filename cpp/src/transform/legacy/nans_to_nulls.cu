...
{
  if (input.size == 0) { return std::pair<bit_mask_t*, cudf::size_type>(nullptr, 0); }
  ...
  switch (input.dtype) {
    ...
    case GDF_FLOAT64:
      return cudf::valid_if(
        source_mask, cudf::detail::predicate_not_nan<double>(input), input.size);
    default: CUDF_FAIL("Unsupported data type for isnan()");
  }
}
//
}  // namespace cudf
