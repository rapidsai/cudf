                                       ...
                                       cudf::memory_resources resources)
{
  ...
  std::unique_ptr<column> keys_column = [&] {
    ...
    auto keys_positions = [&] {
      ...
      thrust::sequence(
        rmm::exec_policy_nosync(stream, resources.get_temporary_mr()), itr, itr + keys_view.size());
      return positions;
    }();
    // copy the non-removed keys ( keys_to_keep_fn(idx)==true )
    auto table_keys =
      cudf::detail::copy_if(
        table_view{{keys_view, keys_positions->view()}}, keys_to_keep_fn, stream, resources)
        ->release();
