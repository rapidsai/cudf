...
struct sort_functor
{
    ...
    __host__ __device__ bool operator()(size_type lhs_index, size_type rhs_index)
    {
        auto comparator = experimental::element_relational_comparator<true>{ // true=has_nulls
                          d_column, d_column, null_order::BEFORE }; // keep nulls up front
                                                ...
                                                cudaStream_t stream = 0)
{
    ...
    // if there are nulls, we need to shift keys left and subtract 1 from all the indices
    if( input_column.has_nulls() )
    {
        d_keys_indices++; // skip the null entry
        unique_count--;
        // subtract 1 from all the indices values
        thrust::transform( execpol->on(stream), d_indices, d_indices+count, d_indices,
                           [] __device__ (size_type value) { return value-1; } );
    }
