...
struct replace_multi_regex_fn
{
    ...
    __device__ size_type operator()(size_type idx)
    {
        ...
        while( ch_pos < nchars )
        {
            ...
            auto itr = thrust::find_if( thrust::seq, d_ranges, d_ranges + number_of_patterns,
                [ch_pos] __device__ (auto range) { return range.first==ch_pos; } );
            ...
            {   // match found, compute and replace the string in the output
                ...
                if( out_ptr )
                {   // copy unmodified content plus new replacement string
                                    ...
                                    cudaStream_t stream = 0)
{
    ...
    if( (regex_insts > MAX_STACK_INSTS) || (regex_insts <= RX_SMALL_INSTS) )
        children = make_strings_children(
            replace_multi_regex_fn<RX_STACK_SMALL>{d_strings,d_progs,static_cast<size_type>(progs.size()),d_found_ranges,d_repls},
            strings_count, null_count, mr, stream);
    else if( regex_insts <= RX_MEDIUM_INSTS )
        children = make_strings_children(
            replace_multi_regex_fn<RX_STACK_MEDIUM>{d_strings,d_progs,static_cast<size_type>(progs.size()),d_found_ranges,d_repls},
            strings_count, null_count, mr, stream);
    else
        children = make_strings_children(
            replace_multi_regex_fn<RX_STACK_LARGE>{d_strings,d_progs,static_cast<size_type>(progs.size()),d_found_ranges,d_repls},
            strings_count, null_count, mr, stream);
    //
