...
gdf_error nvcategory_gather_table(cudf::table source_table, cudf::table destination_table)
{
  ...
  for (int i = 0; i < source_table.num_columns(); i++) {
    ...
    if (original_column->dtype == GDF_STRING_CATEGORY) {
      ...
      nvcategory_gather(output_column,
                        static_cast<NVCategory*>(original_column->dtype_info.category));
    }
  }
  return GDF_SUCCESS;
}
...
gdf_error nvcategory_gather(gdf_column* column, NVCategory* nv_category)
{
  ...
  if (column->null_count > 0) {
    ...
    CUDA_TRY(cudaMemcpyAsync(
      column->data, column_nulls_replaced.data, column->size * byte_width, cudaMemcpyDefault, 0));
  ...
  NVCategory* new_category = nv_category->gather(
    static_cast<nv_category_index_type*>(column->data), column->size, DEVICE_ALLOCATED);
  if (destroy_category) { NVCategory::destroy(nv_category); }
  CHECK_CUDA(0);
                            ...
                            int num_columns)
{
  ...
  combined_category->get_values(static_cast<nv_category_index_type*>(output_column->data), true);
                                 ...
                                 int num_columns)
{
  ...
  for (int column_index = 0; column_index < num_columns; column_index++) {
    GDF_REQUIRE(input_columns[column_index]->size == output_columns[column_index]->size,
