...
CUDF_HDI Result get_array_value(T const* devarr, size_type location)
{
  T result;
...
template <typename Result, typename T>
CUDF_HDI Result linear(T lhs, T rhs, double frac)
{
...
template <typename Result, typename T>
CUDF_HDI Result midpoint(T lhs, T rhs)
{
...
CUDF_HDI Result midpoint(int64_t lhs, int64_t rhs)
{
...
template <>
CUDF_HDI int64_t midpoint(int64_t lhs, int64_t rhs)
{
...
CUDF_HDI Result select_quantile_data(Iterator begin, size_type size, double q, interpolation interp)
{
...
CUDF_HDI bool select_quantile_validity(Iterator begin,
                                       size_type size,
                                       double q,
