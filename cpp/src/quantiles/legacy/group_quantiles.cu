    ...
    thrust::for_each_n(
      ...
      [=] __device__(cudf::size_type i) {
        ...
        thrust::transform(
          ...
          [=](auto q) { return detail::select_quantile(value, segment_size, q, interpolation); });
      });
  }
...
std::pair<cudf::table, cudf::table> group_quantiles(cudf::table const& keys,
  ...
  for (cudf::size_type i = 0; i < values.num_columns(); i++) {
    ...
    detail::group_quantiles(
      sorted_values, group_offsets, group_sizes, result_col, quantiles, interpolation);
