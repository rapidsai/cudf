                              ...
                              bit_mask_t const* __restrict__ replacement_valid)
{
  ...
  if (found_ptr != values_to_replace_end) {
    ...
    if (replacement_has_nulls) { output_is_valid = bit_mask::is_valid(replacement_valid, d); }
  } else {
...
struct replace_kernel_forwarder {
                  ...
                  cudaStream_t stream = 0)
  {
    ...
    if (typed_out_valid != nullptr) {
      ...
      CUDA_TRY(
        cudaMemcpyAsync(&valids, valid_count, sizeof(cudf::size_type), cudaMemcpyDefault, stream));
                                ...
                                cudaStream_t stream = 0)
{
  ...
  CUDF_EXPECTS(
    input_col.dtype == values_to_replace.dtype && input_col.dtype == replacement_values.dtype,
  ...
  if (nullptr == input_col.valid && replacement_values.valid != nullptr) {
    ...
    CUDF_EXPECTS(
      GDF_SUCCESS == gdf_column_view(&output, output.data, valid, input_col.size, input_col.dtype),
...
__global__ void replace_nulls_with_scalar(cudf::size_type size,
...
template <typename Type>
__global__ void replace_nulls_with_column(cudf::size_type size,
                                          const Type* __restrict__ in_data,
                                          const bit_mask_t* __restrict__ in_valid,
                                          const Type* __restrict__ replacement,
                                          Type* __restrict__ out_data)
...
struct replace_nulls_scalar_kernel_forwarder {
                  ...
                  cudaStream_t stream = 0)
  {
    ...
    CUDA_TRY(cudaMemcpyAsync(
      d_replacement, t_replacement, sizeof(col_type), cudaMemcpyHostToDevice, stream));

    replace_nulls_with_scalar<<<grid.num_blocks, BLOCK_SIZE, 0, stream>>>(
      nrows,
      static_cast<const col_type*>(d_in_data),
      reinterpret_cast<bit_mask_t*>(d_in_valid),
      static_cast<const col_type*>(d_replacement),
                         ...
                         cudaStream_t stream)
{
  ...
  if (input.null_count == 0 || input.valid == nullptr) { return cudf::copy(input); }
