  ...
  cudaStream_t stream)
{
  ...
  thrust::for_each_n(thrust::device,
                     matched_view.get_indices_annotated().begin<uint32_t>(),
                     input.size(),
                     [] __device__(auto v) { printf(" %d\n", v); });
  auto values_indices = cudf::dictionary_column_view(matched_values->view()).indices();
  printf("values:\n");
  thrust::for_each_n(
    thrust::device, values_indices.begin<uint8_t>(), values.size(), [] __device__(auto v) {
      printf(" %d\n", v);
    });
  ...
  printf("replacement:\n");
  thrust::for_each_n(thrust::device,
                     replacement_indices.begin<uint8_t>(),
                     replacements.size(),
                     [] __device__(auto v) { printf(" %d\n", v); });

  auto indices_type   = matched_view.indices().type();
  auto new_indices    = cudf::type_dispatcher(indices_type,
