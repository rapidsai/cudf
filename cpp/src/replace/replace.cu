  ...
  rmm::mr::device_memory_resource* mr)
{
  ...
  auto matched_input = [&] {
    auto views    = std::vector<cudf::column_view>({values.keys(), replacements.keys()});
    auto new_keys = cudf::detail::concatenate(cudf::host_span<cudf::column_view>{views}, stream);
