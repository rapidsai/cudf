...
namespace detail {
  ...
  gdf_column find_and_replace_all(const gdf_column &input_col,
    ...
    gdf_column output = cudf::allocate_like(input_col, mask_allocation_policy::RETAIN, stream);
                         ...
                         cudaStream_t stream)
{
  ...
  gdf_column output = cudf::allocate_like(input, mask_allocation_policy::NEVER, stream);
