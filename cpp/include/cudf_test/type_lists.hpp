...
typename std::enable_if_t<cudf::is_fixed_width<TypeParam>() &&
 ...
 */
template <typename TypeParam, typename T>
typename std::enable_if_t<cudf::is_timestamp_t<TypeParam>::value, thrust::host_vector<TypeParam>>
make_type_param_vector(std::initializer_list<T> const& init_list)
{
  thrust::host_vector<TypeParam> vec(init_list.size());
  std::transform(std::cbegin(init_list), std::cend(init_list), std::begin(vec), [](auto const& e) {
...
template <typename TypeParam, typename T>
typename std::enable_if_t<std::is_same_v<TypeParam, std::string>, thrust::host_vector<std::string>>
make_type_param_vector(std::initializer_list<T> const& init_list)
{
