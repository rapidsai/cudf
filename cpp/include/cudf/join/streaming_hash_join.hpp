 ...
 * cumulative rows. Right-side partitions are fed in via `insert()` and are not deep-copied;
 * the caller must keep every inserted partition alive until the streaming_hash_join is destroyed.
 *
 ...
 *
 * @note The `streaming_hash_join` object must not outlive any inserted right-side partition.
 *
...
class streaming_hash_join {
 public:
   ...
   *
   * The partition is not deep-copied; the caller must keep `right_partition` alive until this
   * object is destroyed. Each row in the partition is assigned a global row index equal to
   * `cumulative_rows_so_far + local_row_idx`.
   *
   * @throws std::invalid_argument if `right_partition`'s schema does not match the schema passed
   *                               to the constructor
   * @throws cudf::logic_error if cumulative inserted rows would exceed `total_right_rows`
   *
   ...
   *
   * @throws std::invalid_argument if `left` has nulls but the object was constructed with
   *                               `has_nulls == nullable_join::NO`
   *
