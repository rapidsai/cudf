...
struct csv_read_arg {
  ...
  bool delim_whitespace =
    false;  ///< Use white space as the delimiter; overrides the delimiter argument
  ...
  cudf::size_type skiprows   = 0;   ///< Rows to skip from the start
  cudf::size_type skipfooter = 0;   ///< Rows to skip from the end
  ...
  quote_style quoting =
    QUOTE_MINIMAL;  ///< Treat string fields as quoted item and remove the first and last quotechar
  bool doublequote =
    true;  ///< Whether to interpret two consecutive quotechar inside a field as a single quotechar
  ...
  gdf_time_unit out_time_unit =
    TIME_UNIT_NONE;  ///< The output resolution for date32, date64, and timestamp columns
...
struct json_read_arg {
  ...
  std::string compression =
    "infer";  ///< For on-the-fly decompression, one of `none`, `infer`, `gzip`, `zip`
