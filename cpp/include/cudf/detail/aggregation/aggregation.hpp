...
class hyper_log_log_aggregation final : public groupby_aggregation, public reduce_aggregation {
 public:
  explicit hyper_log_log_aggregation(int const num_registers_per_sketch_)
    : aggregation{HLL}, num_registers_per_sketch(num_registers_per_sketch_)
  {
  }
...
class merge_hyper_log_log_aggregation final : public groupby_aggregation,
 ...
 public:
  explicit merge_hyper_log_log_aggregation(int const num_registers_per_sketch_)
    : aggregation{MERGE_HLL}, num_registers_per_sketch(num_registers_per_sketch_)
  {
  }
  int const num_registers_per_sketch;
...
// Always use list for HLL
template <typename SourceType>
struct target_type_impl<SourceType, aggregation::HLL> {
  using type = list_view;
};
...
// Always use list for MERGE_HLL
template <typename SourceType>
struct target_type_impl<SourceType, aggregation::MERGE_HLL> {
  using type = list_view;
};
                                                              ...
                                                              Ts&&... args)
{
  switch (k) {
    ...
    case aggregation::HLL:
      return f.template operator()<aggregation::HLL>(std::forward<Ts>(args)...);
    case aggregation::MERGE_HLL:
      return f.template operator()<aggregation::MERGE_HLL>(std::forward<Ts>(args)...);
    default: {
