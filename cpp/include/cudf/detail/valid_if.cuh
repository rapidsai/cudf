...
CUDF_KERNEL void valid_if_kernel(
  bitmask_type* output, InputIterator begin, size_type size, Predicate p, size_type* null_count)
{
 ...
 * @return A pair containing a `device_buffer` with the new bitmask and a `device_scalar`
 *         storing null count
 */
template <typename InputIterator, typename Predicate>
std::pair<rmm::device_buffer, rmm::device_scalar<size_type>> valid_if_async(
  InputIterator begin,
  InputIterator end,
  Predicate p,
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  auto null_count = rmm::device_scalar<size_type>{size, stream};
  ...
  if (size > 0) {
    ...
    valid_if_kernel<block_size><<<grid.num_blocks, grid.num_threads_per_block, 0, stream.value()>>>(
      static_cast<bitmask_type*>(null_mask.data()), begin, size, p, null_count.data());
  }
  return std::pair(std::move(null_mask), std::move(null_count));
}
...
std::pair<rmm::device_buffer, size_type> valid_if(InputIterator begin,
                                                  ...
                                                  rmm::device_async_resource_ref mr)
{
  auto [null_mask, null_count] = valid_if_async(begin, end, p, stream, mr);
  return std::pair(std::move(null_mask), null_count.value(stream));
}
