...
/**
 * @brief Get all the row indices of the input table such that the rows corresponding to these
 *        indices are distinct from each other.
 *
 * Given an `input` table_view, an output array of row indices is generated by adding as many
 * indices as possible, in an unspecified order, such that all rows correspond to these indices
