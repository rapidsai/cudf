...
/**
 * @brief The struct to hold temporary data (but needs to be kept alive) to support the output
 * generated by `superimpose_parent_nulls(column_view)` and `superimpose_parent_nulls(table_view)`.
 */
struct temporary_nullable_data {
  /**
   * @brief Store the newly generated null masks (if any).
   */
  std::vector<rmm::device_buffer> temp_null_masks;

  /**
   * @brief Store the newly generated columns with new null masks (if any).
   */
  std::vector<std::unique_ptr<column>> temp_columns;
};
 ...
 *         2. An instance of `temporary_nullable_data` holding the intermediate data that needs
 *         to be kept alive.
 */
[[nodiscard]] std::pair<column_view, temporary_nullable_data> superimpose_parent_nulls(
 ...
 *         to be kept alive.
 */
[[nodiscard]] std::pair<table_view, temporary_nullable_data> superimpose_parent_nulls(
