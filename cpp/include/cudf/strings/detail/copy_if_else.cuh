...
std::unique_ptr<cudf::column> copy_if_else( StringPairIterLeft lhs_begin,
 ...
 * ```
 *
 * @tparam StringPairIterator Pair iterator returning thrust::pair<string_view,bool> where the
 *         bool parameter specifies if the string_view is valid (true) or not (false).
 * @tparam Filter Functor that takes an index and returns a boolean.
 *
 * @param lhs_begin Start of first set of data. Used when filter_fn returns true.
 ...
 * @return New strings column.
 */
template<typename StringPairIterator, typename Filter>
std::unique_ptr<cudf::column> copy_if_else( StringPairIterator lhs_begin,
                                            StringPairIterator lhs_end,
                                            StringPairIterator rhs_begin,
                                            Filter filter_fn,
                                            rmm::mr::device_memory_resource* mr = rmm::mr::get_default_resource(),
                                            cudaStream_t stream=0 )
{
