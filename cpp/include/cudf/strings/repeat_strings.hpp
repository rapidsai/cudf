...
/**
 * @brief The enum specify options to check for overflowing in sizes of the output strings and
 * strings offsets.
 */
enum class check_output_overflow { YES, NO };
 ...
 *        indexed by size_type (offset_type). This is only applicable if the parameter @p
 *        `overflow_checking` is specified as `YES`.
 *
 ...
 * @param overflow_checking Option specifying whether the sizes and offsets of the output strings
 *        will be checked for overflow during computation.
...
std::unique_ptr<column> repeat_strings(
  ...
  size_type repeat_times,
  check_output_overflow overflow_checking = check_output_overflow::NO,
  rmm::mr::device_memory_resource* mr     = rmm::mr::get_current_device_resource());

/**
 * @brief Repeat each string in the given strings column by the number of times given in another
 * numeric column.
 *
 * For a given strings column, an output strings column is generated by repeating each of its string
