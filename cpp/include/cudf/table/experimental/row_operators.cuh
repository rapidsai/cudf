...
struct preprocessed_table {
 ...
 private:
  ...
  // An ID number generated by the factory functions `preprocess::create()`.
  // If two preprocessed_table(s) are generated together using the factory function
  // `preprocessed_table::create(table_view const&, table_view const&)`, they will be assigned the
  // same ID number.
  uint64_t _preprocessed_id;
...
class two_table_comparator {
 public:
  ...
  {
    // If we want to use preprocessed_table(s) that have transformed structs columns for two-table
