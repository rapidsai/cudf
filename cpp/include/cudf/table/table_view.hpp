...
{
  return std::any_of(
    view.begin(), view.end(), [](column_view const& col) { return col.has_nulls(); });
}

inline bool has_nested_nulls(table_view const& view)
{
  return std::any_of(
    view.begin(), view.end(), [](column_view const& col) { return col.has_nested_nulls(); });
}
