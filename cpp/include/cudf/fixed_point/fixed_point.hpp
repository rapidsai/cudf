...
CUDF_HDI constexpr T right_shift(T const& val, scale_type const& scale)
{
 ...
 */
template <typename Rep, Radix Rad, typename T>
CUDF_HDI constexpr T left_shift(T const& val, scale_type const& scale)
{
  return val * ipow<Rep, Rad>(static_cast<int32_t>(-scale));
}
 ...
 */
template <typename Rep, Radix Rad, typename T>
CUDF_HDI constexpr T shift(T const& val, scale_type const& scale)
{
  if (scale == 0)
    return val;
  else if (scale > 0)
...
struct scaled_integer {
  Rep value;
  scale_type scale;
  CUDF_HDI explicit scaled_integer(Rep v, scale_type s) : value{v}, scale{s} {}
};
 ...
 public:
  ...
  template <typename T,
            typename cuda::std::enable_if_t<cuda::std::is_floating_point<T>() &&
                                            is_supported_representation_type<Rep>()>* = nullptr>
  CUDF_HDI explicit fixed_point(T const& value, scale_type const& scale)
  ...
  CUDF_HDI fixed_point(T const& value) : _value{static_cast<Rep>(value)}, _scale{scale_type{0}}
  {
  }
  ...
  CUDF_HDI operator scaled_integer<Rep>() const { return scaled_integer<Rep>{_value, _scale}; }
   ...
   */
  CUDF_HDI rep value() const { return _value; }
   ...
   */
  CUDF_HDI scale_type scale() const { return _scale; }
   ...
   */
  CUDF_HDI explicit constexpr operator bool() const { return static_cast<bool>(_value); }
  ...
  CUDF_HDI fixed_point<Rep1, Rad1>& operator+=(fixed_point<Rep1, Rad1> const& rhs)
  {
    *this = *this + rhs;
    return *this;
  }
   ...
   */
  template <typename Rep1, Radix Rad1>
  CUDF_HDI friend fixed_point<Rep1, Rad1> operator+(fixed_point<Rep1, Rad1> const& lhs,
   ...
   */
  template <typename Rep1, Radix Rad1>
  CUDF_HDI friend bool operator==(fixed_point<Rep1, Rad1> const& lhs,
                                  fixed_point<Rep1, Rad1> const& rhs);
   ...
   */
  CUDF_HDI fixed_point<Rep, Rad> rescaled(scale_type scale) const
  {
...
CUDF_HDI auto addition_overflow(T lhs, T rhs)
{
 ...
 */
template <typename Rep, typename T>
CUDF_HDI auto subtraction_overflow(T lhs, T rhs)
{
 ...
 */
template <typename Rep, typename T>
CUDF_HDI auto division_overflow(T lhs, T rhs)
{
 ...
 */
template <typename Rep, typename T>
CUDF_HDI auto multiplication_overflow(T lhs, T rhs)
{
...
// PLUS Operation
template <typename Rep1, Radix Rad1>
CUDF_HDI fixed_point<Rep1, Rad1> operator+(fixed_point<Rep1, Rad1> const& lhs,
                                           fixed_point<Rep1, Rad1> const& rhs)
{
...
template <typename Rep1, Radix Rad1>
CUDF_HDI bool operator==(fixed_point<Rep1, Rad1> const& lhs, fixed_point<Rep1, Rad1> const& rhs)
{
  auto const scale = std::min(lhs._scale, rhs._scale);
