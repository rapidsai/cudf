...
/**
 * @brief Given a validity bitmask, returns the index of the first unset bit
 * in the range `[start, stop)`.
 *
 * @param bitmask Validity bitmask residing in device memory
 ...
 * @param stream CUDA stream used for device memory operations and kernel launches
 * @return The index of the first unset bit in the specified range,
 *         or `stop-start` if no unset bit is found
 */
