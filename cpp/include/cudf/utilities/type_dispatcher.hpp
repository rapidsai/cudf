...
// clang-format off
template <typename T>
using dispatch_same_width_type =
 ...
 *
 * For example, `cudf::gather` only needs `type_dispatcher<dispatch_on_same_width>(...)` since
 * it only needs to copy the elements and does not need process the values themselves.
 */
template <cudf::type_id Id>
struct dispatch_on_same_width {
  using type = dispatch_same_width_type<typename id_to_type_impl<Id>::type>;
};
