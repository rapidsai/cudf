 ...
 * key at the same row in the keys column, the output column is generated by concatenating the
 * list elements having the same key. If the input lists column contains null list elements, the
 * concatenation process will either ignore those null elements, or will simply set the entire
 ...
 *
 * @param keys The column containing keys.
 * @param values The lists column containing list elements to concatenate.
 * @param null_policy The parameter to specify whether a null list element will be ignored from
 *        concatenation, or any concatenation involving a null element will result in a null list.
 * @param mr Device memory resource used to allocate the returned column's device memory.
 * @return A new column in which each row is a list resulted from concatenating the input list
 *         elements corresponding to the same key.
 */
std::unique_ptr<column> concatenate_by_key(
