...
/**
 * @brief Create new lists columns by copying list entries from the input lists columns ignoring
 * duplicate list entries.
 *
 * Given a pair of keys-values lists columns. At each row, each list entry in one column maps
 * to a list entry in the other column by linear order and vice versa. Therefore, lists at each row
 * in both keys and values columns must have the same size. A pair of output keys and values lists
 * columns are generated by copying entries from the input lists columns. The duplicate entries in
 * each list of the keys column are ignored from copying to extract only the unique list entries.
 * Entries in the values column mapped to these key entries are also copied or ignored by the same
 * way.
 *
 * The users are responsible to have the keys-values lists columns having the same number of entries
 * in each row index. Otherwise, crash may happen, or the results will be undefined.
 *
 * When copying unique entries for the output, depending on the value of the @p keep_option
 * parameter:
 * - KEEP_FIRST: only copy the first entry in the sequence of duplicate ones
 * - KEEP_LAST: only copy the last entry in the sequence of duplicate ones
 * - KEEP_NONE: only copy the entries that are not repeated
 *
...
/**
 * @brief Create a new list column by copying list entries from the input lists column ignoring
 * duplicate list entries.
 *
