 ...
 *
 * Given a pair of keys-values lists columns where both columns have the same number of rows
 * and the size of each corresponding row is the same, remove entries from both columns where the
 * corresponding key values are the same.
 *
 * A pair of output keys and values lists columns are generated by copying entries from the input
 * lists columns. The duplicate entries in each list of the keys column are ignored during copying
 ...
 *        than STRUCT.
 * @throw cudf::logic_error If the input keys and values columns have different numbers of rows.
 *
 ...
 *        column.
 * @param nulls_equal Flag to specify whether null key entries should be considered as equal.
 * @param nans_equal Flag to specify whether NaN key entries should be considered as equal
 ...
 * column in a way such that the duplicate entries in each list are ignored, producing only unique
 * list entries.
 *
