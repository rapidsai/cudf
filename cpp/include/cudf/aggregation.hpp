 ...
 *  - It requires the input values to be a lists column, and
 *  - The value (lists) corresponding to the same key will not result in a list of lists as output
 ...
 * simply merging lists of the input lists column. As such, the input lists column is required to
 * NOT be nullable.
 *
 */
template <typename Base = aggregation>
 ...
 *  - It requires the input values to be a lists column, and
 *  - The value (lists) corresponding to the same key will result in a list generated by merging
 *    them together then dropping the duplicate entries.
 *
 * In practice, this aggregation is used to merge the partial results of multiple (distributed)
 ...
 *
 * Note that the (distributed) partial results should be generated by `COLLECT_LIST` aggregations
 * to avoid the overhead of calling `lists::drop_list_duplicates` before merging lists.
 *
 * Similar to `MERGE_LISTS`, this aggregation does not accept any null handling parameter and
 * requires the input lists column to NOT be nullable.
 *
 */
template <typename Base = aggregation>
