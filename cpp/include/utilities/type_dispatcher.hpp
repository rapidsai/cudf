  ...
  CUDA_HOST_DEVICE_CALLABLE constexpr decltype(auto) type_dispatcher(
    ...
    switch (dtype) {
      ...
      case GDF_STRING_CATEGORY: {
        return f.template operator()<nvstring_category>(
            std::forward<Ts>(args)...);
      }
      default: {
#ifdef __CUDA_ARCH__
        ...
        // The following code will never be reached, but the compiler generates
        // a warning if there isn't a return value.
