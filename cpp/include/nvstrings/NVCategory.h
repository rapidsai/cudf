 ...
 public:
  ...
  /**
     ...
     *
      @return Instance with the strings copied into device memory.
     */
  static NVCategory* create_from_index(std::pair<const char*, size_t>* strs,
                                       unsigned int count,
                                       bool devmem = true);
  /**
   ...
   * The size of each string is determined by computing adjacent offset differences so
   * null-terminators are not allowed.
   ...
   *                    This should be 1 more than the count value with the last value specifying
   * the length of the buffer in bytes.
   * @param[in] nullbitmask Each bit identifies which strings are to be considered null.
   *                        The bits are organized as specified in the Arrow format. If no nulls,
   * this parameter can be null. The size of this byte array should be at least (count+7)/8 bytes.
   * @param nulls The number of nulls identified by the \p nullbitmask.
   * @param devmem Set to true (default) if pointers are to device memory.
   *
   * @return Instance with the strings copied into device memory.
   */
  static NVCategory* create_from_offsets(const char* strs,
                                         unsigned int count,
                                         const int* offsets,
                                         const unsigned char* nullbitmask = 0,
                                         int nulls                        = 0,
                                         bool devmem                      = true);
  /**
   ...
   * @param devmem Identifies the provided \p bitarray parameter points to device memory (default)
   * or CPU memory.
  ...
  /**
   * @brief Creates a new instance removing the keys matching the specified strings and remapping
   * the values.
   *
  ...
  /**
   * @brief Creates a new instance using the specified strings as keys causing add/remove as
   * appropriate.
   *
  ...
  /**
   * @brief Create a new instance identified by the specified values and remap the values to
   * resulting keyset.
   * @param[in] pos The index values to build the strings instance from.
   * @param elems The number of values in the \p pos array.
   * @param devmem Indicates whether the \p results parameter points to device or CPU memory.
