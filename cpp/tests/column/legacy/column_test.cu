...
struct ColumnConcatTest : public GdfTest {
  ...
  void multicolumn_test(std::vector<cudf::size_type> column_sizes,
    ...
    for (auto const& c : gdf_columns) { raw_gdf_columns.push_back(c.get()); }
    ...
    EXPECT_EQ(GDF_SUCCESS, gdf_column_concat(output_gdf_col.get(), columns_to_concat, num_columns));
    ...
    for (cudf::size_type index = 0, col = 0, row = 0; index < total_size; ++index) {
      if (null_init(row, col))
        cudf::util::turn_bit_on(ref_valid.data(), index);
      else
        ref_null_count++;

      if (++row >= column_sizes[col]) {
        row = 0;
        col++;
      }
    }
  ...
  void multicolumn_bench(std::vector<size_t> column_sizes,
    ...
    std::cout << "Time for " << num << " concats of " << num_columns << " columns of " << total_size
              << " total elements:\n";
...
TYPED_TEST(ColumnConcatTest, OutputWrongSize) {
  ...
  cudf::size_type const total_size{std::accumulate(column_sizes.begin(), column_sizes.end(), 0)};
  ...
  EXPECT_EQ(GDF_COLUMN_SIZE_MISMATCH,
            gdf_column_concat(output_gdf_col.get(), input_columns.data(), num_columns));
}
...
TYPED_TEST(ColumnConcatTest, MoreComplicatedColumns) {
  ...
  auto bit_setter = [column_sizes](cudf::size_type row, cudf::size_type col) {
    switch (col) {
      case 0: return (row % 2) != 0;  // column 0 has odd bits set
      case 1: return row < column_sizes[col];
      case 2: return (row % 17) != 0;
      case 3: return row < 3;
    }
    return true;
  };
  ...
  this->multicolumn_test(
    column_sizes, [](int index) { return std::rand(); }, bit_setter);
}
...
TYPED_TEST(ColumnConcatTest, SingleColumn) {
  ...
  this->multicolumn_test(
    column_sizes,
    [](int index) { return std::rand(); },
    [](cudf::size_type row, cudf::size_type col) { return true; });
}
