...
struct transformer_optional_meanvar {
  ...
  CUDF_HOST_DEVICE inline auto operator()(thrust::optional<ElementType> const& optional) -> ResultType
  {
...
struct optional_to_meanvar {
  CUDF_HOST_DEVICE inline auto operator()(const thrust::optional<T>& v) -> T { return v.value_or(T{0}); }
};
