...
TYPED_TEST(IteratorTest, null_iterator_upcast)
{
    ...
    // CPU test
    auto it_hos = cudf::make_iterator<true, T>(
    ...
    // GPU test
    auto it_dev = cudf::make_iterator<true, T>(w_col, T{0});
