...
TYPED_TEST(IteratorTest, null_iterator) {
  ...
  std::transform(std::get<0>(hos).begin(),
                 std::get<0>(hos).end(),
                 host_bools.begin(),
...
TYPED_TEST(IteratorTest, indexed_iterator) {
  ...
  // calculate the expected value by CPU.
  T expected_value =
    std::accumulate(hos_indices.begin(), hos_indices.end(), T{0}, [&](T acc, T_index id) {
      return (acc + hos_array[id]);
    });
...
TYPED_TEST(IteratorTest, large_size_reduction) {
  ...
  cudf::test::scalar_wrapper<T> result = cudf::reduce(w_col, cudf::reduction::SUM, GDF_INT32);
...
TYPED_TEST(IteratorTest, mean_var_output) {
  ...
  expected_value.value_squared =
    std::accumulate(replaced_array.begin(), replaced_array.end(), T_upcast{0}, [](T acc, T i) {
      return acc + i * i;
    });
  ...
  // GPU test
  auto it_dev =
    cudf::make_pair_iterator<true, T>(static_cast<T*>(w_col.get()->data), w_col.get()->valid, init);
  auto it_dev_squared = thrust::make_transform_iterator(it_dev, transformer);
  this->iterator_test_thrust(expected_value, it_dev_squared, w_col.size());
  this->iterator_test_cub(expected_value, it_dev_squared, w_col.size());
}
...
TYPED_TEST(IteratorTest, error_handling) {
  ...
  cudf::test::column_wrapper<T> w_col_null(hos_array, [&](cudf::size_type row) { return true; });
