...
TEST_F(RepeatJoinStringTest, ZeroSizeStringsColumn)
{
  ...
  auto const results = cudf::strings::repeat_join(cudf::strings_column_view(strs), 10);
...
TEST_F(RepeatJoinStringTest, StringsColumnNoNull)
{
  ...
  // Non-positive repeat times.
  {
    ...
    auto results = cudf::strings::repeat_join(cudf::strings_column_view(strs), 0);
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(expected, *results, print_all);

    results = cudf::strings::repeat_join(cudf::strings_column_view(strs), -100);
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(expected, *results, print_all);
  }
