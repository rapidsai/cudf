...
TEST_F(StringsContainsTests, SingleLine)
{
  auto input = cudf::test::strings_column_wrapper({"abc\nfff\nabc", "fff\nabc\nlll", "abc", ""});
  auto view  = cudf::strings_column_view(input);

  auto results = cudf::strings::contains_re(view, "^abc$", cudf::strings::regex_flags::SINGLE_LINE);
  auto expected_contains = cudf::test::fixed_width_column_wrapper<bool>({0, 0, 1, 0});
  ...
  results = cudf::strings::matches_re(view, "^abc$", cudf::strings::regex_flags::SINGLE_LINE);
  auto expected_matches = cudf::test::fixed_width_column_wrapper<bool>({0, 0, 1, 0});
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(*results, expected_matches);
  results          = cudf::strings::matches_re(view, "^abc$");
  expected_matches = cudf::test::fixed_width_column_wrapper<bool>({1, 0, 1, 0});
  CUDF_TEST_EXPECT_COLUMNS_EQUIVALENT(*results, expected_matches);

  results = cudf::strings::count_re(view, "^abc$", cudf::strings::regex_flags::SINGLE_LINE);
  auto expected_count = cudf::test::fixed_width_column_wrapper<int32_t>({0, 0, 1, 0});
...
TEST_F(StringsContainsTests, DotAll)
{
  ...
  auto results = cudf::strings::contains_re(view, "a.*f", cudf::strings::regex_flags::DOT_ALL);
  ...
  results = cudf::strings::matches_re(view, "a.*f", cudf::strings::regex_flags::DOT_ALL);
  ...
  results             = cudf::strings::count_re(view, "a.*?f", cudf::strings::regex_flags::DOT_ALL);
  ...
  auto both_flags = cudf::strings::regex_flags::DOT_ALL | cudf::strings::regex_flags::SINGLE_LINE;
  results =
