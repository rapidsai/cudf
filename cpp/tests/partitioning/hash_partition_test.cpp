...
auto make_view_vector(std::vector<T> const& columns) {
  ...
  std::transform(
      columns.begin(), columns.end(), views.begin(),
...
TEST_F(HashPartition, InvalidColumnsToHash) {
  ...
  EXPECT_THROW(cudf::experimental::hash_partition(input, columns_to_hash,
                                                  num_partitions),
               std::out_of_range);
}
...
TEST_F(HashPartition, ZeroPartitions) {
  ...
  std::tie(output, offsets) = cudf::experimental::hash_partition(
      input, columns_to_hash, num_partitions);
...
TEST_F(HashPartition, MixedColumnTypes) {
  ...
  std::tie(output1, offsets1) = cudf::experimental::hash_partition(
      input, columns_to_hash, num_partitions);
  std::tie(output2, offsets2) = cudf::experimental::hash_partition(
      input, columns_to_hash, num_partitions);
...
TEST_F(HashPartition, NullableStrings) {
  ...
  std::vector<cudf::size_type> offsets;
  std::tie(result, offsets) = cudf::experimental::hash_partition(
      input, columns_to_hash, num_partitions);
...
TEST_F(HashPartition, ColumnsToHash) {
  ...
  std::tie(first_result, first_offsets) = cudf::experimental::hash_partition(
      first_input, columns_to_hash, num_partitions);
  std::tie(second_result, second_offsets) = cudf::experimental::hash_partition(
      second_input, columns_to_hash, num_partitions);
...
TYPED_TEST(HashPartitionFixedWidth, NullableFixedWidth) {
  fixed_width_column_wrapper<TypeParam, int> fixed({1, 2, 3, 4}, {1, 1, 1, 1});
...
template <typename T>
void run_fixed_width_test(size_t cols, size_t rows,
  ...
  if (has_nulls) {
    std::generate(columns.begin(), columns.end(), [rows]() {
      ...
      auto valids = thrust::make_transform_iterator(
          iter, [](auto i) { return i % 4 != 0; });
  ...
  std::adjacent_difference(offsets1.begin() + 1, offsets1.end(),
                           offsets1.begin() + 1);
  ...
  std::for_each(
      offsets1.begin() + 1, offsets1.end(), [&](cudf::size_type const& count) {
  ...
  auto sorted_partitions1 =
      cudf::experimental::sort_by_key(partitions_table, hash1_table);

  auto hash2 = cudf::hash(output2->view());
  cudf::table_view hash2_table({hash2->view()});
  auto sorted_partitions2 =
      cudf::experimental::sort_by_key(partitions_table, hash2_table);
