...
TEST_F(DropDuplicate, NonNullTable)
{
    ...
    cudf::table_view keys {{col1_key, col2_key}};
    ...
    cudf::table_view expected_first {{exp_col1_first, exp_col2_first}};

    auto got_first = drop_duplicates(input, keys, cudf::experimental::duplicate_keep_option::KEEP_FIRST);
    ...
    cudf::table_view expected_last {{exp_col1_last, exp_col2_last}};
    ...
    cudf::table_view expected_unique {{exp_col1_unique, exp_col2_unique}};

    auto got_unique = drop_duplicates(input, keys, cudf::experimental::duplicate_keep_option::KEEP_NONE);
...
TEST_F(DropDuplicate, WithNull)
{
    ...
    cudf::table_view keys {{key}};

    // Keep first of duplicate
    cudf::test::fixed_width_column_wrapper<int32_t> exp_col_first{{4, 5, 5, 8}, {0, 1, 1, 1}};
    cudf::table_view expected_first {{exp_col_first}};
    auto got_first = drop_duplicates(input, keys, cudf::experimental::duplicate_keep_option::KEEP_FIRST, true);

    cudf::test::expect_tables_equal(expected_first, got_first->view());

    // Keep last of duplicate
    cudf::test::fixed_width_column_wrapper<int32_t> exp_col_last{{3, 1, 5, 8}, {1, 1, 1, 1}};
    cudf::table_view expected_last {{exp_col_last}};
    auto got_last = drop_duplicates(input, keys, cudf::experimental::duplicate_keep_option::KEEP_LAST);
    ...
    cudf::table_view expected_unique {{exp_col_unique}};
    auto got_unique = drop_duplicates(input, keys, cudf::experimental::duplicate_keep_option::KEEP_NONE);

    cudf::test::expect_tables_equal(expected_unique, got_unique->view());
}

TEST_F(DropDuplicate, RowSizeMismatch)
{
    cudf::test::fixed_width_column_wrapper<int32_t> col{{5, 4, 3, 5, 8, 1}, {1, 0, 1, 1, 1, 1}};
    cudf::table_view input {{col}};
    cudf::test::fixed_width_column_wrapper<int32_t> key{{20}, {1}};
    cudf::table_view keys {{key}};

    EXPECT_THROW(drop_duplicates(input, keys, cudf::experimental::duplicate_keep_option::KEEP_FIRST, true), cudf::logic_error);
}
