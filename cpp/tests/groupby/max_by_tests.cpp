...
TYPED_TEST_SUITE(groupby_max_by_test, cudf::test::FixedWidthTypes);
...
struct groupby_max_by_string_test : public cudf::test::BaseFixture {};

TEST_F(groupby_max_by_string_test, basic)
{
  cudf::test::fixed_width_column_wrapper<K> keys  {1, 2, 3, 1, 2, 2, 1, 3, 3, 2};
  ...
  cudf::test::fixed_width_column_wrapper<K> expect_keys{1, 2, 3};
  cudf::test::strings_column_wrapper expect_orders{"año", "zit", "₹1"};
  cudf::test::fixed_width_column_wrapper<K> expect_values{4, 4, 2};
  cudf::test::structs_column_wrapper expect_vals{expect_orders, expect_values};
