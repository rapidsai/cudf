...
void tdigest_simple_large_input_double_aggregation(Func op)
{
  ...
  for (int idx = 0; idx < 2; idx++) {
    ...
    // cast to specific types -before- being sent into the aggregation, I can't (safely) just use
    // the expected values that you get when using doubles all the way through.  so I have to
    // pregenerate the correct answers for each type by hand. so, we'll choose a reasonable subset
    // (double, decimal, int, bool)
