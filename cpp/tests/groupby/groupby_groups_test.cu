...
TYPED_TEST(groupby_group_keys_and_values_test, basic_with_values)
{
  ...
  fixed_width_column_wrapper<V, int32_t> values ({0, 0, 1, 1, 2, 2});
  fixed_width_column_wrapper<V, int32_t> expect_grouped_values {2, 2, 1, 1, 0, 0};
...
TYPED_TEST(groupby_group_keys_and_values_test, some_nulls)
{
  ...
  fixed_width_column_wrapper<V, int32_t> expect_grouped_values ({1, 6, 3});
