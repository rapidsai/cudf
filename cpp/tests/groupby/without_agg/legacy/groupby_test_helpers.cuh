...
// at index location and store it in the Ith element of the tuple key
template <std::size_t I = 0, typename... Keys>
inline typename std::enable_if<I == sizeof...(Keys), void>::type extractWithNulls(
...
// Extract a tuple of values (replace nulls with MAX_VALUE of ValueType) from a tuple of vector for
// a given index keys Tuple of vectors of types Keys index Location of the value to be extracted in
// each vector
template <typename... Keys>
...
struct RandomValues {
  ...
  using Distribution = typename std::conditional<std::is_integral<K>::value,
  ...
  RandomValues(const K _min, const K _max) : min(_min), max(_max), gen(rd()), dis(min, max) {}
...
template <typename T>
void createUniqueKeys(const size_t key_count,
...
inline typename std::enable_if<I == sizeof...(K), void>::type initialize_keys(
  std::tuple<std::vector<K>...>& k,
  const size_t key_count,
  const size_t value_per_key,
  const size_t column_range,
  const size_t shuffle_seed,
...
template <std::size_t I = 0, typename... K>
  inline typename std::enable_if <
  I<sizeof...(K), void>::type initialize_keys(std::tuple<std::vector<K>...>& k,
                                              const size_t key_count,
                                              const size_t value_per_key,
                                              const size_t column_range,
                                              const size_t shuffle_seed,
  ...
  initialize_keys<I + 1, K...>(k, key_count, value_per_key, column_range, shuffle_seed, unique);
}
...
void copy_output_with_array(gdf_column** group_by_output_key,
                            multi_column_t& output_key,
  ...
  CUDA_TRY(cudaMemcpy(output_value.data(),
                      group_by_output_value_array,
                      group_by_output_value_size * sizeof(cudf::size_type),
...
void copy_gdf_column_with_nulls(gdf_column* column,
  ...
  vec.resize(column->size);
  CUDA_TRY(cudaMemcpy(vec.data(), column->data, column->size * sizeof(T), cudaMemcpyDeviceToHost));
  CUDA_TRY(cudaMemcpy(output_valids.get(),
                      column->valid,
...
// group_by_output_value is copied to a vector output_value
template <typename gdf_column, typename multi_column_t, typename output_t>
void copy_output_with_nulls(gdf_column** group_by_output_key,
                            multi_column_t& output_key,
                            std::vector<host_valid_pointer>& output_valids,
                            gdf_column* group_by_output_value,
                            std::vector<output_t>& output_value) {
...
template <typename gdf_column, typename multi_column_t>
void copy_output_with_array_with_nulls(gdf_column** group_by_output_key,
                                       multi_column_t& output_key,
                                       std::vector<host_valid_pointer>& output_valids,
                                       cudf::size_type* group_by_output_value_array,
                                       cudf::size_type group_by_output_value_size,
