...
struct GroupByWoAggTest : public GdfTest {
   ...
   */
  void create_input(const size_t key_count,
                    ...
                    bool print = false)
  {
    ...
    gdf_input_value_column =
      init_gdf_column(input_value, 0, [](size_t row, size_t col) { return true; });

    // Fill vector of raw pointers to gdf_columns
    for (auto const& c : gdf_input_key_columns) { gdf_raw_input_key_columns.push_back(c.get()); }
  ...
  map_t compute_reference_solution()
  {
    ...
    for (size_t i = 0; i < input_value.size(); ++i) {
      ...
      if (sch == key_val_map.end()) { key_val_map.emplace(l_key, input_value[i]); }
    }
    return key_val_map;
  }
  ...
  void compute_gdf_result()
  {
    ...
    EXPECT_NO_THROW(std::tie(output_table, indices_col) = gdf_group_by_without_aggregations(
                      input_table, num_columns, groupby_col_indices.data(), &context));

    copy_output_with_array(output_table.begin(),
                           cpu_data_cols_out,
                           static_cast<cudf::size_type*>(indices_col.data),
                           indices_col.size,
                           this->cpu_out_indices);
...
struct GroupValidTest : public GroupByWoAggTest<test_parameters> {
   ...
   */
  /* ----------------------------------------------------------------------------*/
  void create_input_with_nulls(const size_t key_count,
                               const size_t value_per_key,
                               const size_t max_key,
                               const size_t max_val,
                               bool print = false)
  {
    ...
    this->gdf_input_key_columns = initialize_gdf_columns(
      this->input_key,
      [&](size_t row, size_t col) { return gdf_is_valid(input_key_valids[col].get(), row); });
    this->gdf_input_value_column =
      init_gdf_column(this->input_value, 0, [&](size_t row, size_t col) {
        return gdf_is_valid(input_value_valid.get(), row);
      });
    ...
    if (print) {
      std::cout << "Key column(s) created. Size: " << std::get<0>(this->input_key).size()
                << std::endl;
  ...
  void create_gdf_output_buffers_with_nulls(const size_t key_count,
                                            const size_t value_per_key,
                                            const size_t max_key,
                                            const size_t max_val,
                                            bool print = false)
    ...
    for (size_t i = 0; i < this->input_value.size(); i++) {
      if (this->group_output_type == GroupByOutType::PANDAS) {
        ...
        if (all_valid_key) {
          ...
          auto sch   = key_val_map.find(l_key);
          if (sch == key_val_map.end()) { key_val_map.emplace(l_key, this->input_value[i]); }
        }
      } else {
        bool all_valid_key = true;
        auto valid_key     = get_input_key_valids(this->input_key_valids, i, all_valid_key);
  ...
  void compute_gdf_result_with_nulls()
  {
    ...
    gdf_column indices_col;
    EXPECT_NO_THROW(std::tie(output_table, indices_col) = gdf_group_by_without_aggregations(
                      input_table, num_columns, groupby_col_indices.data(), &this->context));
    copy_output_with_array_with_nulls(output_table.begin(),
                                      this->cpu_data_cols_out,
                                      this->cpu_data_cols_out_valid,
                                      static_cast<cudf::size_type*>(indices_col.data),
                                      indices_col.size,
                                      this->cpu_out_indices);
  ...
  void compare_gdf_result_with_nulls(map_t& reference_map)
  {
    ...
    for (size_t i = 0; i < ref_size; ++i) {
      bool all_valid_key = true;
      auto valid_key     = this->get_input_key_valids(
        this->cpu_data_cols_out_valid, this->cpu_out_indices[i], all_valid_key);

      auto l_key =
        extractKeyWithNulls(this->cpu_data_cols_out, valid_key, this->cpu_out_indices[i]);
