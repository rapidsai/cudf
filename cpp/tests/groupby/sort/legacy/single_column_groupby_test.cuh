                                ...
                                bool ignore_null_keys = true)
{
  ...
  CUDF_EXPECT_NO_THROW(
    detail::expect_values_are_equal(sorted_actual_values, sorted_expected_values));
                                      ...
                                      bool ignore_null_keys = true)
{
  ...
  std::transform(expected_values.begin(),
                 expected_values.end(),
                 sorted_expected_values.begin(),
                 [](const gdf_column* col) { return const_cast<gdf_column*>(col); });
