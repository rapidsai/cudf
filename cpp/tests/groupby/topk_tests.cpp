...
TYPED_TEST(groupby_topk_test_types, TopK)
{
  using K = int32_t;
  using V = TypeParam;
  ...
  agg         = cudf::make_top_k_aggregation<cudf::groupby_aggregation>(3, cudf::order::ASCENDING);
  test_single_agg(keys, values, expect_keys, expect_vals, std::move(agg));
}

struct groupby_topk_test : public cudf::test::BaseFixture {};

TEST_F(groupby_topk_test, TopKEmptyInput)
{
