...
TYPED_TEST(groupby_argmin_test, basic)
{
    ...
    fixed_width_column_wrapper<V, int> vals   { 9, 8, 7, 6, 5, 4, 3, 2, 1, 0};
...
TYPED_TEST(groupby_argmin_test, zero_valid_keys)
{
    ...
    fixed_width_column_wrapper<K, int> keys ( { 1, 2, 3}, all_null() );
...
TYPED_TEST(groupby_argmin_test, null_keys_and_values)
{
    ...
    fixed_width_column_wrapper<V, int> vals(  { 9, 8, 7, 6, 5, 4, 3, 2, 1, 0, 4},
