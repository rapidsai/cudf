  ...
  std::vector<cudf::valid_type> const& expected_bitmask = std::vector<cudf::valid_type>{})
{
  ...
  if (expected_values.size() > 0) {
    ...
    EXPECT_TRUE(std::equal(expected_values.begin(), expected_values.end(), actual_values.begin()));

    // Ensure data on device matches expected
    rmm::device_vector<T> expected_device_data(expected_values);
    T const* actual_device_data = static_cast<T const*>(underlying_column->data);
    EXPECT_TRUE(thrust::equal(rmm::exec_policy()->on(0),
                              expected_device_data.begin(),
                              expected_device_data.end(),
                              actual_device_data));
  ...
  if (expected_bitmask.size() > 0) {
    ...
    cudf::size_type const expected_null_count =
      expected_values.size() - count_valid_bits_host(expected_bitmask, underlying_column->size);

    EXPECT_EQ(expected_null_count, col.null_count());

    // Ensure data on device matches expected
    rmm::device_vector<cudf::valid_type> expected_device_bitmask(expected_bitmask);
    ...
    EXPECT_TRUE(thrust::equal(
      ...
      expected_device_bitmask.begin() + gdf_num_bitmask_elements(expected_values.size()) - 1,
      underlying_column->valid));

    EXPECT_TRUE(
      std::equal(expected_bitmask.begin(),
                 expected_bitmask.begin() + gdf_num_bitmask_elements(expected_values.size()) - 1,
    ...
    cudf::size_type valid_bits_last_mask = expected_values.size() % GDF_VALID_BITSIZE;
    if (0 == valid_bits_last_mask) { valid_bits_last_mask = GDF_VALID_BITSIZE; }

    for (cudf::size_type i = 0; i < valid_bits_last_mask; ++i) {
...
TYPED_TEST(ColumnWrapperTest, InitializerListBitInitializer)
{
  ...
  std::vector<cudf::valid_type> expected_bitmask(gdf_valid_allocation_size(list.size()), 0xAA);
  std::vector<TypeParam> expected_values(list);
...
TYPED_TEST(ColumnWrapperTest, ValueBitmaskVectorConstructor)
{
  ...
  std::generate(expected_values.begin(), expected_values.end(), [this]() {
    return TypeParam(this->generator());
  });

  std::vector<cudf::valid_type> expected_bitmask(gdf_valid_allocation_size(size), 0xFF);

  cudf::test::column_wrapper<TypeParam> const col(expected_values, expected_bitmask);
...
TYPED_TEST(ColumnWrapperTest, ValueBitInitConstructor)
{
  ...
  cudf::test::column_wrapper<TypeParam> col(
    size, [](auto row) { return static_cast<TypeParam>(row); }, [](auto row) { return true; });
...
TYPED_TEST(ColumnWrapperTest, ValueVectorBitmaskInitConstructor)
{
  ...
  // Every even bit is null
  std::vector<cudf::valid_type> expected_bitmask(gdf_valid_allocation_size(size), 0xAA);

  auto even_bits_null = [](auto row) { return (row % 2); };

  cudf::test::column_wrapper<TypeParam> const col(expected_values, even_bits_null);
...
TYPED_TEST(ColumnWrapperTest, CopyConstructor)
{
  ...
  EXPECT_EQ(source_column->dtype_info.time_unit, copy_column->dtype_info.time_unit);
  ...
  EXPECT_TRUE(thrust::equal(
    rmm::exec_policy()->on(0), source_device_data, source_device_data + size, copy_device_data));
  ...
  EXPECT_TRUE(std::equal(source_data.begin(), source_data.end(), copy_data.begin()));

  EXPECT_TRUE(std::equal(source_bitmask.begin(), source_bitmask.end(), copy_bitmask.begin()));
...
TYPED_TEST(ColumnWrapperTest, AllValid)
{
  ...
  for (cudf::size_type i = 0; i < size; ++i) { EXPECT_TRUE(gdf_is_valid(bitmask.data(), i)); }
}
