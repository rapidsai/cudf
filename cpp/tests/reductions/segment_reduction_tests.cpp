...
template <typename T>
struct SegmentReductionTest : public cudf::test::BaseFixture {
};

TYPED_TEST_CASE(SegmentReductionTest, NumericTypes);

TYPED_TEST(SegmentReductionTest, Sum)
{
  // TODO: add nulls
  ...
  auto res =
    segment_reduce(input, offsets, make_sum_aggregation(), data_type{type_to_id<TypeParam>()});
