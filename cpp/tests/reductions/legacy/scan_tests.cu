...
struct ScanTest : public GdfTest {
  void scan_test(std::vector<int> const& int_values,
    ...
    std::transform(int_values.begin(), int_values.end(), input_values.begin(), [](int x) {
      T t(x);
      return t;
    });
    ...
    std::equal(exact_values.begin(),
               exact_values.end(),
               host_result.begin(),
               host_result.end(),
               [](int x, UnderlyingType y) {
                 EXPECT_EQ(UnderlyingType(x), y);
                 return true;
               });
  }
...
using Types = testing::Types<int8_t, int16_t, int32_t, int64_t, float, double, cudf::bool8>;
...
TYPED_TEST(ScanTest, Min) {
  ...
  std::for_each(v.begin(), v.end(), [&acc, &exact](int i) {
    acc = std::min(acc, i);
    exact.push_back(acc);
  });
