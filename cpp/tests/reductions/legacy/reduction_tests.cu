...
struct ReductionTest : public GdfTest {
  ...
  static constexpr bool ret_non_arithmetic =
    (std::is_arithmetic<T>::value || std::is_same<T, cudf::bool8>::value) ? true : false;
...
using AllTypes = testing::Types<int8_t,
                                int16_t,
                                int32_t,
                                int64_t,
                                float,
...
TYPED_TEST(ReductionTest, Product)
{
  ...
  this->reduction_test(col, expected_value, this->ret_non_arithmetic, cudf::reduction::PRODUCT);
...
TYPED_TEST(ReductionTest, SumOfSquare)
{
  ...
  this->reduction_test(
    col, expected_value, this->ret_non_arithmetic, cudf::reduction::SUMOFSQUARES);
  ...
  this->reduction_test(
    col_nulls, expected_null_value, this->ret_non_arithmetic, cudf::reduction::SUMOFSQUARES);
}
...
using MultiStepReductionTypes = testing::Types<int8_t, int16_t, int32_t, int64_t, float, double>;
...
TYPED_TEST(MultiStepReductionTest, Mean)
{
  ...
  this->reduction_test(col, expected_value, true, cudf::reduction::MEAN, GDF_FLOAT64);
...
struct ReductionDtypeTest : public GdfTest {
                      ...
                      bool expected_overflow = false)
  {
    ...
    auto statement = [&]() {
      cudf::test::scalar_wrapper<T_out> result = cudf::reduce(col.get(), op, out_dtype);
      if (result.is_valid() && !expected_overflow) { EXPECT_EQ(expected_value, result.value()); }
    };
...
TEST_F(ReductionDtypeTest, different_precision)
{
  ...
  this->reduction_test<int8_t, int64_t>(
    int_values, static_cast<int64_t>(expected_value), true, cudf::reduction::SUM, GDF_INT64);

  this->reduction_test<int8_t, double>(
    int_values, static_cast<double>(expected_value), true, cudf::reduction::SUM, GDF_FLOAT64);
  ...
  this->reduction_test<double, int16_t>(
    int_values, static_cast<int16_t>(expected_value), true, cudf::reduction::SUM, GDF_INT16);

  // not supported case:
  // wrapper classes other than cudf::bool8 are not convertible
  this->reduction_test<cudf::date64, cudf::timestamp>(int_values,
                                                      static_cast<cudf::timestamp>(expected_value),
                                                      false,
                                                      cudf::reduction::SUM,
                                                      GDF_TIMESTAMP);

  this->reduction_test<cudf::date32, cudf::category>(int_values,
                                                     static_cast<cudf::category>(expected_value),
                                                     false,
                                                     cudf::reduction::SUM,
                                                     GDF_CATEGORY);

  this->reduction_test<cudf::date32, cudf::date64>(
    int_values, static_cast<cudf::date64>(expected_value), false, cudf::reduction::SUM, GDF_DATE64);

  this->reduction_test<int8_t, cudf::timestamp>(int_values,
                                                static_cast<cudf::timestamp>(expected_value),
                                                false,
                                                cudf::reduction::SUM,
                                                GDF_TIMESTAMP);
  ...
  this->reduction_test<int8_t, cudf::nvstring_category>(
    int_values,
    static_cast<cudf::nvstring_category>(expected_value),
    false,
    cudf::reduction::SUM,
    GDF_STRING_CATEGORY);

  this->reduction_test<int8_t, cudf::category>(int_values,
                                               static_cast<cudf::category>(expected_value),
                                               false,
                                               cudf::reduction::SUM,
                                               GDF_CATEGORY);

  this->reduction_test<cudf::bool8, cudf::date32>(int_values,
                                                  static_cast<cudf::date32>(expected_value),
                                                  false,
                                                  cudf::reduction::SUM,
                                                  GDF_CATEGORY);
  ...
  this->reduction_test<cudf::bool8, int8_t>(
    int_values, static_cast<int8_t>(expected_bool8), true, cudf::reduction::SUM, GDF_INT8);
  this->reduction_test<cudf::bool8, int16_t>(
    int_values, static_cast<int16_t>(expected_bool8), true, cudf::reduction::SUM, GDF_INT16);
  this->reduction_test<cudf::bool8, int32_t>(
    int_values, static_cast<int32_t>(expected_bool8), true, cudf::reduction::SUM, GDF_INT32);
  this->reduction_test<cudf::bool8, int64_t>(
    int_values, static_cast<int64_t>(expected_bool8), true, cudf::reduction::SUM, GDF_INT64);
  this->reduction_test<cudf::bool8, float>(
    int_values, static_cast<float>(expected_bool8), true, cudf::reduction::SUM, GDF_FLOAT32);
  this->reduction_test<cudf::bool8, double>(
    int_values, static_cast<double>(expected_bool8), true, cudf::reduction::SUM, GDF_FLOAT64);
