...
TEST_F(GroupSTDTest, SingleColumn) {
  ...
  //  { 0, 3, 6, 1, 4, 5, 9, 2, 7, 8}
  auto expect_vals =
    cudf::test::column_wrapper<double>({3, sqrt(131. / 12), sqrt(31. / 3)}, all_valid());
  ...
  ASSERT_EQ(result_keys, expect_keys)
    << "Expected: " << expect_keys.to_str() << "  Actual: " << result_keys.to_str();
...
TEST_F(GroupSTDTest, MultiColumn) {
  ...
  //  { 0, 1, 3,   2, 4, 4, 8,    6, 6, 6}
  auto expect_vals1 =
    cudf::test::column_wrapper<double>({sqrt(7. / 3), sqrt(19. / 3), 0}, all_valid());
...
TEST_F(GroupSTDTest, SingleColumnNullable) {
  ...
  auto keys = cudf::test::column_wrapper<int32_t>(
    keys_data, [&](cudf::size_type row) { return keys_valid[row]; });
  auto vals = cudf::test::column_wrapper<double>(
    vals_data, [&](cudf::size_type row) { return vals_valid[row]; });

  auto expect_keys =
    cudf::test::column_wrapper<int32_t>(expect_keys_data, [](cudf::size_type i) { return true; });
  auto expect_vals = cudf::test::column_wrapper<double>(
    expect_vals_data, [&](cudf::size_type i) { return expect_vals_valid[i]; });
