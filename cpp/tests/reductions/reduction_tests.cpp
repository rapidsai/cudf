...
TEST_P(DictionaryStringReductionTest, MinMaxUnsortedKeys)
{
  std::vector<std::string> host_strings(GetParam());
  cudf::data_type output_type{cudf::type_id::STRING};

  cudf::test::dictionary_column_wrapper<std::string> col(host_strings.begin(), host_strings.end());
  cudf::test::strings_column_wrapper new_keys{"nine", "two", "eight", "five", "six", "three"};
...
TYPED_TEST(DictionaryReductionTest, MinMaxUnsortedKeys)
{
  using T = TypeParam;
  ...
  cudf::test::dictionary_column_wrapper<T> col(v.begin(), v.end());
  std::vector<int> key_values({30, 50, 10, 40, 20});
  ...
  auto min_agg = cudf::make_min_aggregation<reduce_aggregation>();
  EXPECT_EQ(this->template reduction_test<T>(dict->view(), *min_agg, output_type).first, *min_it);
  auto max_agg = cudf::make_max_aggregation<reduce_aggregation>();
  EXPECT_EQ(this->template reduction_test<T>(dict->view(), *max_agg, output_type).first, *max_it);
  ...
  auto index_type = cudf::data_type{cudf::type_id::INT32};
  auto argmin_agg = cudf::make_argmin_aggregation<reduce_aggregation>();
  EXPECT_EQ(this->template reduction_test<int32_t>(dict->view(), *argmin_agg, index_type).first,
            static_cast<int32_t>(std::distance(v.begin(), min_it)));
  auto argmax_agg = cudf::make_argmax_aggregation<reduce_aggregation>();
  EXPECT_EQ(this->template reduction_test<int32_t>(dict->view(), *argmax_agg, index_type).first,
