...
std::unique_ptr<cudf::column> make_numeric_maps_input()
{
  ...
  return cudf::make_lists_column(9, offsets{0, 3, 6, 9, 12, 15, 18, 18, 18, 21}.release(), key_vals.release(), 1, detail::make_null_mask(null_mask_begin, null_mask_begin + num_rows));
}
...
TYPED_TEST(MapsTest, Construction)
{
    ...
    auto const expected_values = list{
        {
            {0,1,2}, 
            {0,1,2}, 
            {3,4,5}, 
