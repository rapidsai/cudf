                                             ...
                                             cudf::column_view const& test_data_comp)
{
   ...
   {                             
      ...
      // compare bitwise
      cudf::test::expect_equal_buffers(mutable_view.head(), test_data_comp.head(), mutable_view.size() * sizeof(T));
   }
   ...
   // returned column overload
   {
      ...
      // compare bitwise
      cudf::test::expect_equal_buffers(out_view.head(), test_data_comp.head(), out_view.size() * sizeof(T));            
   }      
}
