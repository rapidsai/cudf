...
template <typename T>
struct GatherOptTest : GdfTest {};

using test_types =
    ::testing::Types<int8_t, int16_t, int32_t, int64_t, float, double, cudf::bool8>;
TYPED_TEST_CASE(GatherOptTest, test_types);

TYPED_TEST(GatherOptTest, MultiColReverseIdentityTest) {
...
TYPED_TEST(GatherOptTest, MultiColInPlaceTest) {
  constexpr gdf_size_type source_size{1000};
...
TYPED_TEST(GatherOptTest, DtypeMistach){
  ...
  EXPECT_THROW(cudf::opt::gather(&source_table, gather_map.data().get(),
                             &destination_table), cudf::logic_error);
}

TYPED_TEST(GatherOptTest, DestMissingValid){
  constexpr gdf_size_type source_size{1000};
  constexpr gdf_size_type destination_size{1000};
...
TYPED_TEST(GatherOptTest, NumColumnsMismatch){
  constexpr gdf_size_type source_size{1000};
  constexpr gdf_size_type destination_size{1000};
...
TYPED_TEST(GatherOptTest, IdentityTest) {
  constexpr gdf_size_type source_size{1000};
  constexpr gdf_size_type destination_size{1000};
...
TYPED_TEST(GatherOptTest, ReverseIdentityTest) {
  constexpr gdf_size_type source_size{1000};
  constexpr gdf_size_type destination_size{1000};
...
TYPED_TEST(GatherOptTest, AllNull) {
  constexpr gdf_size_type source_size{1000};
  constexpr gdf_size_type destination_size{1000};
...
TYPED_TEST(GatherOptTest, EveryOtherNull) {
