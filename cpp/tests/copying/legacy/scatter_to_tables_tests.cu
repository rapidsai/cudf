...
TYPED_TEST(ScatterToTablesTest, PassTest) {
  ...
  cudf::test::column_wrapper<cudf::size_type> scatter_map(
    scatter_size, [](cudf::size_type row) { return static_cast<cudf::size_type>(row % 2); }, false);
  ...
  EXPECT_NO_THROW(output_tables = cudf::scatter_to_tables(input_table, scatter_map));
...
TYPED_TEST(ScatterToTablesTest, ScatterTypeFailTest) {
  ...
  cudf::test::column_wrapper<TypeParam> scatter_map(
    scatter_size, [](cudf::size_type row) { return static_cast<TypeParam>(row % 2); }, false);
...
auto scatter_columns(std::vector<std::vector<T>> input_cols_data,
  ...
  for (auto g = 0; g <= max; g++) {
    ...
    for (size_t c = 0; c < num_cols; c++) {
      ...
      std::vector<cudf::valid_type> output_col_bitmask(gdf_valid_allocation_size(n_output),
                                                       ~cudf::valid_type(0));
    ...
    EXPECT_EQ(num_cols, output_cols_bitmask[g].size())
      << "Reference solution calculation failure\n";
    EXPECT_EQ(num_cols, output_cols_null_count[g].size())
      << "Reference solution calculation failure\n";
  }
  EXPECT_EQ(size_t(max + 1), output_cols_data.size()) << "Reference solution calculation failure\n";
  EXPECT_EQ(size_t(max + 1), output_cols_bitmask.size())
    << "Reference solution calculation failure\n";
  EXPECT_EQ(size_t(max + 1), output_cols_null_count.size())
    << "Reference solution calculation failure\n";
...
TYPED_TEST(ScatterToTablesTest, FunctionalTest) {
  ...
  for (auto scatter_case :
       std::vector<std::pair<size_t, std::function<cudf::size_type(cudf::size_type)>>>{
         // range 0-0
    ...
    cudf::test::column_wrapper<cudf::size_type> scatter_map(
      scatter_size, scatter_case.second, false);
    std::vector<cudf::table> output_tables;
    EXPECT_NO_THROW(output_tables = cudf::scatter_to_tables(input_table, scatter_map));
    ...
    for (size_t i = 0; i < output_cols_data.size(); i++) {
      ...
      for (int c = 0; c < table_n_cols; c++) {
        ...
        EXPECT_TRUE(cudf::test::column_wrapper<TypeParam>(output_cols_data[i][c],
