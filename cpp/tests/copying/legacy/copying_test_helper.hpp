...
std::vector<cudf::valid_type> slice_cpu_valids(cudf::size_type& bit_set_counter,
  ...
  auto get_data_bitset = [&bitset, &bit_index, &bits_length, &bit_set_counter]() {
    ...
    for (int k = 0; k < 8; ++k) {
      if (bits_length <= bit_index) { return value; }
      cudf::valid_type tmp = bitset[bit_index];
      value |= (tmp << k);
      bit_index++;
      if ((tmp & 1) == 0) { bit_set_counter++; }
    }
    return value;
  };
...
auto slice_columns(std::vector<ColumnType>& input_col_data,
  ...
  for (std::size_t i = 0; i < indexes.size(); i += 2) {
    ...
    if (init_index == end_index) {
      ...
      output_cols_data.emplace_back(std::vector<ColumnType>(input_col_data.begin() + init_index,
                                                            input_col_data.begin() + end_index));

      cudf::size_type bit_set_counter = 0;
      output_cols_bitmask.emplace_back(
        slice_cpu_valids<BITSET_SIZE>(bit_set_counter, input_col_bitmask, init_index, end_index));
...
auto split_columns(std::vector<ColumnType>& input_col_data,
  ...
  for (std::size_t i = 0; i <= indexes.size(); ++i) {
    ...
    if (i != 0) { init_index = indexes[i - 1]; }
    cudf::size_type end_index = input_col_data.size();
    if (i < indexes.size()) { end_index = indexes[i]; }
