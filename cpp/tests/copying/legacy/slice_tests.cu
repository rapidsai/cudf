...
TEST_F(SliceInputTest, InputColumnSizeNull)
{
  ...
  // Create indices
  std::vector<cudf::size_type> indices_host{SIZE / 4, SIZE / 2};
  cudf::test::column_wrapper<cudf::size_type> indices(indices_host);

  // Perform test
  std::vector<gdf_column*> output;
  ASSERT_NO_THROW(call_slice(&input_column,
                             static_cast<cudf::size_type*>(indices.get()->data),
                             indices.get()->size,
                             output));
  ASSERT_EQ(output.size(), std::size_t(0));
}
...
TEST_F(SliceInputTest, InputColumnDataNull)
{
  ...
  // Perform test
  std::vector<gdf_column*> output;
  ASSERT_ANY_THROW(call_slice(input_column_test,
                              static_cast<cudf::size_type*>(indices.get()->data),
                              indices.get()->size,
                              output));
}
...
TEST_F(SliceInputTest, InputColumnBitmaskNull)
{
  ...
  // Perform test
  std::vector<gdf_column*> output;
  ASSERT_NO_THROW(call_slice(input_column_test,
                             static_cast<cudf::size_type*>(indices.get()->data),
                             indices.get()->size,
                             output));
}
...
TEST_F(SliceInputTest, IndexesSizeNotEven)
{
  ...
  std::vector<cudf::size_type> indices_host_test{SIZE / 4, SIZE / 3, SIZE / 2};
  cudf::test::column_wrapper<cudf::size_type> indices_test(indices_host_test);

  // Perform test
  std::vector<gdf_column*> output;
  ASSERT_ANY_THROW(call_slice(input_column.get(),
                              static_cast<cudf::size_type*>(indices_test.get()->data),
                              indices_test.get()->size,
                              output));
}
...
TYPED_TEST(SliceTest, MultipleSlices)
{
  ...
  // Create indices
  std::vector<cudf::size_type> indices_host{7, 13, 17, 37, 43, 43, 17, INPUT_SIZE};
  cudf::test::column_wrapper<cudf::size_type> indices(indices_host);

  // Perform operation
  std::vector<gdf_column*> output_column_ptrs;
  ASSERT_NO_THROW(call_slice(input_column.get(),
                             static_cast<cudf::size_type*>(indices.get()->data),
                             indices.get()->size,
                             output_column_ptrs));
  ...
  std::tie(output_cols_data, output_cols_bitmask, output_cols_null_count) =
    slice_columns<TypeParam>(input_col_data, input_col_bitmask, indices_host);
  ...
  for (std::size_t i = 0; i < output_column_ptrs.size(); i++) {
    validation_columns.emplace_back(
      cudf::test::column_wrapper<TypeParam>(output_cols_data[i], output_cols_bitmask[i]));
...
TYPED_TEST(SliceTest, RangeIndexPosition)
{
  ...
  constexpr cudf::size_type FINAL_INDEX{INPUT_SIZE - SLICE_RANGE};

  // Create input column
  auto input_column = create_random_column<TypeParam>(INPUT_SIZE);
  for (cudf::size_type index = INIT_INDEX; index < FINAL_INDEX; ++index) {
    // Create indices
    std::vector<cudf::size_type> indices_host{index, index + SLICE_RANGE};
    cudf::test::column_wrapper<cudf::size_type> indices(indices_host);
...
TEST_F(SliceInputTest, NVCategoryMultipleSlices)
{
  ...
  // Create input column
  gdf_column* input_column =
    cudf::test::create_nv_category_column_strings(orig_string_data, INPUT_SIZE);
  ...
  for (std::size_t i = 0; i < output_column_ptrs.size(); i++) {
    std::tie(host_output_string_vector[i], host_output_bitmask[i]) =
      cudf::test::nvcategory_column_to_host(output_column_ptrs[i]);
  }
  ...
  std::vector<cudf::size_type> output_cols_null_count;
  std::tie(output_cols_data, output_cols_bitmask, output_cols_null_count) =
    slice_columns<std::string>(input_col_data, input_col_bitmask, indices_host);
  ...
  for (std::size_t i = 0; i < host_output_string_vector.size(); ++i) {
    ...
    for (std::size_t j = 0; j < host_output_string_vector[i].size(); ++j) {
      bool lhs_is_valids = bit_mask::is_valid(
        reinterpret_cast<bit_mask::bit_mask_t*>(host_output_bitmask[i].data()), j);
      bool rhs_is_valids = bit_mask::is_valid(
        reinterpret_cast<bit_mask::bit_mask_t*>(output_cols_bitmask[i].data()), j);
