...
TYPED_TEST(GatherTest, MultiColReverseIdentityTest) {
  ...
  for (size_t i = 0; i < v_src.size(); i++) { vp_src[i] = v_src[i].get(); }
  ...
  for (size_t i = 0; i < v_src.size(); i++) { vp_dest[i] = v_dest[i].get(); }

  cudf::table source_table{vp_src};
  cudf::table destination_table{vp_dest};

  EXPECT_NO_THROW(cudf::gather(&source_table, gather_map.data().get(), &destination_table));
  ...
  for (size_t i = 0; i < n_cols; i++) {
    ...
    expect_column_values_are_equal<TypeParam>(destination_size,
                                              expected_data.data(),
                                              nullptr,
                                              "Expected",
                                              result_data.data(),
...
TYPED_TEST(GatherTest, MultiColInPlaceTest) {
  ...
  EXPECT_NO_THROW(cudf::gather(&source_table, gather_map.data().get(), &source_table));
...
TYPED_TEST(GatherTest, DtypeMistach) {
  ...
  EXPECT_THROW(cudf::gather(&source_table, gather_map.data().get(), &destination_table),
               cudf::logic_error);
}
