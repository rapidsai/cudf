...
struct SanitizedGatherTest : public cudf::test::BaseFixture {
};
...
// List<T>.
TEST_F(SanitizedGatherTest, SingleLevel)
{
  using T = int32_t;
...

  {
    ...
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(results_list_view.child(), values_col_t{{5, 1,2,3,4, 8,9,10}, null_at(3)});
  }
  {
...
// List<List<T>>.
TEST_F(SanitizedGatherTest, TwoLevel)
{
  using T = int32_t;
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(results_lists_view.child(),
    LCW<T>{{1,2,3}, {4,5,6,7}, {8}, {9,1}, {2}, {11,12}, {13,14,15}, {16,17,18}, {19}});

  auto const child_lists_view = lists_column_view(results_lists_view.child());
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(child_lists_view.offsets(), offsets_col_t{0, 3, 7, 8, 10, 11, 13, 16, 19, 20});
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(child_lists_view.child(), 
    values_col_t{1, 2, 3, 4, 5, 6, 7, 8, 9, 1, 2, 11, 12, 13, 14, 15, 16, 17, 18, 19});
}

// List<List<List<T>>>.
TEST_F(SanitizedGatherTest, ThreeLevel)
{
  using T = int32_t;
...
// List<string>.
TEST_F(SanitizedGatherTest, ListOfStrings)
{
  using T = string_view;
...

  {
    ...
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results, expected);
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(results_list_view.offsets(), offsets_col_t{0, 1, 1, 5, 8});
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(results_list_view.child(), strings_column_wrapper{
      {"55555", "1","22","","4444", "88888888","999999999","1010101010"}, 
      null_at(3)});
  }
  {
...
// List<string>.
TEST_F(SanitizedGatherTest, UnsanitizedListOfUnsanitizedStrings)
{
  auto strings = strings_column_wrapper{{
    "1", "22", "3", "44",
    "5",
    "66", "7",
    "8888", "9", "1010"}, //<--- "8888" will be unsanitized.
    no_nulls()
  }.release();
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    strings_column_view(*strings).offsets(), 
    offsets_col_t{0,1,3,4,6,7,9,10,14,15,19} // 10-14 indicates that "8888" is unsanitized.
  );

  // Construct a list column from the strings column.
  auto const lists = make_lists_column(
    4, offsets_col_t{0, 4, 5, 7, 10}.release(), std::move(strings), 
    0, detail::make_null_mask(no_nulls(), no_nulls()+4));
  ...
  auto const expected = LCW<string_view>{
    {
      {"5"},
      {}, // NULL.
      {"1","22","3","44"},
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    results_lists_view.offsets(), 
    offsets_col_t{0, 1, 1, 5, 8} // 1-1 indicates that row#2 is sanitized.
  );
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(child_strings_view.offsets(), offsets_col_t{0, 1, 2, 4, 5, 7, 7, 8, 12});
}
...
TEST_F(SanitizedGatherTest, StructOfList)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
      lists_column_view(structs_input->child(0)).child(),
