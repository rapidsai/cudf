...
TYPED_TEST(ScatterTest, DtypeMistach){
  ...
  EXPECT_THROW(cudf::opt::scatter(&source_table, scatter_map.data().get(),
...
TYPED_TEST(ScatterTest, IdentityTest) {
  ...
  EXPECT_NO_THROW(cudf::opt::scatter(&source_table, scatter_map.data().get(),
                                &destination_table));
...
TYPED_TEST(ScatterTest, PreserveDestBitmask) {
  ...
  cudf::opt::scatter(&source_table, d_scatter_map.data().get(), &destination_table);
