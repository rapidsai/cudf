...
TYPED_TEST_CASE(TypedStructGatherTest, FixedWidthTypesNotBool);
...
TYPED_TEST(TypedStructGatherTest, TestSimpleStructGather)
{
  ...
  auto gathered_struct_col      = gathered_table->get_column(0);
  auto gathered_struct_col_view = cudf::structs_column_view{gathered_struct_col};
...
TYPED_TEST(TypedStructGatherTest, TestGatherStructOfLists)
{
  ...
  auto struct_column = structs_column_wrapper{std::move(vector_of_columns)}.release();
...
TYPED_TEST(TypedStructGatherTest, TestGatherStructOfListOfStructs)
{
  ...
  auto struct_of_list_of_structs = structs_column_wrapper{std::move(vector_of_columns)}.release();
  ...
  auto gathered_table =
    cudf::gather(cudf::table_view{std::vector<cudf::column_view>{struct_of_list_of_structs->view()}},
                 gather_map_col->view());
  ...
  auto expected_numeric_col = 
    fixed_width_column_wrapper<TypeParam, int32_t>{
  ...
  auto expected_struct_of_list_of_struct =
...
TYPED_TEST(TypedStructGatherTest, TestGatherStructOfStructsWithValidity)
{
  ...
  // Validity predicates.
  auto every_3rd_element_null = [](size_type i) { return !(i % 3); };
  auto twelfth_element_null   = [](size_type i) { return i != 11; };
...
TYPED_TEST(TypedStructGatherTest, TestEmptyGather)
{
  ...
  auto struct_validity = std::vector<bool>{1, 1, 1, 1, 1, 0};
  auto struct_column   = structs_column_wrapper{{ages_column}, struct_validity.begin()}.release();

  auto gather_map = std::vector<int>{};
  auto gather_map_col =
    fixed_width_column_wrapper<int32_t>(gather_map.begin(), gather_map.end()).release();
  ...
  auto expected_structs_column = structs_column_wrapper{{expected_ages_column}}.release();
