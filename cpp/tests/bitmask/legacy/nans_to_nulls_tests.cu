...
TYPED_TEST(bitmask_test, nans_to_nulls_source_mask_valid) {
  ...
  cudf::test::column_wrapper<TypeParam> source_column{
    source_size,
    [](cudf::size_type row) {
      return row % 3 ? static_cast<TypeParam>(row) : static_cast<TypeParam>(nan(""));
    },
  ...
  const auto num_bytes =
    cudf::util::packed_bit_sequence_size_in_bytes<bit_mask_t>(source_size) * sizeof(bit_mask_t);
...
TYPED_TEST(bitmask_test, nans_to_nulls_source_mask_null) {
  ...
  cudf::test::column_wrapper<TypeParam> source_column{
    source_size, [](cudf::size_type row) {
      return row % 3 ? static_cast<TypeParam>(row) : static_cast<TypeParam>(nan(""));
    }};
