...
void tables_are_equal(const cudf::table &left, const cudf::table &right) {
  ...
  while (result != right.end()) { EXPECT_TRUE(columns_are_equal(*(*expected++), *(*result++))); }
}
...
TYPED_TEST(OrcWriterTypedParamTest, SingleColumn) {
  ...
  auto filepath = temp_env->get_temp_filepath("OrcWriterSingleColumn") + gdf_col->col_name;
...
TYPED_TEST(OrcWriterTypedParamTest, SingleColumnWithNulls) {
  ...
  cudf::test::column_wrapper<TypeParam> col{random_values<TypeParam>(num_rows), valids_func};
  column_set_name(col.get(), "col_" + std::string{typeid(TypeParam).name()});
  ...
  auto filepath = temp_env->get_temp_filepath("OrcWriterSingleColumnWithNulls") + gdf_col->col_name;
...
TEST_F(OrcWriterTest, MultiColumn) {
  ...
  cudf::test::column_wrapper<int8_t> col1{random_values<int8_t>(num_rows), valids_func};
  cudf::test::column_wrapper<int16_t> col2{random_values<int16_t>(num_rows), valids_func};
  cudf::test::column_wrapper<int32_t> col3{random_values<int32_t>(num_rows), valids_func};
  cudf::test::column_wrapper<float> col4{random_values<float>(num_rows), valids_func};
  cudf::test::column_wrapper<double> col5{random_values<double>(num_rows), valids_func};
...
TEST_F(OrcWriterTest, MultiColumnWithNulls) {
  ...
  cudf::test::column_wrapper<cudf::bool8> col0{random_values<cudf::bool8>(num_rows),
  ...
  auto filepath = temp_env->get_temp_filepath("OrcWriterMultiColumnWithNulls.orc");
...
TEST_P(OrcWriterValueParamTest, Timestamps) {
  ...
  cudf::test::column_wrapper<cudf::timestamp> col{num_rows, values_fn, valids_fn};
...
INSTANTIATE_TEST_CASE_P(OrcWriter,
...
TEST_F(OrcWriterTest, Strings) {
  ...
  cudf::test::column_wrapper<int> col0{random_values<int>(column_size),
                                       [](size_t row) { return true; }};
  cudf::test::column_wrapper<cudf::nvstring_category> col1{column_size, data.data()};
  ...
  auto expect_strs =
    nvcategory_to_strings(static_cast<NVCategory *>(expected.get_column(1)->dtype_info.category));
  auto result_strs = nvstrings_to_strings(static_cast<NVStrings *>(result.get_column(1)->data));
