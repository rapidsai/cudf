...
TEST_F(VariantExtractTest, ExtractInt32TopLevelField)
{
  ...
  auto got = cudf::extract_variant_field(
...
{
  cudf::test::lists_column_wrapper<uint8_t> meta{{0x01, 0x01, 0x00, 0x01, static_cast<uint8_t>('x')},
                                                 {0x01, 0x01, 0x00, 0x01, static_cast<uint8_t>('x')}};
  cudf::test::lists_column_wrapper<uint8_t> val{{0x02, 0x01, 0x00, 0x00, 0x05, 0x14, 0x07, 0x00, 0x00, 0x00},
...
TEST_F(VariantExtractTest, ObjectTwoFieldsSingleRow)
{
  ...
  std::vector<uint8_t> const valb = {0x02, 0x02, 0x00, 0x01, 0x00, 0x05, 0x08, 0x14, 0x07, 0x00, 0x00, 0x00,
                                     0x09, 'h', 'i'};
  cudf::test::lists_column_wrapper<uint8_t> meta(metab.begin(), metab.end());
  cudf::test::lists_column_wrapper<uint8_t> val(valb.begin(), valb.end());
  cudf::test::structs_column_wrapper struc{{meta, val}};
  auto stream = cudf::test::get_default_stream();
  auto x = cudf::extract_variant_field(struc, "x", cudf::data_type{cudf::type_id::INT32}, stream);
  cudf::test::fixed_width_column_wrapper<int32_t> x_exp{7};
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(*x, x_exp);
  auto k = cudf::extract_variant_field(struc, "k", cudf::data_type{cudf::type_id::STRING}, stream);
...
TEST_F(VariantExtractTest, MultiRowDistinctKeysHandBuilt)
{
  ...
  std::vector<uint8_t> const v2 = {0x02, 0x02, 0x00, 0x01, 0x00, 0x05, 0x0a, 0x14, 0x2a, 0x00, 0x00, 0x00,
  ...
  cudf::test::lists_column_wrapper<uint8_t> meta{{m1.begin(), m1.end()},
                                                 {m2.begin(), m2.end()},
                                                 {m3.begin(), m3.end()}};
  cudf::test::lists_column_wrapper<uint8_t> val{{v1.begin(), v1.end()},
                                                {v2.begin(), v2.end()},
                                                {v3.begin(), v3.end()}};
  cudf::test::structs_column_wrapper struc{{meta, val}};
  ...
  auto y = cudf::extract_variant_field(struc, "y", cudf::data_type{cudf::type_id::INT32}, stream);
...
TEST_F(VariantExtractTest, GetVariantFieldReturnsVariantStruct)
{
  // Object { "x": 7, "k": "hi" }
  std::vector<uint8_t> const metab = {0x01, 0x02, 0x00, 0x01, 0x02, 'x', 'k'};
  std::vector<uint8_t> const valb  = {0x02, 0x02, 0x00, 0x01, 0x00, 0x05, 0x08,
                                      0x14, 0x07, 0x00, 0x00, 0x00, 0x09, 'h', 'i'};
  cudf::test::lists_column_wrapper<uint8_t> meta(metab.begin(), metab.end());
  cudf::test::lists_column_wrapper<uint8_t> val(valb.begin(), valb.end());
  cudf::test::structs_column_wrapper struc{{meta, val}};

  auto got = cudf::get_variant_field(struc, "x", cudf::test::get_default_stream());
  ...
  auto casted = cudf::cast_variant(got->view(), cudf::data_type{cudf::type_id::INT32},
...
TEST_F(VariantExtractTest, GetVariantFieldMissingKeyAllNull)
{
  ...
  auto got = cudf::get_variant_field(struc, "missing", cudf::test::get_default_stream());
...
TEST_F(VariantExtractTest, CastVariantLongString)
{
  ...
  for (auto c : payload) { valb.push_back(static_cast<uint8_t>(c)); }
...
TEST_F(VariantExtractTest, GetThenCastMatchesExtract)
{
  ...
  std::vector<uint8_t> const v1 = {
    0x02, 0x02, 0x00, 0x01, 0x00, 0x05, 0x08, 0x14, 0x07, 0x00, 0x00, 0x00, 0x09, 'h', 'i'};
  std::vector<uint8_t> const m2 = {0x01, 0x02, 0x00, 0x01, 0x02, 'x', 'y'};
  std::vector<uint8_t> const v2 = {0x02, 0x02, 0x00, 0x01, 0x00, 0x05, 0x0a, 0x14, 0x2a, 0x00,
                                   0x00, 0x00, 0x14, 0x63, 0x00, 0x00, 0x00};
  std::vector<uint8_t> const m3 = {0x01, 0x01, 0x00, 0x01, 'k'};
  std::vector<uint8_t> const v3 = {0x02, 0x01, 0x00, 0x00, 0x04, 0x0d, 'z', 'z', 'z'};
  ...
  auto extract_x =
    cudf::extract_variant_field(struc, "x", cudf::data_type{cudf::type_id::INT32}, stream);

  // get_variant_field + cast_variant (two-step)
  auto intermediate = cudf::get_variant_field(struc, "x", stream);
  auto two_step_x =
    cudf::cast_variant(intermediate->view(), cudf::data_type{cudf::type_id::INT32}, stream);

  CUDF_TEST_EXPECT_COLUMNS_EQUAL(*extract_x, *two_step_x);

  // Same for string field "k"
  auto extract_k =
    cudf::extract_variant_field(struc, "k", cudf::data_type{cudf::type_id::STRING}, stream);
  auto intermediate_k = cudf::get_variant_field(struc, "k", stream);
  auto two_step_k =
    cudf::cast_variant(intermediate_k->view(), cudf::data_type{cudf::type_id::STRING}, stream);
...
TEST_F(VariantExtractTest, ApachePrimitiveInt32)
{
  ...
  auto got =
    cudf::cast_variant(struc, cudf::data_type{cudf::type_id::INT32}, cudf::test::get_default_stream());
...
TEST_F(VariantExtractTest, ApacheShortString)
{
  ...
  cudf::test::strings_column_wrapper expected({"\x4c\x65\x73\x73\x20\x74\x68\x61\x6e\x20\x36\x34"
...
TEST_F(VariantExtractTest, ApacheObjectPrimitiveExtractString)
{
  ...
  auto got = cudf::extract_variant_field(
    struc, "string_field", cudf::data_type{cudf::type_id::STRING}, cudf::test::get_default_stream());
...
TEST_F(VariantExtractTest, ApacheNestedGetVariantField)
{
  ...
  auto species = cudf::get_variant_field(struc, "species", stream);
  EXPECT_EQ(species->type().id(), cudf::type_id::STRUCT);

  auto name = cudf::get_variant_field(species->view(), "name", stream);
  EXPECT_EQ(name->type().id(), cudf::type_id::STRUCT);

  auto name_str =
    cudf::cast_variant(name->view(), cudf::data_type{cudf::type_id::STRING}, stream);
