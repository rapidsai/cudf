...
auto get_json_object_multiple2(cudf::column_view input,
                               ...
                               std::string lineno)
{
  ...
  auto num_leaf = 0;
  for (auto& ch : result)
    if (ch) num_leaf += count_leaves(*ch);
  std::cout << "num_leaf:" << num_leaf << "\n";
  return result;
}

#define get_json_object(a, b) get_json_object_single(a, b, std::to_string(__LINE__))
#define get_json_object_multiple(...) \
  get_json_object_multiple2(__VA_ARGS__, std::to_string(__LINE__))

auto get_json_object_multiple1(cudf::column_view input,
                               std::vector<std::string> const& paths,
                               std::string lineno)
{
  cudf::scoped_range rng("s" + lineno);
  auto json_paths = pathstrs_to_json_paths(paths);
  return ::spark_rapids_jni::get_json_object_multiple_paths(
    input, json_paths, size_t(4) << 30, 2, cudf::get_default_stream());
}

#define old_get_json_object_multiple1(...) \
  get_json_object_multiple1(__VA_ARGS__, std::to_string(__LINE__))
...
TEST_F(GetJsonTest, DifferentGetJSONObject)
{
  ...
  std::transform(
    files.begin(), files.end(), std::back_inserter(full_files), [dir](auto f) { return dir + f; });
  ...
  // auto iter = std::find_if(result.metadata.schema_info.begin(),
  // result.metadata.schema_info.end(), [requested_column](auto col_info) {
  //   return col_info.name == requested_column;
  // });
  // if(iter==result.metadata.schema_info.end()) throw std::out_of_range("Can't find expected column
  // " + requested_column); auto col_index = iter - result.metadata.schema_info.begin(); auto input
  // = result.tbl->get_column(col_index);
  ...
  {
    auto res1 = old_get_json_object_multiple1(columnD,
                                              {
                                                "$.KPIGLEDEOCFELKLJLAFE",
                                              });
    auto res2 = get_json_object_multiple(columnD,
                                         {
                                           "$.KPIGLEDEOCFELKLJLAFE",
                                         });
    EXPECT_EQ(res1.size(), res2.size());
