...
TEST_F(DLPackTest, InvalidDeviceType)
{
  ...
  DLManagedTensor *mng_tensor = create_DLTensor<T>(1, length, kDLCPU);
  ...
  for (int i = kDLOpenCL; i <= kDLROCM; i++) {
    ...
    ASSERT_EQ(gdf_from_dlpack(&columns, &num_columns, mng_tensor), GDF_INVALID_API_CALL);
  }
...
TEST_F(DLPackTest, UnsupportedDimensions)
{
  ...
  DLManagedTensor *mng_tensor = create_DLTensor<T>(2, length);
  ...
  ASSERT_EQ(gdf_from_dlpack(&columns, &num_columns, mng_tensor), GDF_NOTIMPLEMENTED_ERROR);

  mng_tensor->dl_tensor.ndim = 0;
  ASSERT_EQ(gdf_from_dlpack(&columns, &num_columns, mng_tensor), GDF_DATASET_EMPTY);
  ...
  ASSERT_EQ(gdf_from_dlpack(&columns, &num_columns, mng_tensor), GDF_COLUMN_SIZE_TOO_BIG);
...
TEST_F(DLPackTest, UnsupportedDataType)
{
  ...
  ASSERT_EQ(gdf_from_dlpack(&columns, &num_columns, mng_tensor), GDF_UNSUPPORTED_DTYPE);
...
TYPED_TEST(DLPackTypedTest, FromDLPack_SingleColumn)
{
  ...
  for (int64_t i = 0; i < length; i++) EXPECT_EQ(static_cast<T>(i), output[i]);
...
TYPED_TEST(DLPackTypedTest, FromDLPack_MultiColumn)
{
  ...
  for (int64_t c = 0; c < num_columns; ++c) {
    ...
    for (int64_t i = 0; i < length; i++) EXPECT_EQ(static_cast<T>(i * (c + 1)), output[i]);
...
TYPED_TEST(DLPackTypedTest, ToDLPack_SingleColumn)
{
  ...
  constexpr int64_t length = 100;
  cudf::test::column_wrapper<T> col0(
    length, [](cudf::size_type i) { return i; }, [](cudf::size_type i) { return true; });
...
TYPED_TEST(DLPackTypedTest, ToDLPack_MultiColumn)
{
  ...
  for (int64_t c = 0; c < width; c++) {
    cols[c] = new cudf::test::column_wrapper<T>(
      length,
  ...
  for (int64_t c = 0; c < width; c++) {
    T *o = &output[c * length];
    for (int64_t i = 0; i < length; i++) EXPECT_EQ(static_cast<T>(i * (c + 1)), o[i]);
  }
  delete[] output;
