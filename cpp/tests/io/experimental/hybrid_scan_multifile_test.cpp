 ...
 *
 * Writes the input table to multiple parquet sources and compares `materialize_all_columns` output
 * with the regular multi-source parquet reader. The filter expression used is `col0 >= 100`.
 *
                                ...
                                bool case_sensitive_names = true)
{
  ...
  for (auto const& source_column_chunks : column_chunks_per_source) {
    column_chunk_data.insert(
      column_chunk_data.end(), source_column_chunks.begin(), source_column_chunks.end());
  }
  auto result = reader.materialize_all_columns(row_groups, column_chunk_data, options, stream, mr);
