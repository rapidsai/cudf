...
TEST_F(group_quantile, SingleColumn) {
  ...
  ASSERT_EQ(result_vals, expect_vals)
...
TEST_F(group_quantile, MultiColumn) {
  ...
  ASSERT_EQ(result_vals0, expect_vals0)
    << "Expected: " << expect_vals0.to_str() << "  Actual: " << result_vals0.to_str();
  auto result_vals1 = cudf::test::column_wrapper<double>(*(val_table.get_column(1)));
  ASSERT_EQ(result_vals1, expect_vals1)
