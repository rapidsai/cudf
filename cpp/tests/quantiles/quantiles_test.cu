...
template<typename T>
struct QuantilesIntegerUnderflowTest : public BaseFixture {
};

using QuantilesIntegerUnderflowTestTypes = cudf::test::Types<int8_t, int16_t, int32_t, int64_t>;

TYPED_TEST_CASE(QuantilesIntegerUnderflowTest, QuantilesIntegerUnderflowTestTypes);

TYPED_TEST(QuantilesIntegerUnderflowTest, X)
{
    using T = TypeParam;
    ...
    auto input_a = fixed_width_column_wrapper<T>({ min,
                                                   static_cast<T>(min + 2),
                                                   static_cast<T>(max - 2),
                                                   max });
    auto input = table_view({ input_a });
    auto expected_a = fixed_width_column_wrapper<T>({ static_cast<T>(min + 1),
                                                      static_cast<T>(max - 1) });
    auto expected = table_view({ expected_a });
