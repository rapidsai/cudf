...
TEST_F(DictionaryScatterTest, Scatter)
{
    ...
    cudf::test::strings_column_wrapper strings_target{ "eee", "aaa", "ddd", "bbb", "ccc", "ccc", "ccc", "eee", "aaa" };
    auto target = cudf::dictionary::encode( strings_target );

    cudf::test::fixed_width_column_wrapper<int32_t> scatter_map{0,2,3,7};
    auto table_result = cudf::experimental::scatter(cudf::table_view{{source->view()}}, scatter_map,
                                                    cudf::table_view{{target->view()}} )->release();
    auto decoded = cudf::dictionary::decode(cudf::dictionary_column_view(table_result.front()->view()));
    cudf::test::strings_column_wrapper expected{ "xxx", "aaa", "bbb", "aaa", "ccc", "ccc", "ccc", "ccc", "aaa" };
    cudf::test::expect_columns_equal(expected,decoded->view());

    // empty map test
    cudf::test::fixed_width_column_wrapper<int32_t> empty_map{};
    table_result = cudf::experimental::scatter(cudf::table_view{{source->view()}}, empty_map,
                                               cudf::table_view{{target->view()}} )->release();
    ...
    table_result = cudf::experimental::scatter(cudf::table_view{{empty_dictionary->view()}}, empty_map,
                                               cudf::table_view{{empty_dictionary->view()}} )->release();
    decoded = cudf::dictionary::decode(cudf::dictionary_column_view(table_result.front()->view()));
...
TEST_F(DictionaryScatterTest, WithNulls)
{
    ...
    cudf::test::fixed_width_column_wrapper<int64_t> data_target{ {1,5,5,3,7,1,4,2},{0,1,0,1,1,1,1,1} };
...
TEST_F(DictionaryScatterTest, Error)
{
    ...
    EXPECT_THROW( cudf::experimental::scatter(cudf::table_view{{source->view()}}, scatter_map,
