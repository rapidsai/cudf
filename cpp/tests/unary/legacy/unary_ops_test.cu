...
TEST_F(col_cast_test, usage_example) {
  ...
  std::vector<int64_t> inputInt64Data = {-1528, 1, 19382};

  std::vector<float> inputFloat32Data = {-1528.6, 1.3, 19382.9};

  std::vector<double> inputFloat64Data = {-1528, 17716, 19382};
  ...
  auto inputTimestampMilliCol =
    cudf::test::column_wrapper<cudf::timestamp>(inputTimestampMilliData);
...
template <>
struct DisType<double, double> {
  typedef double value_type;
};
// Specializations for one type is double
template <typename TOUT>
struct DisType<TOUT, double> {
  typedef double value_type;
};
template <typename TFROM>
struct DisType<double, TFROM> {
  typedef double value_type;
};
...
void fill_with_random_values(std::vector<TFROM> &input, size_t size) {
  ...
  if (std::numeric_limits<TFROM>::max() < std::numeric_limits<TOUT>::max())
    floating_dis = std::uniform_real_distribution<T>(std::numeric_limits<TFROM>::min(),
                                                     std::numeric_limits<TFROM>::max());
  else
    floating_dis = std::uniform_real_distribution<T>(std::numeric_limits<TOUT>::min(),
...
void fill_random_bitmap(std::vector<cudf::valid_type> &valid_input, size_t size) {
  ...
  int_dis = std::uniform_int_distribution<cudf::valid_type>(
    std::numeric_limits<cudf::valid_type>::min(), std::numeric_limits<cudf::valid_type>::max());

  std::generate(
    valid_input.begin(), valid_input.end(), [int_dis, eng]() mutable { return int_dis(eng); });
}
...
struct HostUnaryOp {
  static gdf_error launch(gdf_column *input, gdf_column *output) {
    ...
    std::transform((const T *)input->data,
                   (const T *)input->data + input->size,
                   (Tout *)output->data,
                   [](T data) -> Tout { return (Tout)data; });
    return GDF_SUCCESS;
  }
};
  ...
  TEST_F(col_cast_CPU_VS_GPU_TEST, VFROM##_to_##VTO) {                                \
    {                                                                                 \
      ...
      EXPECT_TRUE(gdfError == GDF_SUCCESS);                                           \
                                                                                      \
      for (int i = 0; i < colSize; i++) { EXPECT_TRUE(results[i] == outputData[i]); } \
    }                                                                                 \
  }
  ...
  TEST_F(col_cast_swap_TEST, VFROM##_to_##VTO) {                                     \
    {                                                                                \
      ...
      auto results           = std::get<0>(originalOutputCol.to_host());             \
                                                                                     \
      for (int i = 0; i < colSize; i++) { EXPECT_TRUE(results[i] == inputData[i]); } \
    }                                                                                \
  }
...
TEST_F(gdf_date_casting_TEST, date64_to_timestamp) {
  // date64 to timestamp ms, internally the output is equal to the input
  {
    ...
    auto allValidFunctor = [](cudf::size_type row) { return true; };
    auto inputCol        = cudf::test::column_wrapper<cudf::date64>(
      colSize, [data](cudf::size_type row) { return cudf::date64{data[row]}; }, allValidFunctor);
    auto expectOut = cudf::test::column_wrapper<cudf::timestamp>(
      colSize, [data](cudf::size_type row) { return cudf::timestamp{data[row]}; }, allValidFunctor);
  ...
  // timestamp ms to date64, internally the output is equal to the input
  {
    ...
    auto allValidFunctor = [](cudf::size_type row) { return true; };
    auto inputCol        = cudf::test::column_wrapper<cudf::timestamp>(
      colSize, [data](cudf::size_type row) { return cudf::timestamp{data[row]}; }, allValidFunctor);
    auto expectOut = cudf::test::column_wrapper<cudf::date64>(
      colSize, [data](cudf::size_type row) { return cudf::date64{data[row]}; }, allValidFunctor);
...
TYPED_TEST(gdf_logical_test, LogicalNot) {
  ...
  std::transform(std::begin(h_input_v),
                 std::end(h_input_v),
...
TYPED_TEST(IsNull, sample) {
  ...
  column_wrapper<T> col(
    NUM_ELEM, [](auto row) { return row; }, [](auto row) { return (row % 7 == 0 ? true : false); });
  column_wrapper<cudf::bool8> expected(
    NUM_ELEM, [](auto row) { return cudf::bool8{(row % 7 == 0 ? false : true)}; }, false);
...
TYPED_TEST(IsNull, all_valid) {
  ...
  column_wrapper<T> col(
    NUM_ELEM, [](auto row) { return row; }, [](auto row) { return (true); });
  column_wrapper<cudf::bool8> expected(
    NUM_ELEM, [](auto row) { return cudf::bool8{false}; }, false);
...
TYPED_TEST(IsNull, all_invalid) {
  ...
  column_wrapper<T> col(
    NUM_ELEM, [](auto row) { return row; }, [](auto row) { return (false); });
  column_wrapper<cudf::bool8> expected(
    NUM_ELEM, [](auto row) { return cudf::bool8{true}; }, false);
...
TYPED_TEST(IsNotNull, sample) {
  ...
  column_wrapper<T> col(
    NUM_ELEM, [](auto row) { return row; }, [](auto row) { return (row % 7 == 0 ? true : false); });
  column_wrapper<cudf::bool8> expected(
    NUM_ELEM, [](auto row) { return cudf::bool8{(row % 7 == 0 ? true : false)}; }, false);
