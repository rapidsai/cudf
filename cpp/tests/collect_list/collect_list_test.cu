...
TYPED_TEST(TypedCollectListTest, RollingWindowWithNullLists)
{
  ...
  auto following = 1;
  auto min_periods = 3;
  auto result = rolling_window(input_column, preceding, following, min_periods, make_collect_aggregation());
  ...
  auto expected_result = lists_column_wrapper<T, int32_t>{
    ...
    make_counting_transform_iterator(0, 
                                     [num_elements](auto i) { return i != 0 && i != (num_elements - 1); })
  }.release();
  ...
  auto result_2 = rolling_window(input_column, preceding, following, min_periods, make_collect_aggregation());
  auto expected_result_2 = lists_column_wrapper<T, int32_t>{
    ...
    make_counting_transform_iterator(0, 
                                     [num_elements](auto i) { return i != 0 && i < 4; })
  }.release();
...
TYPED_TEST(TypedCollectListTest, BasicGroupedRollingWindow)
{
  ...
  auto const input_column = fixed_width_column_wrapper<T, int32_t>{10,11,12,13,14, 20,21,22,23}; 
...
TYPED_TEST(TypedCollectListTest, BasicGroupedTimeRangeRollingWindow)
{
  ...
  auto const following = 1;
  auto const min_periods = 1;
  auto const result = grouped_time_range_rolling_window(
...
TEST_F(CollectListTest, BasicGroupedTimeRangeRollingWindowOnStrings)
{
  ...
  auto const time_column  = fixed_width_column_wrapper<cudf::timestamp_D, cudf::timestamp_D::rep>
                                                               {    1, 1, 2, 2, 3,  1, 4, 5, 6};
  auto const group_column = fixed_width_column_wrapper<int32_t>{    1, 1, 1, 1, 1,  2, 2, 2, 2};
  auto const input_column = strings_column_wrapper{"10","11","12","13","14", "20","21","22","23"}; 
  auto const preceding = 2;
  auto const following = 1;
  auto const min_periods = 1;
  auto const result = grouped_time_range_rolling_window(
...
TYPED_TEST(TypedCollectListTest, BasicGroupedTimeRangeRollingWindowOnStructs)
{
  ...
  auto const time_column  = fixed_width_column_wrapper<cudf::timestamp_D, cudf::timestamp_D::rep>
                                                               {    1, 1, 2, 2, 3,  1, 4, 5, 6};
  auto const group_column = fixed_width_column_wrapper<int32_t>{    1, 1, 1, 1, 1,  2, 2, 2, 2};
  auto numeric_member_column = fixed_width_column_wrapper<T, int32_t>{10,11,12,13,14, 20,21,22,23}; 
  auto string_member_column = strings_column_wrapper{"10","11","12","13","14", "20","21","22","23"};
  ...
  auto const struct_column = make_structs_column(9, 
  ...
  auto expected_offsets_column = fixed_width_column_wrapper<size_type>{0, 4, 8, 13, 18, 23, 24, 26, 29, 32}.release();
  auto expected_result = make_lists_column(9, std::move(expected_offsets_column), std::move(expected_structs_column), 0, {});
...
TYPED_TEST(TypedCollectListTest, GroupedTimeRangeRollingWindowOnStructsWithMinPeriods)
{
  ...
  auto expected_offsets_column = fixed_width_column_wrapper<size_type>{0, 4, 8, 13, 18, 23, 23, 23, 23, 23}.release();
  auto expected_validity_iter  = make_counting_transform_iterator(0, [](auto i) {return i < 5; });
  auto expected_null_mask = cudf::test::detail::make_null_mask(expected_validity_iter, expected_validity_iter + 9);
