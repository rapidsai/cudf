    ...
    bool                                  print_all_unequal_pairs = false)
{
    ...
    ASSERT_CUDA_SUCCEEDED(cudf::validate(lhs_gdf_column));
    ASSERT_CUDA_SUCCEEDED(cudf::validate(rhs_gdf_column));
    EXPECT_TRUE(cudf::are_strictly_type_compatible(lhs_gdf_column, rhs_gdf_column));
    if (not cudf::are_strictly_type_compatible(lhs_gdf_column, rhs_gdf_column)) { return; }
