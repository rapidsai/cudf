...
using host_valid_pointer =
  typename std::unique_ptr<cudf::valid_type, std::function<void(cudf::valid_type*)>>;
...
void print_vector_and_valid(std::vector<T>& v, const cudf::valid_type* valid)
{
  ...
  std::transform(
    indexes.begin(), indexes.end(), std::ostream_iterator<std::string>(std::cout, ", "), functor);
  std::cout << std::endl;
}
...
template <std::size_t I = 0, typename... Tp>
inline typename std::enable_if<I == (sizeof...(Tp)), bool>::type rows_equal_using_valids(
...
template <std::size_t I = 0, typename... Tp>
  inline typename std::enable_if <
  I<sizeof...(Tp), bool>::type rows_equal_using_valids(const std::tuple<std::vector<Tp>...>& left,
                                                       const std::tuple<std::vector<Tp>...>& right,
                                                       std::vector<host_valid_pointer>& left_valid,
                                                       std::vector<host_valid_pointer>& right_valid,
                                                       const size_t left_index,
                                                       const size_t right_index)
{
  ...
  if (gdf_is_valid(l_valid, left_index) && gdf_is_valid(r_valid, right_index) &&
      std::get<I>(left)[left_index] == std::get<I>(right)[right_index]) {
    return rows_equal_using_valids<I + 1, Tp...>(
      left, right, left_valid, right_valid, left_index, right_index);
  } else {
    return false;
  }
}
