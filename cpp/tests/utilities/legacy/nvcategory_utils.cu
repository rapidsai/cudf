...
gdf_column *create_nv_category_column(cudf::size_type num_rows, bool repeat_strings)
{
  ...
  gdf_error err =
    gdf_column_view(column, (void *)data, (cudf::valid_type *)valid, num_rows, GDF_STRING_CATEGORY);
  ...
  gdf_column *column)
{
  if (column->dtype == GDF_STRING_CATEGORY && column->dtype_info.category != nullptr &&
      column->size > 0) {
    NVStrings *tptr =
      static_cast<NVCategory *>(column->dtype_info.category)
        ->gather_strings(
          static_cast<nv_category_index_type *>(column->data), column->size, DEVICE_ALLOCATED);
