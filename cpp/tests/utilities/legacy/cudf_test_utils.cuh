 ...
 * @note See the @ref gdf_column variant
 */
void print_gdf_column(gdf_column const* column,
                      unsigned min_element_print_width = 1,
...
gdf_col_pointer create_gdf_column(
  ...
  CUDF_EXPECTS(gdf_col_type != GDF_invalid,
               "Cannot create columns with the GDF_invalid element type");
  ...
  CUDA_TRY(cudaMemcpy(the_column->data,
                      host_vector.data(),
                      host_vector.size() * sizeof(ColumnType),
  ...
  if (valid_vector.size() > 0) {
    ...
    CUDA_TRY(cudaMemcpy(the_column->valid,
                        valid_vector.data(),
                        valid_vector.size() * sizeof(cudf::valid_type),
                        cudaMemcpyHostToDevice));
    the_column->null_count =
      (host_vector.size() - count_valid_bits_host(valid_vector, host_vector.size()));
  } else {
