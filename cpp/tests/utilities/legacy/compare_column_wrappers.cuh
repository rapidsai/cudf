                                    ...
                                    bool print_all_unequal_pairs       = dont_print_unequal_pairs)
{
  ...
  for (cudf::size_type i = 0; i < common_size; i++) {
    auto lhs_element_is_valid =
      lhs_non_nullable or
      cudf::util::bit_is_set<cudf::valid_type, cudf::size_type>(lhs_validity_on_host, i);
    auto rhs_element_is_valid =
      rhs_non_nullable or
      cudf::util::bit_is_set<cudf::valid_type, cudf::size_type>(rhs_validity_on_host, i);
    if (not lhs_element_is_valid and not rhs_element_is_valid) {
      EXPECT_TRUE(treat_distinct_nulls_as_equal);
    } else {
      auto elements_are_equal = (lhs_element_is_valid == rhs_element_is_valid and
                                 almost_equal(lhs_data_on_host[i], rhs_data_on_host[i], 2));
      EXPECT_TRUE(elements_are_equal)
        << std::left << std::setw(max_name_length) << lhs_name << std::right << '[' << i
        << "] = " << (lhs_element_is_valid ? detail::to_string(lhs_data_on_host[i]) : "@") << '\n'
        << std::left << std::setw(max_name_length) << rhs_name << std::right << '[' << i
                                          ...
                                          bool ignore_extra_type_info = false) noexcept
{
  ...
  if (cudf::is_nullable(validated_column_1) != cudf::is_nullable(validated_column_2)) {
    return false;
  }
  if (ignore_extra_type_info) { return true; }
  auto common_dtype = validated_column_1.dtype;
  EXPECT_TRUE(cudf::detail::extra_type_info_is_compatible(
    common_dtype, validated_column_1.dtype_info, validated_column_2.dtype_info));
  return cudf::detail::extra_type_info_is_compatible(
    common_dtype, validated_column_1.dtype_info, validated_column_2.dtype_info);
}

template <typename E>
void expect_columns_are_equal(cudf::test::column_wrapper<E> const& lhs,
                              const std::string& lhs_name,
                              ...
                              bool print_all_unequal_pairs       = dont_print_unequal_pairs)
{
  ...
  return expect_column_values_are_equal(common_size,
                                        lhs_data_on_host,
                                        lhs_validity_on_host,
                                        lhs_name,
                                        rhs_data_on_host,
                                        rhs_validity_on_host,
                                        rhs_name,
