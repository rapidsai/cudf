...
TEST_F(SortSpark, TestExamples)
{
  ...
  using cudf::test::iterators::nulls_at;
    lcw col1{
      lcw{lcw{{0, 0}, nulls_at({0, 1})}},      // 0
      lcw{lcw{{0}, nulls_at({0})}, lcw{-21827}},  // 1
      lcw{lcw{{0}, nulls_at({0})}, lcw{}},
      lcw{lcw{{0}, nulls_at({0})}, lcw{{0}, nulls_at({0})}}  // 2
    };

  cudf::test::print(col1);
  cudf::test::fixed_width_column_wrapper<int32_t> expected1{{2, 1, 3, 0}};
  auto result1 = cudf::sorted_order(cudf::table_view({col1}), {}, {cudf::null_order::AFTER});
  ...
  // auto const col3 = lcw{{lcw{1}, lcw{0}}, nulls_at({1})};
  // auto result3 = cudf::sorted_order(cudf::table_view({col3}), {}, {cudf::null_order::AFTER});
  // cudf::test::print(*result3);
}
...
TYPED_TEST(Sort, WithListColumn)
{
  ...
  cudf::test::print(col);
  auto expect = cudf::test::fixed_width_column_wrapper<cudf::size_type>{1, 0};
