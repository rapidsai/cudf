...
gdf_column *create_column_ints(int32_t *host_data, cudf::size_type num_rows) {
  ...
  gdf_error err =
    gdf_column_view(column, (void *)data, (cudf::valid_type *)valid, num_rows, GDF_INT32);
  return column;
}
...
int32_t *generate_int_data(cudf::size_type num_rows, size_t max_value, bool print = false) {
  ...
  for (cudf::size_type row_index = 0; row_index < num_rows; row_index++) {
    ...
    if (print) std::cout << host_data[row_index] << "\t";
  }
...
struct NVCategoryTest : public GdfTest {
  gdf_column *create_boolean_column(cudf::size_type num_rows) {
    ...
    gdf_error err =
      gdf_column_view(column, (void *)data, (cudf::valid_type *)valid, num_rows, GDF_INT8);
    return column;
  }
...
TEST_F(NVCategoryTest, TEST_NVCATEGORY_SORTING) {
  ...
  if (print) { print_gdf_column(input_columns[0]); }
  ...
  if (print) { print_gdf_column(output_column); }

  int *host_data = new int[rows_size];
  CUDA_TRY(cudaMemcpy(
    host_data, output_column->data, sizeof(int) * output_column->size, cudaMemcpyDeviceToHost));
...
TEST_F(NVCategoryTest, TEST_NVCATEGORY_COMPARISON) {
  ...
  gdf_column *left_column =
    cudf::test::create_nv_category_column_strings(left_string_data, rows_size);
  gdf_column *right_column =
    cudf::test::create_nv_category_column_strings(right_string_data, rows_size);

  gdf_column *output_column = create_boolean_column(rows_size);

  NVStrings *temp_string =
    static_cast<NVCategory *>(right_column->dtype_info.category)->to_strings();
  NVCategory *new_category =
    static_cast<NVCategory *>(left_column->dtype_info.category)->add_strings(*temp_string);
  ...
  CUDA_TRY(cudaMemcpy(left_column->data,
                      indices,
                      sizeof(unsigned int) * left_column->size,
                      cudaMemcpyDeviceToDevice));
  CUDA_TRY(cudaMemcpy(right_column->data,
                      indices + left_column->size,
                      sizeof(unsigned int) * right_column->size,
                      cudaMemcpyDeviceToDevice));
  ...
  CUDF_EXPECT_NO_THROW(cudf::binary_operation(
    output_column, left_column, right_column, gdf_binary_operator::GDF_EQUAL));

  int8_t *data = new int8_t[rows_size];
  CUDA_TRY(
    cudaMemcpy(data, output_column->data, sizeof(int8_t) * rows_size, cudaMemcpyDeviceToHost));
...
struct NVCategoryConcatTest : public GdfTest {
  ...
  std::vector<std::string> compute_gdf_result(bool print = false) {
    ...
    if (print) { print_gdf_column(concat_out); }

    NVStrings *temp_strings =
      static_cast<NVCategory *>(concat_out->dtype_info.category)
        ->gather_strings((nv_category_index_type *)concat_out->data, keys_size, DEVICE_ALLOCATED);

    char **host_strings = new char *[keys_size];
    for (size_t i = 0; i < keys_size; i++) { host_strings[i] = new char[length + 1]; }

    temp_strings->to_host(host_strings, 0, keys_size);

    for (size_t i = 0; i < keys_size; i++) { host_strings[i][length] = 0; }

    std::vector<std::string> strings_vector(host_strings, host_strings + keys_size);

    NVStrings::destroy(temp_strings);

    for (size_t i = 0; i < keys_size; i++) { delete host_strings[i]; }
    delete host_strings;
...
TEST_F(NVCategoryConcatTest, concat_test) {
  ...
  ASSERT_EQ(reference_result.size(), gdf_result.size())
...
// Selects the kind of join operation that is performed
enum struct join_op { INNER, LEFT, FULL };
...
struct NVCategoryJoinTest : public GdfTest {
  ...
  std::vector<result_type> compute_gdf_result(join_op op,
    ...
    size_t result_num_cols =
      gdf_raw_left_columns.size() + gdf_raw_right_columns.size() - left_join_idx.size();
    ...
    switch (op) {
      case join_op::LEFT: {
        result = cudf::left_join(left_gdf_columns,
                                 right_gdf_columns,
                                 left_join_idx,
                                 right_join_idx,
                                 columns_in_common,
      ...
      case join_op::INNER: {
        result = cudf::inner_join(left_gdf_columns,
                                  right_gdf_columns,
                                  left_join_idx,
                                  right_join_idx,
                                  columns_in_common,
      ...
      case join_op::FULL: {
        result = cudf::full_join(left_gdf_columns,
                                 right_gdf_columns,
                                 left_join_idx,
                                 right_join_idx,
                                 columns_in_common,
      ...
      default: std::cout << "Invalid join method" << std::endl; EXPECT_TRUE(false);
    }

    EXPECT_EQ(left_result.size, right_result.size) << "Join output size mismatch";

    // Copy result of gdf join to local result
    EXPECT_EQ(RMM_ALLOC(&result_col->data,
                        result.get_column(0)->size * cudf::size_of(result.get_column(0)->dtype),
                        0),
              RMM_SUCCESS);
    EXPECT_EQ(
      RMM_ALLOC(&result_col->valid,
                gdf_valid_allocation_size(result.get_column(0)->size) * sizeof(cudf::valid_type),
                0),
      RMM_SUCCESS);
    ...
    EXPECT_EQ(cudaMemcpy(result_col->data,
                         result.get_column(0)->data,
                         result.get_column(0)->size * cudf::size_of(result.get_column(0)->dtype),
                         cudaMemcpyDeviceToDevice),
              cudaSuccess);
    EXPECT_EQ(
      cudaMemcpy(result_col->valid,
                 result.get_column(0)->valid,
                 gdf_valid_allocation_size(result.get_column(0)->size) * sizeof(cudf::valid_type),
                 cudaMemcpyDeviceToDevice),
      cudaSuccess);
    ...
    EXPECT_EQ(
      cudaMemcpy(
        host_result.data(), l_join_output, total_pairs * sizeof(int), cudaMemcpyDeviceToHost),
      cudaSuccess);
    ...
    if (sort) { std::sort(host_pair_result.begin(), host_pair_result.end()); }

    if (print) {
      std::cout << "\nGDF result size: " << host_pair_result.size() << std::endl;
      std::cout << "left index\tright index" << std::endl;
      std::copy(host_pair_result.begin(),
                host_pair_result.end(),
                std::ostream_iterator<result_type>(std::cout, ""));
      std::cout << "\n";
    }
  ...
  void check_output(join_op op,
    ...
    if (result_size > 0) {
      NVStrings *temp_strings =
        static_cast<NVCategory *>(result_column->dtype_info.category)
          ->gather_strings(
            (nv_category_index_type *)result_column->data, result_size, DEVICE_ALLOCATED);

      char **host_strings = new char *[result_size];
      for (size_t i = 0; i < result_size; i++) { host_strings[i] = new char[length + 1]; }

      temp_strings->to_host(host_strings, 0, result_size);

      for (size_t i = 0; i < result_size; i++) { host_strings[i][length] = 0; }

      std::vector<std::string> result_output =
        std::vector<std::string>(host_strings, host_strings + result_size);
      ...
      if (print) {
        for (auto str : result_output) { std::cout << str << "\t"; }
        std::cout << std::endl;
      }

      NVStrings::destroy(temp_strings);

      for (size_t i = 0; i < result_size; i++) { delete host_strings[i]; }
      delete host_strings;

      for (size_t i = 0; i < result_size; i++) { EXPECT_EQ(reference_output[i], result_output[i]); }
    }
  }
};
...
TEST_F(NVCategoryJoinTest, join_test) {
  ...
  std::vector<result_type> gdf_result =
    this->compute_gdf_result(op, left_join_idx, right_join_idx, print);
...
TEST_F(NVCategoryJoinTest, join_test_bug) {
  ...
  gdf_column *right_column =
    cudf::test::create_nv_category_column_strings(column_right_b, right_size);
  right_column->valid               = nullptr;
  ...
  gdf_column *result_column_joined =
    cudf::test::create_nv_category_column_strings(column_left_b, left_size);
