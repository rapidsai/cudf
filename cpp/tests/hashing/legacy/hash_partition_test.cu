...
struct HashPartitionTest : public GdfTest {
  ...
  void create_input(size_t num_rows, size_t max_value, bool print = false)
  {
    ...
    for (auto const& c : gdf_input_columns) { this->raw_gdf_input_columns.push_back(c.get()); }
    for (auto const& c : gdf_output_columns) { this->raw_gdf_output_columns.push_back(c.get()); }

    if (print) {
      std::cout << "Input column(s) created. Size: " << std::get<0>(input_columns).size()
                << std::endl;
  ...
  std::vector<int> compute_gdf_result(const int num_partitions, bool print = false)
  {
    ...
    if (print) {
      ...
      for (int i = 0; i < num_partitions; ++i) { std::cout << partition_offsets[i] << " "; }
      std::cout << std::endl;
    }
  ...
  void verify_gdf_result(int num_partitions, std::vector<int> partition_offsets, bool print = false)
  {
    ...
    switch (gdf_hash_function) {
      ...
      default: std::cerr << "Invalid GDF hash function.\n";
    }
    ...
    for (int partition_number = 0; partition_number < num_partitions; ++partition_number) {
      ...
      for (int i = partition_start; i < partition_stop; ++i) {
        EXPECT_EQ(partition_number, host_row_partition_numbers[i])
          << "Partition number for row: " << i << " doesn't match!";
      }
    }
  }
};

template <typename tuple_of_vectors, gdf_hash_func hash, int... cols>
struct TestParameters {
  static_assert(
    (std::tuple_size<tuple_of_vectors>::value >= sizeof...(cols)),
...
template <typename tuple_of_vectors, gdf_hash_func hash, int... cols>
constexpr const std::array<int, sizeof...(cols)>
  TestParameters<tuple_of_vectors, hash, cols...>::cols_to_hash;
...
typedef ::testing::Types<
  TestParameters<VTuple<int32_t>, GDF_HASH_IDENTITY, 0>,
