...
get_nanoarrow_host_tables(cudf::size_type length, rmm::device_async_resource_ref mr)
{
  ...
  auto keys       = cudf::test::to_host<int64_t>(view.keys(), temporary_mr).first;
  auto indices    = cudf::test::to_host<uint32_t>(view.indices(), temporary_mr).first;
