...
struct JoinTest : public GdfTest {
  ...
  gdf_context ctxt = {
    test_parameters::join_type == gdf_method::GDF_SORT, test_parameters::join_type, 0};
   ...
   * -------------------------------------------------------------------------*/
  void create_input(size_t left_column_length,
                    size_t left_column_range,
    ...
    initialize_tuple(right_columns,
                     right_column_length,
                     right_column_range,
                     static_cast<size_t>(ctxt.flag_sorted));
    ...
    for (auto const& c : gdf_left_columns) { gdf_raw_left_columns.push_back(c.get()); }

    for (auto const& c : gdf_right_columns) { gdf_raw_right_columns.push_back(c.get()); }

    if (print) {
      std::cout << "Left column(s) created. Size: " << std::get<0>(left_columns).size()
                << std::endl;
      print_tuples_and_valids(left_columns, left_valids);

      std::cout << "Right column(s) created. Size: " << std::get<0>(right_columns).size()
                << std::endl;
  ...
  std::vector<result_type> compute_gdf_result(bool print                = false,
    ...
    switch (op) {
      case join_op::LEFT: {
        cudf::left_join(left_gdf_columns,
                        right_gdf_columns,
                        range,
                        range,
                        columns_in_common,
      ...
      case join_op::INNER: {
        cudf::inner_join(left_gdf_columns,
                         right_gdf_columns,
                         range,
                         range,
                         columns_in_common,
      ...
      case join_op::FULL: {
        cudf::full_join(left_gdf_columns,
                        right_gdf_columns,
                        range,
                        range,
                        columns_in_common,
...
TYPED_TEST(MaxJoinTest, InputTooLarge) {
  ...
  CUDF_EXPECT_THROW_MESSAGE(this->compute_gdf_result(print_result, sort_result, expected_error),
                            "right column size is too big");
}
