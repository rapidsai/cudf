...
TEST_F(AstVsJitFilterJoinTest, SimpleGreaterThanComparison)
{
  ...
  for (auto join_kind : join_kinds) {
    ...
    // Compare results
    EXPECT_EQ(ast_left->size(), jit_left->size())
      << "Size mismatch for join_kind: " << static_cast<int>(join_kind);
    EXPECT_EQ(ast_right->size(), jit_right->size())
      << "Size mismatch for join_kind: " << static_cast<int>(join_kind);
  }
}
