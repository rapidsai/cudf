...
class SortMergeJoinThreadSafetyTest : public cudf::test::BaseFixture {
 protected:
  // Create a simple test table with sequential values
  static std::unique_ptr<cudf::table> create_sequential_table(cudf::size_type num_rows,
                                                              cudf::size_type start_value = 0)
  {
    std::vector<cudf::size_type> values(num_rows);
    std::iota(values.begin(), values.end(), start_value);

    auto col = column_wrapper<cudf::size_type>(values.begin(), values.end());
    std::vector<std::unique_ptr<cudf::column>> cols;
    cols.push_back(col.release());
    return std::make_unique<cudf::table>(std::move(cols));
  }

  // Create a test table with values that will produce predictable join matches
  static std::unique_ptr<cudf::table> create_test_table(cudf::size_type num_rows,
                                                        cudf::size_type multiplier,
                                                        cudf::size_type offset)
  {
    std::vector<cudf::size_type> values(num_rows);
    for (cudf::size_type i = 0; i < num_rows; ++i) {
      values[i] = (i * multiplier + offset) % 100;  // Values in range [0, 99]
    }
...
// Test that concurrent inner_join calls produce correct results
TEST_F(SortMergeJoinThreadSafetyTest, ConcurrentInnerJoins)
{
  constexpr int num_threads = 4;

  // Create right table with values [0, 1, 2, ..., 49]
  auto right_table = create_sequential_table(50);

  // Create the sort_merge_join object (shared across threads)
  cudf::sort_merge_join join_obj(
    right_table->view(), cudf::sorted::NO, cudf::null_equality::EQUAL, cudf::get_default_stream());

  // Create different left tables for each thread
  std::vector<std::unique_ptr<cudf::table>> left_tables;
  for (int i = 0; i < num_threads; ++i) {
    left_tables.push_back(create_test_table(30, i + 1, i * 10));
  }

  // Launch concurrent joins using std::async
  std::vector<std::future<std::pair<std::unique_ptr<rmm::device_uvector<cudf::size_type>>,
                                    std::unique_ptr<rmm::device_uvector<cudf::size_type>>>>>
    futures;

  for (int i = 0; i < num_threads; ++i) {
    futures.push_back(std::async(std::launch::async, [&join_obj, &left_tables, i]() {
      rmm::cuda_stream stream;
      return join_obj.inner_join(left_tables[i]->view(), cudf::sorted::NO, stream);
    }));
  }

  // Collect results and verify each one
  for (int i = 0; i < num_threads; ++i) {
    auto [left_indices, right_indices] = futures[i].get();

    // Verify that indices are valid
    auto host_left = cudf::detail::make_host_vector_sync(*left_indices, cudf::get_default_stream());
    auto host_right =
      cudf::detail::make_host_vector_sync(*right_indices, cudf::get_default_stream());

    EXPECT_EQ(host_left.size(), host_right.size());
    for (size_t j = 0; j < host_left.size(); ++j) {
      EXPECT_GE(host_left[j], 0);
      EXPECT_LT(host_left[j], left_tables[i]->num_rows());
      EXPECT_GE(host_right[j], 0);
      EXPECT_LT(host_right[j], right_table->num_rows());
    }
  }
}

// Test that concurrent left_join calls produce correct results
TEST_F(SortMergeJoinThreadSafetyTest, ConcurrentLeftJoins)
{
  constexpr int num_threads = 4;
  ...
  for (int i = 0; i < num_threads; ++i) {
    futures.push_back(std::async(std::launch::async, [&join_obj, &left_tables, i]() {
      rmm::cuda_stream stream;
      return join_obj.left_join(left_tables[i]->view(), cudf::sorted::NO, stream);
    }));
  }
  ...
  for (int i = 0; i < num_threads; ++i) {
    ...
    // For left join, output size should be at least as large as left table
    EXPECT_GE(left_indices->size(), static_cast<size_t>(left_tables[i]->num_rows()));
    EXPECT_EQ(left_indices->size(), right_indices->size());
...
// Stress test with many concurrent join operations
TEST_F(SortMergeJoinThreadSafetyTest, StressTestConcurrentJoins)
{
  constexpr int num_threads    = 8;
  constexpr int num_iterations = 10;

  auto right_table = create_sequential_table(100);
  ...
  for (int t = 0; t < num_threads; ++t) {
    threads.emplace_back([&, t]() {
      ...
      for (int iter = 0; iter < num_iterations; ++iter) {
        try {
          auto left_table = create_test_table(50, t + 1, iter * 5);

          if ((t + iter) % 2 == 0) {
            auto [left_idx, right_idx] =
              join_obj.inner_join(left_table->view(), cudf::sorted::NO, stream);
            stream.synchronize();
            if (left_idx->size() == right_idx->size()) { success_count++; }
          } else {
            auto [left_idx, right_idx] =
              join_obj.left_join(left_table->view(), cudf::sorted::NO, stream);
            stream.synchronize();
            if (left_idx->size() >= static_cast<size_t>(left_table->num_rows()) &&
