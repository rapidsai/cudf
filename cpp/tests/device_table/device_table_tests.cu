...
TEST_F(DeviceTableTest, AllRowsDifferentWithNulls)
{
  ...
  thrust::device_vector<cudf::size_type> indices(table->num_rows());
  thrust::sequence(indices.begin(), indices.end());
  EXPECT_TRUE(
    thrust::all_of(rmm::exec_policy()->on(0),
                   thrust::make_zip_iterator(thrust::make_tuple(indices.begin(), indices.begin())),
  ...
  EXPECT_FALSE(thrust::equal(row_hashes.begin() + 1, row_hashes.end(), row_hashes.begin()));

  // Every row should be different from every other row other than itself
  for (cudf::size_type i = 0; i < table->num_rows(); ++i) {
    thrust::device_vector<cudf::size_type> left_indices(table->num_rows(), i);
    thrust::device_vector<cudf::size_type> right_indices(table->num_rows());
    ...
    EXPECT_FALSE(thrust::all_of(
      ...
      thrust::make_zip_iterator(thrust::make_tuple(left_indices.end(), right_indices.end())),
      row_comparison<true>{*table, *table, true}));
  }
}
...
TEST_F(DeviceTableTest, TwoTablesAllRowsEqual)
{
  ...
  std::vector<gdf_column*> left_cols{left_col0, left_col1, left_col2, left_col3};
  ...
  std::vector<gdf_column*> right_cols{right_col0, right_col1, right_col2, right_col3};
  ...
  thrust::device_vector<cudf::size_type> indices(left_table->num_rows());
  thrust::sequence(indices.begin(), indices.end());
  EXPECT_TRUE(
    thrust::all_of(rmm::exec_policy()->on(0),
                   thrust::make_zip_iterator(thrust::make_tuple(indices.begin(), indices.begin())),
...
TEST_F(DeviceTableTest, CopyRowsNoNulls)
{
  ...
  std::vector<gdf_column*> source_cols{source_col0, source_col1, source_col2, source_col3};
  auto source_table = device_table::create(source_cols.size(), source_cols.data());
  ...
  std::vector<gdf_column*> target_cols{target_col0, target_col1, target_col2, target_col3};
  auto target_table = device_table::create(target_cols.size(), target_cols.data());
  ...
  // host
  std::vector<cudf::size_type> indices(source_table->num_rows());
  std::iota(indices.begin(), indices.end(), 0);
  std::shuffle(indices.begin(), indices.end(), std::default_random_engine{});
  thrust::device_vector<cudf::size_type> target_indices(indices);

  std::shuffle(indices.begin(), indices.end(), std::default_random_engine{});
  thrust::device_vector<cudf::size_type> source_indices(indices);

  // Copy source_table row @ source_indices[i] to target_table @
  // target_indices[i]
  EXPECT_NO_THROW(thrust::for_each(
    rmm::exec_policy()->on(0),
    thrust::make_zip_iterator(thrust::make_tuple(target_indices.begin(), source_indices.begin())),
    thrust::make_zip_iterator(thrust::make_tuple(target_indices.end(), source_indices.end())),
  ...
  EXPECT_TRUE(thrust::all_of(
    rmm::exec_policy()->on(0),
    thrust::make_zip_iterator(thrust::make_tuple(source_indices.begin(), target_indices.begin())),
    thrust::make_zip_iterator(thrust::make_tuple(source_indices.end(), target_indices.end())),
...
TEST_F(DeviceTableTest, CopyRowsSourceValidTargetNull)
{
  ...
  cudf::table target_host_table{target_cols.data(),
                                static_cast<cudf::size_type>(target_cols.size())};
