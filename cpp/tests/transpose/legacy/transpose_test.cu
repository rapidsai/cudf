...
class TransposeTest : public GdfTest {
 protected:
  ...
  void compute_gdf_result(void) {
    ...
    std::transform(
      in_gdf_columns.begin(), in_gdf_columns.end(), in_gdf_column_ptr.begin(), to_raw_ptr);

    std::transform(
      out_gdf_columns.begin(), out_gdf_columns.end(), out_gdf_column_ptr.begin(), to_raw_ptr);
  ...
  void create_reference_output(void) {
    ...
    for (auto& c : ref_data) { c.resize(_ncols); }

    // copy transposed from in_columns to ref_data
    for (size_t i = 0; i < _nrows; i++)
      for (size_t j = 0; j < _ncols; j++) (ref_data[i])[j] = (in_columns[j])[i];
