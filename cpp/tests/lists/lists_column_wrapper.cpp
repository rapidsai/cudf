 ...
 * 
 * This function handles a special case.  For convenience of declaration, we want to treat these two cases 
 * as equivalent   
 *   
 * List<int>      = { 0, 1 }   
 * List<int>      = { {0, 1} }  
 ...
 *
 */
void lists_column_wrapper::build_from_nested(std::initializer_list<lists_column_wrapper> elements, std::vector<bool> const& v)
{
  ...
  std::transform(elements.begin(), elements.end(), std::back_inserter(cols), [&](lists_column_wrapper const& l){
    // potentially unwrap
