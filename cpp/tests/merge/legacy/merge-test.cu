...
TYPED_TEST(MergeTest, MismatchedColumnDypes)
{
  ...
  cudf::test::column_wrapper<double> rightColWrap1(inputRows,
                                                   [](cudf::size_type row) { return row; });
  ...
  EXPECT_THROW(
    cudf::merge(
      cudf::table{leftColWrap1.get()}, cudf::table{rightColWrap1.get()}, sortByCols, orderByTypes),
    cudf::logic_error);
}
...
TYPED_TEST(MergeTest, MergeWithEmptyColumn)
{
  ...
  inputRows =
    (cudf::detail::unwrap(std::numeric_limits<TypeParam>::max()) < inputRows ? 40 : inputRows);
...
TYPED_TEST(MergeTest, Merge1KeyColumns)
{
  ...
  auto leftColWrap1 = columnFactory.make(inputRows, [](cudf::size_type row) -> cudf::size_type {
    if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
      return 1;
    else
      return 2 * row;
  });
  auto leftColWrap2 = columnFactory.make(inputRows, [](cudf::size_type row) -> cudf::size_type {
    if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
      return 0;
    else
      return row;
  ...
  auto rightColWrap1 = columnFactory.make(inputRows, [](cudf::size_type row) -> cudf::size_type {
    if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
      return 0;
    else
      return 2 * row + 1;
  });
  auto rightColWrap2 = columnFactory.make(inputRows, [](cudf::size_type row) -> cudf::size_type {
    if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
      return 0;
    else
      return row;
  ...
  const cudf::size_type outputRows = leftColWrap1.size() + rightColWrap1.size();
  auto expectedDataWrap1 =
    columnFactory.make(outputRows, [=](cudf::size_type row) -> cudf::size_type {
      if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
        return row >= outputRows / 2;
      else
        return row;
    });
  auto expectedDataWrap2 =
    columnFactory.make(outputRows, [](cudf::size_type row) -> cudf::size_type {
      if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
        return 0;
      else
...
TYPED_TEST(MergeTest, Merge2KeyColumns)
{
  ...
  auto leftColWrap1 = columnFactory.make(inputRows, [=](cudf::size_type row) -> cudf::size_type {
    if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
      return row >= inputRows / 2;
    else
      return row;
  });
  auto leftColWrap2 = columnFactory.make(inputRows, [=](cudf::size_type row) -> cudf::size_type {
    if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
      return (row / (inputRows / 4)) % 2 == 0;
    else
      return 2 * row;
  });
...
TYPED_TEST(MergeTest, Merge1KeyNullColumns)
{
  ...
  auto leftColWrap1 = columnFactory.make(
    inputRows,
  ...
  auto rightColWrap1 = columnFactory.make(
    inputRows,
    [](cudf::size_type row) -> cudf::size_type {
      if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
        return 1;
  ...
  auto expectedDataWrap1                  = columnFactory.make(
    outputRows,
    [=](cudf::size_type row) -> cudf::size_type {
      if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
        return row >= (outputRows - column1TotalNulls) / 2;
      else
        return row;
    },
...
TYPED_TEST(MergeTest, Merge2KeyNullColumns)
{
  ...
  auto leftColWrap2 = columnFactory.make(
    inputRows,
    [=](cudf::size_type row) -> cudf::size_type {
      if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
        return (row / (inputRows / 4)) % 2 == 0;
  ...
  auto rightColWrap2 = columnFactory.make(
    inputRows,
    [=](cudf::size_type row) -> cudf::size_type {
      if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
        return (row / (inputRows / 4)) % 2 == 0;
  ...
  auto expectedDataWrap2 = columnFactory.make(
    outputRows,
    [=](cudf::size_type row) -> cudf::size_type {
      if (cudf::gdf_dtype_of<TypeParam>() == GDF_BOOL8)
        return (row / (outputRows / 8)) % 2 == 0;
      else
