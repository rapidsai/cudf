...
class RollingTest : public GdfTest {
 protected:
                    ...
                    gdf_agg_op agg)
  {
    ...
    CUDF_EXPECTS(min_periods.size() == 0 || min_periods.size() == (size_t)input.size(),
                 "Min periods array size != input column size");
    CUDF_EXPECTS(forward_window.size() == 0 || forward_window.size() == (size_t)input.size(),
                 "Forward window array size != input column size");
    ...
    cudf::size_type *d_window         = NULL;
    cudf::size_type *d_min_periods    = NULL;
    cudf::size_type *d_forward_window = NULL;

    // copy sizes to the gpu
    if (window.size() > 0) {
      EXPECT_EQ(RMM_ALLOC(&d_window, window.size() * sizeof(cudf::size_type), 0), RMM_SUCCESS);
      CUDA_TRY(cudaMemcpy(
        d_window, window.data(), window.size() * sizeof(cudf::size_type), cudaMemcpyDefault));
    }
    if (min_periods.size() > 0) {
      EXPECT_EQ(RMM_ALLOC(&d_min_periods, min_periods.size() * sizeof(cudf::size_type), 0),
                RMM_SUCCESS);
      CUDA_TRY(cudaMemcpy(d_min_periods,
                          min_periods.data(),
                          min_periods.size() * sizeof(cudf::size_type),
                          cudaMemcpyDefault));
    }
    if (forward_window.size() > 0) {
      EXPECT_EQ(RMM_ALLOC(&d_forward_window, forward_window.size() * sizeof(cudf::size_type), 0),
                RMM_SUCCESS);
      CUDA_TRY(cudaMemcpy(d_forward_window,
                          forward_window.data(),
                          forward_window.size() * sizeof(cudf::size_type),
                          cudaMemcpyDefault));
    }

    out_gdf_col = {
      cudf::rolling_window(*input.get(), w, m, f, agg, d_window, d_min_periods, d_forward_window),
      deleter};
 ...
 private:
  ...
  void create_reference_output(cudf::size_type window,
                               ...
                               const std::vector<cudf::size_type> &forward_window_col)
  {
    ...
    for (cudf::size_type i = 0; i < nrows; i++) {
      ...
      // compute bounds
      cudf::size_type start_index = std::max((cudf::size_type)0, i - window + 1);
      cudf::size_type end_index   = std::min(nrows, i + forward_window + 1);  // exclusive
      // aggregate
                               ...
                               const std::vector<cudf::size_type> &forward_window_col)
  {
    ...
    switch (agg) {
      case GDF_SUM:
        create_reference_output<cudf::DeviceSum, false>(
          window, min_periods, forward_window, window_col, min_periods_col, forward_window_col);
        break;
      case GDF_MIN:
        create_reference_output<cudf::DeviceMin, false>(
          window, min_periods, forward_window, window_col, min_periods_col, forward_window_col);
        break;
      case GDF_MAX:
        create_reference_output<cudf::DeviceMax, false>(
          window, min_periods, forward_window, window_col, min_periods_col, forward_window_col);
        break;
      case GDF_COUNT:
        create_reference_output<cudf::DeviceCount, false>(
          window, min_periods, forward_window, window_col, min_periods_col, forward_window_col);
        break;
      case GDF_AVG:
        create_reference_output<cudf::DeviceSum, true>(
          window, min_periods, forward_window, window_col, min_periods_col, forward_window_col);
        break;
      default: FAIL() << "aggregation type not supported";
    }
  }
  ...
  void compare_gdf_result()
  {
    ...
    CUDA_TRY(cudaMemcpy(
      out_col.data(), static_cast<T *>(out_gdf_col->data), nrows * sizeof(T), cudaMemcpyDefault));
    ...
    CUDA_TRY(cudaMemcpy(out_col_mask.data(),
                        static_cast<cudf::valid_type *>(out_gdf_col->valid),
                        nmasks * sizeof(cudf::valid_type),
                        cudaMemcpyDefault));

    // create column wrappers and compare
    cudf::test::column_wrapper<T> out(
      out_col, [&](cudf::size_type i) { return gdf_is_valid(out_col_mask.data(), i); });
    cudf::test::column_wrapper<T> ref(ref_data,
...
TYPED_TEST(RollingTest, SimpleStatic)
{
  ...
  // static sizes
  this->run_test_col_agg(col_data,
                         col_mask,
                         2,
                         2,
                         0,
...
TYPED_TEST(RollingTest, RandomStaticAllValid)
{
  ...
  cudf::test::column_wrapper<TypeParam> input{
    num_rows, [&](cudf::size_type row) { return this->random_value(rng); }};
...
TYPED_TEST(RollingTest, RandomDynamicAllValid)
{
  ...
  this->run_test_col_agg(input, 0, 0, 0, window, min_periods, forward_window);
}
...
TYPED_TEST(RollingTest, RandomDynamicWindowStaticPeriods)
{
  ...
  this->run_test_col_agg(
    input, 0, min_periods, 0, window, std::vector<cudf::size_type>(), forward_window);
}
...
TYPED_TEST(RollingTest, NegativeSizes)
{
  ...
  EXPECT_THROW(this->run_test_col_agg(col_data,
                                      col_mask,
                                      -2,
                                      2,
                                      2,
...
TYPED_TEST(RollingTest, WindowArraySizeMismatch)
{
  ...
  EXPECT_THROW(this->run_test_col_agg(col_data,
                                      ...
                                      std::vector<cudf::size_type>({2, 1, 2, 1})),
               cudf::logic_error);
}

// ------------- non-arithmetic types --------------------

using NonArithmeticTypes =
...
TYPED_TEST(RollingTestNonArithmetic, SumAvgNonArithmetic)
{
  ...
  EXPECT_THROW(this->run_test_col(input,
                                  2,
                                  2,
                                  0,
                                  std::vector<cudf::size_type>(),
...
BB0_3:
  ...
  cudf::test::column_wrapper<int> input{size,
  ...
  EXPECT_NO_THROW(
    output = cudf::rolling_window(
      *input.get(), 2, 4, 2, ptx, GDF_NUMBA_GENERIC_AGG_OPS, GDF_INT64, nullptr, nullptr, nullptr));
