...
TEST_F(OffsetRowWindowTest, OffsetRowWindow_3_to_Minus_1)
{
  ...
  auto run_rolling     = rolling_runner{preceding, following, 1};
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    *run_rolling(*cudf::make_min_aggregation<cudf::rolling_aggregation>()),
    ints_column{{null, 0, 0, 1, 2, 3, null, 6, 6, 7}, nulls_at({0, 6})});

  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    *run_rolling(*cudf::make_max_aggregation<cudf::rolling_aggregation>()),
    ints_column{{null, 0, 1, 2, 3, 4, null, 6, 7, 8}, nulls_at({0, 6})});

  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    *run_rolling(*cudf::make_sum_aggregation<cudf::rolling_aggregation>()),
    bigints_column{{null, 0, 1, 3, 5, 7, null, 6, 13, 15}, nulls_at({0, 6})});

  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    *run_rolling(*cudf::make_collect_list_aggregation<cudf::rolling_aggregation>()),
...
TEST_F(OffsetRowWindowTest, OffsetRowWindow_0_to_2)
{
  ...
  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    *run_rolling(*cudf::make_min_aggregation<cudf::rolling_aggregation>()),
    ints_column{{1, 2, 3, 4, 5, null, 7, 8, 9, null}, nulls_at({5, 9})});

  CUDF_TEST_EXPECT_COLUMNS_EQUAL(
    *run_rolling(*cudf::make_max_aggregation<cudf::rolling_aggregation>()),
    ints_column{{2, 3, 4, 5, 5, null, 8, 9, 9, null}, nulls_at({5, 9})});
