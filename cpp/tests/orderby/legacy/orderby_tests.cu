...
struct OrderByTest : public GdfTest {
  ...
  gdf_col_pointer create_gdf_column(std::vector<col_type> const& host_vector,
    ...
    EXPECT_EQ(RMM_ALLOC(&(the_column->data), host_vector.size() * sizeof(col_type), 0),
              RMM_SUCCESS);
    EXPECT_EQ(cudaMemcpy(the_column->data,
                         host_vector.data(),
    ...
    if (host_valid != nullptr) {
      EXPECT_EQ(
        RMM_ALLOC((void**)&(the_column->valid), gdf_valid_allocation_size(host_vector.size()), 0),
        RMM_SUCCESS);
      EXPECT_EQ(cudaMemcpy(the_column->valid,
                           host_valid,
                           gdf_num_bitmask_elements(host_vector.size()),
                           cudaMemcpyHostToDevice),
                cudaSuccess);
   ...
   * -------------------------------------------------------------------------*/
  void create_input(size_t orderby_column_length,
    ...
    for (auto const& c : gdf_orderby_columns) { gdf_raw_orderby_columns.push_back(c.get()); }
    ...
    if (print) {
      std::cout << "orderby column(s) created. Size: " << std::get<0>(orderby_columns).size()
                << std::endl;
  ...
  template <std::size_t I = 0, typename... Tp>
  inline typename std::enable_if<I == sizeof...(Tp), void>::type sort_multi_column(
    std::tuple<std::vector<Tp>...>& t,
    std::vector<host_valid_pointer>& valids,
  ...
  template <std::size_t I = 0, typename... Tp>
    ...
    for (size_t i = 0; i < column.size(); ++i) {
      ...
      if (!gdf_is_valid(column_valids, indices[j])) {
        if (nulls_at_front) {
          std::rotate(
            indices.begin() + invalid_count, indices.begin() + i, indices.begin() + i + 1);
        } else {
          std::rotate(
            indices.rbegin() + invalid_count, indices.rbegin() + i, indices.rbegin() + i + 1);
        }
        ++invalid_count;
      }
    }
    ...
    auto cmp = [&](size_t i1, size_t i2) {
      return (asc_desc[col_index] == GDF_ORDER_ASC ? column[i1] < column[i2]
                                                   : column[i1] > column[i2]);
    };
  ...
  std::vector<size_t> compute_gdf_result(bool use_default_sort_order = false,
    ...
    result_error =
      gdf_order_by(columns_to_sort,
                   (use_default_sort_order ? nullptr : (int8_t*)(sort_order_types->data)),
                   num_columns,
                   sorted_indices_output,
                   &ctxt);

    EXPECT_EQ(expected_result, result_error)
      << "The gdf order by function did not complete successfully";
    ...
    if (GDF_SUCCESS != expected_result) { return std::vector<size_t>(); }
    ...
    EXPECT_EQ(
      cudaMemcpy(
        host_result.data(), device_result, output_size * sizeof(int), cudaMemcpyDeviceToHost),
      cudaSuccess);
    ...
    if (print) {
      ...
      std::cout << "Indices:" << std::endl;
      std::copy(
        host_result.begin(), host_result.end(), std::ostream_iterator<size_t>(std::cout, ", "));
      std::cout << "\n";
    }
...
// tests.
template <typename tuple_of_vectors, bool smaller_nulls = true>
