  ...
  namespace cudf::datagen
{
  ...
  constexpr std::array vocab_priorities{
  ...
  std::unique_ptr<cudf::table> generate_orders_independent(
    double scale_factor, rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr)
  {
    CUDF_FUNC_RANGE();
    ...
    auto o_orderkey = [&]() {
      ...
      auto const o_orderkey_unsorted =
        cudf::sample(cudf::table_view({o_orderkey_candidates->view()}),
                     o_num_rows,
    ...
    double scale_factor, rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr)
  {
    ...
    auto ps_partkey = [&]() {
      auto const p_partkey = generate_primary_key_column(
        cudf::numeric_scalar<cudf::size_type>(1), p_num_rows, stream, mr);
    ...
    auto ps_suppkey =
      calculate_ps_suppkey(ps_partkey->view(), scale_factor, ps_num_rows, stream, mr);
    ...
    double scale_factor, rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr)
  {
    ...
    auto p_name = [&]() {
      ...
      auto const p_name_e = generate_random_string_column_from_set(
        ...
        stream,
        mr);
      return cudf::strings::concatenate(cudf::table_view({p_name_a->view(),
                                                          p_name_b->view(),
                                                          p_name_c->view(),
                                                          p_name_d->view(),
                                                          p_name_e->view()}),
   ...
   */
  std::
    tuple<std::unique_ptr<cudf::table>, std::unique_ptr<cudf::table>, std::unique_ptr<cudf::table>>
      ...
      double scale_factor, rmm::cuda_stream_view stream, rmm::device_async_resource_ref mr)
  {
    ...
    lineitem_partial_columns.insert(lineitem_partial_columns.begin() + 6,
