...
/**
 * @brief Fixed-point values are parameterized with a distribution type, scale and bounds of the same type.
 */
template <typename T>
 ...
 public:
  ...
  distribution_params<T> get_distribution_params() const
  {
    ...
    if (it == decimal_params.end()) {
      ...
      auto& desc = it->second;
      return {desc.id, static_cast<rep>(desc.lower_bound), static_cast<rep>(desc.upper_bound), desc.scale};
    }
  }
  ...
  // Users should pass integral values for bounds hen setting the parameters for fixed-point. 
  ...
  {
    for (auto tid : get_type_or_group(static_cast<int32_t>(type_or_group))) {
      decimal_params[tid] = {dist, static_cast<__int128_t>(lower_bound), 
        static_cast<__int128_t>(upper_bound), scale};
    }
  }
