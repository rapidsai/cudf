...
void table_write_common(cudf::table_view const& view, io_type sink_type, nvbench::state& state)
{
...
void BM_table_write_data_sizes(nvbench::state& state)
{
  ...
  table_write_common(view, sink_type, state);
}

template <data_type DataType>
void BM_table_write_data_common(nvbench::state& state,
...
template <data_type DataType>
void BM_table_write_data_types(nvbench::state& state,
                               nvbench::type_list<nvbench::enum_type<DataType>> type_list)
{
  BM_table_write_data_common<DataType>(state, data_profile{}, type_list);
}

template <data_type DataType>
void BM_table_write_num_columns(nvbench::state& state,
                                nvbench::type_list<nvbench::enum_type<DataType>> type_list)
{
  auto const d_type = get_type_or_group(static_cast<int32_t>(DataType));
...
using d_type_list_reduced = nvbench::enum_type_list<data_type::INTEGRAL,
                                                    ...
                                                    data_type::LIST,
                                                    data_type::STRUCT>;
NVBENCH_BENCH_TYPES(BM_table_write_data_types, NVBENCH_TYPE_AXES(d_type_list_reduced))
  .set_name("table_write_data_types")
  .set_type_axes_names({"data_type"})
  .add_string_axis("io_type", {"FILEPATH", "HOST_BUFFER", "VOID"})
  .set_min_samples(4)
  .add_int64_axis("data_size", {128 << 20});

NVBENCH_BENCH(BM_table_write_data_sizes)
  .set_name("table_write_data_sizes")
  .set_min_samples(4)
  .add_string_axis("io_type", {"FILEPATH", "HOST_BUFFER", "VOID"})
  .add_int64_power_of_two_axis("data_size", nvbench::range(24, 31, 1));  // 16MB to 2GB

NVBENCH_BENCH_TYPES(BM_table_write_num_columns,
                    NVBENCH_TYPE_AXES(nvbench::enum_type_list<data_type::STRING>))
  .set_name("table_write_num_columns")
  .set_type_axes_names({"data_type"})
