...
struct ColumnHandle {
  ...
  void operator()(gdf_column* source_column, int work_per_thread, cudaStream_t stream = 0) {
    ...
    // Launch the kernel.
    host_dispatching_kernel<functor_type>
      <<<grid_size, block_size, 0, stream>>>(source_data, n_rows);
  }
};
  ...
  while (index < n_rows) {
    for (cudf::size_type i = 0; i < source.num_columns(); i++) {
      cudf::type_dispatcher(
        source.get_column(i)->dtype, RowHandle<functor_type>{}, *source.get_column(i), index);
    }
    index += blockDim.x * gridDim.x;
  }  // while
}
...
void launch_kernel(cudf::table& input, T** d_ptr, int work_per_thread) {
  ...
  if (dispatching_type == HOST_DISPATCHING) {
    ...
    for (int c = 0; c < n_cols; c++) {
      cudf::type_dispatcher(input.get_column(c)->dtype,
                            ColumnHandle<functor_type>{},
                            input.get_column(c),
                            work_per_thread);
    }
