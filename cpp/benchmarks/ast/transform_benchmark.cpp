...
static void CustomRanges(benchmark::internal::Benchmark* b)
{
  auto row_counts = std::vector<cudf::size_type>{100'000,
                                                 ...
                                                 50'000'000,
                                                 100'000'000};
  // auto row_counts       = std::vector<cudf::size_type>{100'000, 1'000'000, 10'000'000,
  // 100'000'000};
  auto operation_counts = std::vector<cudf::size_type>{1, 2, 5, 10, 20, 50};
  for (auto const& row_count : row_counts) {
    for (auto const& operation_count : operation_counts) {
      // Size limit imposed by 16 GB memory
      if (static_cast<long>(row_count) * static_cast<long>(operation_count) <= 2'500'000'000L) {
