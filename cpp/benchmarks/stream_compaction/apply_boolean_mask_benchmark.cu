...
void calculate_bandwidth(benchmark::State& state, cudf::size_type num_columns)
{
  ...
  int64_t validity_bytes_in =
    (fraction >= 1.0f / 32) ? gdf_valid_allocation_size(column_size) : 4 * column_size_out;
...
void BM_apply_boolean_mask(benchmark::State& state, cudf::size_type num_columns)
{
  ...
  std::transform(
    columns.begin(), columns.end(), raw_columns.begin(), [](wrapper& c) { return c.get(); });
...
#define ABM_BENCHMARK_DEFINE(name, type, n_columns)         \
  ...
  (::benchmark::State & st) { BM_apply_boolean_mask<TypeParam>(st, n_columns); }
