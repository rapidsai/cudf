...
void calculate_bandwidth(benchmark::State& state, cudf::size_type num_columns) {
  ...
  int64_t const mask_size = sizeof(bool) * column_size + cudf::bitmask_allocation_size_bytes(column_size);
  ...
  int64_t const bytes_read = (column_bytes_in + validity_bytes_in) * num_columns + // reading columns
                             mask_size; // reading boolean mask
  int64_t const bytes_written = (column_bytes_out + validity_bytes_out) * num_columns; // writing columns
...
void BM_apply_boolean_mask(benchmark::State& state, cudf::size_type num_columns) {
  ...
  auto mask_iter = cudf::test::make_counting_transform_iterator(0, [&](auto i){
    return random_int(0, 100) < percent_true;
  });
  ...
  std::transform(columns.begin(), columns.end(),
                 column_views.begin(), [](auto const& col) { return static_cast<cudf::column_view>(col); });
