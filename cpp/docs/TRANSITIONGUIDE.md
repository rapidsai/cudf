...
This is the data type for a STRING column like int32_t is the data type for an INT32 column. As it's name implies, this is a read-only object instance that points to device memory inside the strings column. It's lifespan is the same as the column it was created from.
...
```
A null string is not the same as an empty string. Use the `string_scalar` class if you need to instance of a class object to represent a null string.
...
The data for a `string_view` instance is required to be [UTF-8](#UTF-8) and all operators and methods expect this encoding. Unless otherwise, specified
position and length parameters are specified in characters and not bytes.
The class also includes a `string_view::const_iterator` which can be used to navigate through individual characters within the string more quickly.

This is also the data type that will be resolved by the type-dispatcher when invoked on a STRING column. It is currently the only non-fixed-width type and the only compound type.


### UTF-8

The cudf strings column only supports UTF-8 encoding for strings data. [UTF-8](https://en.wikipedia.org/wiki/UTF-8) is a variable length character encoding where each character can be 1-4 bytes. This means the length of a string is not same as its size in bytes. For this reason, it recommended to use the `string_view` class to access these characters for most operations.
