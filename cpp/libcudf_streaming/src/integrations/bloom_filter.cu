...
BloomFilter::BloomFilter(std::size_t num_blocks,
                         std::uint64_t seed,
                         void* storage,
...
BloomFilter const BloomFilter::view(std::size_t num_blocks,
                                    std::uint64_t seed,
                                    void const* storage,
                                    rmm::cuda_stream_view stream)
{
...
std::unique_ptr<rmm::device_buffer> BloomFilter::storage(std::size_t num_blocks,
                                                         rmm::cuda_stream_view stream,
...
void BloomFilter::add(cudf::table_view const& values_to_hash,
                      rmm::cuda_stream_view stream,
                      rmm::device_async_resource_ref mr)
{
...
rmm::device_uvector<bool> BloomFilter::contains(cudf::table_view const& values,
                                                rmm::cuda_stream_view stream,
...
rmm::cuda_stream_view BloomFilter::stream() const noexcept { return stream_; }

void* BloomFilter::data() noexcept { return storage_; }

void const* BloomFilter::data() const noexcept { return storage_; }

std::size_t BloomFilter::size() const noexcept { return num_blocks_ * sizeof(StorageType); }
