  ...
  if (comm->nranks() == 1) {
    ...
    if (chunks.size() == 1) {
      ...
      cuda_stream_join(chunks | std::views::transform([](auto&& chunk) { return chunk.stream(); }),
                       ...
                       &event);
      co_return streaming::to_message(
        0, std::make_unique<streaming::TableChunk>(std::move(result), gather_stream));
    }
  } else {
    ...
    if (result.size() == 1) {
      co_return streaming::to_message(0,
                                   ...
                                   CudaEvent* left_event)
{
  ...
  return streaming::to_message(
    sequence, std::make_unique<streaming::TableChunk>(std::move(result_table), chunk_stream));
}
                                    ...
                                    KeepKeys keep_keys)
{
  ...
  while (!ch_out->is_shutdown()) {
    ...
    auto build_chunk  = co_await left_msg.release<streaming::TableChunk>().make_available(ctx);
                                               ...
                                               KeepKeys keep_keys)
{
  ...
  while (!ch_out->is_shutdown()) {
    ...
    auto right_chunk = co_await right_msg.release<streaming::TableChunk>().make_available(ctx);
                                        ...
                                        KeepKeys keep_keys)
{
  ...
  while (!ch_out->is_shutdown()) {
    ...
    auto left_chunk  = co_await left_msg.release<streaming::TableChunk>().make_available(ctx);
    auto right_chunk = co_await right_msg.release<streaming::TableChunk>().make_available(ctx);
