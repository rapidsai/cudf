...
//
// cd ../build
// g++ -std=c++11 ../tests/replace_multi.cpp -I/usr/local/cuda/include -L. -lNVStrings -lNVText -o replace_multi -Wl,-rpath,.:
//
...
void test_regex2()
{
    ...
    std::vector<const char*> data_ptrs;
    for( int idx=0; idx < 1000000; ++idx )
        data_ptrs.push_back(hstrs[idx % 7]); // dist(mt)
    NVStrings* strs = NVStrings::create_from_array(data_ptrs.data(), data_ptrs.size());
    printf("strings(%d): (%ld bytes)\n", strs->size(), strs->memsize());
    strs->print(0,10);

    const char* stop_words_re[] = {"\\bi\\b","\\bme\\b","\\bmy\\b","\\bmyself\\b","\\bwe\\b","\\bour\\b","\\bours\\b","\\bourselves\\b","\\byou\\b","\\byour\\b","\\byours\\b","\\byourself\\b",
...
void test_text2()
{
    ...
    std::vector<const char*> data_ptrs;
    for( int idx=0; idx < 1000000; ++idx )
        data_ptrs.push_back(hstrs[idx % 7]);
    NVStrings* strs = NVStrings::create_from_array(data_ptrs.data(), data_ptrs.size());
    printf("strings(%d): (%ld bytes)\n", strs->size(), strs->memsize());
    strs->print(0,10);
    ...
    data_ptrs.clear();;
    for( int idx=0; idx < tcount; ++idx )
        data_ptrs.push_back(stop_words[idx]);
    ...
    double st = GetTime();
    NVStrings* result = NVText::replace_tokens(*strs,*tgts,*rpls,"\t\f\n "); // delimiter default is whitespace
    double et = GetTime() - st;
    printf("result: (%ld bytes)\n", result->memsize());
    result->print(0,10);
    printf("  %g seconds\n",et);
