...
struct edit_distance_matrix_levenshtein_algorithm :
    ...
    cudf::index_type count;   // size of nvstrings object
    ...
    __device__ void operator() (cudf::index_type idx)
    {
      cudf::index_type row = idx / count;
      cudf::index_type col = idx % count;
      if( row > col )
         return;
                                           ...
                                           bool results_is_device_memory )
{
    ...
    cudf::index_type count = strs.size();
    if( count==0 )
        return 0; // nothing to do
    auto execpol = rmm::exec_policy(0);
    ...
    cudf::index_type n_upper = (count*(count-1))/2;
    rmm::device_vector<size_t> sizes(n_upper,0);
    size_t* d_sizes = sizes.data().get();
    thrust::for_each_n(execpol->on(0),
        thrust::make_counting_iterator<cudf::index_type>(0), count*count,
        [d_strings, d_sizes, count] __device__(cudf::index_type idx){
            cudf::index_type row = idx / count;
            cudf::index_type col = idx % count;
            if( row >= col )
                return;
