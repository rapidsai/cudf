...
/// \param[in] a first column to join (larger table)
/// \param[in] b second column to join (smaller table)
/// \param[in] dependency optional dependency to wait for before this operation is started.
template<typename input_it, typename size_type>
			  ...
			  std::shared_future<void>* dependency = 0)
{
  ...
  probe_hash_tbl<INNER_JOIN, multimap_type, key_type, size_type, joined_type, block_size, DEFAULT_CUDA_CACHE_SIZE>
                 <<<(a_count + block_size-1) / block_size, block_size>>>
                  (hash_tbl.get(), a, a_count, static_cast<joined_type*>(*out), out_count, max_out_count);
...
/// \brief Performs a hash based inner join of columns a and b, stores only rows that have matching values in columns a2 and b2 (two-column join)
/// 
/// It is possible to pass in preallocated ArrayData, in which case new memory via the MemoryPool associated
/// with handle is only allocated if the capacity of the passed in ArrayData container is not sufficient.
/// \param[in] compute_ctx The CudaComputeContext to shedule this to.
...
/// \param[in] a2 second column from table1
/// \param[in] b2 second column from table2
/// \param[in] dependency optional dependency to wait for before this operation is started.
			  ...
			  std::shared_future<void>* dependency = 0)
{
  ...
  probe_hash_tbl<multimap_type, key_type, key_type2, size_type, joined_type, block_size, DEFAULT_CUDA_CACHE_SIZE>
                 <<<(a_count + block_size-1) / block_size, block_size>>>
                  (hash_tbl.get(), a, a2, a_count, b2, static_cast<joined_type*>(*out), out_count, max_out_count);
  error = cudaDeviceSynchronize();
                          ...
                          std::shared_future<void>* dependency = 0)
{
  ...
  probe_hash_tbl<LEFT_JOIN, multimap_type, key_type, size_type, joined_type, block_size, DEFAULT_CUDA_CACHE_SIZE>
                 <<<(a_count + block_size-1) / block_size, block_size>>>
                  (hash_tbl.get(), a, a_count, static_cast<joined_type*>(*out), out_count, max_out_count);
  error = cudaDeviceSynchronize();
