...
gdf_error gpu_hash_columns(gdf_column ** columns_to_hash, int num_columns, gdf_column * output_column, cudaStream_t * stream){
	...
	for(int i = 0; i <  num_columns; i++){
		get_column_byte_width(columns_to_hash,host_widths[i]);
	}
	...
	thrust::transform(thrust::cuda::par.on(*stream),
			...
			thrust::make_counting_iterator<gdf_size_type>(0) + columns_to_hash[0].size,
			thrust::detail::make_normal_iterator(thrust::device_ptr_cast((unsigned long long *) output_column.data))
	,op);
