...
def pdf(request):
    ...
    test_pdf = test_pdf.astype('object')\
                       ...
                       .astype(typer)\
                       .rename({'col_datetime64[ms]': 'col_datetime64'},
                               axis=1)
...
def hdf_files(request, tmp_path_factory, pdf):
    ...
    for column in pdf.columns:
        fname_series[column] = (tmp_path_factory.mktemp("hdf") /
                                "test_series.hdf")
        pdf[column].to_hdf(fname_series[column], 'hdf_series_tests',
...
def test_hdf_writer(tmpdir, pdf, gdf, complib, format):
    ...
    for column in pdf.columns:
        ...
        pdf[column].to_hdf(pdf_series_fname, 'hdf_tests', format=format,
                           complib=complib)
        gdf[column].to_hdf(gdf_series_fname, 'hdf_tests', format=format,
                           complib=complib)
