...
def gpu_mark_segment_begins(arr, markers):
    ...
    if i == 0:
        ...
        if not markers[i]:
            # This arithmetic handles NaNs/Infs while still maintaining consistency for other
            # values. Since a NaN/Inf value is never equal to itself OR itself plus/minus any value,
            # we use this unique property to return correctly for NaN/Inf AND other values
