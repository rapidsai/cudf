...
def _datetime_timedelta_find_and_replace(
    ...
    if isinstance(to_replace, original_col_class):
        to_replace = to_replace.astype(dtype=np.dtype(np.int64))
    if isinstance(replacement, original_col_class):
        replacement = replacement.astype(dtype=np.dtype(np.int64))
    try:
        result_col = (
            original_column.astype(dtype=np.dtype(np.int64))
