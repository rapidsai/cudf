...
def random_bitmask(size):
    ...
    data = np.random.default_rng(2).integers(0, 255, dtype="u1", size=sz)
    return data.view("i1")
...
def gen_rand(dtype, size, **kwargs):
    ...
    if dtype.kind == "f":
        ...
        return (
            ...
            .integers(low=low, high=high, size=size)
            .astype(dtype)
        )
    elif dtype.kind == "i":
        ...
        return pd.to_datetime(
            np.random.default_rng(2).integers(low=low, high=high, size=size),
            unit=time_unit,
        )
    elif dtype.kind in ("O", "U"):
        low = kwargs.get("low", 10)
        high = kwargs.get("high", 11)
        nchars = np.random.default_rng(2).integers(low=low, high=high, size=1)[
            0
        ]
        char_options = np.array(list(string.ascii_letters + string.digits))
        all_chars = "".join(
            np.random.default_rng(2).choice(char_options, nchars * size)
        )
        return np.array(
