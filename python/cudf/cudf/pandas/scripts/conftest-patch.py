...
NODEIDS_TO_ALWAYS_SKIP = {
    # These tests fail because the fast and slow paths measure memory differently:
    # - obj.memory_usage(deep=...) is called inside cudf.pandas, so
    #   getattr_real_or_wrapped sees the fast attribute.
    #   The memory usage is calculated from the GPU object's device
    #   buffers and ignores pandas' deep kwarg.
    #
    # - sys.getsizeof(obj) is called from the pandas test module, which
    #   lives outside cudf.pandas. getattr_real_or_wrapped only intercepts
    #   lookups for modules on its allowlist, so external calls like this
    #   go straight to the slow object. As a result, sys.getsizeof dispatches
    #   to pandas' implementation, which calls the object's __sizeof__ and
    #   could add extra bytes from the garbage collector.
    ...
    "tests/base/test_misc.py::test_memory_usage[range]",
    "tests/base/test_misc.py::test_memory_usage[tuples]",
    "tests/base/test_misc.py::test_memory_usage[nullable_bool]",
    ...
    # These test_series_constructor tests do an identity check (`is`) between
    # the input ArrowExtensionArray and the ArrowExtensionArray backing the Series
    # which was shallow copied from the input. These identity checks are incompatible
    # with cudf.pandas
    "tests/extension/test_arrow.py::TestArrowArray::test_series_constructor[binary]",
    "tests/extension/test_arrow.py::TestArrowArray::test_series_constructor[bool]",
    "tests/extension/test_arrow.py::TestArrowArray::test_series_constructor[date32[day]]",
