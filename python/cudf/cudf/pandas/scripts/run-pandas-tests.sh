...
PYTEST_IGNORES="--ignore=tests/io/test_user_agent.py --ignore=tests/interchange/test_impl.py --ignore=tests/window/test_dtypes.py"
...
PANDAS_CI="1" python -m pytest -p cudf.pandas \
    ...
    -k "not test_overwrite_warns and not test_complex_series_frame_alignment and not test_to_parquet_gcs_new_file and not test_qcut_nat and not test_add and $TEST_NUMPY_UFUNCS_BASIC_FLAKY" \
    --durations=50 \
