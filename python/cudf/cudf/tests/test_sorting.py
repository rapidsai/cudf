...
def test_dataframe_sort_values(nelem, dtype):
    df = DataFrame()
    df["a"] = aa = (100 * np.random.default_rng(2).random(nelem)).astype(dtype)
    df["b"] = bb = (100 * np.random.default_rng(2).random(nelem)).astype(dtype)
...
def test_dataframe_sort_values_sliced(nelem, sliceobj):
    df = pd.DataFrame()
    df["a"] = np.random.default_rng(2).random(nelem)
...
def test_series_argsort(nelem, dtype, asc):
    sr = Series((100 * np.random.default_rng(2).random(nelem)).astype(dtype))
...
def test_series_sort_index(nelem, asc):
    sr = Series(100 * np.random.default_rng(2).random(nelem))
    psr = sr.to_pandas()
...
def test_dataframe_nlargest_nsmallest(nelem, n, op, columns):
    aa = np.random.default_rng(2).random(nelem)
    bb = np.random.default_rng(2).random(nelem)
...
def test_dataframe_nlargest_sliced(counts, sliceobj):
    ...
    df = pd.DataFrame()
    df["a"] = np.random.default_rng(2).random(nelem)
    df["b"] = np.random.default_rng(2).random(nelem)
...
def test_series_nlargest_nelem(nelem):
    elems = np.random.default_rng(2).random(nelem)
...
def test_dataframe_scatter_by_map(map_size, nelem, keep):
    ...
    df = DataFrame()
    df["a"] = np.random.default_rng(2).choice(strlist[:map_size], nelem)
    df["b"] = np.random.default_rng(2).uniform(
        low=0, high=map_size, size=nelem
    )
    df["c"] = np.random.default_rng(2).integers(map_size, size=nelem)
