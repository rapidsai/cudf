...
def make_params():
    ...
    aa = np.random.default_rng(2).integers(0, 50, 100)
    bb = np.random.default_rng(2).integers(0, 50, 100)
    for how in hows:
        yield (aa, bb, how)

    # Test floating point inputs
    aa = np.random.default_rng(2).random(50)
    bb = np.random.default_rng(2).random(50)
    for how in hows:
        yield (aa, bb, how)
...
def test_dataframe_join_suffix():
    df = cudf.DataFrame(
        np.random.default_rng(2).integers(0, 5, (5, 3)), columns=list("abc")
    )
...
def test_dataframe_merge_on(on):
    ...
    nelem = 500
    df_left["key1"] = np.random.default_rng(2).integers(0, 40, nelem)
    df_left["key2"] = np.random.default_rng(2).integers(0, 50, nelem)
    df_left["left_val"] = np.arange(nelem)

    df_right = cudf.DataFrame()
    nelem = 500
    df_right["key1"] = np.random.default_rng(2).integers(0, 30, nelem)
    df_right["key2"] = np.random.default_rng(2).integers(0, 50, nelem)
...
def test_dataframe_merge_no_common_column():
    ...
    df_right = cudf.DataFrame()
    nelem = 500
    df_right["key3"] = np.random.default_rng(2).integers(0, 30, nelem)
    df_right["key4"] = np.random.default_rng(2).integers(0, 50, nelem)
    df_right["right_val"] = np.arange(nelem)
...
def test_dataframe_pairs_of_triples(pairs, max, rows, how):
    ...
    for left_column in pairs[0]:
        pdf_left[left_column] = np.random.default_rng(2).integers(0, max, rows)
    for right_column in pairs[1]:
        pdf_right[right_column] = np.random.default_rng(2).integers(
            0, max, rows
        )
...
def test_safe_merging_with_left_empty():
    ...
    pdf_right = pd.DataFrame()
    for left_column in pairs[0]:
        pdf_left[left_column] = np.random.default_rng(2).integers(0, 10, 0)
    for right_column in pairs[1]:
        pdf_right[right_column] = np.random.default_rng(2).integers(0, 10, 5)
    gdf_left = cudf.from_pandas(pdf_left)
    gdf_right = cudf.from_pandas(pdf_right)
