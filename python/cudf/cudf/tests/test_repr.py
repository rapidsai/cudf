...
def test_null_series(nrows, dtype):
    size = 5
    sr = cudf.Series(np.random.default_rng(2).integers(1, 9, size)).astype(
        dtype
    )
    sr[np.random.default_rng(2).choice([False, True], size=size)] = None
...
def test_null_dataframe(ncols):
    ...
    for idx, dtype in enumerate(dtype_categories):
        sr = cudf.Series(
            np.random.default_rng(2).integers(0, 128, size)
        ).astype(dtype)
        sr[np.random.default_rng(2).choice([False, True], size=size)] = None
        gdf[dtype] = sr
    pdf = gdf.to_pandas()
...
def test_full_series(nrows, dtype):
    size = 20
    ps = pd.Series(np.random.default_rng(2).integers(0, 100, size)).astype(
        dtype
    )
...
def test_full_dataframe_20(dtype, size, nrows, ncols):
    pdf = pd.DataFrame(
        {
            idx: np.random.default_rng(2).integers(0, 100, size)
...
def test_MI():
    gdf = cudf.DataFrame(
        {
            "a": np.random.default_rng(2).integers(0, 4, 10),
            "b": np.random.default_rng(2).integers(0, 4, 10),
            "c": np.random.default_rng(2).integers(0, 4, 10),
        }
    )
...
def test_generic_index(length, dtype):
    psr = pd.Series(
        range(length),
        index=np.random.default_rng(2)
        .integers(0, high=100, size=length)
        .astype(dtype),
