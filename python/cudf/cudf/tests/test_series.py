...
def test_series_datetime_value_counts(data, nulls, normalize, dropna):
    ...
    if len(data) > 0:
        if nulls == "one":
            p = np.random.default_rng(2).integers(0, len(data))
            psr[p] = None
        elif nulls == "some":
    ...
    pd_cat = pd.Categorical(
        pd.Series(
            np.random.default_rng(2).choice(
                list(ascii_letters + digits), num_elements
            ),
            dtype="category",
        )
    )
...
def test_series_value_counts(dropna, normalize):
    for size in [10**x for x in range(5)]:
        arr = np.random.default_rng(2).integers(low=-1, high=10, size=size)
        mask = arr != -1
...
@pytest.mark.parametrize(
    "arr",
    [
        np.random.default_rng(2).normal(-100, 100, 1000),
        np.random.default_rng(2).integers(-50, 50, 1000),
        np.zeros(100),
...
def test_series_round(arr, decimals):
    ...
    arr.ravel()[
        np.random.default_rng(2).choice(
            arr.shape[0], arr.shape[0] // 2, replace=False
        )
    ] = np.nan
...
@pytest.mark.parametrize(
    ...
    [
        ...
        np.random.default_rng(2).integers(-100, 100, 200),
...
@pytest.mark.parametrize(
    "values",
    [
        np.random.default_rng(2).integers(-100, 100, 10),
        [],
        [np.nan, None, -1, 2, 3],
...
def test_isin_numeric(data, values):
    index = np.random.default_rng(2).integers(0, 100, len(data))
...
def test_series_digitize(num_rows, num_bins, right, dtype, series_bins):
    data = np.random.default_rng(2).integers(0, 100, num_rows).astype(dtype)
    bins = np.unique(
        np.sort(
            np.random.default_rng(2).integers(2, 95, num_bins).astype(dtype)
        )
    )
    s = cudf.Series(data)
    if series_bins:
...
def test_series_digitize_invalid_bins():
    s = cudf.Series(
        np.random.default_rng(2).integers(0, 30, 80), dtype="int32"
    )
...
def test_series_ordered_dedup():
    ...
    sr = cudf.Series(np.random.default_rng(2).integers(0, 100, 1000))
