...
cdef extern from "cudf/unary.hpp" namespace "cudf::experimental" nogil:
    ...
    cdef extern unique_ptr[column] is_null(column_view input)
    cdef extern unique_ptr[column] is_valid(column_view input)
    # cdef extern unique_ptr[column] cast(column_view input, data_type out_type)
    cdef extern unique_ptr[column] is_nan(column_view input)
    cdef extern unique_ptr[column] is_not_nan(column_view input)
