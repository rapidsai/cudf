...
class StringColumn(ColumnBase, Scannable):
    ...
    def as_decimal_column(self, dtype: DecimalDtype) -> DecimalBaseColumn:
        with self.access(mode="read", scope="internal"):
            ...
            result = cast(DecimalBaseColumn, ColumnBase.from_pylibcudf(plc_column))
    ...
    def minhash(
        ...
        with self.access(mode="read", scope="internal"):
            ...
            if isinstance(seed, int):
                ...
                seed = np.uint32(seed)
            return cast(ListColumn, type(self).from_pylibcudf(
    ...
    def jaccard_index(self, other: Self, width: int) -> NumericalColumn:
        with self.access(mode="read", scope="internal"):
            ...
            return cast(NumericalColumn, type(self).from_pylibcudf(result))
    ...
    def generate_character_ngrams(self, ngrams: int) -> ListColumn:
        with self.access(mode="read", scope="internal"):
            ...
            return cast(ListColumn, type(self).from_pylibcudf(result))
