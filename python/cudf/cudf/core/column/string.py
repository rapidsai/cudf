...
class StringMethods(ColumnMethods):
    ...
    def get_json_object(self, json_path, allow_single_quotes=False, strip_quotes_from_single_strings=True, missing_fields_as_nulls=False):
        r"""
        ...
        allow_single_quotes: bool, default False
            If True, representing strings with single quotes is allowed. 
            If False, strings must only be represented with double quotes.  
        strip_quotes_from_single_strings: bool, default True
            If True, strip the quotes from the return value of a given row if it is a string.
            If False, values returned for a given row include quotes if they are strings.
        missing_fields_as_nulls: bool, default False
            If True, when an object is queried for a field it does not contain, "null" is returned.
            If False, when an object is queried for a field it does not contain, None is returned.
        ...
        try:
            res = self._return_or_inplace(
                libstrings.get_json_object(
                    self._column, cudf.Scalar(json_path, "str"), allow_single_quotes, strip_quotes_from_single_strings, missing_fields_as_nulls
                )
            )
