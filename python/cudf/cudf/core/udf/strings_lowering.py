...
def register_binary_string_func(binary_func, cuda_func, retty):
    """
    ...
    if isinstance(binary_func, str):
        cuda_lower(f"StringView.{binary_func}", string_view, string_view)(
            binary_func_impl
        )
        cuda_lower(
        ...
        cuda_lower(binary_func, string_view, string_view)(binary_func_impl)


def create_unary_identifier_func(id_func, cuda_func):
    """
    Provide a wrapper around numba's low-level extension API which
    ...
    cuda_lower(f"StringView.{id_func}", string_view)(id_func_impl)
    cuda_lower(f"UDFString.{id_func}", string_view)(id_func_impl)


def create_upper_or_lower(id_func, cuda_func):
    """
    Provide a wrapper around numba's low-level extension API which
...
def register_strings_lowering():
    ...
    cuda_lowering_registry.lower_cast(types.StringLiteral, string_view)(
        cast_string_literal_to_string_view
    )
    ...
    cuda_lowering_registry.lower_cast(managed_udf_string, string_view)(
        cast_managed_udf_string_to_string_view
    )
    ...
    cuda_lower(
        ...
        types.Integer,
        types.Any,
    )(setitem_cpointer_managed_udf_string)

    cuda_lower(len, string_view)(len_impl)
    cuda_lower(NRT_decref, managed_udf_string)(decref_managed_udf_string)

    cuda_lower(operator.add, string_view, string_view)(concat_impl)

    cuda_lower("StringView.replace", string_view, string_view, string_view)(
        replace_impl
    )

    register_binary_string_func(
        operator.contains, contains_impl, types.boolean
    )
    register_binary_string_func(operator.eq, eq_impl, types.boolean)
    register_binary_string_func(operator.ne, ne_impl, types.boolean)
    register_binary_string_func(operator.ge, ge_impl, types.boolean)
    register_binary_string_func(operator.le, le_impl, types.boolean)
    register_binary_string_func(operator.gt, gt_impl, types.boolean)
    register_binary_string_func(operator.lt, lt_impl, types.boolean)
    register_binary_string_func("strip", strip_impl, udf_string)
    register_binary_string_func("lstrip", lstrip_impl, udf_string)
    register_binary_string_func("rstrip", rstrip_impl, udf_string)
    register_binary_string_func("startswith", startswith_impl, types.boolean)
    register_binary_string_func("endswith", endswith_impl, types.boolean)
    register_binary_string_func("count", count_impl, size_type)
    register_binary_string_func("find", find_impl, size_type)
    register_binary_string_func("rfind", rfind_impl, size_type)

    create_upper_or_lower("upper", upper_impl)
    create_upper_or_lower("lower", lower_impl)

    create_unary_identifier_func("isdigit", isdigit_impl)
    create_unary_identifier_func("isalnum", isalnum_impl)
    create_unary_identifier_func("isalpha", isalpha_impl)
    create_unary_identifier_func("isnumeric", isnumeric_impl)
    create_unary_identifier_func("isdecimal", isdecimal_impl)
    create_unary_identifier_func("isspace", isspace_impl)
    create_unary_identifier_func("isupper", isupper_impl)
    create_unary_identifier_func("islower", islower_impl)
    create_unary_identifier_func("istitle", istitle_impl)
