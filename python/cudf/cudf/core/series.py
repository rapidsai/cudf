...
class Series(Frame):
    ...
    def clip(self, lower=None, upper=None, inplace=False):
        ...
        Thresholds can be singular values, and
        in the latter case the clipping is performed element-wise.
        ...
        if not is_scalar(lower) or not is_scalar(upper):
            raise ValueError("lower/upper needs to be a scalar")

        if lower is not None and upper is not None and lower > upper:
            lower, upper = upper, lower

        result = self._column.clip(lower, upper)

        if inplace is True:
            if is_categorical_dtype(self._column):
                # As column type changes from categorical to
                # non-categorical, can't fo _mimic_inplace
                self._column = result
            else:
