...
class GroupBy(Serializable, Reducible, Scannable):
    ...
    def dtypes(self):
        ...
        return pd.DataFrame(
            {name: [obj_dtypes[name]] * len(index) for name in self.obj._data.names},
            index=index,
        )
    ...
    def nth(self, n):
        ...
        result._index = self.obj.index.take(result._data["__groupbynth_order__"])
        ...
        if isinstance(chunk_results, ColumnBase) or cudf.api.types.is_scalar(
            ...
            # group is the same length as the original group
                ...
                if total_rows == len(group_names):
                    ...
                    # a nameless UDF.
                    result = result.to_frame(name=grouped_values._data.names[0])
                else:
    ...
    @_cudf_nvtx_annotate
    def apply(self, function, *args, engine="auto", include_groups: bool = True):
