...
def get_variant_field(variant_column, field_name):
    ...
    encoded bytes of the named field's value for each row. Null is produced
    when the row is null, the field is absent, or the value blob is not an
    object.
    ...
    -------
    cudf.Series
        VARIANT struct Series with the extracted field's encoded bytes.
    """
...
def extract_variant_field(variant_column, field_name, dtype):
    """Extract a top-level VARIANT field and decode it into a typed Series.
    ...
    cudf.Series
        ...
        null, the field is missing, or the encoded value does not match
        ``dtype``.
    """
    from cudf.utils.dtypes import (
        dtype_from_pylibcudf_column,
    ...
    with src.access(mode="read", scope="internal"):
        plc_result = plc.io.experimental.extract_variant_field(
            src.plc_column, field_name, plc_dtype
        )
    return _wrap_variant_result(
        variant_column, plc_result, dtype_from_pylibcudf_column(plc_result)
    )
