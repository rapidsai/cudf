...
def rolling(inp, window, min_periods, center, op):
    ...
    cdef index_type c_window = 0
    cdef index_type c_forward_window = 0
    cdef gdf_agg_op c_op
    cdef index_type *c_window_col = NULL
    cdef index_type *c_min_periods_col = NULL
    cdef index_type *c_forward_window_col = NULL
    ...
    cdef index_type c_min_periods = min_periods
    ...
    if isinstance(window, numba.cuda.devicearray.DeviceNDArray):
        ...
        c_window_col = <index_type*> c_window_ptr
    else:
        if center:
            c_window = (window // 2) + 1
