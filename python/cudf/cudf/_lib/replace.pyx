...
def clip(Column input_col, object lo, object hi):
    ...
    lo_scalar = as_device_scalar(lo, dtype=input_col.dtype if lo is None else None)
    hi_scalar = as_device_scalar(hi, dtype=input_col.dtype if hi is None else None)
