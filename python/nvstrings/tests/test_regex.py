...
def test_replace(find, replace):
    s = [
        ...
        "over the",
        "lazy @dog",
        "hello http://www.world.com I'm here @home",
    ]
    pstrs = pd.Series(s)
    nvstrs = nvstrings.to_device(s)
...
def test_replace_multi():
    s = [
        "xxx 1281151 xxxxxx xxxxxxx xxxx xxxx - xxxxx xxxx xx 24",
...
@pytest.mark.parametrize("pattern", ["[hH]", "[bB][aA]"])
...
@pytest.mark.parametrize("pattern", ["a", "[aA]"])
