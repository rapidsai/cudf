...
def _select_local_split_candidates(
    ...
    if df.num_rows == 0:
        ...
        return DataFrame(
            [
                *df.columns,
                Column(
                    plc.column_factories.make_empty_column(part_id_dtype.plc_repr),
                    dtype=part_id_dtype,
                    name=next(name_gen),
                ),
                Column(
    ...
    row_id = plc.Column.from_iterable_of_py(candidates, part_id_dtype.plc_repr)

    res = plc.copying.gather(df.table, row_id, plc.copying.OutOfBoundsPolicy.DONT_CHECK)
    part_id = plc.Column.from_scalar(
        plc.Scalar.from_py(my_part_id, part_id_dtype.plc_repr),
        len(candidates),
    )
