...
def _build_sales_agg(
    ...
    return (
        sales
        .join(item_lf, left_on=sales_item_sk, right_on="i_item_sk")
...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    target_week = date_dim.filter(pl.col("d_date") == pl.date(2002, 2, 25)).select("d_week_seq")
    week_dates = date_dim.join(target_week, on="d_week_seq", how="inner").select(["d_date_sk"])

    ss_items = _build_sales_agg(
        store_sales, item, week_dates,
        "ss_item_sk", "ss_sold_date_sk", "ss_ext_sales_price", "ss_item_rev"
    )
    cs_items = _build_sales_agg(
        catalog_sales, item, week_dates,
        "cs_item_sk", "cs_sold_date_sk", "cs_ext_sales_price", "cs_item_rev"
    )
    ws_items = _build_sales_agg(
        web_sales, item, week_dates,
        "ws_item_sk", "ws_sold_date_sk", "ws_ext_sales_price", "ws_item_rev"
    )

    return (
        ss_items
        .join(cs_items, on="item_id", how="inner")
        ...
        .with_columns([
            (pl.col("ss_item_rev") + pl.col("cs_item_rev") + pl.col("ws_item_rev")).alias("total_rev"),
            ((pl.col("ss_item_rev") + pl.col("cs_item_rev") + pl.col("ws_item_rev")) / 3).alias("average"),
        ])
        .with_columns([
