...
def polars_impl(run_config: RunConfig) -> QueryResult:
    ...
    store_returns_filtered = (
        store_returns
        .join(d2_dates, left_on="sr_returned_date_sk", right_on="d_date_sk", how="semi")
    ...
    store_sales_filtered = (
        ...
        .select(["ss_customer_sk", "ss_item_sk", "ss_ticket_number", "ss_quantity", "i_item_id", "i_item_desc", "s_store_id", "s_store_name"])
    )

    catalog_sales_filtered = (
        catalog_sales
        .join(d3_dates, left_on="cs_sold_date_sk", right_on="d_date_sk", how="semi")
        .join(
            sr_customer_item,
    ...
    return QueryResult(
        frame=(
            ...
            .select(["ss_customer_sk", "ss_item_sk", "ss_quantity", "sr_return_quantity", "i_item_id", "i_item_desc", "s_store_id", "s_store_name"])
            .join(
                catalog_sales_filtered,
                left_on=["ss_customer_sk", "ss_item_sk"],
...
def polars_impl_naive(run_config: RunConfig) -> QueryResult:
    ...
    result = (
        ...
        # SQL: WHERE d1.d_moy={month} AND d1.d_year={year} AND d2.d_moy BETWEEN {month} AND {month}+3
        # SQL:   AND d2.d_year={year} AND d3.d_year IN (year, year+1, year+2)
        .filter(
            (pl.col("d_moy") == month)
            & (pl.col("d_year") == year)
            & pl.col("d_moy_d2").is_between(month, month + 3)
            & (pl.col("d_year_d2") == year)
            & pl.col("d_year_d3").is_in([year, year + 1, year + 2])
        )
        # SQL: GROUP BY i_item_id, i_item_desc, s_store_id, s_store_name
        .group_by(["i_item_id", "i_item_desc", "s_store_id", "s_store_name"])
        # SQL: {agg}(ss_quantity) AS store_sales_quantity, {agg}(sr_return_quantity) AS store_returns_quantity, {agg}(cs_quantity) AS catalog_sales_quantity
        .agg(
            [
                getattr(pl.col("ss_quantity"), polars_agg)().alias(
                    "store_sales_quantity"
        ...
        # SQL: ORDER BY i_item_id, i_item_desc, s_store_id, s_store_name
        .sort(sort_by.keys(), nulls_last=True)
        # SQL: LIMIT 100
        .limit(limit)
    )
