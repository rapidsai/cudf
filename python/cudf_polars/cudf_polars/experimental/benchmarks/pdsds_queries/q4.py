...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    t_s_firstyear = (
        store_sales.join(
            date_2001, left_on="ss_sold_date_sk", right_on="d_date_sk", how="inner"
        )
    ...
    t_s_secyear = (
        store_sales.join(
            date_2002, left_on="ss_sold_date_sk", right_on="d_date_sk", how="inner"
        )
        .join(customer, left_on="ss_customer_sk", right_on="c_customer_sk", how="inner")
        .group_by(
            [
    ...
    t_c_firstyear = (
        catalog_sales.join(
            date_2001, left_on="cs_sold_date_sk", right_on="d_date_sk", how="inner"
        )
        .join(
            customer,
        ...
        .filter(pl.col("year_total") > 0)
        .select([pl.col("c_customer_id").alias("customer_id"), pl.col("year_total")])
    )
    # Catalog sales - second year (2002)
    t_c_secyear = (
        catalog_sales.join(
            date_2002, left_on="cs_sold_date_sk", right_on="d_date_sk", how="inner"
        )
        .join(
            customer,
            left_on="cs_bill_customer_sk",
    ...
    t_w_firstyear = (
        web_sales.join(
            date_2001, left_on="ws_sold_date_sk", right_on="d_date_sk", how="inner"
        )
        .join(
            customer,
    ...
    # Web sales - second year (2002)
    t_w_secyear = (
        web_sales.join(
            date_2002, left_on="ws_sold_date_sk", right_on="d_date_sk", how="inner"
        )
        .join(
            customer,
            left_on="ws_bill_customer_sk",
    ...
    return (
        ...
        .filter(
            ...
            (
                ...
                .then(
                    pl.col("year_total") / pl.col("year_total_sf")
                )  # t_s_secyear.year_total
                .otherwise(None)
            )
            &
