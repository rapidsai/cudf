...
def polars_impl_naive(run_config: RunConfig) -> QueryResult:
    ...
    Not refactored to use year_sales_agg(): the helper joins
    sales.join(customer, left_on=customer_key, right_on=customer_key) using the
    same key name on both sides, but q4 uses different key names
    (c_customer_sk vs ss_customer_sk / cs_bill_customer_sk / ws_bill_customer_sk).
    """
    params = load_parameters(
        int(run_config.scale_factor), query_id=4, qualification=run_config.qualification
    )
    ...
    # SQL: FROM customer, store_sales WHERE c_customer_sk = ss_customer_sk
    store_total = (
        ...
        # SQL: GROUP BY c_customer_id, c_first_name, c_last_name, ..., d_year
        .group_by(
            [
                "c_customer_id",
                "c_first_name",
        ...
        # SQL: Sum(((ss_ext_list_price - ss_ext_wholesale_cost - ss_ext_discount_amt) + ss_ext_sales_price) / 2) AS year_total
        .agg(
            (
                (
        ...
        # SQL: SELECT c_customer_id AS customer_id, ..., 's' AS sale_type
        .select(
            [
                pl.col("c_customer_id").alias("customer_id"),
                pl.col("c_first_name").alias("customer_first_name"),
    ...
    # SQL: UNION ALL SELECT ... FROM customer, catalog_sales WHERE c_customer_sk = cs_bill_customer_sk
    catalog_total = (
        customer.join(
            catalog_sales, left_on="c_customer_sk", right_on="cs_bill_customer_sk"
        )
        # SQL: JOIN date_dim ON cs_sold_date_sk = d_date_sk
        .join(date_dim, left_on="cs_sold_date_sk", right_on="d_date_sk")
        # SQL: GROUP BY c_customer_id, ..., d_year
        .group_by(
            [
                "c_customer_id",
                "c_first_name",
        ...
        # SQL: Sum(((cs_ext_list_price - cs_ext_wholesale_cost - cs_ext_discount_amt) + cs_ext_sales_price) / 2) AS year_total
        .agg(
            (
                (
        ...
        # SQL: SELECT c_customer_id AS customer_id, ..., 'c' AS sale_type
        .select(
            [
                pl.col("c_customer_id").alias("customer_id"),
                pl.col("c_first_name").alias("customer_first_name"),
    ...
    # SQL: UNION ALL SELECT ... FROM customer, web_sales WHERE c_customer_sk = ws_bill_customer_sk
    web_total = (
        customer.join(
        ...
        # SQL: Sum(((ws_ext_list_price - ws_ext_wholesale_cost - ws_ext_discount_amt) + ws_ext_sales_price) / 2) AS year_total
        .agg(
            (
                (
        ...
        # SQL: SELECT c_customer_id AS customer_id, ..., 'w' AS sale_type
        .select(
            [
                pl.col("c_customer_id").alias("customer_id"),
                pl.col("c_first_name").alias("customer_first_name"),
    ...
    # SQL: year_total CTE = store_total UNION ALL catalog_total UNION ALL web_total
    year_total = pl.concat([store_total, catalog_total, web_total])

    # SQL: FROM year_total t_s_firstyear, t_s_secyear, t_c_firstyear, t_c_secyear, t_w_firstyear, t_w_secyear
    ...
    t_c_firstyear = year_total.rename(
        {
            "customer_id": "c_fy_customer_id",
            "customer_first_name": "c_fy_customer_first_name",
            "customer_last_name": "c_fy_customer_last_name",
            "customer_preferred_cust_flag": "c_fy_customer_preferred_cust_flag",
            "customer_birth_country": "c_fy_customer_birth_country",
            "customer_login": "c_fy_customer_login",
            "customer_email_address": "c_fy_customer_email_address",
            "dyear": "c_fy_dyear",
            "year_total": "c_fy_year_total",
            "sale_type": "c_fy_sale_type",
        }
    )
    t_c_secyear = year_total.rename(
        {
            "customer_id": "c_sy_customer_id",
            "customer_first_name": "c_sy_customer_first_name",
            "customer_last_name": "c_sy_customer_last_name",
            "customer_preferred_cust_flag": "c_sy_customer_preferred_cust_flag",
            "customer_birth_country": "c_sy_customer_birth_country",
            "customer_login": "c_sy_customer_login",
            "customer_email_address": "c_sy_customer_email_address",
            "dyear": "c_sy_dyear",
            "year_total": "c_sy_year_total",
            "sale_type": "c_sy_sale_type",
        }
    )
    ...
    # SQL: WHERE t_s_firstyear.customer_id = t_s_secyear.customer_id = ... AND sale_type filters AND dyear filters
    joined = (
        t_s_firstyear.join(
            t_s_secyear,
        ...
        .join(
            t_c_firstyear,
            left_on="s_fy_customer_id",
            right_on="c_fy_customer_id",
            how="inner",
        )
        .join(
            t_c_secyear,
            left_on="s_fy_customer_id",
            right_on="c_sy_customer_id",
            how="inner",
        )
        .join(
            t_w_firstyear,
        ...
        # SQL: AND t_s/c/w_firstyear.year_total > 0 AND CASE WHEN c_fy>0 THEN c_sy/c_fy END > CASE WHEN s_fy>0 THEN s_sy/s_fy END AND > CASE WHEN w_fy>0
        .filter(
            (pl.col("s_fy_sale_type") == "s")
            & (pl.col("c_fy_sale_type") == "c")
            & (pl.col("w_fy_sale_type") == "w")
            & (pl.col("s_sy_sale_type") == "s")
            & (pl.col("c_sy_sale_type") == "c")
            & (pl.col("w_sy_sale_type") == "w")
            & (pl.col("s_fy_dyear") == year)
            & (pl.col("s_sy_dyear") == year + 1)
            & (pl.col("c_fy_dyear") == year)
            & (pl.col("c_sy_dyear") == year + 1)
            & (pl.col("w_fy_dyear") == year)
            & (pl.col("w_sy_dyear") == year + 1)
            & (pl.col("s_fy_year_total") > 0)
            & (pl.col("c_fy_year_total") > 0)
            & (pl.col("w_fy_year_total") > 0)
            & (
                pl.when(pl.col("c_fy_year_total") > 0)
                .then(pl.col("c_sy_year_total") / pl.col("c_fy_year_total"))
                .otherwise(None)
    ...
    return QueryResult(
        frame=(
            # SQL: SELECT t_s_secyear.customer_id, customer_first_name, customer_last_name, customer_preferred_cust_flag
            joined.select(
                [
                    ...
                    pl.col("s_sy_customer_preferred_cust_flag").alias(
                        "customer_preferred_cust_flag"
                    ),
                ]
            )
            # SQL: ORDER BY customer_id, customer_first_name, customer_last_name, customer_preferred_cust_flag
            .sort(sort_cols)
            # SQL: LIMIT 100
            .limit(100)
        ),
