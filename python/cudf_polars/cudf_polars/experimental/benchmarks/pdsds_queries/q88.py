...
def polars_impl(run_config: RunConfig) -> QueryResult:
    ...
    filtered_store = store.filter(pl.col("s_store_name") == s_store_name).select("s_store_sk")
    ...
    counts_lf = (
        ...
        .select(
            pl.when((pl.col("t_hour") == 8) & (pl.col("t_minute") >= 30)).then(pl.lit(0))
            .when((pl.col("t_hour") == 9) & (pl.col("t_minute") < 30)).then(pl.lit(1))
            .when((pl.col("t_hour") == 9) & (pl.col("t_minute") >= 30)).then(pl.lit(2))
            .when((pl.col("t_hour") == 10) & (pl.col("t_minute") < 30)).then(pl.lit(3))
            .when((pl.col("t_hour") == 10) & (pl.col("t_minute") >= 30)).then(pl.lit(4))
            .when((pl.col("t_hour") == 11) & (pl.col("t_minute") < 30)).then(pl.lit(5))
            .when((pl.col("t_hour") == 11) & (pl.col("t_minute") >= 30)).then(pl.lit(6))
            .when((pl.col("t_hour") == 12) & (pl.col("t_minute") < 30)).then(pl.lit(7))
            .alias("bucket")
        )
        .group_by("bucket")
