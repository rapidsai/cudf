...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    cs_ui_items = (
        ...
        .agg([
            ...
            (pl.col("cr_refunded_cash") + pl.col("cr_reversed_charge") + pl.col("cr_store_credit")).sum().alias("refund"),
        ])
    ...
    filtered_items = (
        item
        .filter(
            pl.col("i_color").is_in(["cyan", "peach", "blush", "frosted", "powder", "orange"])
    ...
    base = (
        store_sales
        .join(filtered_items, left_on="ss_item_sk", right_on="i_item_sk", how="inner")
        .join(cs_ui_items, left_on="ss_item_sk", right_on="cs_item_sk", how="semi")
        .join(store_returns.select(["sr_item_sk", "sr_ticket_number"]),
              left_on=["ss_item_sk", "ss_ticket_number"],
              right_on=["sr_item_sk", "sr_ticket_number"],
              how="inner")
        .select([
            "ss_item_sk", "ss_store_sk", "ss_sold_date_sk",
            "ss_customer_sk", "ss_cdemo_sk", "ss_addr_sk",
    ...
    base = (
        base
        .join(store.select(["s_store_sk", "s_store_name", "s_zip"]),
              left_on="ss_store_sk", right_on="s_store_sk", how="inner")
        .join(date_dim.select(["d_date_sk", "d_year"]).rename({"d_year": "syear"}),
              left_on="ss_sold_date_sk", right_on="d_date_sk", how="inner")
        .join(customer.select([
                "c_customer_sk",
                "c_current_cdemo_sk", "c_current_addr_sk",
                "c_first_sales_date_sk", "c_first_shipto_date_sk"
            ]),
            left_on="ss_customer_sk", right_on="c_customer_sk", how="inner")
        .join(customer_demographics.select(["cd_demo_sk", "cd_marital_status"]).rename({"cd_marital_status": "cd1_marital"}),
              left_on="ss_cdemo_sk", right_on="cd_demo_sk", how="inner")
        .join(customer_demographics.select(["cd_demo_sk", "cd_marital_status"]).rename({
                "cd_demo_sk": "cd2_demo_sk", "cd_marital_status": "cd2_marital"
              }),
              left_on="c_current_cdemo_sk", right_on="cd2_demo_sk", how="inner")
        .filter(pl.col("cd1_marital") != pl.col("cd2_marital"))
        .join(customer_address.select([
            ...
            left_on="ss_addr_sk", right_on="b_addr_sk", how="inner")
        .join(customer_address.select([
                "ca_address_sk", "ca_street_number", "ca_street_name", "ca_city", "ca_zip"
            ]).rename({
            ...
            left_on="c_current_addr_sk", right_on="c_addr_sk", how="inner")
        .join(date_dim.select(["d_date_sk", "d_year"]).rename({"d_date_sk": "d2_date_sk", "d_year": "fsyear"}),
              left_on="c_first_sales_date_sk", right_on="d2_date_sk", how="inner")
        .join(date_dim.select(["d_date_sk", "d_year"]).rename({"d_date_sk": "d3_date_sk", "d_year": "s2year"}),
              left_on="c_first_shipto_date_sk", right_on="d3_date_sk", how="inner")
    )

    cross_sales = (
        base
        .group_by([
            "i_product_name", "ss_item_sk",
            "s_store_name", "s_zip",
            "b_street_number", "b_streen_name", "b_city", "b_zip",
    ...
    return (
        ...
        .select([
            "product_name", "store_name", "store_zip",
            "b_street_number", "b_streen_name", "b_city", "b_zip",
            "c_street_number", "c_street_name", "c_city", "c_zip",
            "syear", "cnt", "s1", "s2", "s3",
