...
def polars_impl_naive(run_config: RunConfig) -> QueryResult:
    ...
    # SQL: CTE ws_wh — web_sales orders shipped from multiple warehouses
    ws_wh = (
        ...
        # SQL: AND ws_warehouse_sk <> ws_warehouse_sk2 (filter multi-warehouse)
        .filter(pl.col("ws_warehouse_sk") != pl.col("ws_warehouse_sk_ws2"))
        .select("ws_order_number")
        .unique()
    )

    # SQL: web_returns JOIN ws_wh ON wr_order_number = ws_order_number
    ...
    return QueryResult(
        frame=(
            ...
            # SQL: AND ws_order_number IN (SELECT ws_order_number FROM ws_wh)
            .join(ws_wh, on="ws_order_number", how="inner")
            # SQL: AND ws_order_number IN (SELECT wr_order_number FROM web_returns, ws_wh)
