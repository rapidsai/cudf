...
def polars_impl(run_config: RunConfig) -> pl.LazyFrame:
    ...
    base = (
        store_sales
        .join(date_dim.select(["d_date_sk", "d_week_seq", "d_day_name"]), left_on="ss_sold_date_sk", right_on="d_date_sk")
    ...
    wss = (
        ...
        .agg([
            pl.when(pl.col("d_day_name") == "Sunday").then(pl.col("ss_sales_price")).otherwise(None).sum().alias("sun_sales"),
            pl.when(pl.col("d_day_name") == "Monday").then(pl.col("ss_sales_price")).otherwise(None).sum().alias("mon_sales"),
            pl.when(pl.col("d_day_name") == "Tuesday").then(pl.col("ss_sales_price")).otherwise(None).sum().alias("tue_sales"),
            pl.when(pl.col("d_day_name") == "Wednesday").then(pl.col("ss_sales_price")).otherwise(None).sum().alias("wed_sales"),
            pl.when(pl.col("d_day_name") == "Thursday").then(pl.col("ss_sales_price")).otherwise(None).sum().alias("thu_sales"),
            pl.when(pl.col("d_day_name") == "Friday").then(pl.col("ss_sales_price")).otherwise(None).sum().alias("fri_sales"),
            pl.when(pl.col("d_day_name") == "Saturday").then(pl.col("ss_sales_price")).otherwise(None).sum().alias("sat_sales"),
        ])
    )
    wss_enriched = (
        wss
        .join(store.select(["s_store_sk", "s_store_id", "s_store_name"]), left_on="ss_store_sk", right_on="s_store_sk")
    ...
    return (
        projected
        .sort(["s_store_name1", "s_store_id1", "d_week_seq1"], descending=[False, False, False], nulls_last=True)
        .limit(100)
    )
