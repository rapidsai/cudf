...
class Agg(Expr):
    ...
    def __init__(
        ...
        if name == "min":
            ...
            if plc.traits.is_duration(child.dtype.plc_repr):
    ...
    def _reduce(
        ...
        return Column(
            plc.Column.from_scalar(
                plc.reduce.reduce(column.obj, request, self.dtype.plc_repr),
                1,
            ),
            name=column.name,
            dtype=self.dtype,
    ...
    def _count(self, column: Column, *, include_nulls: bool) -> Column:
        ...
        return Column(
            plc.Column.from_scalar(
                plc.Scalar.from_py(column.size - null_count, self.dtype.plc_repr),
                1,
            ),
            name=column.name,
            dtype=self.dtype,
    ...
    def _sum(self, column: Column) -> Column:
        if column.size == 0 or column.null_count == column.size:
            return Column(
                plc.Column.from_scalar(
                    plc.Scalar.from_py(0, self.dtype.plc_repr),
                    1,
                ),
                name=column.name,
                dtype=self.dtype,
    ...
    def _min(self, column: Column, *, propagate_nans: bool) -> Column:
        if propagate_nans and column.nan_count > 0:
            return Column(
                plc.Column.from_scalar(
                    plc.Scalar.from_py(float("nan"), self.dtype.plc_repr),
                    1,
                ),
                name=column.name,
                dtype=self.dtype,
