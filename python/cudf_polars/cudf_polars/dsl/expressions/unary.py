...
class Cast(Expr):
    ...
    def __init__(self, dtype: DataType, value: Expr) -> None:
        ...
        if not dtypes.can_cast(value.dtype.plc_repr, self.dtype.plc_repr):
    ...
    def do_evaluate(
        ...
        return Column(
            plc.Column.from_scalar(
                plc.Scalar.from_py(df.num_rows, self.dtype.plc_repr),
                1,
            ),
            dtype=self.dtype,
        )
...
class UnaryFunction(Expr):
    ...
    def do_evaluate(
        ...
        if self.name == "null_count":
            ...
            return Column(
                plc.Column.from_scalar(
                    plc.Scalar.from_py(column.null_count, self.dtype.plc_repr),
                    1,
                ),
                dtype=self.dtype,
            )
        if self.name == "round":
            ...
            if isinstance(fill_value, Literal):
                arg = plc.Scalar.from_py(fill_value.value, fill_value.dtype.plc_repr)
            else:
            ...
            if isinstance(arg, plc.Scalar) and dtypes.can_cast(
                column.dtype.plc_repr, arg.type()
            ):  # pragma: no cover
                arg = (
            ...
            if strategy == "forward":
                ...
                replacement = plc.scalar.Scalar.from_py(0, dtype=column.dtype.plc_repr)
            elif strategy == "one":
                replacement = plc.scalar.Scalar.from_py(1, dtype=column.dtype.plc_repr)
            else:
            ...
            column = self.children[0].evaluate(df, context=context)
            if column.dtype.plc_repr.id() != self.dtype.id():
                arg = plc.unary.cast(column.obj, self.dtype.plc_repr)
            else:
                arg = column.obj
            return Column(
            ...
            column = self.children[0].evaluate(df, context=context)
            plc_col = column.obj
            col_type = column.dtype.plc_repr
            # cum_sum casts
            ...
            if (
                ...
                self.name == "cum_sum"
                and column.dtype.plc_repr.id() == plc.TypeId.BOOL8
            ):
