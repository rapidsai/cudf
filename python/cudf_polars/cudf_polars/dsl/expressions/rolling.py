...
class FixedSizeRollingWindow(Expr):
    ...
    __slots__ = ("agg_name", "fn_params", "following", "min_periods", "preceding")
    _non_child = (
        "dtype",
        "agg_name",
        "preceding",
    ...
    _aggregations: ClassVar[dict[str, Callable[..., plc.aggregation.Aggregation]]] = {
        "sum": lambda: plc.aggregation.sum(),
        "min": lambda: plc.aggregation.min(),
        "max": lambda: plc.aggregation.max(),
        "mean": lambda: plc.aggregation.mean(),
        "var": lambda ddof: plc.aggregation.variance(ddof=ddof),
        "std": lambda ddof: plc.aggregation.std(ddof=ddof),
    }
