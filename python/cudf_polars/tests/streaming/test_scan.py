...
def test_split_scan_is_equal() -> None:
    parquet_options = ParquetOptions(prefetch_file_metadata=True)
    base_scan = _make_parquet_scan(["file.parquet"], parquet_options=parquet_options)
    cached_info = [
    ...
    assert scan_1.get_hashable() == scan_2.get_hashable()
    assert hash(scan_1) == hash(scan_2)

    assert not scan_1.is_equal(scan_3)
    assert scan_1.get_hashable() != scan_3.get_hashable()
    assert hash(scan_1) != hash(scan_3)


def test_fused_scan_is_equal() -> None:
    parquet_options = ParquetOptions(prefetch_file_metadata=True)
    base_scan = _make_parquet_scan(
        ["file_1.parquet", "file_2.parquet"], parquet_options=parquet_options
    )
    other_base_scan = _make_parquet_scan(
        ["other.parquet"], parquet_options=parquet_options
    )
    cached_info = [
        CachedParquetInfo(path="file_1.parquet", size=100, file_metadata=None),  # type: ignore[arg-type]
        CachedParquetInfo(path="file_2.parquet", size=200, file_metadata=None),  # type: ignore[arg-type]
    ]
...
def test_streaming_scan_is_equal() -> None:
    parquet_options = ParquetOptions(prefetch_file_metadata=True)
    base_scan = _make_parquet_scan(["file.parquet"], parquet_options=parquet_options)
    cached_info = [
        CachedParquetInfo(path="file.parquet", size=100, file_metadata=None)  # type: ignore[arg-type]
    ...
    streaming_1 = StreamingScan([split_1], base_scan, "split")
    streaming_2 = StreamingScan([split_2], base_scan, "split")
    streaming_3 = StreamingScan([split_3], base_scan, "split")

    assert streaming_1.is_equal(streaming_2)
    assert streaming_1.get_hashable() == streaming_2.get_hashable()
    assert hash(streaming_1) == hash(streaming_2)

    assert not streaming_1.is_equal(streaming_3)
    assert streaming_1.get_hashable() != streaming_3.get_hashable()
    assert hash(streaming_1) != hash(streaming_3)
