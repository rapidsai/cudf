...
In practice, `state` is some `TypedDict` that's specific to that
`CachingVisitor`. To add a field to the state dict, add it to the
`TypeDict` passed in as its `state` parameter.
