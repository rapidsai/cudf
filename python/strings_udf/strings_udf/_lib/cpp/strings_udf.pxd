...
cdef extern from "cudf/strings/udf/udf_apis.hpp":
    ...
    cdef unique_ptr[column] call_udf(udf_module, string, size_type, vector[column_view])
    cdef unique_ptr[device_buffer] to_string_view_array(column_view)
    cdef unique_ptr[column] from_dstring_array(void*, size_t)

cdef extern from "cudf/strings/detail/char_tables.hpp" namespace "cudf::strings::detail":
