...
def test_padding(n):
    data = list(range(n))
    padded, valid = sorting._pad_data_to_length(data)
    assert len(data) == valid
    assert sorting.is_power_of_2(len(padded))
...
def test_compare_frame(seed, nelem):
    ...
    # Sort by column "a"
    got_a = sorting._compare_frame(lhs, rhs, max_part_size, by="a")
    # Check
    ...
    # Sort by column "b"
    got_b = sorting._compare_frame(lhs, rhs, max_part_size, by="b")
    # Check
...
def test_compare_frame_with_none():
    ...
    max_part_size = 1
    df["a"] = [0]
    res = sorting._compare_frame(df, None, max_part_size, by="a")
    assert res[0] is not None, res[1] is None
    res = sorting._compare_frame(None, df, max_part_size, by="a")
    assert res[0] is not None, res[1] is None
    res = sorting._compare_frame(None, None, max_part_size, by="a")
