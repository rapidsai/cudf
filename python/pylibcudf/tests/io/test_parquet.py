...
@pytest.mark.parametrize(
    "metadata_size_hint_factory",
    [
        lambda file_size: 0,
        lambda file_size: 9,
        lambda file_size: 1024,
        lambda file_size: file_size + 1024,
    ],
    ids=[
        "hint-0",
        "hint-9",
        "hint-1024",
        "hint-larger-than-file",
    ],
)
def test_read_parquet_metadata_with_size_hint(
    tmp_path, metadata_size_hint_factory
) -> None:
    table = pa.table({"a": [1, 2, 3], "b": ["x", "y", "z"]})
    path = tmp_path / "metadata_hint.parquet"
    write_table(table, path)
    metadata_size_hint = metadata_size_hint_factory(path.stat().st_size)

    source_info = plc.io.SourceInfo([path])
    metadata = plc.io.parquet_metadata.read_parquet_metadata(
        source_info, metadata_size_hint=metadata_size_hint
    )
...
def test_read_parquet_footers_with_size_hint(
    tmp_path, metadata_size_hint_factory
) -> None:
    table = pa.table({"a": [1, 2, 3], "b": ["x", "y", "z"]})
