...
cdef extern from "cudf/io/text/data_chunk_source_factories.hpp" \
    ...
    unique_ptr[data_chunk_source] make_source(string data) except +libcudf_exception_handler
    unique_ptr[data_chunk_source] \
        make_source_from_file(string filename) except +libcudf_exception_handler
    unique_ptr[data_chunk_source] \
        make_source_from_bgzip_file(string filename) except +libcudf_exception_handler
    unique_ptr[data_chunk_source] \
        make_source_from_bgzip_file(string filename,
                                    uint64_t virtual_begin,
                                    uint64_t virtual_end) except +libcudf_exception_handler
