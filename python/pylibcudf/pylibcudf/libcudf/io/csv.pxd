...
cdef extern from "cudf/io/csv.hpp" \
    ...
    cdef cppclass csv_reader_options:
        ...
        # Filter settings
        void set_use_cols_names(vector[string] col_names) except +libcudf_exception_handler
        ...
        void set_detect_whitespace_around_quotes(bool val) except +libcudf_exception_handler
    ...
    cdef cppclass csv_reader_options_builder:
        ...
        csv_reader_options_builder& byte_range_offset(size_t val) except +libcudf_exception_handler
        csv_reader_options_builder& byte_range_size(size_t val) except +libcudf_exception_handler
        csv_reader_options_builder& names(vector[string] val) except +libcudf_exception_handler
        csv_reader_options_builder& prefix(string pfx) except +libcudf_exception_handler
        csv_reader_options_builder& mangle_dupe_cols(bool val) except +libcudf_exception_handler
        ...
        csv_reader_options_builder& nrows(size_type n_rows) except +libcudf_exception_handler
        csv_reader_options_builder& skiprows(size_type val) except +libcudf_exception_handler
        csv_reader_options_builder& skipfooter(size_type val) except +libcudf_exception_handler
        csv_reader_options_builder& header(size_type hdr) except +libcudf_exception_handler

        # Parsing settings
        csv_reader_options_builder& lineterminator(char val) except +libcudf_exception_handler
        csv_reader_options_builder& delimiter(char val) except +libcudf_exception_handler
        csv_reader_options_builder& thousands(char val) except +libcudf_exception_handler
        csv_reader_options_builder& decimal(char val) except +libcudf_exception_handler
        csv_reader_options_builder& comment(char val) except +libcudf_exception_handler
        csv_reader_options_builder& windowslinetermination(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& delim_whitespace(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& skipinitialspace(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& skip_blank_lines(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& quoting(
            cudf_io_types.quote_style style
        ) except +libcudf_exception_handler
        csv_reader_options_builder& quotechar(char val) except +libcudf_exception_handler
        csv_reader_options_builder& doublequote(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& detect_whitespace_around_quotes(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& parse_dates(vector[string]) except +libcudf_exception_handler
        csv_reader_options_builder& parse_dates(vector[int]) except +libcudf_exception_handler

        # Conversion settings
        csv_reader_options_builder& dtypes(vector[string] types) except +libcudf_exception_handler
        ...
        csv_reader_options_builder& keep_default_na(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& na_filter(bool val) except +libcudf_exception_handler
        csv_reader_options_builder& dayfirst(bool val) except +libcudf_exception_handler
    ...
    cdef cppclass csv_writer_options:
        ...
        # setter
        void set_metadata(cudf_io_types.table_metadata* val) except +libcudf_exception_handler
    ...
    cdef cppclass csv_writer_options_builder:
        ...
        csv_writer_options_builder& names(vector[string] val) except +libcudf_exception_handler
        csv_writer_options_builder& na_rep(string val) except +libcudf_exception_handler
        csv_writer_options_builder& include_header(bool val) except +libcudf_exception_handler
        csv_writer_options_builder& rows_per_chunk(size_type val) except +libcudf_exception_handler
        csv_writer_options_builder& line_terminator(string term) except +libcudf_exception_handler
        csv_writer_options_builder& inter_column_delimiter(char delim) except +libcudf_exception_handler
        csv_writer_options_builder& true_value(string val) except +libcudf_exception_handler
        csv_writer_options_builder& false_value(string val) except +libcudf_exception_handler
