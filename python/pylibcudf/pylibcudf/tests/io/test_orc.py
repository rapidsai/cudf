...
def test_read_orc_basic(table_data, binary_source_or_sink, nrows_skiprows, columns):
    ...
    # let's cast to int64
    _, new_fields = _convert_types(pa_table, pa.types.is_unsigned_integer, pa.int64())
    pa_table = pa_table.cast(pa.schema(new_fields))

    source = make_source(binary_source_or_sink, pa_table, **_COMMON_ORC_SOURCE_KWARGS)
