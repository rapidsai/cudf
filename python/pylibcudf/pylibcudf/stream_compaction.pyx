...
cpdef Table drop_nulls(
    ...
    with nogil:
        c_result = cpp_stream_compaction.drop_nulls(
            source_table.view(), c_keys, keep_threshold, stream.view(), mr.c_ref.value()
        )
...
cpdef Table apply_boolean_mask(
    ...
    with nogil:
        c_result = cpp_stream_compaction.apply_boolean_mask(
            source_table.view(), boolean_mask.view(), stream.view(), mr.c_ref.value()
        )
    return Table.from_libcudf(move(c_result), stream, mr)
...
cpdef Table unique(
    ...
    with nogil:
        c_result = cpp_stream_compaction.unique(
            input.view(), c_keys, keep, nulls_equal, stream.view(), mr.c_ref.value()
        )
    return Table.from_libcudf(move(c_result), stream, mr)
...
cpdef Column distinct_indices(
    ...
    with nogil:
        c_result = cpp_stream_compaction.distinct_indices(
            input.view(), keep, nulls_equal, nans_equal, stream.view(), mr.c_ref.value()
        )
