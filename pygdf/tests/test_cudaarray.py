...
def test_reshape_ravel():
    ...
    for shapes in ((110,), (10,11),(5,2,11)):
        np.testing.assert_equal(carr.reshape(*shapes).copy_to_host(), arr.reshape(*shapes))
        np.testing.assert_equal(carr.reshape(*shapes, **dict(order='F')).copy_to_host(), arr.reshape(*shapes, **dict(order='F')))
        np.testing.assert_equal(carr.reshape(*shapes).ravel().copy_to_host(), arr.reshape(*shapes).ravel())
        np.testing.assert_equal(carr.reshape(*shapes, **dict(order='C')).ravel(order='C').copy_to_host(), arr.reshape(*shapes, **dict(order='C')).ravel(order='C'))
        np.testing.assert_equal(carr.reshape(*shapes, **dict(order='F')).ravel(order='F').copy_to_host(), arr.reshape(*shapes, **dict(order='F')).ravel(order='F'))
        if len(shapes)>1:
            with pytest.raises(NotImplementedError, message="Expecting NotImplementedError") as excinfo:
