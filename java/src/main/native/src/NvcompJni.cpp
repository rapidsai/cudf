...
// methods for zstd
JNIEXPORT jlong JNICALL
Java_ai_rapids_cudf_nvcomp_NvcompJni_batchedZstdCompressGetTempSize(JNIEnv *env,
                                                                    jclass,
                                                                    jlong j_batch_size,
                                                                    jlong j_max_chunk_size)
{
  try {
    ...
    auto max_chunk_size = static_cast<std::size_t>(j_max_chunk_size);
    std::size_t temp_size = 0;
    auto status = nvcompBatchedZstdCompressGetTempSize(batch_size, max_chunk_size,
...
JNIEXPORT jlong JNICALL
Java_ai_rapids_cudf_nvcomp_NvcompJni_batchedZstdCompressGetMaxOutputChunkSize(JNIEnv *env,
                                                                              jclass,
                                                                              jlong j_max_chunk_size)
{
  try {
                                                              ...
                                                              jlong j_stream)
{
  try {
    ...
    auto compressed_out_sizes = reinterpret_cast<std::size_t *>(j_compressed_sizes_out_ptr);
    auto stream = reinterpret_cast<cudaStream_t>(j_stream);
    auto status = nvcompBatchedZstdCompressAsync(in_ptrs, in_sizes, chunk_size, batch_size,
        temp_ptr, temp_size, out_ptrs, compressed_out_sizes, nvcompBatchedZstdDefaultOpts, stream);
    check_nvcomp_status(env, status);
  }
  CATCH_STD(env, );
}
...
JNIEXPORT jlong JNICALL
Java_ai_rapids_cudf_nvcomp_NvcompJni_batchedZstdDecompressGetTempSize(JNIEnv *env,
                                                                      jclass,
                                                                      jlong j_batch_size,
                                                                      jlong j_chunk_size)
{
                                                                ...
                                                                jlong j_stream)
{
  try {
    ...
    auto uncompressed_statuses = rmm::device_uvector<nvcompStatus_t>(batch_size, stream);
    auto actual_uncompressed_sizes = rmm::device_uvector<std::size_t>(batch_size, stream);
    auto status = nvcompBatchedZstdDecompressAsync(
                                                                       ...
                                                                       jlong j_stream)
{
  try {
    ...
    auto batch_size = static_cast<std::size_t>(j_batch_size);
    auto stream = reinterpret_cast<cudaStream_t>(j_stream);
    auto status = nvcompBatchedZstdGetDecompressSizeAsync(compressed_ptrs, compressed_sizes,
