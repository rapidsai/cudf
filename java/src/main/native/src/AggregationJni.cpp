                                                                     ...
                                                                     jint num_registers_per_sketch)
{
  try {
    ...
    switch (kind) {
      case 35:  // HLL
        ret = cudf::make_hyper_log_log_aggregation(num_registers_per_sketch);
        break;
      case 36:  // MERGE_HLL
        ret = cudf::make_merge_hyper_log_log_aggregation(num_registers_per_sketch);
        break;
      default: throw std::logic_error("Unsupported HLL Aggregation Operation");
    }
    return reinterpret_cast<jlong>(ret.release());
  }
  CATCH_STD(env, 0);
