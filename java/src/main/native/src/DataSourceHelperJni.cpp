...
namespace jni {
  bool cache_data_source_jni(JNIEnv *env) {
    ...
    hostRead_method =
      env->GetStaticMethodID(cls, "hostRead", "(L" DATA_SOURCE_CLASS ";JJJ)J");
    ...
    deviceRead_method =
      env->GetStaticMethodID(cls, "deviceRead", "(L" DATA_SOURCE_CLASS ";JJJJ)J");
  ...
  public:
    ...
    host_buffer_done_callback(host_buffer_done_callback && other) : jvm(other.jvm), ds(other.ds), id(other.id) {
      other.jvm = nullptr;
      other.ds = nullptr;
      other.id = -1;
    }
      ...
      if (jvm != nullptr) {
        ...
        if (jvm->GetEnv(reinterpret_cast<void **>(&env), cudf::jni::MINIMUM_JNI_VERSION) == JNI_OK) {
          env->CallStaticVoidMethod(
                DataSource_jclass, hostReadBuffDone_method, this->ds, id);
        }
      }
    }
  private:
  ...
  public:
    explicit jni_datasource(JNIEnv *env, jobject ds, size_t ds_size, bool device_read_supported,
            size_t device_read_cutoff) : ds_size(ds_size), 
    ...
    std::unique_ptr<buffer> host_read(size_t offset, size_t size) override {
      ...
      return std::make_unique<owning_buffer<cudf::jni::host_buffer_done_callback>>(std::move(cb), ptr, length);
    }
    ...
    size_t host_read(size_t offset, size_t size, uint8_t* dst) override {
      ...
      jlong amount_read = env->CallStaticLongMethod(
              DataSource_jclass, hostRead_method, this->ds, offset, size, 
    ...
    size_t device_read(size_t offset, size_t size, uint8_t* dst, rmm::cuda_stream_view stream) override
    {
      ...
      jlong amount_read = env->CallStaticLongMethod(
              DataSource_jclass, deviceRead_method, this->ds, offset, size, 
              reinterpret_cast<jlong>(dst), reinterpret_cast<jlong>(stream.value()));
      if (env->ExceptionOccurred()) {
...
JNIEXPORT jlong JNICALL Java_ai_rapids_cudf_DataSourceHelper_createWrapperDataSource(
  ...
  try {
    ...
    auto source = new cudf::jni::jni_datasource(env, ds, ds_size, device_read_supported,
            device_read_cutoff);
...
JNIEXPORT void JNICALL Java_ai_rapids_cudf_DataSourceHelper_destroyWrapperDataSource(
