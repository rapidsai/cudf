...
JNIEXPORT jlong JNICALL Java_ai_rapids_cudf_ColumnView_getJSONObject(
  ...
  try {
    ...
    std::unique_ptr<cudf::column> result_col_ptr;
    try {
      result_col_ptr = cudf::get_json_object(n_strings_col_view, *n_scalar_path, options);
    } catch (std::invalid_argument const &err) {
      auto const null_scalar = cudf::string_scalar(std::string(""), false);
      result_col_ptr = cudf::make_column_from_scalar(null_scalar, n_strings_col_view.size());
    }
