...
public class MultiBufferDataSource extends DataSource {
    ...
    public MultiBufferDataSource(HostMemoryBuffer ... buffers) {
        ...
        allocator = DefaultHostMemoryAllocator.get();
    }
    ...
    private int getStartBufferIndexForOffset(long offset) {
        ...
        while (startIndex != endIndex) {
            ...
            if (offset >= midStartOffset) {
                ...
                long midEndOffset = Long.MAX_VALUE;
                if (midIndex < startOffsets.length - 1) {
                    midEndOffset = startOffsets[midIndex + 1];
                }
    ...
    private <T extends MemoryBuffer> long read(long offset, long amount,
                                               T dest, DoCopy<T> doCopy) {
    ...
    public HostMemoryBuffer hostRead(long offset, long amount) {
        ...
        if (bufferAmount == realAmount) {
            ...
            try {
                long amountRead = read(offset, amount, ret, HostMemoryBuffer::copyFromHostBuffer);
    ...
    @Override
    public long hostRead(long offset, long amount, HostMemoryBuffer dest) {
        long ret = read(offset, amount, dest, HostMemoryBuffer::copyFromHostBuffer);
        hostReads += 1;
        hostReadBytes += ret;
        return ret;
    }
    ...
    public long deviceRead(long offset, long amount, DeviceMemoryBuffer dest,
                           Cuda.Stream stream) {
        long ret = read(offset, amount, dest, (destParam, destOffset, src, srcOffset, srcAmount) ->
