...
/**
 * Provides a convenient way to setup a prediction for a device data operation. The prediction
 ...
 */
final class DevicePrediction implements AutoCloseable {
   ...
   */
  DevicePrediction(long amount, String note) {
    this.note = note;
    MemoryListener.devicePrediction(amount, note);
  }

  @Override
  public void close() {
    MemoryListener.deviceEndPrediction(note);
  }
}
