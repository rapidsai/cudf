...
cpdef _get_column_data_ptr(obj):
    return _get_ctype_ptr(obj._data.mem)

cpdef _get_column_valid_ptr(obj):
    return _get_ctype_ptr(obj._mask.mem)
...
cdef gdf_column* column_view_from_column(col):
    ...
    if len(col) > 0:
        data_ptr = _get_column_data_ptr(col)
    else:
        data_ptr = 0

    if col._mask is not None and col.null_count > 0:
        valid_ptr = _get_column_valid_ptr(col)
    else:
        valid_ptr = 0
...
cdef gdf_column* column_view_from_NDArrays(size, data, mask, dtype,
    ...
    if data is not None:
        data_ptr = _get_ctype_ptr(data)
    else:
        data_ptr = 0

    if mask is not None:
        valid_ptr = _get_ctype_ptr(mask)
    else:
        valid_ptr = 0
